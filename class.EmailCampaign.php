...
class EmailCampaign extends DB_Table {
    ...
    function get_contents_array() {
        ...
        $sql                  = sprintf(
            "SELECT `Email Content Color Scheme Historic Key`,`Email Content Template Postcard Key`,`Email Content Color Scheme Key`,`Email Content Template Type`,`Email Content Type`,`Email Content Subject`,`Email Content Type`,C.`Email Content Key`,`Email Content Text`,`Email Content HTML`,`Email Template Header Image Key`,`Email Content Metadata` FROM  `Email Content Dimension`   C  LEFT JOIN `Email Campaign Content Bridge` B ON (B.`Email Content Key`=C.`Email Content Key`) WHERE `Email Campaign Key`=%d ",
            $this->id
        );
        $res                  = mysql_query($sql);
        // print $sql;
        while ($row = mysql_fetch_assoc($res)) {
            ...
            $sql2           = sprintf(
                "SELECT * FROM `Email Content Paragraph Dimension` WHERE `Email Content Key`=%d ORDER BY `Paragraph Order`", $row['Email Content Key']
            );
            ...
            $sql  = sprintf(
                "SELECT * FROM `Email Template Color Scheme Dimension` WHERE `Email Template Color Scheme Key`=%d ", $row['Email Content Color Scheme Key']
            );
            ...
            $sql  = sprintf(
                "SELECT * FROM `Email Template Historic Color Scheme Dimension` WHERE `Email Template Historic Color Scheme Key`=%d ", $row['Email Content Color Scheme Historic Key']
            );
            $res2 = mysql_query($sql);
            if ($row2 = mysql_fetch_assoc($res2)) {
            ...
            if ($row['Email Template Header Image Key']) {
                $sql = sprintf(
                    "SELECT `Image Key` FROM `Email Template Header Image Dimension` WHERE `Email Template Header Image Key`=%d ", $row['Email Template Header Image Key']
                );
            ...
            if ($row['Email Content Template Postcard Key']) {
                $sql  = sprintf(
                    "SELECT `Image Key` FROM `Email Template Postcard Dimension` WHERE `Email Template Postcard Key`=%d ", $row['Email Content Template Postcard Key']
                );
                $res2 = mysql_query($sql);
                if ($row2 = mysql_fetch_assoc($res2)) {
    ...
    function get_content_data_keys() {
        $sql          = sprintf(
            "SELECT `Email Content Key` FROM `Email Campaign Content Bridge` WHERE `Email Campaign Key`=%d", $this->id
        );
        $content_keys = array();
        $result       = mysql_query($sql);
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            $content_keys[$row['Email Content Key']]
                = $row['Email Content Key'];
        }
    ...
    function create($raw_data) {
        ...
        $data['Email Campaign Last Updated Date']
                                              = $data['Email Campaign Creation Date'];
        ...
        $sql    = sprintf(
            "INSERT INTO `Email Campaign Dimension` %s %s", $keys, $values
        );
        ...
        if (mysql_query($sql)) {
            ...
            $sql = sprintf(
                "INSERT INTO `Email Content Dimension` (`Email Content Type`,`Email Content Subject`,`Email Content Text`,`Email Content HTML`) VALUES (%s,'','','')",
            ...
            $paragraph_data = array(
                array(
                    'type'     => 'Main',
                    'title'    => 'Donec eleifend nunc ut libero fringilla posuere',
                    'subtitle' => 'Duis mauris massa',
                    'content'  => 'Lorem ipsum dolor sit amet, consectetur adipiscing elit. Quisque pretium sapien nec augue dictum tincidunt. Phasellus in vulputate nibh. Morbi ac odio lorem. Suspendisse ut nibh vel nibh malesuada ullamcorper vitae sed magna. Aliquam erat volutpat.'
                ),
                array(
                    'type'     => 'Main',
                    'title'    => 'Nullam interdum posuere ultricies',
                    'subtitle' => 'In sagittis augue tellus',
                    'content'  => 'Morbi porttitor posuere venenatis. Aliquam tincidunt scelerisque porttitor. Vivamus vulputate tortor ut augue eleifend semper. Curabitur venenatis placerat porta. Aliquam semper magna vitae libero porttitor vulputate.'
                ),
                array(
                    'type'     => 'Side',
                    'title'    => 'Pellentesque sed sapien',
                    'subtitle' => 'Aliquam urna dui',
                    'content'  => 'Quisque in purus eu purus malesuada porttitor. Proin sed arcu nisi. Ut in enim arcu. Cras consectetur commodo dolor, id tempus tortor imperdiet quis. Donec iaculis interdum congue. Nullam ultrices hendrerit lectus, vitae lobortis magna sagittis et.'
                )

            );

            foreach (
                $paragraph_data as $_paragraph_data_key => $_paragraph_data
            ) {

                $sql = sprintf(
                    "INSERT INTO `Email Content Paragraph Dimension` (
                    ...
                    $email_content_key, $_paragraph_data_key, prepare_mysql('Main'), prepare_mysql($_paragraph_data['title']), prepare_mysql($_paragraph_data['subtitle']),
            ...
            $sql = sprintf(
                "INSERT INTO `Email Campaign Content Bridge`  VALUES (%d,%d)", $this->id, $email_content_key
            );
            mysql_query($sql);
            $email_content_key = mysql_insert_id();
            $this->get_data('id', $this->id);
    ...
    function delete_paragraph($email_content_key, $paragraph_key) {
        $sql = sprintf(
            "DELETE FROM  `Email Content Paragraph Dimension` WHERE `Email Paragraph Key`=%d ", $paragraph_key
        );
        mysql_query($sql);
        // print "$sql";
        if (mysql_affected_rows()) {
    ...
    function get_content_type() {
        ...
        $types = array();
        $sql   = sprintf(
            "SELECT `Email Content Type` FROM  `Email Content Dimension` C  LEFT JOIN `Email Campaign Content Bridge` B ON (B.`Email Content Key`=C.`Email Content Key`) WHERE `Email Campaign Key`=%d ",
            $this->id
        );
        $res   = mysql_query($sql);
        while ($row = mysql_fetch_assoc($res)) {
    ...
    function get_content_html($email_content_key) {
        $content_html = '';
        $sql          = sprintf(
            "SELECT `Email Content HTML` FROM  `Email Content Dimension`  WHERE `Email Content Key`=%d", $email_content_key
        );
        $res          = mysql_query($sql);
        if ($row = mysql_fetch_assoc($res)) {
    ...
    function get_templete_html($html_data, $email_mailing_list_key = false, $_email_content_key = false, $color_scheme_key = false) {
        ...
        if (isset($color_scheme_key) and is_numeric($color_scheme_key)) {
            $color_scheme = array();
            $sql          = sprintf(
                "SELECT * FROM `Email Template Color Scheme Dimension` WHERE `Email Template Color Scheme Key`=%d ", $_REQUEST['color_scheme_key']
            );
            $res2         = mysql_query($sql);
            if ($row2 = mysql_fetch_assoc($res2)) {
        ...
        if (preg_match_all('/\%[a-z]+\%/', $output, $matches)) {
            foreach ($matches[0] as $match) {
                $output = preg_replace(
                    '/'.$match.'/', $customer->get(preg_replace('/\%/', '', $match)), $output
                );
            }
        }
    ...
    function get_subject($email_content_key) {
        $subject = '';
        $sql     = sprintf(
            "SELECT `Email Content Subject` FROM  `Email Content Dimension`  WHERE `Email Content Key`=%d", $email_content_key
        );
        $res     = mysql_query($sql);
        if ($row = mysql_fetch_assoc($res)) {
            $subject = $row['Email Content Subject'];
    ...
    function get_template_type($email_content_key) {
        ...
        $template_type = '';
        $sql           = sprintf(
            "SELECT `Email Content Template Type` FROM  `Email Content Dimension`  WHERE `Email Content Key`=%d", $email_content_key
        );
    ...
    function get_color_scheme($email_content_key) {
        ...
        $color_scheme = '';
        $sql          = sprintf(
            "SELECT `Email Content Color Scheme Key` FROM  `Email Content Dimension`  WHERE `Email Content Key`=%d", $email_content_key
        );
    ...
    function move_paragraph_before_target($email_content_key, $paragraph_key, $target_key, $paragraph_type, $change_original = true) {
        ...
        if ($target_key == 0) {
            ...
            return $this->move_paragraph_to_the_end(
                $email_content_key, $paragraph_key, $paragraph_type, $change_original
            );
        }

        $sql = sprintf(
            "SELECT `Paragraph Type` FROM `Email Content Paragraph Dimension` WHERE `Email Content Key`=%d  AND `Email Paragraph Key`=%d   ", $email_content_key, $target_key
        );
        $res = mysql_query($sql);
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            $sql = sprintf(
                "SELECT `Email Paragraph Key`,`Paragraph Order` FROM `Email Content Paragraph Dimension` WHERE `Email Content Key`=%d AND `Paragraph Type`=%s ORDER BY `Paragraph Order`",
                $email_content_key, prepare_mysql($paragraph_type)
            ...
            foreach ($new_order as $_paragraph_key => $paragraph_order) {
                $sql = sprintf(
                    "UPDATE `Email Content Paragraph Dimension`  SET `Paragraph Type`=%s , `Paragraph Order`=%d WHERE `Email Paragraph Key`=%d ",

                    prepare_mysql($paragraph_type), $paragraph_order, $_paragraph_key
                );
                mysql_query($sql);
                if ($change_original) {
                    $sql = sprintf(
                        "UPDATE `Email Content Paragraph Dimension`  SET `Paragraph Original Type`=%s , `Paragraph Order`=%d WHERE `Email Paragraph Key`=%d ",
    ...
    function move_paragraph_to_the_end($email_content_key, $paragraph_key, $paragraph_type = 'Main', $change_original = true) {
        $sql = sprintf(
            "SELECT `Email Paragraph Key`,`Paragraph Order` FROM `Email Content Paragraph Dimension` WHERE `Email Content Key`=%d AND `Paragraph Type`=%s ORDER BY `Paragraph Order` DESC LIMIT 1",
            $email_content_key, prepare_mysql($paragraph_type)
        ...
        if ($last_paragraph_key != $paragraph_key) {
            ...
            $sql = sprintf(
                "UPDATE `Email Content Paragraph Dimension`  SET `Paragraph Order`=%d , `Paragraph Type`=%s WHERE `Email Paragraph Key`=%d ", $last_order_index, prepare_mysql($paragraph_type),
                $paragraph_key
            );
            ...
            if ($change_original) {
                $sql = sprintf(
                    "UPDATE `Email Content Paragraph Dimension`  SET  `Paragraph Original Type`=%s WHERE `Email Paragraph Key`=%d ", prepare_mysql($paragraph_type), $paragraph_key
                );
    ...
    function update_content_text($value, $email_content_key) {
        $sql = sprintf(
            "UPDATE `Email Content Dimension` SET `Email Content Text`=%s WHERE `Email Content Key`=%d", prepare_mysql($value), $email_content_key
        );
        mysql_query($sql);
    ...
    function get_content_text($email_content_key) {
        $content_text = '';
        $sql          = sprintf(
            "SELECT `Email Content Text` FROM  `Email Content Dimension`  WHERE `Email Content Key`=%d", $email_content_key
        );
        $res          = mysql_query($sql);
        if ($row = mysql_fetch_assoc($res)) {
    ...
    function update_subject($value, $email_content_key) {
        ...
        $sql = sprintf(
            "UPDATE `Email Content Dimension` SET `Email Content Subject`=%s WHERE `Email Content Key`=%d", prepare_mysql($value), $email_content_key
        );
        mysql_query($sql);
    ...
    function update_content($email_content_key, $key, $value) {
        ...
        $valid_keys = array(
            'Email Content Type',
            'Email Content Template Type',
            'Email Content Color Scheme Key',
            'Email Template Header Image Key',
            'Email Content Template Postcard Key'
        );
        if (in_array($key, $valid_keys)) {

            $sql = sprintf(
                "SELECT `%s` AS old_value FROM  `Email Content Dimension`  WHERE `Email Content Key`=%d ", $key, $email_content_key
            );
            mysql_query($sql);
            $res = mysql_query($sql);
            if ($row = mysql_fetch_assoc($res)) {
            ...
            $sql = sprintf(
                "UPDATE `Email Content Dimension` SET `%s`=%s  WHERE `Email Content Key`=%d ", $key, prepare_mysql($value), $email_content_key
            );
            mysql_query($sql);
            if (mysql_affected_rows()) {
                ...
                if ($key == 'Email Content Template Type') {
                    ...
                    if ($value == 'Basic' or $value == 'Postcard') {
                        ...
                        $sql = sprintf(
                            "SELECT `Email Paragraph Key` FROM  `Email Content Paragraph Dimension`  WHERE `Paragraph Type`='Side' AND `Email Content Key`=%d ",
                        ...
                        while ($row = mysql_fetch_assoc($res)) {
                            $sql = sprintf(
                                "UPDATE `Email Content Paragraph Dimension`  SET `Paragraph Type`='Main' WHERE `Email Paragraph Key=%d ", $row['Email Paragraph Key']
                            );
                            mysql_query($sql);
                            //  print $sql;
                            $this->move_paragraph_to_the_end(
                                $email_content_key, $row['Email Paragraph Key'], 'Main', $change_original = false
                            );
                        }
                        ...
                        $sql = sprintf(
                            "SELECT `Email Paragraph Key` FROM  `Email Content Paragraph Dimension`  WHERE `Paragraph Original Type`='Side' AND `Paragraph Type`='Main' AND `Email Content Key`=%d ",
                            $email_content_key
                        );
                        mysql_query($sql);
                        $res = mysql_query($sql);
                        while ($row = mysql_fetch_assoc($res)) {
                            $sql = sprintf(
                                "UPDATE `Email Content Paragraph Dimension`  SET `Paragraph Type`='Side' WHERE `Email Paragraph Key=%d ", $row['Email Paragraph Key']
                            );
                            mysql_query($sql);
                            $this->move_paragraph_to_the_end(
                                $email_content_key, $row['Email Paragraph Key'], 'Side', $change_original = false
                            );
                        }
                    ...
                    $this->data['Email Campaign Content Type']
                         = $this->get_content_type();
                    $sql = sprintf(
                        "UPDATE `Email Campaign Dimension` SET `Email Campaign Content Type`=%s WHERE `Email Campaign Key`=%d", prepare_mysql(
                            $this->data['Email Campaign Content Type']
                        ), $this->id
                    );
                    mysql_query($sql);
                    ...
                    $this->update_content_from_other_type(
                        $email_content_key, $old_value, $value
                    );
    ...
    function update_content_from_other_type($email_content_key, $old_type, $new_type) {
        ...
        $content = $this->get_content_from_other_type(
            $email_content_key, $old_type, $new_type
        );
        ...
        switch ($new_type) {
            ...
            case 'HTML Template':
                $sql = sprintf(
                    "DELETE FROM  `Email Content Paragraph Dimension` WHERE `Email Content Key`=%d ", $email_content_key
                );
                mysql_query($sql);
    ...
    function get_content_from_other_type($email_content_key, $old_type, $new_type) {
        ...
        switch ($old_type) {
            ...
            case 'HTML Template':
                ...
                foreach (
                    $this->content_data[$email_content_key]['paragraphs'] as $paragraph_data
                ) {
    ...
    function update_content_html($value, $email_content_key) {
        ...
        $sql = sprintf(
            "UPDATE `Email Content Dimension` SET `Email Content HTML`=%s WHERE `Email Content Key`=%d", prepare_mysql($value), $email_content_key
        );
        mysql_query($sql);
    ...
    function update_links($email_content_key) {
        ...
        $sql = sprintf(
            "DELETE FROM `Email Campaign Objective Dimension` WHERE `Email Campaign Objective Type`='Link' AND `Email Campaign Key`=%d ", $this->id
        );
        mysql_query($sql);
        ...
        foreach ($links as $url => $link_data) {
            ...
            if (!$page->id or $page->data['Page Type'] != 'Store' or $page->data['Page Store Key'] != $this->data['Email Campaign Store Key']) {
                ...
                if (preg_match('/\/$/i', $url)) {
                    ...
                    $alternative_urls[] = preg_replace(
                        '/index\.(php|html|asp)$/i', '', $url
                    );
                } else {
                    $alternative_urls[] = $url.'/index.html';
                    $alternative_urls[] = $url.'/index.php';
    ...
    function add_paragraph($email_content_key, $paragraph_data) {
        ...
        $sql              = sprintf(
            "SELECT `Email Paragraph Key`,`Paragraph Order` FROM `Email Content Paragraph Dimension` WHERE `Email Content Key`=%d ORDER BY `Paragraph Order` DESC LIMIT 1", $email_content_key
        );
        $res              = mysql_query($sql);
        $last_order_index = 1;
        if ($row = mysql_fetch_assoc($res)) {
        ...
        if ($paragraph_data['title'] or $paragraph_data['subtitle'] or $paragraph_data['content']) {
            ...
            $sql = sprintf(
                ...
                $email_content_key, $last_order_index, prepare_mysql($paragraph_data['type']), prepare_mysql($paragraph_data['title'], false), prepare_mysql($paragraph_data['subtitle'], false),
    ...
    function update_paragraph($email_content_key, $paragraph_key, $data) {
        ...
        $sql = sprintf(
            "UPDATE `Email Content Paragraph Dimension` SET `Paragraph Title`=%s,`Paragraph Subtitle`=%s,`Paragraph Content`=%s WHERE `Email Paragraph Key`=%d ", prepare_mysql($data['title']),
            prepare_mysql($data['subtitle']), prepare_mysql($data['content']), $paragraph_key
        );
        mysql_query($sql);
        //print_r($sql);
        if (mysql_affected_rows()) {
