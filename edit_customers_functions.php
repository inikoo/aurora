			...
			if (preg_match('/^Cat\d+$/i',$data_key)) {
				...
				if (is_numeric( $category_key) ) {
					...
					if ($category->id) {
						...
						if ($category->data['Is Category Field Other']=='Yes' and array_key_exists('Cat'.$parent_category_key.'_Other_Value',$data)) {
							$category_other_value=$data['Cat'.$parent_category_key.'_Other_Value'];
						}else {
