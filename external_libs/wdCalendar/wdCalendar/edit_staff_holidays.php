...
function getCalendarByRange($id){
  try{
   // $db = new DBConnection();
   // $db->getConnection();
    $sql = "select * from `Staff Event Dimension` where `Staff Event Key` = " . $id;
...
<html xmlns="http://www.w3.org/1999/xhtml" >
  <head>    
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            //debugger;
            var DATA_FEED_URL = "php/datafeed_staff_holidays.php";
            var arrT = [];
            var tt = "{0}:{1}";
            for (var i = 0; i < 24; i++) {
  ...
  <body>    
   <a href="abc.php">   <div>      
      ...
      <div class="infocontainer"> <?php $staff_event_key="Staff Event Key"; ?>                     
        <form action="php/datafeed_staff_holidays.php?method=adddetails<?php echo isset($event)?"&id=".$event->$staff_event_key:""; ?>" class="fform" id="fmEdit" method="post">                 
          <label>                    
            <span>                        *Subject:              
            </span>                    
 ...
 <label><span>Staff Id:</span>                    
            <input MaxLength="200" id="StaffKey" name="StaffKey" style="width:95%;" type="text" value="<?php echo isset($event)?$event->$staff_key:""; ?>" />                 
          </label> 
