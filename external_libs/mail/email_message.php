    ...
    Function GetPartHeaders(&$headers, $part) {
        ...
        switch ($type) {
            ...
            case "message":
                ...
                $headers["Content-Type"] = $full_type.(IsSet($this->parts[$part]["CHARSET"]) ? "; charset=".$this->parts[$part]["CHARSET"] : "").(IsSet($this->parts[$part]["NAME"]) ? "; name=\""
                        .$filename."\"" : "");
                ...
                if (IsSet($this->parts[$part]["FILENAME"])) {
                    ...
                    if (GetType($size) == "integer"
                        ...
                        return ($this->OutputError(
                            "the length of the file that was read does not match the size of the part file ".$this->parts[$part]["FILENAME"]." due to possible data corruption"
                        ));
                    }
                } else {
...
*/
    ...
    Function EndSendingMessage() {
        ...
        if (strcmp(
            $error = $this->SendMail(
                strlen($has["to"]) ? $has["to"] : (IsSet($has["cc"]) ? "" : "undisclosed-recipients: ;"), $has["subject"], $this->delivery["Body"], $headers_text,
                IsSet($has["return-path"]) ? $has["return-path"] : ""
            ), ""
        )) {
            return ($error);
        }
...
*/
    ...
    Function GenerateMessageID($sender) {
        ...
        return ($this->FormatHeader(
            "Message-ID", "<".strftime("%Y%m%d%H%M%S", $seconds).substr($micros, 1, 5).".".preg_replace('/[^A-Za-z]/', '-', $local)."@".preg_replace('/[^.A-Za-z_-]/', '', $host).">"
        ));
    }
