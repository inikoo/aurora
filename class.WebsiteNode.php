...
class WebsiteNode extends DB_Table {
    ...
    function get_data($key, $tag, $tag2 = false) {
        ...
        if ($key == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Website Node Dimension` WHERE `Website Node Key`=%d", $tag
            );
        } else {
            return;
        }
        ...
        if ($this->data = $this->db->query($sql)->fetch()) {
            ...
            $this->webpage = new Webpage(
                $this->get('Website Node Webpage Key')
            );
...
*/
        ...
        if ($this->db->exec($sql)) {
           ...
           // print "=============";
            ...
            if (!$this->error) {
                ...
                $this->update(
                    array('Website Node Webpage Key' => $webpage->id), 'no_history'
                );
            ...
            if ($webpage->new) {
                ...
                if ($webpage->found) {
                    ...
                    $this->error_metadata = json_encode(
                        array($webpage->duplicated_field)
                    );
    ...
    function get_webpage_key_delete() {
        ...
        $webpages = array();
        $sql      = sprintf(
            'SELECT `Webpage Key`,`Webpage Display Probability` FROM `Webpage Dimension` WHERE `Webpage Website Node Key`=%d AND `Webpage Display Probability`>0', $this->id
        );
    ...
    function update_webpages() {
        $sql             = sprintf(
            'SELECT count(*) AS number FROM `Webpage Dimension` WHERE `Webpage Website Node Key`=%d', $this->id
        );
        $number_webpages = 0;
        ...
        $this->update(
            array('Website Node Number Pages' => $number_webpages), 'no_history'
        );
    ...
    function get_number_webpages() {
        ...
        $sql = sprintf(
            'SELECT count(*) AS num FROM `Webpage Dimension` WHERE `Webpage Website Node Key`=%d', $this->id
        );
