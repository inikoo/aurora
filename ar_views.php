...
case 'views':
	...
	if ($data['old_state']['section']!=$state['section']
	...
	$response['table']=get_table($state);
...
function get_table($data) {
	...
	$html='table'.$data['module'].' '.$data['module'];
	switch ($data['module']) {
	case 'customers':
		switch ($data['section']) {
		case 'customers':
			
			$html=$smarty->fetch('table_customers.tpl');
			break;
		default:
...
if (isset($tabs[$data['tab']]) )$tabs[$data['tab']]['selected']=true;
