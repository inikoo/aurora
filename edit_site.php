...
if (isset($_REQUEST['pages_block_view'])) {
	$valid_views=array('pages','flags');
	if (in_array($_REQUEST['pages_block_view'], $valid_views))
		$_SESSION['state']['site']['editing_pages']=$_REQUEST['pages_block_view'];
...
$smarty->assign('pages_block_view',$_SESSION['state']['site']['editing_pages']);
...
$flags=array();
$sql=sprintf("select `Site Flag Number Pages` as pages ,`Site Flag Key` as id ,`Site Flag Color` as color, `Site Flag Label`as  label ,`Site Flag Active` as display from `Site Flag Dimension` where `Site Key`=%d ",$site->id);
$res=mysql_query($sql);
while ($row=mysql_fetch_assoc($res)) {
	$row['icon']='flag_'.strtolower($row['color']).'.png';
	$row['pages']=number($row['pages']);
	$row['default']=($site->data['Site Default Flag Color']==$row['color']?1:0);
	$flags[]=$row;
}
