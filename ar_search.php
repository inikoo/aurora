...
function search_locations($db, $account, $memcache_ip, $data) {
	...
	if ($data['scope']=='warehouse') {
		if (in_array($data['scope_key'], $user->stores)) {
			...
			$where_warehouse=sprintf(' and `Location Warehouse Key`=%d', $data['scope_key']);
		}else {
		...
		if (count($user->stores)==$account->data['Warehouses']) {
			...
			$where_warehouse=sprintf(' and `Location Warehouse Key` in (%s)', join(',', $user->stores));
		}
	...
	if (!$results_data or $cache) {
		...
		$sql=sprintf("select `Location Key`,`Location Code` from `Location Dimension` where true $where_warehouse and `Location Code` like '%s%%' limit 20 ",
			$q);
		...
		$sql=sprintf("select `Location Code`,`Location Warehouse Key`,`Location Key`, `Warehouse Code` from `Location Dimension` left join `Warehouse Dimension` on (`Location Warehouse Key`=`Warehouse Key`) where `Location Key` in (%s)",
			$customer_keys);
