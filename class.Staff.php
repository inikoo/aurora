...
class Staff extends DB_Table{
	...
	function get_deleted_data( $tag) {
		...
		$this->deleted=true;
		$sql=sprintf("select * from `Staff Deleted Dimension` where `Staff Deleted Key`=%d", $tag);

		if ($this->data = $this->db->query($sql)->fetch()) {
			$this->id=$this->data['Staff Deleted Key'];
			$deleted_data=json_decode(gzuncompress($this->data['Staff Deleted Metadata']), true);
	...
	function get($key) {
		...
		case ('Deleted Date'):
			...
			return $this->data['Staff '.$key]==''  ?'':strftime("%a %e %b %Y %H:%M %Z", strtotime($this->data['Staff '.$key]));
	...
	function create($data) {
		...
		if ($this->db->exec($sql)) {
			...
			if ($this->get('Staff Type')=='Contractor') {
				$abstract=sprintf(_('Contractor %s created'), sprintf('<span class="button" onClick="change_view(\'contractor/%d\')">%s</span>', $this->id, $this->get('Alias')));
			}else {
				$abstract=sprintf(_('Employee %s created'), sprintf('<span class="button" onClick="change_view(\'employee/%d\')">%s</span>', $this->id, $this->get('Alias')));
			...
			$sql=sprintf('insert into `HR History Bridge`  (`HR Key`,`History Key`,`Type`) values (1,%d,"Changes")  ', $history_key);
			$this->db->exec($sql);
	...
	function delete() {
		...
		$sql=sprintf("insert into `Staff Deleted Dimension`  (`Staff Deleted Key`,`Staff Deleted Type`,`Staff Deleted ID`,`Staff Deleted Alias`,`Staff Deleted Name`,`Staff Deleted Date`,`Staff Deleted Metadata`) values (%d,%s,%s,%s,%s,%s,%s) ",
			$this->id,
			prepare_mysql(($this->get('Staff Type')=='Contractor'?'Contractor':'Employee')),
		...
		$sql=sprintf("select `Image Subject Key`,`Image Subject Image Key` from `Image Subject Bridge` where `Image Subject Object`='Staff' and `Image Subject Object Key`=%d",
			$this->id
		);
		if ($result=$this->db->query($sql)) {
			foreach ($result as $row) {
		...
		if ($this->get('Staff Type')=='Contractor') {
			$abstract=sprintf(_('Contractor %s deleted'), sprintf('<span class="button" onClick="change_view(\'contractor/%d\')">%s</span>', $this->id, $this->get('Alias')));
		}else {
			$abstract=sprintf(_('Employee %s deleted'), sprintf('<span class="button" onClick="change_view(\'employee/%d\')">%s</span>', $this->id, $this->get('Alias')));
