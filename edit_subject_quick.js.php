...
var dialog_quick_edit_Subject_Name;
var dialog_quick_edit_Subject_Main_Email;
var dialog_quick_edit_Subject_Main_Address;
var dialog_quick_edit_Subject_Main_Telephone;
var dialog_quick_edit_Subject_Main_Mobile;
var dialog_quick_edit_Subject_Website;
var dialog_quick_edit_Subject_Main_FAX;
...
function validate_subject_telephone(query){
    validate_general('quick_telephone','Subject_Main_Telephone',unescape(query));
    if(query==''){
        validate_scope_data.subject_quick.telephone.validated=true;
	    validate_scope('quick_telephone'); 
	    if(Dom.get('Subject_Main_Telephone').getAttribute('ovalue')!='')
	    Dom.get(validate_scope_data.subject_quick.telephone.name+'_msg').innerHTML='<?php echo _('Will remove the telephone')?>';
    }
}
...
function show_dialog_quick_edit_Subject_Main_Telephone(){
...
function save_edit_quick_telephone(telephone_key){
	
    save_edit_general_bulk('quick_telephone');
}
...
'quick_telephone':{
'Subject_Main_Telephone':{'changed':false,'validated':true,'required':false,'group':1,'type':'item','name':'Subject_Main_Telephone','validation':[{'regexp':regex_valid_tel,'invalid_msg':'<?php echo _('Invalid Telephone')?>'}]},
'Subject_Main_Telephone_comment':{'changed':false,'validated':true,'required':false,'group':1,'type':'item','name':'Subject_Main_Telephone_comment','validation':false}
},
...
'subject_quick':{'type':'edit','ar_file':'ar_edit_subjects.php','subject':Dom.get('subject').value,'key':Dom.get('subject_key').value}
,'quick_telephone':{'type':'edit','ar_file':'ar_edit_subjects.php','subject':Dom.get('subject').value,'key':Dom.get('subject_key').value}
...
Event.addListener('quick_edit_main_telephone', "click", show_dialog_quick_edit_Subject_Main_Telephone);

Event.addListener('save_edit_quick_telephone', "click", save_edit_quick_telephone, true);


Event.addListener('close_edit_quick_telephone', "click", dialog_quick_edit_Subject_Main_Telephone.hide,dialog_quick_edit_Subject_Main_Telephone , true);

 var subject_telephone_oACDS = new YAHOO.util.FunctionDataSource(validate_subject_telephone);
    subject_telephone_oACDS.queryMatchContains = true;
    var subject_telephone_oAutoComp = new YAHOO.widget.AutoComplete("Subject_Main_Telephone","Subject_Main_Telephone_Container", subject_telephone_oACDS);
    subject_telephone_oAutoComp.minQueryLength = 0; 
    subject_telephone_oAutoComp.queryDelay = 0.1;
