...
trait SupplierCategory {
    ...
    function update_supplier_category_sales($interval, $this_year = true, $last_year = true) {
        ...
        if ($this_year) {
            $sales_data = $this->get_supplier_category_sales_data(
                $from_date, $to_date
            );
    ...
    function update_supplier_category_previous_quarters_data() {
        ...
        foreach (range(1, 4) as $i) {
            ...
            $sales_data_1yb = $this->get_supplier_category_sales_data(
    ...
    function update_supplier_category_parts() {
        ...
        if ($parts_skus != '') {
            ...
            if ($parts_skus != '') {
                ...
                $sql = sprintf(
                    'SELECT count(*) AS num , sum(if(`Part Stock Status`="Surplus",1,0)) AS surplus, sum(if(`Part Stock Status`="Optimal",1,0)) AS optimal, sum(if(`Part Stock Status`="Low",1,0)) AS low, sum(if(`Part Stock Status`="Critical",1,0)) AS critical, sum(if(`Part Stock Status`="Out_Of_Stock",1,0)) AS out_of_stock FROM `Supplier Part Dimension` SP  LEFT JOIN `Part Dimension` P ON (P.`Part SKU`=SP.`Supplier Part Part SKU`)  WHERE `Supplier Part Part SKU` IN (%s) ',
