...
class part extends DB_Table {
	...
	function get_materials() {
		...
		$sql=sprintf("select * from `Part Material Bridge` B left join `Material Dimension` MD on (MD.`Material Key`=B.`Material Key`) where `Part SKU`=%d order by `Ratio` desc,`Part Material Key` ",
			$this->sku
	...
	function update_materials($value) {
		...
		foreach ($materials as $key=>$_value) {
			...
			if ($material->id) {
				$sql=sprintf("insert into `Part Material Bridge` (`Part SKU`,`Material Key`,`Ratio`) values (%d,%d,%s) ",
					$this->sku,
					$material->id,
