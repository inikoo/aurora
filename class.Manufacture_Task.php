...
class Manufacture_Task extends DB_Table {
    ...
    function get_data($tipo, $tag) {
        ...
        if ($tipo == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Manufacture Task Dimension` WHERE `Manufacture Task Key`=%d", $tag
            );
        } else {
            return;
        }
    ...
    function find($raw_data, $options) {
        ...
        $sql = sprintf(
            "SELECT `Manufacture Task Key` FROM `Manufacture Task Dimension` WHERE `Manufacture Task Name`=%s", prepare_mysql($data['Manufacture Task Name'])
        );
    ...
    function create($data) {
        ...
        foreach ($this->data as $key => $value) {
            ...
            if (in_array(
                $key, array(
                    ...
                    'Manufacture Task Lower Target',
                    'Manufacture Task Upper Target'
                )
            )) {
        ...
        $sql
            = "insert into `Manufacture Task Dimension` ($keys) values ($values)";
    ...
    function get($key = '') {
        ...
        switch ($key) {
            case 'Targets':
                ...
                return _('Lower').': <span title="'.$this->get('Lower Target').'">'.$this->get('Lower Target Per Hour').'</span><br>'._('Upper').': <span title="'.$this->get('Upper Target').'">'
                .$this->get('Upper Target Per Hour').'</span>';
            ...
            case 'Operative Reward Amount':
                if ($this->data['Manufacture Task '.$key] == '') {
                    return '';
                }
