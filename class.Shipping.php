...
class Shipping extends DB_Table {
    ...
    function get_data($tipo, $tag) {
        ...
        if ($tipo == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Shipping Dimension` WHERE `Shipping Key`=%d", $tag
            );
        }
        //    elseif($tipo=='code')
    ...
    function find($raw_data, $options) {
        ...
        if (count($raw_data) == 1) {
            ...
            foreach ($raw_data as $key => $value) {
                if (preg_match(
                    '/^(Country Name|Country Code|Country)$/i', $key
                )) {
                    ...
                    $sql = sprintf(
                        "SELECT `Shipping Key`  FROM `Shipping Dimension` WHERE `Shipping Destination Type`='Country' AND `Shipping Destination Code`=%s AND (`Shipping Begin Date` IS NULL OR `Shipping Begin Date`<=%s )  AND (`Shipping Expiration Date` IS NULL OR `Shipping Expiration Date`>=%s )  ",
            ...
            $sql
                = "SELECT `Shipping Key` FROM `Shipping Dimension` WHERE  TRUE ";
            //print_r($fields);
    ...
    function create($data) {
        ...
        $sql    = sprintf(
            "INSERT INTO `Shipping Dimension` %s %s", $keys, $values
        );
        //   print "$sql\n";
        if (mysql_query($sql)) {
        ...
        switch ($key) {
            case('Country Name'):
                if ($this->data['Shipping Destination Type'] == 'Country') {
                    $country = new Country (
                        'code', $this->data['Shipping Destination Code']
                    );
