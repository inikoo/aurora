	    	...
	    	oOverlay.css({
   			...
   			$this.click(function(){
				...
				o = $.extend(o,{oImgZoomBox:oImgZoomBox});
   				...
   				var strTitle = imgTarget.attr('title');
   				if(strTitle==''){
   				strTitle = imgTarget.attr('alt');
   				}
				...
				//be shure that the image to display is loaded open the zoom box, if not display a loading Image.
   				...
   				var $fctEndLoading = function(){
					if(bCancelLoading) {bCancelLoading=false;}
						...
						if(__getFileName(imgPreload.src).trim() == __getFileName($('img:first',oImgZoomBox).attr('src')).trim() ){
