...
YAHOO.util.Event.addListener(window, "load", function() {
    YAHOO.suppliers.XHR_JSON = new function() {
	    ...
	    this.SuppliersDataSource = new YAHOO.util.DataSource("ar_suppliers.php?tipo=index&tid="+tableid);
	    ...
	    this.SuppliersDataTable = new YAHOO.widget.DataTable(tableDivEL, SuppliersColumnDefs,
								   this.SuppliersDataSource, {
								     sortedBy: {key:"<?=$_SESSION['tables']['suppliers_list'][0]?>", dir:"<?=$_SESSION['tables']['suppliers_list'][1]?>"}
								       ,renderLoopSize: 50
								   }
								   );
	...
	this.SuppliersDataTable.filterMenu = new YAHOO.widget.Menu('filternewmenu'+tableid,  {context:['filterselector'+tableid,"tr", "br"]  });
 	this.SuppliersDataTable.filterMenu.addItems([{ text: "<?=_('Code')?>", onclick:{fn:changeFilter,obj:{col:'code',text:"<?=_('Code')?>"},scope:this.SuppliersDataTable}  } ]);
 	this.SuppliersDataTable.filterMenu.addItems([{ text: "<?=_('Name')?>", onclick:{fn:changeFilter,obj:{col:'name',text:"<?=_('Name')?>"},scope:this.SuppliersDataTable}  } ]);
 	this.SuppliersDataTable.filterMenu.addItems([{ text: "<?=_('Low Stock')?>", onclick:{fn:changeFilter,obj:{col:'low',text:"<?=_('Low Stock')?>"},scope:this.SuppliersDataTable}  } ]);
 	this.SuppliersDataTable.filterMenu.addItems([{ text: "<?=_('Out of Stock')?>", onclick:{fn:changeFilter,obj:{col:'outofstock',text:"<?=_('Out of Stock')?>"},scope:this.SuppliersDataTable}  } ]);
...
function init(){
 ...
 function mygetTerms(query) {
   ...
   var Dom = YAHOO.util.Dom;
	...
	var table=YAHOO.suppliers.XHR_JSON.SuppliersDataTable;
	table.myreload();
...
// 	data.sendRequest(newrequest,{success:table.onDataReturnInitializeTable, scope:table});
	...
	var handleSuccess = function(o) {
	    ...
	    var response = YAHOO.lang.JSON.parse(o.responseText);
		 ...
		 // relocate to new supplier to add telephone and another stodd

		 location.href="supplier.php?id="+response.supplier_id
