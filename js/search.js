...
var submit_search_on_enter = function(e, tipo) {
        var key;
        if (window.YAHOO.util.Event) key = window.YAHOO.util.Event.keyCode; //IE
        else key = e.which; //firefox
        if (key == 13) submit_search(e, tipo);
    };
...
function init_search(type) {
    ...
    case 'sites':
        subject = 'sites';
        search_scope = '';
        var store_name_oACDS = new YAHOO.util.FunctionDataSource(search_sites);
        break;
    case 'products_store':
        ...
        YAHOO.util.Connect.asyncRequest('POST', request, {
            success: function(o) {
               // 		alert(o.responseText)
                ...
                } else Dom.get(data.container + '_search_msg').innerHTML = r.msg;
            }
        });
    }
...
/*
                ...
                if (r.results > 0) {
                    ...
                    for (result_key in r.data) {
                        ...
                        }else if (subject == 'sites') {
                            oTR.setAttribute('key', r.data[result_key].key);
                            oTR.setAttribute('link', r.data[result_key].link);


 var oTD = oTR.insertCell(1);
                            oTD.innerHTML = r.data[result_key].site_code;
                            var oTD = oTR.insertCell(2);
                            oTD.innerHTML = r.data[result_key].code;
                            var oTD = oTR.insertCell(3);
                            oTD.innerHTML = r.data[result_key].name;
                        ...
                        if (result_number == 0) oTR.setAttribute('prev', r.results - 1);
                        else oTR.setAttribute('prev', result_number - 1);
                        if (result_number == r.results - 1) oTR.setAttribute('next', 0);

                        else oTR.setAttribute('next', result_number + 1);
...
function search_events(e, subject) {
    ...
    if (key == 13) goto_search_result(subject);
    else if (key == 40) {
...
function goto_search_result(subject) {
    ...
    tr = elements_array[0];
    if (tr != undefined && tr.getAttribute('link')) location.href = tr.getAttribute('link') + tr.getAttribute('key');
