...
YAHOO.supplier.views[0]=<?=$_SESSION['tables']['po_item'][4][2]?>;
    YAHOO.supplier.po_id=<?=$_REQUEST['po_id']?>;
YAHOO.util.Event.addListener(window, "load", function() {
	YAHOO.supplier.XHR_JSON = new function() {
		...
		var SuppliersColumnDefs1 = [
					    ...
					    {key:"stock", label:"<?=_('Stock')?>",width:60,sortable:true,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}<?=($_SESSION['tables']['po_item'][4][3]==0?'':',hidden:true')?>}
					    ,{key:"code", label:"<?=_('Our Code')?>", width:<?=($_SESSION['tables']['po_item'][4][3]==1?90:90)?>,sortable:true,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_ASC}}
					   ...
					   ,{key:"description", label:"<?=_('Description')?>",width:320, formatter:this.description,sortable:true,className:"aleft",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_ASC}}
					   ,{key:"ordered", label:"<?=_('Ordered')?>",width:95, sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_ASC},editor:'textbox'
					     <?=($_SESSION['tables']['po_item'][4][3]==2?',hidden:true':'')?>}
					    ,{key:"received", label:"<?=_('Received')?>",width:70, sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_ASC},editor:'textbox'
					     <?=($_SESSION['tables']['po_item'][4][3]!=2?',hidden:true':'')?>}
					    ,{key:"damage", label:"<?=_('Damaged')?>",width:60, sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_ASC},editor:'textbox'
					      <?=(($_SESSION['tables']['po_item'][4][3]!=2 or $_SESSION['tables']['po_item'][4][3]!=1)?',hidden:true':'')?>}
					    ,{key:"eprice", label:"<?=_('Cost')?>",width:95, sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_ASC}}
...
var Dom   = YAHOO.util.Dom;
	...
	YAHOO.util.Connect.asyncRequest(
					...
					request,{
						...
						if (r.state == 200) {
						    ...
						    var tipo=<?=$_SESSION['tables']['po_item'][4][3]?>;
	...
	if(YAHOO.util.Dom.hasClass(elCell, "yui-dt-editable")) {
		...
		this.SuppliersDataTable1.filterMenu.addItems([{ text: "<?=_('Supplier Code')?>", onclick:{fn:changeFilter,obj:{col:'code',text:"<?=_('Family Code')?>"},scope:this.SuppliersDataTable1}  } ]);
		this.SuppliersDataTable1.filterMenu.addItems([{ text: "<?=_('Description')?>", onclick:{fn:changeFilter,obj:{col:'description',text:"<?=_('Description')?>"},scope:this.SuppliersDataTable1}  } ]);
...
YAHOO.util.Event.addListener("receiving", "click", receiving);
 ...
 YAHOO.supplier.dialog1  = new YAHOO.widget.Dialog("submiting_form",
						   { width : "20em",
						     ...
						     postmethod:"form",
						     buttons : [ { text:"<?=_('Submit')?>", handler:handleSubmit, isDefault:true },{ text:"<?=_('Cancel')?>", handler:handleCancel } ]
...
// YAHOO.supplier.cal3 = new YAHOO.widget.Calendar("cal3","cal3Container", { title:"<?=_('Choose a date')?>:", close:true } );
...
function myCalhandleSelect(type,args,obj) {
 ...
 YAHOO.supplier.cal2 = new YAHOO.widget.Calendar("cal2","cal2Container", { title:"<?=_('Choose a date')?>:", close:true } );
