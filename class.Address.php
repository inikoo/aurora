...
class _Address extends DB_Table {
    ...
    */
    ...
    function get_data($tipo, $id = false) {
        ...
        if ($tipo == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Address Dimension` WHERE  `Address Key`=%d", $id
            );
        } elseif ('tipo' == 'fuzzy country') {
            $sql = sprintf(
                "SELECT * FROM `Address Dimension` WHERE  `Address Fuzzy`='Yes' AND `Address Fuzzy Type`='country' AND `Address Country Key`=%d   ", $id
            );
        } else {
            $sql = sprintf(
                "SELECT * FROM `Address Dimension` WHERE  `Address Fuzzy`='Yes' AND `Address Fuzzy Type`='all' ", $id
            );
        }
    ...
    function prepare_data($raw_data, $options = '') {
        ...
        */
        ...
        switch ($data['Address Input Format']) {
            case('3 Line'):
                $data = $this->prepare_3line(
                    $data, 'untrusted', $this->get_default('Country 2 Alpha Code')
                );
    ...
    function prepare_3line($raw_data, $args = 'untrusted', $default_2alpha_code = 'XX') {
        ...
        if (preg_match(
            '/^St. Thomas.*Virgin Islands$/i', $data['Address Town']
        )) {
        ...
        if (preg_match(
            '/Caledonia|Calédonie|Caledonie/i', $data['Address Country Name']
        )) {
            $data['Address Country Name'] = 'France';
            if ($data['Address Country First Division'] == '') {
        ...
        if (preg_match(
            '/Saint Martin|St Martin/i', $data['Address Country Name']
        )) {
            $data['Address Country Name'] = 'France';
            if ($data['Address Country First Division'] == '') {
        ...
        if (preg_match(
            '/^england$|^inglaterra$/i', $data['Address Country Name']
        )) {
            $data['Address Country Name'] = 'United Kingdom';
            if ($data['Address Country First Division'] == '') {
            ...
            if (preg_match(
                '/^nor.*ireland$|n\.{2}ireland/i', $data['Address Country Name']
            )) {
                $data['Address Country Name'] = 'United Kingdom';
                if ($data['Address Country First Division'] == '') {
                ...
                if (preg_match(
                    '/^r.*ireland$|^s.*ireland|^eire$/i', $data['Address Country Name']
                )) {
                    $data['Address Country Name'] = 'Ireland';
                } else {
                    if (preg_match(
                        '/me.ico|m.xico/i', $data['Address Country Name']
                    )) {
                        $data['Address Country Name'] = 'Mexico';
                    } else {
                        if (preg_match(
                            '/scotland|escocia/i', $data['Address Country Name']
                        )) {
                            ...
                            if (preg_match(
                                '/.*\s+(w|g)ales$/i', $data['Address Country Name']
                            )) {
                                $data['Address Country Name']
                                    = 'United Kingdom';
                                if ($data['Address Country First Division'] == '') {
                                ...
                                if (preg_match(
                                    '/canarias$/i', $data['Address Country Name']
                                )) {
                                    ...
                                    if (preg_match(
                                        '/^Channel Islands$/i', $data['Address Country Name']
                                    )) {
        ...
        if ($data['Address Country Name'] == '') {
            if ($default_2alpha_code == 'GB') {
                ...
                if (Address::is_valid_postcode(
                    ...
                    $data['Address Country Name'], 'GBR'
                )
                ) {
                    $data['Address Country First Division'] = _trim(
                        $data['Address Country First Division'].' '.$data['Address Postal Code']
                    );
                ...
                if (preg_match(
                        '/^\d{5}$/', _trim($raw_data['Address Country Name'])
                    ) and (_trim($data['Address Postal Code']) == '' or preg_match(
                            '/^(spain|espa.{0,2}a|ESPA.{0,2}A)$/i', _trim($data['Address Postal Code'])
                        ))
                ) {
                    ...
                    if (preg_match(
                        '/^(spain|Espa.{0,2}a)$/i', _trim($data['Address Country First Division'])
                    )) {
                        $data['Address Country First Division'] = '';
                    }

                    if (preg_match(
                        '/^(spain|Espa.{0,2}a)$/i', _trim($data['Address Country Second Division'])
                    )) {
                        $data['Address Country Second Division'] = '';
                    }
                }
        ...
        if ($untrusted) {
            ...
            if (preg_match('/^\d{1,}\s*\,\s*/', $raw_data['Address Line 1'])) {
                $raw_data['Address Line 1'] = _trim(
                    preg_replace('/,/', ' ', $raw_data['Address Line 1'], 1)
                );
            }
            ...
            if ($number_lines == 1 and $data['Address Town'] == '' and $data['Address Country First Division'] == '' and $data['Address Country Second Division'] == '') {
                // try to sepatate
                //split by worlds
                ...
                if ($num_words > 1) {
                    if (Address::is_country_d1(
                        $words[$num_words - 1], $data['Address Country 2 Alpha Code']
                    )
                    ) {
                        $data['Address Country First Division'] = array_pop(
                            $words
                        );
                        $num_words                              = count($words);
                    }
                ...
                if ($num_words > 1) {
                    if (Address::is_country_d2(
                        ...
                        $data['Address Country Second Division'] = array_pop(
                            $words
                        );
                        $num_words                               = count(
                            $words
            ...
            if ($data['Address Town'] == '') {
                ...
                if (Address::is_town(
                    $raw_data['Address Line 3'], $data['Address Country 2 Alpha Code']
                )
                ) {
                    ...
                    if (Address::is_town(
                        $data['Address Country Second Division'], $data['Address Country 2 Alpha Code']
                    )
                    ) {
                        $data['Address Town']
                                                                 = $data['Address Country Second Division'];
            ...
            if (preg_match(
                '/^\d[a-z]?(bis)?\s*,/', $raw_data['Address Line 1']
            )) {
                $raw_data['Address Line 1'] = preg_replace(
                    '/\s*,\s*/', ' ', $raw_data['Address Line 1']
                );
            }
            if (preg_match(
                '/^\d[a-z]?(bis)?\s*,/', $raw_data['Address Line 2']
            )) {
                $raw_data['Address Line 2'] = preg_replace(
                    '/\s*,\s*/', ' ', $raw_data['Address Line 2']
                );
            }
            if (preg_match(
                '/^\d[a-z]?(bis)?\s*,/', $raw_data['Address Line 3']
            )) {
                $raw_data['Address Line 3'] = preg_replace(
                    '/\s*,\s*/', ' ', $raw_data['Address Line 3']
                );
            }

            $raw_data['Address Line 1'] = preg_replace(
                '/,\s*$/', ' ', $raw_data['Address Line 1']
            );
            $raw_data['Address Line 2'] = preg_replace(
                '/,\s*$/', ' ', $raw_data['Address Line 2']
            );
            $raw_data['Address Line 3'] = preg_replace(
                '/,\s*$/', ' ', $raw_data['Address Line 3']
            );
            ...
            if ($raw_data['Address Line 1'] != '' and $raw_data['Address Line 2'] == '' and $raw_data['Address Line 3'] == '') {// one line
                $splited_address = preg_split(
                    '/\s*,\s*/i', $raw_data['Address Line 1']
                );
                ...
                if (count($splited_address) == 1) {
                    ...
                    if (count($splited_address) == 2) {
                        ...
                        if (Address::is_internal($parte_0) and Address::is_street($parte_1)) {
                            ...
                            $data['Address Town First Division'] = preg_replace(
            ...
            if ($f_a1 and $f_a2 and $f_a3) { // caso 1 all filled a1,a2 and a3
                ...
                if ($s_a1 and !$s_a2 and !$s_a3) { //caso    soo  (moviing 2 )
                    ...
                    if (!$f_ta and !$f_td and !$f_t) { // caso ooo (towns)
                        ...
                        if (!$f_ta and !$f_td and $f_t) {// caso oot
                            ...
                            $raw_data['Address Line 3']
                                                        = $raw_data['Address Line 1'].', '.$raw_data['Address Line 2'].', '.$raw_data['Address Line 3'];
                            $raw_data['Address Line 2'] = '';
                            $raw_data['Address Line 1'] = '';
                    ...
                    if ((!$s_a1 and $s_a2 and !$s_a3) or ($s_a1 and $s_a2 and !$s_a3)) { //caso    oso OR  sso  (move one)
        ...
        if (preg_match(
            '/^P\.o\.box\s+\d+$|^po\s+\d+$|^p\.o\.\s+\d+$/i', $data['Address Town Second Division']
        )) {

            $po                                   = $data['Address Town Second Division'];
            $data['Address Town Second Division'] = '';
            $po                                   = preg_replace(
                '/^P\.o\.box\s+|^po\s+|^p\.o\.\s+/i', 'PO BOX ', $po
            );
            if ($raw_data['Address Line 1'] == '') {
        ...
        switch ($data['Address Country Key']) {
            case(30)://UK
                ...
                if (preg_match(
                    '/Andover.*\sHampshire/i', $data['Address Town']
                )) {
                ...
                if ($data['Address Town'] == '') {
                    if ($data['Address Town First Division'] != '') {
                        ...
                        if ($raw_data['Address Line 2'] != '' and $raw_data['Address Line 1'] != '') {
            ...
            case(78)://Italy
                $data['Address Postal Code'] = preg_replace(
                    '/italy|italia/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/\s/i', '', $data['Address Postal Code']
                );
                ...
                if (preg_match(
                    '/Villasor.*Cagliari/i', $data['Address Town']
                )) {
                ...
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
                );
            ...
            case(75)://Ireland
                ...
                $data['Address Country Second Division'] = _trim(
                    $data['Address Country Second Division']
                );
                $data['Address Postal Code']             = preg_replace(
                    '/County COrK/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code']             = preg_replace(
                    '/^co\.\s*|Republique of Ireland|Louth Ireland|ireland/i', '', $data['Address Postal Code']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/^co\.\s*|republic of ireland|republic of|ireland/i', '', $data['Address Country Second Division']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/(co|county)\s+[a-z]+$/i', '', $data['Address Country Second Division']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/(co|county)\s+[a-z]+,?\s*(ireland)?/i', '', $data['Address Country Second Division']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/(co|county)\s+[a-z]+$/i', '', $data['Address Country Second Division']
                );
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/\,+\s*^ireland$/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/(co|county)\s+[a-z]+,?\s*(ireland)?/i', '', $data['Address Postal Code']
                );
                $data['Address Town']        = preg_replace(
                    '/(co|county)\s+[a-z]+$/i', '', $data['Address Town']
                );
                ...
                $data['Address Postal Code']             = preg_replace(
                    '/co\s*Donegal|eire|republic of ireland|rep\? of Ireland|n\/a|^ireland$|/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code']             = _trim(
                    $data['Address Postal Code']
                );
                ...
                //print "country_d2: $data['Address Country Second Division']\n";
                $data['Address Town'] = preg_replace(
                    '/\-?\s*eire|\s*\-?\s*ireland/i', '', $data['Address Town']
                );
                //exit;
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/Rep.?of/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = str_replace(
                    ',', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = str_replace(
                    '.', '', $data['Address Postal Code']
                ...
                $data['Address Postal Code'] = str_replace(
                    'DUBLIN', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = str_replace(
                    'N/A', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/Republic\s?of/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/Erie/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/county/i', '', $data['Address Postal Code']
                );

                $data['Address Postal Code'] = preg_replace(
                    '/^co/i', 'County ', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/\s{2,}/', ' ', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
                );

                $valid_postalcodes = array(
                    'D1',
                    'D2',
                    'D3',
                    'D4',
                ...
                if ($data['Address Postal Code'] != '') {
                    $sql
                        = "SELECT `Country Second Division Name` AS name FROM kbase.`Country Second Division Dimension` WHERE  `Country Code`='IRL' AND `Country Second Division Name` LIKE '%"
                        .addslashes($data['Address Postal Code'])."%'";
                    //print "$sql\n";
                ...
                if (preg_match(
                    '/COMAYORepublicof|COGALWAY|RepublicofTIPPERARY|Republiqueof|NCW|eire|WD3|123|CoKerry,EIRE|COCORK|COOFFALY|WICKLOW|CoKerry/i', $data['Address Postal Code']
                )) {
                    $data['Address Postal Code'] = '';
                }

                if (preg_match(
                    '/^co\.?\s+|^country\s+/i', $data['Address Postal Code']
                )) {
                    $data['Address Postal Code'] = '';
                    if ($data['Address Country Second Division'] == '') {
                        $data['Address Country Second Division']
                ...
                $data['Address Town']                    = preg_replace(
                    '/\s+ireland\s*/i', '', $data['Address Town']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/\s+ireland\s*/i', '', $data['Address Country Second Division']
                );


                $data['Address Town'] = preg_replace(
                    '/co\.\s*/i', 'Co ', $data['Address Town']
                );
                $data['Address Town'] = preg_replace(
                    '/county\s+/i', 'Co ', $data['Address Town']
                );

                // print "$data['Address Town']";
                $split_town = preg_split(
                    '/\s*-\s*|\s*,\s*/i', $data['Address Town']
                );
                ...
                if (preg_match('/^co\s+/i', $data['Address Town'])) {
                    ...
                    $data['Address Town'] = preg_replace(
                        '/^co\s+/i', '', $data['Address Town']
                    );
                }

                $data['Address Country Second Division'] = preg_replace(
                    '/co\.?\s+/i', '', $data['Address Country Second Division']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/county\s+/i', '', $data['Address Country Second Division']
                );

                if (preg_match(
                    '/\s*Cork\sCity\s*/i', $data['Address Town Second Division']
                )) {
                    $data['Address Town Second Division'] == '';
                    if ($data['Address Town'] == '') {
                ...
                if (preg_match(
                    '/^dublin\s+\d+$/i', $data['Address Town Second Division']
                )) {
                    ...
                    if ($data['Address Town First Division'] == '') {
                        $data['Address Town First Division'] = preg_replace(
                            '/dublin\s+/i', '', $data['Address Town Second Division']
                        );
                    }
                    if ($data['Address Postal Code'] == preg_replace(
                            '/dublin\s+/i', '', $data['Address Town Second Division']
                        )
                    ) {
                        $data['Address Postal Code'] = '';
                ...
                if (preg_match(
                    '/^dublin\s*\d{1,2}$/i', $data['Address Postal Code']
                )) {
                    $data['Address Postal Code'] = preg_replace(
                        '/^dublin\s*/i', '', $data['Address Postal Code']
                    );
                }
                ...
                //  print "$data['Address Town'] +++++++++++++++\n";
                $data['Address Town'] = preg_replace(
                    '/\s*,?\s*Leinster/i', '', $data['Address Town']
                );
                ...
                //  print "$data['Address Town'] +++++++++++++++\n";
                if (preg_match(
                    '/^dublin\s*\-\s*\d$/i', $data['Address Town']
                )) {
                    $data['Address Postal Code'] = preg_replace(
                        '/^dublin\s*\-\s*/i', '', $data['Address Town']
                    );
                    $data['Address Town']        = 'Dublin';
                }

                if (preg_match(
                    '/^dublin\s*d?\d{1,2}$/i', $data['Address Town']
                )) {
                    $data['Address Postal Code'] = preg_replace(
                        '/^dublin\s*/i', '', $data['Address Town']
                    );
                    $data['Address Town']        = 'Dublin';
                }
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/MUNSTER|County RK/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
                );
            ...
            case(89)://Canada
                $data['Address Postal Code'] = preg_replace(
                    '/\s*canada\s*/i', '', $data['Address Postal Code']
                );
            ...
            case(208)://Czech Republic
                $data['Address Postal Code'] = preg_replace(
                    '/\s*Czech Republic\s*/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/\s*/i', '', $data['Address Postal Code']
                );
            ...
            case(108)://Cypruss
                $data['Address Postal Code'] = preg_replace(
                    '/\s*cyprus\s*/i', '', $data['Address Postal Code']
                );

                $data['Address Postal Code'] = preg_replace(
                    '/^cy\-?/i', '', $data['Address Postal Code']
                );
            ...
            case(240):
                $data['Address Town'] = preg_replace(
                    '/\,?\s*Guernsey Islands$/i', '', $data['Address Town']
                );
                $data['Address Town'] = preg_replace(
                    '/\,?\s*Guernsey$/i', '', $data['Address Town']
                );
                $data['Address Town'] = preg_replace(
                    '/\,?\s*Channel Islands$/i', '', $data['Address Town']
                );
                $data['Address Town'] = preg_replace(
                    '/\,?\s*CI$/i', '', $data['Address Town']
                );
                $data['Address Town'] = preg_replace(
                    '/\,?\s*C.I.$/i', '', $data['Address Town']
                );
                ...
                if ($data['Address Town'] == '') {
                    if ($data['Address Town First Division'] != '') {
                        ...
                        if (!preg_match(
                            '/^rue\s/i', $raw_data['Address Line 3']
                        )
                        ) {
                            $data['Address Town']       = $raw_data['Address Line 3'];
                            $raw_data['Address Line 3']
            ...
            case(104):// Greece
                $data['Address Postal Code'] = preg_replace(
                    '/greece/i', '', $data['Address Postal Code']
                );

                $data['Address Postal Code'] = preg_replace(
                    '/^(GK|T\.?k\.?)/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/\s/i', '', $data['Address Postal Code']
                );
                ...
                if (preg_match(
                    '/^(Attica|Ionian Islands)$/i', $data['Address Town']
                )) {
                    $data['Address Town'] = '';
                }
            ...
            case(229)://USA
                ...
                //apo address
                if (preg_match(
                    '/^(09|96|340)\d+$/', $data['Address Postal Code']
                )) {
                    ...
                    $raw_data['Address Line 1']                         = _trim(
                        $raw_data['Address Line 1'].' '.$raw_data['Address Line 2'].' '.$raw_data['Address Line 3'].' '.$data['Address Town'].' '.$data['Address Country Second Division'].' '
                        .$data['Address Country First Division']
                    );
                    $raw_data['Address Line 2']                         = '';
                    $raw_data['Address Line 3']                         = '';
                    ...
                    $military_installation['military base postal code']
                                                                        = $data['Address Postal Code'];
                    if (preg_match('/apo ae$/i', $raw_data['Address Line 1']) or preg_match(
                            '/\sapo ae\s+/i', $raw_data['Address Line 1']
                        )
                    ) {
                        $raw_data['Address Line 1']                  = _trim(
                            preg_replace(
                                '/apo ae/i', '', $raw_data['Address Line 1']
                            )
                        );
                ...
                $data['Address Town'] = preg_replace(
                    '/Lousiana/i', 'Louisiana', $data['Address Town']
                );
                ...
                if (preg_match(
                    '/^[a-z]\s*[a-z]$/i', $data['Address Country First Division']
                )) {
                    $data['Address Country First Division'] = preg_replace(
                        '/\s/', '', $data['Address Country First Division']
                    );
                }
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/united states of america/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/\s*u\s*s\s*a\s*|^United States\s+|United Stated|usa|^united states$|^united states of america$|^america$/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
                );
                ...
                if ($data['Address Country First Division'] == '') {
                    ...
                    if (preg_match(
                        $regex, $data['Address Postal Code'], $match
                    )) {
                        $data['Address Country First Division'] = preg_replace(
                            '/[^a-z]/i', '', $match[0]
                        );
                        $data['Address Postal Code']            = preg_replace(
                            $regex, '', $data['Address Postal Code']
                        );
                    }
                    $regex = '/\([a-z]{2}\)/i';
                ...
                if ($data['Address Postal Code'] == '' and preg_match(
                        '/\s*\d{4,5}\s*/', $data['Address Town'], $match
                    )
                ) {
                    $data['Address Postal Code'] = trim(trim($match[0]));
                    $data['Address Town']        = _trim(
                        preg_replace(
                            '/\s*\d{4,5}\s*/', '', $data['Address Town']
                        )
                    );
                }

                $data['Address Town'] = preg_replace(
                    '/\s*\-\s*$/', '', $data['Address Town']
                );
                ...
                if (strlen($data['Address Country First Division']) == 2) {
                    if (array_key_exists(
                        strtoupper($data['Address Country First Division']), $states
                    )) {
                        $data['Address Country First Division']
                            = $states[strtoupper(
                            $data['Address Country First Division']
            ...
            case(105)://Croatia
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/croatia/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/^hr-?/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
                );
            ...
            case(160)://Portugal
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/portugal/i', '', $data['Address Postal Code']
                );
                $data['Address Town']        = preg_replace(
                    '/\-?\s*portugal/i', '', $data['Address Town']
                );


                if ($data['Address Postal Code'] == '' and preg_match(
                        '/\s*\d{4}\s*/', $data['Address Town'], $match
                    )
                ) {
                    $data['Address Postal Code'] = trim(trim($match[0]));
                    $data['Address Town']        = _trim(
                        preg_replace('/\s*\d{4}\s*/', '', $data['Address Town'])
                    );
                }
            ...
            case(21)://Belgium
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/belgium/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/^b\-?/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
                );
                ...
                if (count($t) == 2) {
                    if (Address::is_country_d1(
                        $t[1], $data['Address Country 2 Alpha Code']
                    )
                    ) {
                ...
                if (Address::is_country_d1(
                        $data['Address Town'], $data['Address Country 2 Alpha Code']
                    ) and $data['Address Country First Division'] == '' and ($raw_data['Address Line 2'] != '' and $raw_data['Address Line 3'] != '')
                ) {
                    $data['Address Country First Division']
                                          = $data['Address Town'];
                    $data['Address Town'] = '';
                ...
                if (Address::is_country_d1(
                        $data['Address Town'], $data['Address Country 2 Alpha Code']
                    ) and $data['Address Country First Division'] == '' and $data['Address Town Second Division'] != ''
                ) {
                    $data['Address Country First Division']
                                                          = $data['Address Town Second Division'];
                    $data['Address Town Second Division'] = '';
            ...
            case(15)://Australia
                $data['Address Postal Code'] = preg_replace(
                    '/\s*australia\s*/i', '', $data['Address Postal Code']
                );
                $regex                       = '/\(QLD\)/i';
            ...
            case(47)://Spain
                ...
                $raw_data['Address Line 3'] = preg_replace(
                    '/^c\/\s*/i', 'Calle ', $raw_data['Address Line 3']
                );
                $regex_calle
                                            = '/^C(alle)? [a-záéíóúñ\s]+,\s?\d+/i';

                if (preg_match(
                    $regex_calle, $raw_data['Address Line 3'], $match
                )) {

                    $calle                      = $match[0];
                    $tmp                        = preg_replace(
                        $regex_calle, '', $raw_data['Address Line 3']
                    );
                ...
                if (preg_match(
                    '/Balearic Islands|Balearic Island/i', $data['Address Country First Division']
                )) {
                    $data['Address Country First Division']
                        = 'Balearic Islands';
                }
                if (preg_match(
                    '/Balearic Islands|Balearic Island/i', $data['Address Country Second Division']
                )) {
                    $data['Address Country Second Division']
                        = 'Balearic Islands';
                }
                ...
                if (preg_match('/Baleares/i', $raw_data['Address Line 3']) and preg_match(
                        '/Palma de Mallorca/i', $raw_data['Address Line 2']
                    )
                ) {
                ...
                if (preg_match(
                    '/Zugena - Provincia Almeria/i', $data['Address Town']
                )) {
                ...
                if (preg_match(
                    '/Mijas Costa, Malaga/i', $data['Address Town']
                )) {
                ...
                if (preg_match(
                    '/Ciutadella - Menorca/i', $data['Address Town']
                )) {
                ...
                if (preg_match('/malaga/i', $data['Address Town'])) {
                    if (preg_match(
                        '/Marbella/i', $raw_data['Address Line 3']
                    )) {
                        $raw_data['Address Line 3'] = '';
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/spain/i', '', $data['Address Postal Code']
                );
                ...
                $data['Address Country First Division'] = _trim(
                    preg_replace(
                        '/^Adaluc.a$/i', 'Andalusia', _trim($data['Address Country First Division'])
                    )
                );
            ...
            case(126)://Malta
                $data['Address Postal Code'] = preg_replace(
                    '/malta/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
                );
                ...
                if (preg_match('/[a-z]*/i', $data['Address Postal Code'], $ap) and preg_match(
                        '/[0-9]{1,}/i', $data['Address Postal Code'], $xxx
                    )
                ) {
                ...
                $data['Address Town'] = preg_replace(
                    '/-?\s*malta|gozo\s*\-?/i', '', $data['Address Town']
                );
            ...
            case(110)://Latvia
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/Latvia/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/LV\s*\-?\s*/i', '', $data['Address Postal Code']
                );
                $data['Address Town']        = _trim($data['Address Town']);
                $data['Address Postal Code'] = _trim(
                    $data['Address Postal Code']
            ...
            case(117)://Luxembourg
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/Luxembourg/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/L\s*\-?\s*/i', '', $data['Address Postal Code']
                );
                $data['Address Town']        = preg_replace(
                    '/\-?\s*Luxembourg/i', '', $data['Address Town']
                );
                if ($data['Address Town'] == '') {
            ...
            case(165)://France
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/FRANCE|french republic/i', '', $data['Address Postal Code']
                );
                if ($data['Address Postal Code'] == '' and preg_match(
                        '/\s*\d{4,5}\s*/', $data['Address Town'], $match
                    )
                ...
                if (preg_match(
                    '/Digne les Bains|Dignes les Bains/i', $data['Address Town']
                )) {
                    $data['Address Town'] = 'Digne-les-Bains';
                }

                $data['Address Town'] = preg_replace(
                    '/,\s*france\s*$/i', '', $data['Address Town']
                );
            ...
            case(196)://Switzerland
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/Switzerland/i', '', $data['Address Postal Code']
                );
                ...
                if (preg_match('/^\d{4}\s+/', $data['Address Town'], $match)) {
                    if ($data['Address Postal Code'] == '' or $data['Address Postal Code'] == trim($match[0])) {
                        ...
                        $data['Address Town']        = preg_replace(
                            '/^\d{4}\s+/', '', $data['Address Town']
                        );
                    }
                }

                $data['Address Postal Code'] = preg_replace(
                    '/^CH\-/i', '', $data['Address Postal Code']
                );
                break;
            case(193)://Findland
                $data['Address Postal Code'] = _trim(
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/findland|finland/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/^fi\s*\-?\s*/i', '', $data['Address Postal Code']
                );
            ...
            case(241)://Jersey
                ...
                $data['Address Town']                    = preg_replace(
                    '/^jersey$|^jersey\s*c\.?i\.?$/i', '', $data['Address Town']
                );
                $data['Address Town']                    = preg_replace(
                    '/\,?\s*Channel Islands$/i', '', $data['Address Town']
                );
                ...
                $data['Address Town']                    = preg_replace(
                    '/\-?\s*jersey$/i', '', $data['Address Town']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/\-?\s*jersey$|Jersy Channel Isles/i', '', $data['Address Country Second Division']
                );
                //  print "1$raw_data['Address Line 1'] 2$raw_data['Address Line 2'] 3$raw_data['Address Line 3']\n";
                if ($data['Address Town'] == '') {
                    if ($data['Address Town First Division'] != '') {
                ...
                $data['Address Town'] = _trim($data['Address Town']);
                if ($data['Address Town Second Division'] == '' and preg_match(
                        '/\w+\.?\s*St\.? Helier$/i', $data['Address Town']
                    )
                ) {
                    $data['Address Town Second Division'] = _trim(
                        preg_replace(
                            '/St\.? Helier$/i', '', $data['Address Town']
                        )
                    );
                    $data['Address Town']                 = 'St Helier';
                }

                $data['Address Town Second Division'] = preg_replace(
                    '/\./', '', $data['Address Town Second Division']
                );
                $data['Address Town']                 = preg_replace(
                    '/^St\s{1,}/', 'St. ', $data['Address Town']
                );
            ...
            case(171)://Sweden
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/sweden/i', '', $data['Address Postal Code']
                );

                $data['Address Postal Code'] = preg_replace(
                    '/^SE\-?/i', '', $data['Address Postal Code']
                );
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/\s/', '', $data['Address Postal Code']
                );

                if (Address::is_country_d1(
                        $data['Address Town'], $data['Address Country 2 Alpha Code']
                    ) and $raw_data['Address Line 1'] = '' and $raw_data['Address Line 2'] != '' and $raw_data['Address Line 3'] != ''
                ) {
                    $data['Address Country First Division']
                                                = $data['Address Town'];
                    $raw_data['Address Line 3'] = $raw_data['Address Line 2'];
            ...
            case(149)://Norway
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/norway/i', '', $data['Address Postal Code']
                );
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/^NO\s*\-?\s*/i', '', $data['Address Postal Code']
                );

                $data['Address Postal Code'] = preg_replace(
                    '/^N\-/i', '', $data['Address Postal Code']
                );
                if (preg_match('/^\d{3}$/', $data['Address Postal Code'])) {
                    $data['Address Postal Code'] = '0'.$data['Address Postal Code'];
                }
            ...
            case(2)://Netherlands
                $data['Address Town']                    = preg_replace(
                    '/Noord Brabant/i', 'Noord-Brabant', $data['Address Town']
                );
                $data['Address Country First Division']  = preg_replace(
                    '/Noord Brabant/i', 'Noord-Brabant', $data['Address Country First Division']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/Noord Brabant/i', 'Noord-Brabant', $data['Address Country Second Division']
                );
                $data['Address Town']                    = preg_replace(
                    '/Zuid Holland/i', 'Zuid-Holland', $data['Address Town']
                );
                $data['Address Country First Division']  = preg_replace(
                    '/Zuid Holland/i', 'Zuid-Holland', $data['Address Country First Division']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/Zuid Holland/i', 'Zuid-Holland', $data['Address Country Second Division']
                );
                $data['Address Town']                    = preg_replace(
                    '/Noord Holland/i', 'Noord-Holland', $data['Address Town']
                );
                $data['Address Country First Division']  = preg_replace(
                    '/Noord Holland/i', 'Noord-Holland', $data['Address Country First Division']
                );
                $data['Address Country Second Division'] = preg_replace(
                    '/Noord Holland/i', 'Noord-Holland', $data['Address Country Second Division']
                );
                $data['Address Town']                    = preg_replace(
                    '/Gerderland/i', 'Gelderland', $data['Address Town']
                );
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/Netherlands|holland/i', '', $data['Address Postal Code']
                );

                if ($data['Address Postal Code'] == '') {
                    if (preg_match(
                        '/\s*\d{4,6}\s*[a-z]{2}\s*/i', $data['Address Town'], $match2
                    )) {
                        $data['Address Postal Code'] = _trim($match2[0]);
                    }
                }
                $data['Address Postal Code'] = strtoupper(
                    $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/\s/', '', $data['Address Postal Code']
                ...
                if (preg_match(
                    '/^\d{4}[a-z]{2}$/i', $data['Address Postal Code']
                )) {
                    $data['Address Town']        = str_replace(
                        $data['Address Postal Code'], '', $data['Address Town']
                    );
                    $data['Address Town']        = str_replace(
                        strtolower($data['Address Postal Code']), '', $data['Address Town']
                    );
                    $_postcode                   = substr(
                            $data['Address Postal Code'], 0, 4
                        ).' '.substr($data['Address Postal Code'], 4, 2);
                ...
                if (Address::is_country_d1(
                        $raw_data['Address Line 3'], $data['Address Country 2 Alpha Code']
                    ) and $data['Address Country First Division'] == '' and $data['Address Town'] == '' and ($raw_data['Address Line 1'] != '' and $raw_data['Address Line 2'] != '')
                ) {
                    $data['Address Country First Division']
                                                = $raw_data['Address Line 3'];
                    $raw_data['Address Line 3'] = '';
                ...
                if (Address::is_country_d1(
                        $data['Address Town'], $data['Address Country 2 Alpha Code']
                    ) and $data['Address Country First Division'] == '' and (($raw_data['Address Line 1'] != '' and $raw_data['Address Line 2'] != '') or ($raw_data['Address Line 2'] != ''
                            and $raw_data['Address Line 3'] != '') or ($raw_data['Address Line 1'] != '' and $raw_data['Address Line 3'] != ''))
                ) {
                    $data['Address Country First Division']
                                          = $data['Address Town'];
                    $data['Address Town'] = '';
                ...
                similar_text(
                    $data['Address Country First Division'], $data['Address Country Second Division'], $w
                );
                if ($w > 90) {
                    $data['Address Country Second Division'] = '';
                }
                ...
                if (preg_match(
                    '/Zuid.Holland|ZuidHolland/i', $data['Address Country First Division']
                )) {
                    $data['Address Country First Division'] = 'Zuid Holland';
                }
            ...
            case(177):// Germany
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/germany/i', '', $data['Address Postal Code']
                );
                if ($data['Address Country Second Division'] != '' and $data['Address Country First Division'] == '') {
                    $data['Address Country First Division']
                                                             = $data['Address Country Second Division'];
                ...
                if (preg_match(
                    '/^bavaria$/i', $data['Address Country First Division']
                )) {
            ...
            case(201)://Denmark
                ...
                $data['Address Postal Code'] = preg_replace(
                    '/denmark|Demnark/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code'] = preg_replace(
                    '/^dk\s*\-?\s*/i', '', $data['Address Postal Code']
                );
                $data['Address Town']        = _trim($data['Address Town']);

                if ($data['Address Postal Code'] == '' and preg_match(
                        '/^\d{4}\s+/', $data['Address Town'], $match
                    )
                ) {
                    $data['Address Postal Code'] = trim($match[0]);
                    $data['Address Town']        = preg_replace(
                ...
                if (preg_match(
                    '/^DK\- 7470 Karup J$/i', $raw_data['Address Line 3']
                )) {
                    $raw_data['Address Line 3']  = '';
                ...
                if (preg_match(
                    '/Sjalland|Zealand|Sjælland|Sealand/i', $data['Address Country Second Division']
                )) {
                    $data['Address Country Second Division'] = '';
                }
        ...
        if ($data['Address Country First Division'] != '') {
            $sql = sprintf(
                "SELECT `Country First Division Code` AS id  FROM  kbase.`Country First Division Dimension` WHERE `Country First Division Name`=%s AND `Country Code`=%s",
                prepare_mysql($data['Address Country First Division']), prepare_mysql($data['Address Country Code'])
            );
            // print "$sql\n";
        ...
        if ($data['Address Country Second Division'] != '') {
            $sql = sprintf(
                "SELECT `Country Second Division Code`  AS id, `Country First Division Code`   FROM kbase.`Country Second Division Dimension`   WHERE `Country Second Division Name`=%s   AND `Country Code`=%s"

                , prepare_mysql($data['Address Country Second Division']), prepare_mysql($data['Address Country Code'])
            );
            ...
            if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
                ...
                if (mysql_num_rows($result) == 1) {
                    $data['Address Country First Division Code']
                        = $row['Country First Division Code'];
                }
        ...
        $sql = sprintf(
            "SELECT `Geography Key` AS id,`Country Second Division Code` , `Country First Division Code` FROM kbase.`Town Dimension` WHERE (`Town Name`='%s'  ) AND `Country Code`=%d",
            addslashes($data['Address Town']), addslashes($data['Address Town']), addslashes($data['Address Town']), $data['Address Country Code']
        );
        // print $sql;
        $res = mysql_query($sql);
        ...
        if (mysql_num_rows($res) == 1) {
            ...
            if ($data['Address Country Second Division Code'] == 0) {
                $data['Address Country Second Division Code']
                    = $row['Country Second Division Code'];
            }
        ...
        $raw_data['Address Line 1'] = preg_replace(
            '/^P\.o\.box\s+/i', 'PO BOX ', $raw_data['Address Line 1']
        );
        $raw_data['Address Line 2'] = preg_replace(
            '/^P\.o\.box\s+/i', 'PO BOX ', $raw_data['Address Line 2']
        );
        $raw_data['Address Line 3'] = preg_replace(
            '/^P\.o\.box\s+/i', 'PO BOX ', $raw_data['Address Line 3']
        );
        $raw_data['Address Line 3'] = preg_replace(
            '/^p o box\s+/i', 'PO BOX ', $raw_data['Address Line 3']
        );
        $raw_data['Address Line 3'] = preg_replace(
            '/^NULL$/i', '', $raw_data['Address Line 3']
        );

        $raw_data['Address Line 1']           = _trim(
            $raw_data['Address Line 1']
        );
        $raw_data['Address Line 2']           = _trim(
            $raw_data['Address Line 2']
        );
        $raw_data['Address Line 3']           = _trim(
            $raw_data['Address Line 3']
        );
        $data['Address Town']                 = _trim($data['Address Town']);
        $data['Address Town First Division']  = _trim(
            $data['Address Town First Division']
        );
        $data['Address Town Second Division'] = _trim(
            $data['Address Town Second Division']
        ...
        $data['Address Fuzzy Type'] = preg_replace(
    ...
    function is_street($string) {
        ...
        if (preg_match(
            '/\s+rd\.?\s*$|\s+road\s*$|^\d+[a-z]?\s+\w|^\s*calle\s+|\s+close\s*$|\s+lane\s*$|\s+street\s*$|\s+st\.?\s*$/i', $string
        )) {
            $is_street = true;
    ...
    */
    ...
    public static function is_town($town, $country_2acode = false) {
        ...
        if ($country_2acode) {
            $sql = sprintf(
                "SELECT `Geography Key` AS id FROM kbase.`Town Dimension` WHERE `Town Name`=%s  AND `Country 2 Alpha Code`=%s", prepare_mysql($town), prepare_mysql($country_2acode)
            );
        } else {
            $sql = sprintf(
                "SELECT `Geography Key` AS id FROM kbase.`Town Dimension` WHERE `Town Name`=%s ", prepare_mysql($town)
            );
        }
    ...
    */
    ...
    function is_internal($string) {
        ...
        if (preg_match(
            '/lot\s*(n-)?\s*\d|suite\s*\d|shop\s*\d|apt\s*\d/i', $string
        )) {
            return true;
        } else {
            return false;
    ...
    function get_default($key) {
        ...
        if (!isset($this->default_country)) {
            $this->default_country = new Country(
                'code', $this->default_country_code
            );
        }
    ...
    function find_complete($data, $subject_data) {
        ...
        if ($data['Address Fuzzy'] != 'Yes') {
            ...
            $fields = array(
                ...
                'Address Street Name',
                'Address Street Type',
                'Address Town Second Division',
                'Address Town First Division',
                'Address Town',
                'Address Country First Division',
                'Address Country Second Division',
                'Address Country Key',
                'Address Postal Code',
                'Military Address',
                'Military Installation Address',
                'Military Installation Name'
            );

            $sql
                = "SELECT A.`Address Key`,`Subject Key`,`Subject Type` FROM `Address Dimension`  A  LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`) WHERE `Subject Type`='Contact' ";
    ...
    function find_in_subject($data, $subject_data) {
        ...
        foreach ($address_keys as $address_key) {
            ...
            foreach ($data as $key => $value) {
                ...
                if ($value != '' and !preg_match(
                        '/World|Continent|Fuzzy|Location|Format|FAX|Telephone|Update/', $key
                    )
                ) {
    ...
    function find_fuzzy($data, $subject_data) {
            ...
            switch ($field) {
                case 'Address Postal Code':
                    ...
                    if ($postal_code_no_spaces != $value) {
                        $postal_code = prepare_mysql($value).','.prepare_mysql(
                                $postal_code_no_spaces
                            );
                    } else {
                    ...
                    $sql = sprintf(
                        "SELECT A.`Address Key`,`Subject Key` FROM `Address Dimension`  A  LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`) WHERE `Subject Type`='Contact' AND `Address Postal Code` IN (%s)",
                        $postal_code
                    );
                    // print $sql;
                    $res         = mysql_query($sql);
                ...
                case 'Address Town':
                    ...
                    $sql = sprintf(
                        "SELECT A.`Address Key`,`Subject Key` FROM `Address Dimension`  A  LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`) WHERE `Subject Type`='Contact' AND `Address Town`=%s ",
                        prepare_mysql($value)
                    );
                    // print "$sql\n";
                    $res         = mysql_query($sql);
        ...
        if ($number_filled_fields == 1) {
            ...
            if ($column) {
                ...
                $sql         = sprintf(
                    "SELECT A.`Address Key` ,damlev(UPPER(`%s`),%s) AS dist,  `Subject Key`AS contact_key FROM `Address Dimension` A LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`) WHERE  `Subject Type`='Contact'  AND `Address Country Code`=%s AND damlevlim(UPPER(`%s`),%s,2)<2  ORDER BY dist LIMIT 20   ",
                    $column, prepare_mysql(strtoupper($data[$column])), prepare_mysql($data['Address Country Code']), $column, prepare_mysql(strtoupper($data[$column]))
                );
                $result      = mysql_query($sql);
                ...
                $sql         = sprintf(
                    "SELECT `Address Town`, A.`Address Key` ,damlev(UPPER(`%s`),%s) AS dist,  `Subject Key` AS contact_key FROM `Address Dimension` A LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`) WHERE  `Subject Type`='Contact'  AND `Address Country Code`!=%s AND damlevlim(UPPER(`%s`),%s,2)<2  ORDER BY dist LIMIT 20   ",
                    $column, prepare_mysql(strtoupper($data[$column])), prepare_mysql($data['Address Country Code']), $column, prepare_mysql(strtoupper($data[$column]))
                );
                $result      = mysql_query($sql);
                $len_keyword = strlen($data[$column]);
            ...
            $sql                       = sprintf(
                "SELECT SQL_CACHE count(*) AS addresses FROM `Address Dimension` "
            );
            $res                       = mysql_query($sql);
            $address_multiplicity_data = mysql_fetch_array($res);
            $address_multiplicity
                                       = $address_multiplicity_data['addresses'];

            $sql = sprintf(
                "SELECT SQL_CACHE count(*) AS addresses FROM `Address Dimension` WHERE   `Address Country Code`=%s ", prepare_mysql($data['Address Country Code'])
            );
            $res = mysql_query($sql);

            $country_multiplicity_data = mysql_fetch_array($res);
            $country_multiplicity
                                       = $country_multiplicity_data['addresses'];
            ...
            if (($data['Address Street Name'] != '') or $data['Address Building'] != '') {
                ...
                if ($data['Address Town'] != '') {
                    $sql_town  = sprintf(
                        "damlev(UPPER(`Address Town`),%s) as dist_town,", prepare_mysql(strtoupper($data['Address Town']))
                    );
                    $order     = 'dist_town';
                    $with_town = true;
                ...
                if ($data['Address Postal Code'] != '') {
                    $sql_postal_code       = sprintf(
                        "damlevlim(UPPER(`Address Postal Code`),%s,3) as dist_postal_code,", prepare_mysql(strtoupper($data['Address Postal Code']))
                    );
                    $sql_where_postal_code = sprintf(
                        "and  damlevlim(UPPER(`Address Postal Code`),%s,3)<3 ", prepare_mysql(strtoupper($data['Address Postal Code']))
                    );
                    $order .= ($order != '' ? ',' : '').'dist_postal_code';
                    $with_postal_code = true;
                }
                if ($data['Address Street Number'] != '') {
                    $sql_where_street_number = sprintf(
                        "and `Address Street Number`=%s ", prepare_mysql(
                            strtoupper($data['Address Street Number'])
                        )
                    );
                } elseif ($data['Address Building'] != '') {
                    $sql_where_street_number = sprintf(
                        "and  damlevlim(UPPER(`Address Building`),%s,6)<6 ", prepare_mysql(strtoupper($data['Address Building']))
                    );
                }
                if ($data['Address Internal'] != '') {
                    $sql_where_internal = sprintf(
                        "and `Address Internal`=%s", prepare_mysql($data['Address Internal'], false)
                    );
                }
                ...
                $sql = sprintf(
                    "SELECT `Address Town`,`Address Country Code`,`Address Postal Code`, A.`Address Key` ,%s  %s  `Subject Key` FROM `Address Dimension` A LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`) WHERE  `Subject Type`='Contact'  AND `Address Country Code`=%s %s %s %s %s LIMIT 250 ",
                    $sql_postal_code, $sql_town, prepare_mysql(strtoupper($data['Address Country Code'])), $sql_where_internal, $sql_where_street_number, $sql_where_postal_code, $order
                );
            ...
            if ($nothing_found) {
                ...
                if ($data['Address Postal Code'] != '') {
                    $sql    = sprintf(
                        "SELECT   `Address Country Code`,`Address Postal Code`, A.`Address Key` ,damlev(UPPER(`Address Postal Code`),%s) AS dist1,`Subject Key` FROM `Address Dimension` A LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`)  WHERE  `Subject Type`='Contact'  AND   damlev(UPPER(`Address Postal Code`),%s) <3   ORDER BY dist1  LIMIT 250 ",
                        prepare_mysql(strtoupper($data['Address Postal Code'])), prepare_mysql(strtoupper($data['Address Postal Code']))
                    );
                    $result = mysql_query($sql);
                ...
                if ($data['Address Town'] != '') {
                    ...
                    $sql = sprintf(
                        "SELECT   `Address Country Code`,`Address Town`, A.`Address Key` ,damlev(UPPER(`Address Town`),%s) AS dist1,`Subject Key` FROM `Address Dimension` A LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`)  WHERE  `Subject Type`='Contact'     ORDER BY dist1  LIMIT 50 ",
                        prepare_mysql(strtoupper($data['Address Town'])), prepare_mysql(strtoupper($data['Address Town']))
                    );
                ...
                if ($data['Address Street Name'] != '') {
                    $sql        = sprintf(
                        "SELECT   `Address Country Code`,`Address Town`, A.`Address Key` ,damlev(UPPER(`Address Street Name`),%s) AS dist1,`Subject Key` FROM `Address Dimension` A LEFT JOIN `Address Bridge` AB  ON (AB.`Address Key`=A.`Address Key`)  WHERE  `Subject Type`='Contact'     ORDER BY dist1  LIMIT 50 ",
    ...
    public function update($data, $options = '', $metadata = '') {
        ...
        if (!$this->updated) {
            ...
            $sql = sprintf(
                "UPDATE `Address Dimension` SET `Address Data Last Update`=%s WHERE `Address Key`=%d", prepare_mysql(gmdate('Y-m-d H:i:s')), $this->id
            );
            mysql_query($sql);
            ...
            $sql = sprintf(
                "UPDATE `Address Dimension` SET `Address Fuzzy`=%s, `Address Fuzzy Type`=%s, `Address Location`=%s,`Address Plain`=%s  WHERE `Address Key`=%d  ", prepare_mysql($fuzzy),
                prepare_mysql($fuzzy_type, false), prepare_mysql($location), prepare_mysql($plain), $this->id
    ...
    public static function parse_country($country) {
        ...
        if ($len == 2) {
            $sql    = sprintf(
                "SELECT `Country Code` FROM kbase.`Country Dimension`  WHERE `Country 2 Alpha Code`=%s", prepare_mysql($country)
            );
            $result = mysql_query($sql);
            if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            ...
            $sql    = sprintf(
                "SELECT `Country Code` FROM kbase.`Country Dimension`  WHERE `Country Code`=%s", prepare_mysql($country)
            );
            $result = mysql_query($sql);
            if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
                return $row['Country Code'];
        ...
        $sql    = sprintf(
            "SELECT `Country Code` FROM kbase.`Country Dimension`  WHERE `Country Name`=%s", prepare_mysql($country)
        );
        $result = mysql_query($sql);
        if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            return $row['Country Code'];
        ...
        $sql = sprintf(
            "SELECT `Country Alias Code` FROM kbase.`Country Alias Dimension` WHERE `Country Alias`=%s", prepare_mysql($country)
        );
        //  print "$sql\n";
        $result = mysql_query($sql);
        if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    function update_country_code($code) {
        ...
        $sql  = sprintf(
            'UPDATE `Address Dimension` SET `Address Country Key`=%d,`Address Country Name`=%s,`Address Country Code`=%s,`Address Country 2 Alpha Code`=%s,`Address World Region`=%s,`Address Continent`=%s WHERE `Address Key`=%d  ',
            $data['Address Country Key'], prepare_mysql($data['Address Country Name']), prepare_mysql($data['Address Country Code'])

            , prepare_mysql($data['Address Country 2 Alpha Code']), prepare_mysql($data['Address World Region']), prepare_mysql($data['Address Continent']), $this->id
        );
        mysql_query($sql);
    ...
    public static function prepare_country_data($data, $default_2alpha_code = 'XX') {
        ...
        if ($data['Address Country Key'] == '' and $data['Address Country Code'] == '' and $data['Address Country 2 Alpha Code'] == '' and $data['Address Country Name'] == '') {

            // try to get the counbtry form the town
            if ($data['Address Country First Division'] != '') {
                $country_in_other = new Country(
                    'find', $data['Address Country First Division']
                );
                ...
                $country_in_other = new Country(
                    'find', $data['Address Country Second Division']
                );
                if ($country_in_other->id != 244) {
                    $data['Address Country Key']             = $country_in_other->id;
                    $data['Address Country Second Division'] = '';
        ...
        if ($data['Address Country Key'] and Address::is_country_key(
            ...
            if ($data['Address Country Code'] != 'UNK' and Address::is_country_code($data['Address Country Code'])) {
                ...
                $country = new Country(
                    '2 alpha code', $data['Address Country 2 Alpha Code']
                );
            } else {
        ...
        switch ($field) {
            ...
            case('Address Postal Code'):
                ...
                foreach ($data as $postcode_field => $postcode_value) {
                    ...
                    $this->update_field(
                        $postcode_field, $postcode_value, $postcode_options
                    );
    ...
    function get_telecom_type_keys($type) {
        $sql = sprintf(
            "SELECT TB.`Telecom Key` FROM `Telecom Bridge` TB LEFT JOIN `Telecom Dimension` T ON (T.`Telecom Key`=TB.`Telecom Key`)  WHERE  `Subject Type`='Address' AND `Telecom Type`=%s AND  `Subject Key`=%d ",
    ...
    function create_telecom_bridge($telecom_key, $type, $swap_principal = true) {
        $sql = sprintf(
            "INSERT INTO `Telecom Bridge` (`Subject Type`,`Subject Key`,`Telecom Key`) VALUES ('Address',%d,%d)  ", $this->id, $telecom_key
        );
        mysql_query($sql);
        $this->updated = true;
    ...
    function get_principal_telecom_key($type) {
        ...
        $sql = sprintf(
            "SELECT TB.`Telecom Key` FROM `Telecom Bridge` TB  LEFT JOIN `Telecom Dimension` T ON (T.`Telecom Key`=TB.`Telecom Key`)  WHERE  `Telecom Type`=%s AND   `Subject Type`='Address' AND `Subject Key`=%d AND `Is Main`='Yes'",
            prepare_mysql($type), $this->id
        );
        //print $sql;
        $res = mysql_query($sql);
    ...
    function update_principal_telecom($telecom_key, $type, $old_principal_telecom = '') {
        ...
        if ($main_telecom_key != $telecom_key) {
            ...
            $sql             = sprintf(
                "update `Telecom Bridge` B left join `Telecom Dimension` T on (T.`Telecom Key`=B.`Telecom Key`)   set `Is Main`='No' where `Subject Type`='Address'   and `Subject Key`=%d and `Telecom Type`='$type' ",
                $this->id
            ...
            mysql_query($sql);
            $sql = sprintf(
                "UPDATE `Telecom Bridge`  SET `Is Main`='Yes' WHERE `Subject Type`='Address'  AND  `Subject Key`=%d  AND `Telecom Key`=%d", $this->id, $telecom->id
            );
            mysql_query($sql);


            $sql = sprintf(
                "update `Address Dimension` set  `Address Main $type Key`=%d where `Address Key`=%d", $telecom->id, $this->id
            );
            //print "$sql\n";
            ...
            $telecom->update_parents(
                $add_parent_history = true, $old_principal_telecom
            );
    ...
    function update_parents_principal_telecom_keys($type, $parents = false) {
        ...
        foreach ($parents as $parent) {
            $sql = sprintf(
                "select `$parent Key` as `Parent Key`   from  `$parent Dimension` where `$parent Main Address Key`=%d group by `$parent Key`", $this->id
            );
            //print "$sql\n";
            $res = mysql_query($sql);
            while ($row = mysql_fetch_array($res)) {
                ...
                if (!array_key_exists($telecom_key, $parent_telecoms)) {
                    $sql = sprintf(
                        "insert into  `Telecom Bridge` (`Telecom Key`,`Subject Type`, `Subject Key`,`Is Main`) values (%d,'$parent',%d,'No')  ", $telecom_key, $parent_object->id
                    );
                    mysql_query($sql);
                    // print "$sql\n";
                }
                ...
                $sql = sprintf(
                    "update `Telecom Bridge` B left join `Telecom Dimension` T on (T.`Telecom Key`=B.`Telecom Key`) set `Is Main`='No'   where   `Subject Type`='$parent' and  `Subject Key`=%d and `Telecom Type`='$type' ",
                    $parent_object->id
                ...
                //  print "$sql\n";
                $sql = sprintf(
                    "update `Telecom Bridge`  set `Is Main`='Yes' where  `Subject Type`='$parent' and  `Subject Key`=%d  and `Telecom Key`=%d", $parent_object->id, $telecom_key
                );
                mysql_query($sql);
                //    print "$sql\n";
                ...
                $sql = sprintf(
                    "update `$parent Dimension` set `$parent Main $type Key`=%d where `$parent Key`=%d", $telecom_key, $parent_object->id
                );
                //print "$sql\n";
                mysql_query($sql);
    ...
    function parse_postcode($postcode, $country_code = '') {
        ...
        switch ($country_code) {
            case 'GBR':
                $data['Address Postal Code Separator'] = ' ';
                $data['Address Postal Code']           = preg_replace(
                    '/,?\s*scotland\s*$|united kingdom/i', '', $data['Address Postal Code']
                );
                $data['Address Postal Code']           = preg_replace(
                    '/\s/', '', $data['Address Postal Code']
                );
                if (preg_match('/^bfpo\s*\d/i', $data['Address Postal Code'])) {
                    $data['Address Postal Code']        = preg_replace(
                        '/bfpo/i', 'BFPO ', $data['Address Postal Code']
                    );
                    $data['Address First Postal Code']  = 'BFPO';
                    $data['Address Second Postal Code'] = preg_replace(
                        '/bfpo /i', '', $data['Address Postal Code']
                    );
                } else {
                    $data['Address Postal Code']        = substr(
                            $data['Address Postal Code'], 0, strlen($data['Address Postal Code']) - 3
                        ).' '.substr($data['Address Postal Code'], -3, 3);
                    $postcode_parts                     = preg_split(
                        '/ /', $data['Address Postal Code']
                    );
    ...
    function get_fuzzines($data) {
        ...
        foreach ($data as $key => $value) {
            if (preg_match(
                    '/Address Internal|Address Building|Address Street Name/', $key
                ) and $value != ''
            ) {
                $street_empty = false;
                $total_empty  = false;
        ...
        switch ($tipo) {
            ...
            case('3lines'):
                ...
                if ($this->data['Address Internal'] != '' and $this->data['Address Building'] != '') {
                    ...
                    $lines[1] = _trim(
                        $this->data['Address Internal'].' '.$this->data['Address Building']
                    );
            ...
            case('street'):
                ...
                if ($this->data['Address Street Number Position'] == 'Right') {
                    return _trim(
                        $this->data['Address Street Name'].' '.$this->data['Address Street Type'].' '.$this->data['Address Street Number']
                    );

                } else {
                    return _trim(
                        $this->data['Address Street Number'].' '.$this->data['Address Street Name'].' '.$this->data['Address Street Type']
                    );
                ...
                switch ($this->data['Address Country Code']) {
                    case('ESP'):
                        return _trim(
                            $this->data['Address Street Type'].' '.$this->data['Address Street Name'].' '.$this->data['Address Street Number']
                        );
                        break;
                    default:
                        return _trim(
            ...
            case('label'):
                ...
                $country_name = $this->get_localized_country_name(
                    $this->data['Address Country Code'], $locale
                );
    ...
    public static function location($data, $flag = 'left') {
        ...
        if ($data['Military Address'] == 'Yes') {
            $location = sprintf(
                '<img src="/art/flags/%s.gif" title="%s"> %s', strtolower($data['Address Country 2 Alpha Code']), $data['Address Country Code'], $data['Military Installation Type']
            );
        } else {
            ...
            if ($data['Address Fuzzy'] == 'Yes') {
                if (preg_match('/country/i', $data['Address Fuzzy Type'])) {
                    $location = sprintf(
                        '<img src="/art/flags/%s.gif" title="%s"> %s', strtolower($data['Address Country 2 Alpha Code']), $data['Address Country Code'], _('Unknown')
                    );

                    return _trim($location);
                } elseif (preg_match('/town/i', $data['Address Fuzzy Type'])) {
                    $location = sprintf(
                        '<img src="/art/flags/%s.gif" title="%s"> %s', strtolower($data['Address Country 2 Alpha Code']), $data['Address Country Code'],
                        _('Somewhere in').' '.$data['Address Country Name']
                    );
            ...
            if ($flag == 'none') {
                $location = sprintf(
                    '%s %s', $data['Address Town'], $data['Address Country Code']
                );
            } else {
                if ($flag == 'right') {
                    $location = sprintf(
                        '%s <img src="/art/flags/%s.gif" title="%s">', $data['Address Town'], strtolower($data['Address Country 2 Alpha Code']), $data['Address Country Code']
                    );
                } else {
                    $location = sprintf(
                        '<img src="/art/flags/%s.gif" title="%s"> %s', strtolower($data['Address Country 2 Alpha Code']), $data['Address Country Code'], $data['Address Town']
                    );
                }
            }
    ...
    function plain($data, $locale = 'en_GB') {
        ...
        if ($data['Military Address'] == 'Yes') {
            ...
            $address        = '';
            $header_address = _trim(
                $data['Address Internal'].' '.$data['Address Building']
            );
            if ($header_address != '') {
                $address .= $header_address.$separator;
            }

            $street_address = _trim(
                $data['Address Street Number'].' '.$data['Address Street Name'].' '.$data['Address Street Type']
            );
            if ($street_address != '') {
                $address .= $street_address.$separator;
            }
    ...
    function get_formatted_address($separator, $locale = 'en_GB') {
        ...
        if ($this->data['Military Address'] == 'Yes') {
            ...
            if ($locale == 'es_ES') {
                ...
                $ps_address = _trim(
                    $this->data['Address Postal Code'].' '.$this->data['Address Country Second Division']
                );
                if ($ps_address != '') {
                    $address .= $ps_address.$separator;
                }
    ...
    function update_parents($parents = false, $add_parent_history = true) {
        ...
        foreach ($parents as $parent) {
            ...
            while ($row = mysql_fetch_array($res)) {
                ...
                if ($parent == 'Contact') {
                    $parent_object = new Contact(
                        $row['Parent Key']
                    );
                    $old_princial_address
                                   = $parent_object->data[$parent.' Main XHTML Address'];
                    $parent_label  = _(
                        'Contact'
                    );
                    $parent_object->editor
                                   = $this->editor;
                    $parent_object->data[$parent.' Main XHTML Address']
                                   = $this->display(
                        'xhtml'
                    );
                    $parent_object->data[$parent.' Main Plain Address']
                                   = $this->display(
                        'plain'
                    );
                    ...
                    $store                 = new Store(
                        $parent_object->data['Customer Store Key']
                    );
                    ...
                    $sql = sprintf(
                        'UPDATE `Customer Dimension` SET `Customer Main Plain Postal Code`=%s, `Customer Main Address Incomplete`=%s WHERE `Customer Key`=%d ', prepare_mysql(
                            preg_replace(
                                '/[^a-z^A-Z^\d]/', '', $this->data['Address Postal Code']
                            ), false
                        ), prepare_mysql(($this->data['Address Fuzzy'])), $row['Parent Key']
                    );
                    mysql_query($sql);
                    $lines = $this->display('3lines', $locale);
                    ...
                    $secondary_country_division
                        = $this->data['Address Country Third Division'];
                    if ($secondary_country_division != '') {
                        $secondary_country_division .= ', ';
                    }
                    ...
                    $this->data['Customer Main Postal Code Country Second Division']
                                                               = _trim(
                        $this->data['Address Postal Code'].' '.$secondary_country_division
                    );
                    $this->data['Customer Main Country Second Division']
                                                               = _trim(
                        $secondary_country_division
                    );

                    $sql = sprintf(
                        'UPDATE `Customer Dimension` SET `Customer Main Address Line 1`=%s,`Customer Main Address Line 2`=%s,`Customer Main Address Line 3`=%s,`Customer Main Address Lines`=%s,`Customer Main Postal Code Country Second Division`=%s,`Customer Main Country Second Division`=%s WHERE `Customer Key`=%d '


                        , prepare_mysql(
                            $this->data['Customer Main Address Line 1'], false
                        ), prepare_mysql(
                            $this->data['Customer Main Address Line 2'], false
                        ), prepare_mysql(
                            $this->data['Customer Main Address Line 3'], false
                        ), prepare_mysql(
                            $this->data['Customer Main Address Lines'], false
                        ), prepare_mysql(
                            $this->data['Customer Main Postal Code Country Second Division'], false
                        ), prepare_mysql(
                            $this->data['Customer Main Country Second Division'], false
                        )
                ...
                $parent_object->data[$parent.' Main Country']
                    = $this->data['Address Country Name'];
                $parent_object->data[$parent.' Main Country Code']
                    = $this->data['Address Country Code'];
                $parent_object->data[$parent.' Main Location']
                    = $this->location(
                    $this->data
                );
                ...
                $sql = sprintf(
                    "update `$parent Dimension` set `$parent Main Plain Address`=%s,`$parent Main XHTML Address`=%s,`$parent Main Country Key` =%d,`$parent Main Country` =%s,`$parent Main Country Code` =%s,`$parent Main Location` =%s where `$parent Key`=%d",
                    prepare_mysql(
                        $parent_object->data[$parent.' Main Plain Address']
                    ), prepare_mysql(
                        $parent_object->data[$parent.' Main XHTML Address']
                    ), $parent_object->data[$parent.' Main Country Key'], prepare_mysql($parent_object->data[$parent.' Main Country']), prepare_mysql(
                        $parent_object->data[$parent.' Main Country Code']
                    ), prepare_mysql(
                        $parent_object->data[$parent.' Main Location']
                    ), $parent_object->id
                ...
                if ($parent == 'Customer') {
                    $sql = sprintf(
                        "update `$parent Dimension` set `$parent Main Town` =%s,`$parent Main Postal Code` =%s,`$parent Main Country First Division` =%s ,`Customer Main Country 2 Alpha Code`=%s where `$parent Key`=%d",
                        prepare_mysql($this->data['Address Town']), prepare_mysql($this->data['Address Postal Code']), prepare_mysql(
                            $this->data['Address Country First Division']
                        ), prepare_mysql(
                            $this->data['Address Country 2 Alpha Code']
                        )
                ...
                if ($principal_address_changed and $add_parent_history) {
                    ...
                    if ($old_princial_address == '') {
                        ...
                        $history_data['History Abstract']
                                                             = 'Address Associated';
                        $history_data['History Details']
                                                             = '<div class="history_address" style="border:1px solid grey;padding:5px;width:250px">'.$this->display('xhtml')."</div> "._(
                                'address associated with'
                            )." ".$parent_object->get_name()." ".$parent_label;
                        $history_data['Action']              = 'associated';
                        $history_data['Direct Object']       = $parent;
                        $history_data['Direct Object Key']
                                                             = $parent_object->id;
                        ...
                        $history_data['History Details']     = _(
                                'Address changed from'
                            ).' <div  class="history_address" style="border:1px solid grey;padding:5px;width:250px">'.$old_princial_address.'</div> '._('to')
                            .' <div  class="history_address" style="border:1px solid grey;padding:5px;width:250px">'.$this->display('xhtml')."</div> "._('in')." ".$parent_object->get_name()." "
                            .$parent_label;
        ...
        $sql  = sprintf(
            "SELECT `Customer Key`  FROM  `Customer Dimension` WHERE `Customer Main Delivery Address Key`=%d GROUP BY `Customer Key`", $this->id
        );
        ...
        while ($row2 = mysql_fetch_array($res2)) {
            ...
            $sql = sprintf(
                'UPDATE `Customer Dimension` SET `Customer XHTML Main Delivery Address`=%s,`Customer Main Delivery Address Lines`=%s,`Customer Main Delivery Address Town`=%s,`Customer Main Delivery Address Country`=%s ,`Customer Main Delivery Address Postal Code`=%s,`Customer Main Delivery Address Country Code`=%s,`Customer Main Delivery Address Country 2 Alpha Code`=%s,`Customer Main Delivery Address Country Key`=%d  WHERE `Customer Key`=%d ',
                prepare_mysql($this->display('xhtml', $locale)), prepare_mysql($this->display('lines', $locale), false), prepare_mysql($this->data['Address Town'], false),
                prepare_mysql($this->data['Address Country Name']), prepare_mysql($this->data['Address Postal Code'], false), prepare_mysql($this->data['Address Country Code']),
                prepare_mysql($this->data['Address Country 2 Alpha Code']), $this->data['Address Country Key'], $row2['Customer Key']
            );
        ...
        $sql  = sprintf(
            "SELECT `Customer Key`  FROM  `Customer Dimension` WHERE `Customer Billing Address Key`=%d GROUP BY `Customer Key`", $this->id
        );
        $res2 = mysql_query($sql);
        while ($row2 = mysql_fetch_array($res2)) {
            ...
            $sql = sprintf(
                'UPDATE `Customer Dimension` SET `Customer XHTML Billing Address`=%s,`Customer Billing Address Lines`=%s,
						...
						WHERE `Customer Key`=%d ', prepare_mysql($this->display('xhtml', $locale)), prepare_mysql($this->display('lines', $locale), false), prepare_mysql($lines[1], false),
                prepare_mysql($lines[2], false), prepare_mysql($lines[3], false), prepare_mysql($this->data['Address Town'], false), prepare_mysql($this->data['Address Postal Code'], false)

                , prepare_mysql($this->data['Address Country Code']), prepare_mysql($this->data['Address Country 2 Alpha Code'])
    ...
    public static function is_country_d1($country_d1, $country_2a_code) {
        ...
        if ($country_2a_code) {
            $extra_where = sprintf(
                'and `Country 2 Alpha Code`=%s', prepare_mysql($country_2a_code)
            );
        }
        $sql = sprintf(
            "SELECT `Geography Key` AS id FROM kbase.`Country First Division Dimension` WHERE `Country First Division Name`=%s %s", prepare_mysql($country_d1), $extra_where
        );
    ...
    public static function is_country_d2($str, $country_2a_code = false, $country_d1_code = false) {
        ...
        if ($country_d1_code) {
            $extra_where .= sprintf(
                'and `Country First Division Code`=%s', prepare_mysql($country_d1_code)
            );
        }
        $sql = sprintf(
            "SELECT `Geography Key` AS id FROM kbase.`Country Second Division Dimension` WHERE `Country Second Division Name`=%s %s", prepare_mysql($str), $extra_where
        );
    ...
    public static function is_country_key($key) {
        ...
        $sql = sprintf(
            "SELECT `Country Key` FROM kbase.`Country Dimension`  WHERE `Country Key`=%d", $key
        );
        //    PRINT $sql;
        $result = mysql_query($sql);
        if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    public static function is_country_code($code) {
        ...
        $sql    = sprintf(
            "SELECT `Country Key` FROM kbase.`Country Dimension`  WHERE `Country Code`=%s", prepare_mysql($code)
        );
        $result = mysql_query($sql);
        if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            return true;
    ...
    public static function is_country_2alpha_code($code) {
        ...
        $sql    = sprintf(
            "SELECT `Country Key` FROM kbase.`Country Dimension`  WHERE `Country 2 Alpha Code`=%s", prepare_mysql($code)
        );
        $result = mysql_query($sql);
        if ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            return true;
    ...
    public static function is_valid_postcode($postcode, $country_code) {
        ...
        switch ($country_code) {
            case 'GBR':
                ...
                if (preg_match(
                    '/^([A-PR-UWYZ][A-HK-Y0-9][A-HJKSTUW0-9]?[ABEHMNPRVWXY0-9]?\s?[0-9][ABD-HJLN-UW-Z]{2}|GIR 0AA|BT\d{2}\s*\d[a-z]2)$/i', $postcode
                )) {
                    return true;
                } else {
                    return false;
    ...
    function update_address_type($raw_new_address_types) {
        ...
        foreach ($raw_new_address_types as $raw_new_address_type) {
            if (in_array($raw_new_address_type, $valid_types)) {
                $new_address_types[$raw_new_address_type]
                    = $raw_new_address_type;
            }
        }
        ...
        foreach ($this->data['Type'] as $type) {
            if (!in_array($type, $new_address_types)) {
                //print "deleting $type\n";
                $sql = sprintf(
                    "DELETE FROM `Address Bridge` WHERE `Address Key`=%s AND `Subject Type`=%s AND `Subject Key`=%d  AND `Address Type`=%s ", $this->id, prepare_mysql($this->scope), $this->scope_key,
                    prepare_mysql($type)
                );
                //print "$sql\n";
                mysql_query($sql);
        ...
        foreach ($new_address_types as $type) {
            ...
            if (!in_array($type, $this->data['Type'])) {
                foreach ($this->data['Function'] as $function) {
                    $sql    = sprintf(
                        "SELECT *  FROM `Address Bridge` WHERE `Address Key`=%s AND `Subject Type`=%s AND `Subject Key`=%d AND `Address Function`=%s ", $this->id, prepare_mysql($this->scope),
                        $this->scope_key, prepare_mysql($function)
                    );
                    $res    = mysql_query($sql);
                    $active = 'Yes';
                    $main   = 'No';
                    ...
                    $sql = sprintf(
                        'INSERT INTO `Address Bridge` VALUES (%d,%s,%d,%s,%s,%s,%s)', $this->id, prepare_mysql($this->scope), $this->scope_key, prepare_mysql($type), prepare_mysql($function),
                        prepare_mysql($active, false), prepare_mysql($main, false)
                    );
                    //print "$sql\n";
                    mysql_query($sql);
    ...
    function load_metadata() {
        ...
        $where_scope = sprintf(
            ' and `Subject Type`=%s', prepare_mysql($this->scope)
        );

        $where_scope_key = '';
        if ($this->scope_key) {
            $where_scope_key = sprintf(
                ' and `Subject Key`=%d', $this->scope_key
            );
        }


        $sql = sprintf(
            "SELECT * FROM `Address Bridge` WHERE `Address Key`=%d %s  %s  ORDER BY `Is Main`", $this->id, $where_scope, $where_scope_key
        );
        $res = mysql_query($sql);
        ...
        while ($row = mysql_fetch_array($res)) {
            ...
            $this->data['Address Type'][$row['Address Type']]
                                         = $row['Address Type'];
            $this->data['Address Function'][$row['Address Function']]
                                         = $row['Address Function'];
            $this->data['Address Is Main'][$row['Address Type']]
                                         = $row['Is Main'];
            $this->data['Address Is Active'][$row['Address Type']]
                                         = $row['Is Active'];
    ...
    function get($key) {
        ...
        switch ($key) {
            ...
            case('Address World Region Code'):
                $country = new Country(
                    'code', $this->data['Address Country Code']
                );
    ...
    function set_scope($raw_scope = '', $scope_key = 0) {
        ...
        if (preg_match('/^customers?$/i', $raw_scope)) {
            ...
            if (preg_match('/^(contacts?|person)$/i', $raw_scope)) {
                $scope = 'Contact';
            } else {
                if (preg_match('/^(company?|bussiness)$/i', $raw_scope)) {
                    $scope = 'Company';
                } else {
                    if (preg_match('/^(supplier)$/i', $raw_scope)) {
                        $scope = 'Supplier';
                    } else {
                        if (preg_match('/^(staff)$/i', $raw_scope)) {
                            $scope = 'Staff';
                        }
                    }
                }
    ...
    function get_country_d2_name($id = '') {
        ...
        $sql = sprintf(
            "SELECT `Country Second Division Name` AS name FROM kbase.`Country Second Division Dimension` WHERE `Country Second Division Code`=%d", $id
        );
        //  print $sql;
    ...
    function get_customer_keys() {
        $sql           = sprintf(
            "SELECT `Customer Key` FROM `Customer Dimension` WHERE `Customer Type`='Person' AND `Customer Main Address Key`=%d  ", $this->id
        );
        $customer_keys = array();
        $result        = mysql_query($sql);
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    function get_company_key() {
        $sql = sprintf(
            "SELECT `Company Key` FROM `Company Dimension` WHERE `Company Main Address Key`=%d  ", $this->id
        );
        //print $sql;
        $company_keys = array();
        $result       = mysql_query($sql);
    ...
    function get_contact_key() {
        $sql          = sprintf(
            "SELECT `Contact Key` FROM `Contact Dimension` WHERE `Contact Main Address Key`=%d  ", $this->id
        );
        $contact_keys = array();
        $result       = mysql_query($sql);
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    function disassociate_telecom($telecom_key, $type, $swap_principal = true) {
        ...
        $sql               = sprintf(
            "DELETE FROM `Telecom Bridge` WHERE `Subject Type`='Address' AND  `Subject Key`=%d AND `Telecom Key`=%d ", $this->id, $telecom_key
        ...
        if (count($telecom_keys) == 0) {
            ...
            $sql = sprintf(
                "UPDATE `Address Dimension` SET `Address Main %s Key`=0 , `Address Main Plain %s`='', `Address Main XHTML %s`=''  WHERE `Address Key`=%d ", addslashes($type), addslashes($type),
                addslashes($type), $this->id
            );
            mysql_query($sql);
            //   print "$sql\n";

        } else {
            if ($principal_adddess == $telecom_key and $swap_principal) {
    ...
    function associate_telecom_to_parents($type = 'Telephone', $parent, $parent_key, $telecom_key, $set_as_main = true) {
        ...
        if ($set_as_main and $old_principal_telecom_key != $telecom_key) {
            ...
            $sql = sprintf(
                "update `Telecom Bridge`  set `Is Main`='No' where `Subject Type`='$parent' and  `Subject Key`=%d ", $parent_object->id
    ...
    function get_number_of_associated_telecoms($type) {
        $sql = sprintf(
            "SELECT  count(*) AS num  FROM `Telecom Bridge` TB LEFT JOIN `Telecom Dimension` T ON (T.`Telecom Key`=TB.`Telecom Key`)  WHERE  `Subject Type`='Address' AND `Telecom Type`=%s AND  `Subject Key`=%d ",
            prepare_mysql($type), $this->id
        );
        //print $sql;
        $telecoms = 0;
    ...
    function get_formatted_principal_telephone() {
        ...
        $sql = sprintf(
            "SELECT TB.`Telecom Key` FROM `Telecom Bridge` TB  LEFT JOIN `Telecom Dimension` T ON (T.`Telecom Key`=TB.`Telecom Key`)  WHERE  `Telecom Type`=%s AND   `Subject Type`='Address' AND `Subject Key`=%d AND `Is Main`='Yes'",
            prepare_mysql('Telephone'), $this->id
        );
        // print $sql;
        $res = mysql_query($sql);
        if ($row = mysql_fetch_array($res)) {
    ...
    function get_data_for_ship_to() {
        ...
        $data['Ship To Postal Code']
                                = $this->data['Address Postal Code'];
        $data['Ship To Country Name']
                                = $this->data['Address Country Name'];
        $data['Ship To Country Key']
                                = $this->data['Address Country Key'];
        $data['Ship To Country Code']
                                = $this->data['Address Country Code'];
        $data['Ship To Country 2 Alpha Code']
                                = $this->data['Address Country 2 Alpha Code'];
    ...
    function has_parents() {
        $has_parents = false;
        $sql         = sprintf(
            "SELECT count(*) AS total FROM `Address Bridge`  WHERE  `Address Key`=%d  ", $this->id
        );
        $res         = mysql_query($sql);
        if ($row = mysql_fetch_array($res)) {
            if ($row['total'] > 0) {
    ...
    function delete() {
        ...
        foreach ($parents as $parent) {
            ...
            $res = mysql_query($sql);
            while ($row = mysql_fetch_array($res)) {
                $this->add_history_record_to_parent_after_removed(
                    $parent, $row['Parent Key']
                );
            }
        ...
        $sql = sprintf(
            "SELECT `Customer Key`  ,`Customer Main Address Key` FROM  `Customer Dimension` WHERE `Customer Billing Address Key`=%d ", $this->id
        );
        $res = mysql_query($sql);
        while ($row = mysql_fetch_array($res)) {
        ...
        $sql = sprintf(
            "SELECT `Customer Key`  ,`Customer Main Address Key` FROM  `Customer Dimension` WHERE `Customer Main Delivery Address Key`=%d ", $this->id
        );
        //print "$sql\n";
        $res = mysql_query($sql);
        while ($row = mysql_fetch_array($res)) {
        ...
        $sql = sprintf(
            "DELETE FROM `Address Dimension` WHERE `Address Key`=%d", $this->id
        );
        mysql_query($sql);
        ...
        $sql = sprintf(
            "DELETE FROM `Address Bridge`  WHERE  `Address Key`=%d", $this->id
        );
        mysql_query($sql);
    ...
    function add_history_record_to_parent_after_removed($parent, $parent_key) {
        ...
        $history_data['History Details']     = _('Address').' '.$this->display(
                'plain'
            )." "._('has been deleted from')." ".$parent_object->get_name()." ".$parent_label;
...
*/
    ...
    function get_telecom_keys() {
        $sql = sprintf(
            "SELECT `Telecom Key` FROM `Telecom Bridge` WHERE  `Subject Type`='Address'   AND `Subject Key`=%d "
    ...
    function get_parent_keys($type) {
        ...
        if (!preg_match(
            '/^(Contact|Company|Supplier|User|Staff|Customer)$/', $type
        )
        ) {
            return $keys;
        }
        $sql    = sprintf(
            "SELECT `Subject Key` FROM `Address Bridge` WHERE `Subject Type`=%s AND `Address Key`=%d  ", prepare_mysql($type), $this->id
        );
        $result = mysql_query($sql);
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    function get_ship_to($data) {
        ...
        $shipping_address['Ship To Town']
                                                   = $this->data['Address Town'];
        $shipping_address['Ship To Postal Code']
                                                   = $this->data['Address Postal Code'];
        $shipping_address['Ship To Country Name']
                                                   = $this->data['Address Country Name'];
        $shipping_address['Ship To Country Key']
                                                   = $this->data['Address Country Key'];
        $shipping_address['Ship To Country Code']
                                                   = $this->data['Address Country Code'];
        $shipping_address['Ship To Country 2 Alpha Code']
                                                   = $this->data['Address Country 2 Alpha Code'];
        $shipping_address['Ship To XHTML Address'] = $this->display(
            'xhtml'
        );

        $shipping_address['Ship To Country First Division']
            = $this->data['Address Country First Division'];
        $shipping_address['Ship To Country Second Division']
            = $this->data['Address Country Second Division'];


        if ($this->data['Address Main XHTML Telephone'] != '') {
            $shipping_address['Ship To Telephone']
                = $this->data['Address Main XHTML Telephone'];
        } else {
        ...
        if ($this->data['Address Contact'] == '') {
            ...
            $shipping_address['Ship To Contact Name']
    ...
    function get_billing_to($data) {
        ...
        $billing_address['Billing To Town']
                                                     = $this->data['Address Town'];
        $billing_address['Billing To Postal Code']
                                                     = $this->data['Address Postal Code'];
        $billing_address['Billing To Country Name']
                                                     = $this->data['Address Country Name'];
        $billing_address['Billing To Country Key']
                                                     = $this->data['Address Country Key'];
        $billing_address['Billing To Country Code']
                                                     = $this->data['Address Country Code'];
        $billing_address['Billing To Country 2 Alpha Code']
                                                     = $this->data['Address Country 2 Alpha Code'];
        $billing_address['Billing To XHTML Address'] = $this->display(
            'xhtml'
        );

        $billing_address['Billing To Country First Division']
            = $this->data['Address Country First Division'];
        $billing_address['Billing To Country Second Division']
            = $this->data['Address Country Second Division'];


        if ($this->data['Address Main XHTML Telephone'] != '') {
            $billing_address['Billing To Telephone']
                = $this->data['Address Main XHTML Telephone'];
        } else {
            $billing_address['Billing To Telephone']
                = $data['Billing To Telephone'];
        }
        ...
        $billing_address['Billing To Company Name']
                                                    = $data['Billing To Company Name'];
