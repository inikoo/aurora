...
$number_records = 0;
$sql            = sprintf(
    'SELECT `Timeseries Key`,`Timeseries Number Records` FROM `Timeseries Dimension` WHERE `Timeseries Parent`="Category" AND `Timeseries Parent Key`=%s AND `Timeseries Frequency`="Daily" AND  `Timeseries Type`="PartCategorySales" ',
    $state['key']
);
    ...
    json_encode(
        array(
            'valid_from'              => $category->get(
                'Part Category Valid From'
            ),
            'valid_to'                => ($category->get('Category Status') == 'NotInUse'
