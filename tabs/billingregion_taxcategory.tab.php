...
$smarty->assign(
    'js_code', 'js/injections/billingregion_taxcategory.'.(_DEVEL ? '' : 'min.').'js'
);


$smarty->assign(
    'aux_templates', array('table.aux/billingregion_taxcategory.tpl')
);

$stores_data = array();
$sql         = sprintf(
    'SELECT `Store Key`,`Store Code`,`Store Name` FROM `Store Dimension`'
);
...
if ($result = $db->query($sql)) {
    ...
    foreach ($result as $data) {
        ...
        $stores_data[$data['Store Key']] = array(
            'label'   => $data['Store Name'],
            'checked' => (in_array(
                $data['Store Key'], $excluded_stores
            ) ? false : true)
        );
    }
