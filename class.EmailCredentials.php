...
class EmailCredentials extends DB_Table {
    ...
    function get_data($key, $tag) {
        ...
        if ($key == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Email Credentials Dimension` WHERE `Email Credentials Key`=%d", $tag
            );
        } else {
            return;
        }
    ...
    function find($raw_data, $options) {
        ...
        $sql = sprintf(
            "SELECT `Email Credentials Key` FROM `Email Credentials Dimension` WHERE `Email Address`=%s  ", prepare_mysql($data['Email Address'])
        );
    ...
    function create($data) {
        ...
        $sql    = sprintf(
            "INSERT INTO `Email Credentials Dimension` %s %s", $keys, $values
        );
        //print $sql;
        if (mysql_query($sql)) {
            ...
            $this->msg = "Error can not create email credentials";
        }
    }
    ...
    function get_amazon_Data() {
        return array(
            'email'      => $this->data['Email Address Amazon Mail'],
            'access_key' => $this->data['Amazon Access Key'],
            'secret_key' => $this->data['Amazon Secret Key']
        );
    }
    ...
    function get_smtp_data() {
        switch ($this->data['Email Provider']) {
            case 'Gmail' :
                return array(
                    'login'           => $this->data['Email Address Gmail'],
                    'password'        => $this->get_password(CKEY),
                    'outgoing_server' => 'smtp.gmail.com'
                );
                break;
            case 'Other':
                return array(
                    'login'           => $this->data['Email Address Other'],
                    'password'        => $this->get_password(CKEY),
                    'outgoing_server' => $this->data['Outgoing Mail Server']
                );
                break;
        }
    ...
    function save_gmail_data($login, $password) {
        ...
        $sql      = sprintf(
            "INSERT INTO `Email Credentials Dimension` (`Email Credentials Method`, `Email Provider`, `Email Address Gmail`, `Password Gmail`, `Outgoing Mail Server`) VALUES ('%s', '%s', '%s','%s', '%s')",
            'SMTP', 'Gmail', $login, $password, 'smtp.gmail.com'
        );
        mysql_query($sql);
    ...
    function save_other_data($email, $login, $password, $incoming_server, $outgoing_server) {
        $password = $this->encrypt_password($password);
        $sql      = sprintf(
            "INSERT INTO `Email Credentials Dimension` (`Email Credentials Method`, `Email Provider`, `Email Address Other`, `Login Other`, `Password Other`, `Incoming Mail Server`, `Outgoing Mail Server`) VALUES ('%s', '%s', '%s','%s','%s', '%s', '%s')",
            'SMTP', 'Other', $email, $login, $password, $incoming_server, $outgoing_server
        );
        mysql_query($sql);
    ...
    function delete() {
        ...
        $sql = sprintf(
            "DELETE FROM `Email Credentials Dimension` WHERE `Email Credentials Key`=%d", $this->id
        );
        mysql_query($sql);
    }
