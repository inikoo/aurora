    ...
    function layoutNode(node, parentMaxWidth) {
        ...
        for (i = 0; i < node.children.length; ++i) {
            ...
            if (getAlignItem(node, child) === CSS_ALIGN_STRETCH &&
                getPositionType(child) === CSS_POSITION_RELATIVE && !isUndefined(node.layout[dim[crossAxis]]) && !isDimDefined(child, crossAxis)) {
                ...
                for (ii = 0; ii < 2; ii++) {
                    ...
                    if (!isUndefined(node.layout[dim[axis]]) && !isDimDefined(child, axis) &&
        ...
        while (endLine < node.children.length) {
            ...
            for (i = startLine; i < node.children.length; ++i) {
                ...
                if (isFlexWrap(node) && !isUndefined(node.layout[dim[mainAxis]]) &&
