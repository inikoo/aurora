...
Event.addListener(window, "load", function() {
	    ...
	    var OrdersColumnDefs = [
			     {key:"code", label:"<?php echo _('Code')?>",width:70,sortable:true,className:"aleft",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_ASC}}
				     ...
				     ,{key:"quantity",label:"<?php echo _('Qty')?>", width:50,sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}}
				       ,{key:"gross",label:"<?php echo _('Amount')?>",<?php echo(($_SESSION['state']['order_cancelled']['items']['view']=='basket')  ?'':'hidden:true,')?>  width:70,sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}}
				       ,{key:"discount",label:"<?php echo _('Discounts')?>",<?php echo(($_SESSION['state']['order_cancelled']['items']['view']=='basket')  ?'':'hidden:true,')?>  width:70,sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}}
				     ,{key:"to_charge",label:"<?php echo _('To Charge')?>",<?php echo(($_SESSION['state']['order_cancelled']['items']['view']=='basket')  ?'':'hidden:true,')?> width:85,sortable:false,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}}
				     ,{key:"created",label:"<?php echo _('Created')?>", <?php echo(($_SESSION['state']['order_cancelled']['items']['view']=='times')  ?'':'hidden:true,')?>width:190,sortable:true,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}}
				     ,{key:"last_updated",label:"<?php echo _('Updated')?>",<?php echo(($_SESSION['state']['order_cancelled']['items']['view']=='times')  ?'':'hidden:true,')?> width:190,sortable:true,className:"aright",sortOptions:{defaultDir:YAHOO.widget.DataTable.CLASS_DESC}}
					 ];
						     ...
						     this.dataSource0, {draggableColumns:true,
							   renderLoopSize: 50,generateRequest : myRequestBuilder
								       ,paginator : new YAHOO.widget.Paginator({
									       rowsPerPage    : <?php echo$_SESSION['state']['order_cancelled']['items']['nr']?>,containers : 'paginator0', 
								     ...
								     ,sortedBy : {
									 key: "<?php echo$_SESSION['state']['order_cancelled']['items']['order']?>",
									 dir: "<?php echo$_SESSION['state']['order_cancelled']['items']['order_dir']?>"
								     }
							   ,dynamicData : true
  ...
  this.table0.table_id=tableid;
     this.table0.subscribe("renderEvent", myrenderEvent);
	    ...
	    this.table0.filter={key:'<?php echo$_SESSION['state']['order_cancelled']['items']['f_field']?>',value:'<?php echo$_SESSION['state']['order_cancelled']['items']['f_value']?>'};
