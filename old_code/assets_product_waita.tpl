...
<span class="nav2"><a href="assets_family.php?id={$family_id}">{$family}</a></span>
  ...
  <div id="yui-main">
	...
	<div style="width:220px;margin:0 10px 0 0;background:green"  >
...
{if $outall>0 and $view_sales}
	<table class="show_info_product" style="margin-top:10px">
	  <tr><td>{t}TS{/t}:</td><td class="aright" >{$tsall}</td><td class="aright" >{$outall}</td></tr>
	  <tr><td>{t}AWS{/t}:</td><td class="aright">{$awtsall}</td><td class="aright" >{$awsall}</td></tr>
	  <tr><td>{t}LQAQS{/t}:</td><td class="aright">{$awtsq}</td><td class="aright" >{$awsq}</td></tr>
	  <tr><td>{t}LWS{/t}:</td><td class="aright">{$tsw}</td><td class="aright" >{$outw}</td></tr>
	</table>
	{/if}
	...
	<h1>{$description}</h1>
	<div style="width:200px;margin:0 10px 0 0"">
	<table class="show_info_product" "{if $edit}class="edit"{/if}>
	 {if $edit} <tr><td colspan="2">{t}Product Identification{/t}</td></tr>{/if}
	  ...
	  <tr>
	    <td>{t}Product Family{/t}:</td><td>{$family}</td>{if $edit}<td class="aright"><span style="float:left">{t}Current Family:{/t}<span>{$family}</span> <span>[{t}Families list{/t}]</span></span> {t}Family unique code: {/t}<input value="{$family_id}"> </td>{/if}
	  </tr>
	  	  <tr>
...
<br>
	<table  class="show_info_product" {if $edit}class="edit"{/if}>
	  ...
	  <td>{t}Item units{/t}:</td><td>{$units_tipo}</td>{if $edit}
	  <td class="aright">
	  ...
	  {/if}
	  <tr ><td>{t}Outer Weight{/t}:</td>{if $edit}<td>{$ow}</td><td  class="aright" ><input type="text" value="{$ow}"> {t}Kg{/t}</td>{/if}</tr>
	  <tr><td>{t}Outer Dimensions{/t}:</td><td>{$odim}</td>{if $edit}<td  class="aright" ><span id="v_odim_shape_ex" >{$ashape_example[$oshape]}</span><input id="v_odim"  type="text" value="{$odim}"  onkeypress="return key_filter(event,{$key_filter_number})"  > 
	   ...
	   <tr><td>{t}Colour{/t}:</td><td>{$color}</td>{if $edit}<td  class="aright"><span>{t}colour1,colour2,...{/t} <span>[{t}Colour list{/t}]</span></span><input type="text" value="{$color}"> 
	   </td>{/if}
	   </tr>
	</table>
<br>
...
<div style="border:none;padding:0;margin:0;width:270px" >
	...
	<br>
	<table  class="show_info_product" {if $edit}class="edit"{/if}   >
	  <tr><td colspan="2">{t}Suppliers{/t}</td>{if $edit}<td class="aright" ><button>{t}Add new Supplier{/t}</button></td>{/if}</tr>
	  {if $view_suppliers}
	  {if $suppliers>0}
	  {foreach from=$suppliers_name item=supplier key=supplier_id }
	  <tr><td>{t}Supplier{/t}:</td>{if $edit}<td><a href="supplier.php?id={$supplier_id}">{$supplier}</a></td><td  class="aright" ><button>{t}Delete{/t}</button> <button>{t}Desactivate{/t}</button> </td>{/if}</tr>
	  <tr><td>{t}Supplier code{/t}:</td>{if $edit}<td>{$suppliers_code[$supplier_id]}</td><td  class="aright" ><input value="{$suppliers_code[$supplier_id]}" type="text"></td>{/if}</tr>
	  <tr><td>{t}Unit Cost Price{/t}:</td>{if $edit}<td>{$suppliers_price[$supplier_id]}</td><td class="aright" >£<input value="{$suppliers_price[$supplier_id]}" type="text"/></td>{/if}</tr>
	  {/foreach}
	  {else}
...
{/if}
	...
	<table  class="show_info_product"  {if $edit}class="edit"{/if}>
	  ...
	  <tr>
	    <td>{t}Units per outer{/t}:</td><td>{$units}</td>{if $edit}<td class="aright"><input type="text" value="{$units}"> 
	    </td>{/if}
	  </tr>
	  <tr>
...
<div  style="border:none;padding:0;margin:0;background:green;width:100%"    >

	  <div style="border:none" id="extended_description">{$short_description}ssss</div>
	</div>
...
<input type="hidden" onchange="alert('caca')" name="product_id" value="{$id}" />
