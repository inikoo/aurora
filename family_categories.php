...
if (isset($_REQUEST['store_id']) ) {
	...
	if (count($user->storess)==0) {
		...
		header('Location: family_categories.php?store_id='.$user->stores[0]);
		exit;
...
$block_view=$_SESSION['state']['family_categories']['root_block_view'];
...
$tipo_filter=$_SESSION['state']['family_categories']['main_categories']['f_field'];
$smarty->assign('filter1',$tipo_filter);
$smarty->assign('filter_value1',$_SESSION['state']['family_categories']['main_categories']['f_value']);
...
$smarty->assign('title', _('Family Categories'));
...
$sql=sprintf("select count(*) as num ,`Type` from  `Product Family Category History Bridge` where  `Store Key`=%d group by  `Type`",$store->id);
...
$smarty->display('family_categories.tpl');
?>
