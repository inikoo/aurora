...
switch ($_REQUEST['action']) {
    ...
    case 'get_delivery_note_items':
        ...
        $sql =
            sprintf('select `Inventory Transaction Key`,P.`Part SKU`,`Part Package Description`,`Part Package Weight`,`Required`,L.`Location Key`,`Part SKO Barcode` from `Inventory Transaction Fact` ITF left join `Part Dimension` P on (P.`Part SKU`=ITF.`Part SKU`) left join `Location Dimension` L on (L.`Location Key`=ITF.`Location Key`)  where `Delivery Note Key`=%d ', $_REQUEST['delivery_note_key']);
