...
function mailing_list() {
	...
	$rtext=sprintf("%s %s",number($total_records),ngettext("recipient", "recipients", $total_records));
...
function color_schemes() {
	...
	$rtext=sprintf("%s %s",number($total_records),ngettext("scheme", "schemes", $total_records));
	if ($total_records>$number_results)
		$rtext_rpp=sprintf("(%d%s)",$number_results,_('rpp'));
	else
		$rtext_rpp='('._('Showing all').')';
