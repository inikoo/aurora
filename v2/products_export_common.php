...
if ($row=mysql_fetch_assoc($res)) {
	...
	foreach ($default_fields as $default_field) {
		...
		case '`Product Main Department Code`':
			$field_label=_('Department');
			break;
		case '`Product Family Code`':
			$field_label=_('Family');
			break;
		case 'P.`Product ID`':
			$field_label=_('Id');
			break;
		...
		case '`Image URL`':
			$field_label=_('Image URL');
			break;		
		case '`Product Total Acc Invoiced Amount`':
			$field_label=_('Sales').' ('._('all').')';
			break;	
		case '`Product 1 Year Acc Invoiced Amount`':
