				...
				if (preg_match('/\(.*\)/',$match,$match_products)) {
					...
					foreach($match_products as $match_product){
						...
						foreach($_code as $product_code){
							...
							$sql=sprintf("select * from `Product Dimension` where `Product Family Key`=%d and `Product Web State`!='Offline' and `Product Store Key`='1' %s %s", $product_code, $_range, $_limit);
							//print "$sql\n";
							$result=mysql_query($sql);
							
							while($row=mysql_fetch_array($result)){
								print $row['Product Code']."\n";
								$sql=sprintf("insert into `Product Page Bridge` (`Product ID`,`Page Key`,`Type`) values (%d, %d, '%s')", $row['Product ID'], $page_key, 'List');
								print $sql."\n";
								mysql_query($sql);
							}
						}
			...
			foreach($matches[0] as $match){
				if (preg_match('/\(.*\)/',$match,$match_products)) {
					...
					foreach($match_products as $match_product){
						...
						$result=mysql_query($sql);
						if($row=mysql_fetch_array($result)){
							$sql=sprintf("insert into `Product Page Bridge` (`Product ID`,`Page Key`,`Type`) values (%d, %d, '%s')", $row['Product ID'], $page_key, 'Button');
							//print $sql;
							mysql_query($sql);
						}
					}
