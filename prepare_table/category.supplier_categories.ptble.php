...
$group_by='';
$table='`Category Dimension` C  left join `Supplier Category Dimension` SC on (`Category key`=SC.`Supplier Category Key`) left join `Supplier Category Data` SCD on (`Category key`=SCD.`Supplier Category Key`) ';
...
if (in_array($db_period, array('Total', '3 Year'))) {
	...
	$fields_1yb="`Supplier Category $db_period Acc 1Yb Invoiced Amount` as sales_1y";
