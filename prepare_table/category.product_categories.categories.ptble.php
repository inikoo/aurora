...
$where = sprintf(
    "where C.`Category Parent Key`=%d  ", $parameters['parent_key']
);
...
if ($order == 'code') {
    ...
    $order = 'products';
}elseif ($order == 'subdepartments') {
    $order = '`Product Category Subdepartments`';
...
$fields =
    ...
    `Category Number Subjects` as subjects, `Product Category Subdepartments` as  sub_departments,
...
`Product Category Currency Code`,$yb_fields
 ...
 ";
$table
    = ' `Category Dimension` C   left join `Product Category Dimension` P on (P.`Product Category Key`=C.`Category Key`) left join `Product Category Data` D on (D.`Product Category Key`=C.`Category Key`) left join `Product Category DC Data` DC on (DC.`Product Category Key`=C.`Category Key`)';
