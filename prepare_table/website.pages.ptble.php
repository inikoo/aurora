...
$table='`Page Store Dimension` PS left join `Page Store Data Dimension` PSD on (PS.`Page Key`=PSD.`Page Key`) left join `Page Dimension` P on (P.`Page Key`=PS.`Page Key`) left join `Site Dimension` S on (S.`Site Key`=PS.`Page Site Key`) ';
...
case('site'):
	...
	break;
case('department'):
	$where.=sprintf('  and `Page Parent Key`=%d  and `Page Store Section`="Department Catalogue"  ', $parameters['parent_key']);
	break;
case('product'):
	$where.=sprintf('  and `Page Parent Key`=%d  and `Page Store Section`="Product Description"  ', $parameters['parent_key']);
	break;
case('family'):
	$where.=sprintf('  and `Page Parent Key`=%d  and `Page Store Section`="Family Catalogue"  ', $parameters['parent_key']);
	break;
case('product_form'):
	$where.=sprintf('  and `Product ID`=%d   ', $parameters['parent_key']);
...
$sql_totals="select count(Distinct PS.`Page Key`) as num from $table  $where  ";

$fields="*,`Site SSL`,(`Page Store Number Out of Stock Products`/`Page Store Number Products`) as percentage_out_of_stock,`Site Code`,S.`Site Key`,`Page Short Title`,`Page Preview Snapshot Image Key`,`Page Store Section`,`Page Parent Code`,`Page Parent Key`,`Page URL`,P.`Page Key`,`Page Store Title`,`Page Code`
";
?>
