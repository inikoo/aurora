...
if (isset($parameters['elements'])) {
	...
	case('tax_status'):
		...
		foreach ($parameters['elements'][$parameters['elements_type']]['items'] as $_element=>$element_data) {
...
if ($order=='tax_number') {
	...
	$order='refunds';
}else if ($order=='country_code') {
	$order='`Invoice Billing Country 2 Alpha Code`,`Invoice Tax Number`';

	if ($order_direction!='') {
		$order='`Invoice Billing Country 2 Alpha Code` desc ,`Invoice Tax Number`';
