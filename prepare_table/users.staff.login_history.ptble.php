...
if ($parameters['f_field'] == 'handle' and $f_value != '') {
    ...
    if ($parameters['f_field'] == 'ip' and is_numeric($f_value)) {
...
$table
    = ' `User Log Dimension` UL left join `User Dimension` U on (U.`User Key`=UL.`User Key`) ';

$sql_totals
    = "select count(Distinct `User Log Key`) as num from $table  $where  ";


$fields
    = "`User Alias`,`User Parent Key`,U.`User Key`,`User Log Key`,`User Handle`,`IP`,`Start Date`,`Logout Date`";
