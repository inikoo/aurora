...
/*
 ...
 Created: 19 August 2017 at 13:04:19 GMT+5:30,  Delhi Airport, India
...
$where = 'where (( `Order State`="InWarehouse"  and `Order Delivery Note Alert`="Yes") or `Order Replacements In Warehouse with Alerts`>0) ';
...
$fields
    = '`Order Invoiced`,`Order Number Items`,`Order Store Key`,`Payment Account Name`,`Order Payment Method`,`Order Balance Total Amount`,`Order Payment State`,`Order State`,`Order Type`,`Order Currency Exchange`,`Order Currency`,O.`Order Key`,O.`Order Public ID`,`Order Customer Key`,`Order Customer Name`,O.`Order Last Updated Date`,O.`Order Date`,`Order Total Amount`,
     (select group_concat(`Delivery Note Key`) from `Delivery Note Dimension` where `Delivery Note Order Key`=O.`Order Key`   ) as delivery_notes,`Order Priority Level`,`Order Care Level`,`Order Shipping Level`,`Order Checkout Block Payment Account Key`,`Order To Pay Amount`
