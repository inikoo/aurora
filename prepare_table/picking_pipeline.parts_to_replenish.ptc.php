...
class prepare_table_picking_pipeline_parts_to_replenish extends prepare_table {
    ...
    function prepare_table() {
        ...
        $this->table = "
                        `Part Location Dimension` PL left join `Location Dimension` L on (PL.`Location Key`=L.`Location Key`) 
                         left join `Location Picking Pipeline Bridge` on (PL.`Location Key`=`Location Picking Pipeline Location Key`) 
                            left join `Part Dimension` P on (PL.`Part SKU`=P.`Part SKU`) left join 
                            `Warehouse Flag Dimension` on (`Warehouse Flag Key`=`Location Warehouse Flag Key`)
     ";

        $this->fields = ' `Part Symbol`, `Part Distinct Locations`,  P.`Part Current On Hand Stock`, ';
        $this->fields .= ' `Location Warehouse Key`,`Quantity On Hand`,`Minimum Quantity`,`Maximum Quantity`,PL.`Location Key`,`Location Code`,P.`Part Reference`,`Warehouse Flag Color`,`Warehouse Flag Key`,`Warehouse Flag Label`,PL.`Part SKU`,';
        $this->fields .= "   IFNULL((select GROUP_CONCAT(L.`Location Key`,':',L.`Location Code`,':',`Can Pick`,':',`Quantity On Hand` SEPARATOR ',') from `Part Location Dimension` PLD  left join `Location Dimension` L on (L.`Location Key`=PLD.`Location Key`) where PLD.`Part SKU`=P.`Part SKU`),'') as location_data,";
        $this->fields .= '    `Part Next Deliveries Data`,`Part Units Per Package`,`Part Package Description` ,  IFNULL(JSON_EXTRACT(`Part Location Metadata`,\'$.stock_in_process\'),0) + IFNULL(JSON_EXTRACT(`Part Location Metadata`,\'$.stock_ordered_paid\'),0)  as ordered_quantity,';
        $this->fields .= '  (`Quantity On Hand`- IFNULL(JSON_EXTRACT(`Part Location Metadata`,\'$.stock_in_process\'),0) - IFNULL(JSON_EXTRACT(`Part Location Metadata`,\'$.stock_ordered_paid\'),0) )  as effective_stock  ';
...
<span class="link"  onClick="change_view(\'locations/'.$data['Location Warehouse Key'].'/'.$_locations_data[0].'\')" >'.$_locations_data[1].'</span>
            ...
            foreach ($next_deliveries_array as $next_delivery) {
                ...
                $next_deliveries .= '<div class="as_row "><div class="as_cell padding_left_5" style="min-width: 100px" >'.$next_delivery['formatted_link'].'</div><div class="padding_left_10 as_cell strong" style="text-align: right;min-width: 40px" title="'._(
                        'SKOs ordered'
                    ).'">+'.number(
