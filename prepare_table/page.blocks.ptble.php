...
$table
    = '`Webpage Block Bridge` B left join  `Webpage Dimension` P on (`Webpage Block Webpage Key`=`Webpage Key`)';

switch ($parameters['parent']) {
    case('page'):
        $where = sprintf(
            ' where  `Webpage Block Webpage Key`=%d  ', $parameters['parent_key']
        );
        break;
    case('website'):
        $where = sprintf(
...
$sql_totals
    = "select count(Distinct B.`Webpage Block Key`) as num from $table  $where  ";
