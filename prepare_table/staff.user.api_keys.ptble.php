...
switch ($parameters['parent']) {
    case 'user':
        $where=sprintf(" where  AKD.`API Key User Key`=%d ",$parameters['parent_key']);
        break;
    default:
        $where=" where  true";
        break;
...
$table=' `API Key Dimension` AKD left join `User Dimension` U on (U.`User Key`=AKD.`API Key User Key`) ';

$sql_totals="select count(Distinct `API Key Key`) as num from $table  $where  ";

//print $sql_totals;
$fields="`User Alias`,`User Handle`,AKD.`API Key User Key`,`API Key Key`,`API Key Valid From`,`API Key Valid To`,`API Key Allowed IP`,`API Key Active`,
