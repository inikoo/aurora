...
$where
    = "where `Supplier Part Status`='Available' and `Part Status` not in ('Not In Use','Discontinuing') ";
$table
...
`Part Current Stock`,`Part Stock Status`,`Part Status`,`Part Package Weight`,P.`Part SKU`,`Part 1 Year Acc Dispatched`,`Part Next Deliveries Data`,
...
if ($parameters['parent'] == 'purchase_order') {
    if ($purchase_order->get('Purchase Order Parent') == 'Supplier') {
        ...
        $where .= sprintf(
            " and  `Supplier Part Supplier Key`=%d", $purchase_order->get('Purchase Order Parent Key')
        );


    } else {
        $where .= sprintf(
            "  and  `Agent Supplier Agent Key`=%d", $purchase_order->get('Purchase Order Parent Key')
        );
