...
if (count($user->stores)==0)
	...
	case('store'):
		if (in_array($parameters['parent_key'], $user->stores))
			$where=sprintf("where  `Product Department Store Key`=%d", $parameters['parent_key']);
		else
...
$sql_totals="select count(distinct  D.`Product Department Key`) as num from $table $where";
