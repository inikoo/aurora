...
case('category'):
	...
	$associated_field=sprintf("(select `Category Key` from `Category Bridge` C  where C.`Category Key`=%d and `Subject Key`=P.`Product ID` ) as associated, ",
		$parameters['parent_key']);
	$where=sprintf(" where    `Product Store Key`=%d", $category->get('Category Store Key'));
	break;
default:
