...
function invoices_awhere($awhere) {
    ...
    if ($where_data['tax_code'] != '') {
        $where_delivery_geo_constraints = sprintf(
            " and `Invoice Tax Code`='%s'", $where_data['tax_code']
        );
    }


    $date_interval_invoice_created = prepare_mysql_dates(
        $where_data['invoice_date_from'], $where_data['invoice_date_to'], '`Invoice Date`', 'only_dates'
    );
    $date_interval_invoice_paid    = prepare_mysql_dates(
        $where_data['invoice_paid_date_from'], $where_data['invoice_paid_date_to'], '`Invoice Paid Date`', 'only_dates'
    );
    ...
    foreach ($where_data['paid_status'] as $paid_status) {
        switch ($paid_status) {
            case 'partially':
                $paid_status_where .= sprintf(
                    " or `Invoice Paid`='Partially' "
                );
                break;
            case 'yes':
    ...
    $not_paid_status_where = preg_replace(
        '/^\s*or/', '', $not_paid_status_where
    );
    ...
    foreach ($where_data['total_net_amount'] as $total_net_amount) {
        switch ($total_net_amount) {
            case 'less':
                $total_net_amount_where .= sprintf(
                    " and `Invoice Total Net Amount`<'%s' ", $where_data['total_net_amount_lower']
                );
                break;
            case 'equal':
                $total_net_amount_where .= sprintf(
            ...
            case 'more':
                $total_net_amount_where .= sprintf(
                    " and `Invoice Total Net Amount`>'%s'  ", $where_data['total_net_amount_upper']
                );
                break;
            case 'between':
                $total_net_amount_where .= sprintf(
                    " and  `Invoice Total Net Amount`>'%s'  and `Invoice Total Net Amount`<'%s'", $where_data['total_net_amount_lower'], $where_data['total_net_amount_upper']
                );
                break;
        }
    }
    $total_net_amount_where = preg_replace(
        '/^\s*and/', '', $total_net_amount_where
    );
    ...
    foreach ($where_data['total_tax_amount'] as $total_tax_amount) {
        switch ($total_tax_amount) {
            case 'less':
                $total_tax_amount_where .= sprintf(
                    " and `Invoice Total Tax Amount`<'%s' ", $where_data['total_tax_amount_lower']
                );
                break;
            case 'equal':
                $total_tax_amount_where .= sprintf(
            ...
            case 'more':
                $total_tax_amount_where .= sprintf(
                    " and `Invoice Total Tax Amount`>'%s'  ", $where_data['total_tax_amount_upper']
                );
                break;
            case 'between':
                $total_tax_amount_where .= sprintf(
                    " and  `Invoice Total Tax Amount`>'%s'  and `Invoice Total Tax Amount`<'%s'", $where_data['total_tax_amount_lower'], $where_data['total_tax_amount_upper']
                );
                break;
        }
    }
    $total_tax_amount_where = preg_replace(
        '/^\s*and/', '', $total_tax_amount_where
    );
    ...
    foreach ($where_data['total_profit'] as $total_profit) {
        switch ($total_profit) {
            case 'less':
                $total_profit_where .= sprintf(
                    " and `Invoice Total Profit`<'%s' ", $where_data['total_profit_lower']
                );
                break;
            case 'equal':
                $total_profit_where .= sprintf(
            ...
            case 'more':
                $total_profit_where .= sprintf(
                    " and `Invoice Total Profit`>'%s'  ", $where_data['total_profit_upper']
                );
                break;
            case 'between':
                $total_profit_where .= sprintf(
                    " and  `Invoice Total Profit`>'%s'  and `Invoice Total Profit`<'%s'", $where_data['total_profit_lower'], $where_data['total_profit_upper']
                );
                break;
        }
    }
    ...
    foreach ($where_data['total_amount'] as $total_amount) {
        switch ($total_amount) {
            case 'less':
                $total_amount_where .= sprintf(
                    " and `Invoice Total Amount`<'%s' ", $where_data['total_amount_lower']
                );
                break;
            case 'equal':
                $total_amount_where .= sprintf(
            ...
            case 'more':
                $total_amount_where .= sprintf(
                    " and `Invoice Total Amount`>'%s'  ", $where_data['total_amount_upper']
                );
                break;
            case 'between':
                $total_amount_where .= sprintf(
                    " and  `Invoice Total Amount`>'%s'  and `Invoice Total Amount`<'%s'", $where_data['total_amount_lower'], $where_data['total_amount_upper']
                );
                break;
        }
    }
    ...
    */
    ...
    foreach ($where_data['category'] as $category) {
        $sql          = sprintf(
            "SELECT `Subject Key` FROM `Category Bridge` WHERE `Category Key`=%d", $category
        );
