...
function get_picked_offline_input($qty, $pending, $quantity_on_location, $itf_key, $part_sku, $picked, $part_stock, $barcode, $part_reference, $part_description, $part_image_key) {
    ...
    $picked_offline_input = sprintf(
        '<span class="picked_quantity_done">  
  ...
  <span class="picked_quantity_cant_pick %s"> 
        ...
        <i  class="fa  %s fa-fw button warning add_picked " aria-hidden="true"/>
                ...
                <span data-settings=\'{"field": "Picked_Offline", "transaction_key":%d,"item_key":%d ,"on":1, "reference":"%s" , "description":"%s" , "image_key":%d  ,"stock":"%f"}\' class="picked_quantity %s"  >
                <i onClick="picked_offline_items_qty_change(this)" class="fa  fa-minus fa-fw button picked_offline_items_qty_change " aria-hidden="true"></i>
                    <input class="picked_offline width_50" style="" value="%s" ovalue="%s" barcode="%s" to_pick="%s"  max="%s"     > <i onClick="picked_offline_items_qty_change(this)" class="fa  fa-plus fa-fw button picked_offline_items_qty_change %s" aria-hidden="true"></i>
                </span>',
        ...
        ($pending != 0 && $quantity_on_location >= 1 ? '' : 'hide'), $to_pick_value, $to_pick_value, $barcode, $to_pick,$max_qty_can_be_picked, ''
    );
...
function get_item_location($pending, $quantity_on_location, $date_picked, $location_key, $location_code, $part_stock) {
    ...
    $location .= sprintf(
        '<span class="%s location"   data-available="%f"  location_key = "%d" >%s </span >', ($pending > 0 ? 'discreet' : ''), $quantity_on_location, $location_key, $location_code
    );
