...
function get_useragent_key($useragent) {
    ...
    $sql = sprintf(
        "SELECT `User Agent Key` FROM `User Agent Dimension` WHERE `User Agent String`=%s  ", prepare_mysql($useragent)
    );
    $res = mysql_query($sql);
    if ($row = mysql_fetch_assoc($res)) {
        ...
        $sql = sprintf(
            "INSERT INTO `User Agent Dimension` (`User Agent String`) VALUES (%s) ", prepare_mysql($useragent)
        );
        mysql_query($sql);
...
function get_user_browser($useragent) {
    ...
    if (strpos($useragent, "MSIE") !== false
        ...
        //deal with IE
        $found = preg_match(
            "/MSIE ([0-9]{1}\.[0-9]{1,2})/", $useragent, $mathes
        );
        if ($found) {
        ...
        //if firefox
        $found = preg_match(
            "/Firefox\/([0-9]{1}\.[0-9]{1}(\.[0-9])?)/", $useragent, $mathes
        );
        if ($found) {
        ...
        $found = preg_match(
            "/Netscape\/([0-9]{1}\.[0-9]{1}(\.[0-9])?)/", $useragent, $mathes
        );
        if ($found) {
        ...
        $found = preg_match(
            "/Safari\/([0-9]{2,3}(\.[0-9])?)/", $useragent, $mathes
        );
        if ($found) {
        ...
        $found = preg_match(
            "/Galeon\/([0-9]{1}\.[0-9]{1}(\.[0-9])?)/", $useragent, $mathes
        );
        if ($found) {
        ...
        $found = preg_match(
            "/Konqueror\/([0-9]{1}\.[0-9]{1}(\.[0-9])?)/", $useragent, $mathes
        );
        if ($found) {
        ...
        //deal with Opera
        $found = preg_match(
            "/Opera[\/ ]([0-9]{1}\.[0-9]{1}([0-9])?)/", $useragent, $mathes
        );
        if ($found) {
        ...
        //deal with Lynx
        $found = preg_match(
            "/Lynx\/([0-9]{1}\.[0-9]{1}(\.[0-9])?)/", $useragent, $mathes
        );
        if ($found) {
