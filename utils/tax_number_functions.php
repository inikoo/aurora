...
function check_tax_number($tax_number, $country_2alpha_code) {
...
function check_european_tax_number($country_code, $tax_number) {
    ...
    try {
        ...
        if (preg_match('/INVALID_INPUT/i', $msg)) {
            ...
            $response['msg'] = _('Validations server is busy please try later');


        } else {

            $response['msg']
                = "<div style='padding:10px 0px'><img src='art/icons/error.png'/> "._('Invalid Tax Number').'<br/><span style="margin-left:22px">'.$country_code.' '.$tax_number
                .'</span><div style="padding:10px 22px">'.$msg.'</div></div>';
    ...
    if ($result->valid) {
        ...
        $response['Tax Number Validation Date'] = gmdate('Y-m-d H:i:s');
        $response['msg']
                                                = "<div style='padding:10px 0px'><img style='height:12px;position:relative;top:1px' src='art/icons/error.png'/> "._('Invalid Tax Number')
            .'<br/><span style="margin-left:16px">'.$country_code.' '.$tax_number.'</span></div>';
