...
function check_european_tax_number($country_code,$tax_number) {
	...
	try {
		...
		if (preg_match('/INVALID_INPUT/i',$msg)) {
			...
			$response['msg']="<div style='padding:10px 0px'><img src='art/icons/error.png'/> "._('Invalid Tax Number').'<br/><span style="margin-left:22px">'.$country_code.' '.$tax_number.'</span><div style="padding:10px 22px">'.$msg.'</div></div>';
