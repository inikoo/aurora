...
function unformat_money($number) {
    ...
    $number = preg_replace(
        '/\\'.$locale_info['decimal_point'].'/', '.', $number
    );
...
function parse_money($amount, $currency = false) {
    ...
    if (preg_match('/$|£|¥|€|zł/i', $amount, $match)) {
        ...
        if (preg_match(
            '/usd|eur|gbp|jpy|cad|aud|inr|pkr|mxn|nok/i', $match[0]
        )) {
    ...
    $locale_info = localeconv();
    $amount      = preg_replace(
        "/[^\d\.".$locale_info['decimal_point']."\-]/i", "", $amount
    );
...
function mb_ucwords($str) {
    ...
    foreach ($words as $word) {
        if (preg_match('/([a-z]\.){1,}$/i', $word)) {
            ...
            continue;
        } elseif (!$first and preg_match(
                '/^(UK|USA|HP|IBM|GB|MB|CD|DVD|USB)$/i', $word
            )
        ) {
            $result .= ' '.strtoupper($word);
            continue;
        } elseif (!$first and preg_match(
                '/^(and|y|o|or|of|at|des|les|las|le)$/i', $word
            )
        ) {
            $result .= ' '.strtoupper($word);
            continue;
...
function mb_ucwordsols($str) {
    ...
    foreach ($exceptions as $find => $replace) {
        $return = preg_replace(
            '/\s+'.$find.'\s+|\s+'.$find.'$/', ' '.$replace.' ', $return
        );
...
function strlcs($str1, $str2) {
    ...
    for ($i = 0; $i < strlen($str1); $i++) {
        for ($j = 0; $j < strlen($str2); $j++) {
             ...
             */
            if (substr($str1, $i, 2) == substr(
                    $str2, $j, 2
                )
            ) { //initial match found
                $substring = substr(
                    $str1, $i, 2
                ); //start with first 2 matching characters
                ...
                $arySubstrings[] = trim(
                    $substring
                ); //remove excess whitespace and add to array
            }
        }
    }
    $arySubstrings = array_unique(
        $arySubstrings
    ); //remove duplicate common substrings
