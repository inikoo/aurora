...
function extract_geo_groups($str, $q_country_code = 'C.`Customer Main Country Code`', $q_wregion_code = 'C.`Customer Main Country Code`', $q_town_name = 'C.`Customer Main Town`',
    $q_post_code = 'C.`Customer Main Postal Code`'
) {
    if ($str == '') {
        return '';
    }
    ...
    if (preg_match_all('/pc\([a-z0-9\-\,]*\)/i', $str, $matches)) {
        foreach ($matches[0] as $match) {
            $_post_codes = preg_replace(
                '/\)$/i', '', preg_replace('/^pc\(/i', '', $match)
            );
    ...
    if (preg_match_all('/wr\([a-z0-9\-\,]*\)/i', $str, $matches)) {
        ...
        foreach ($matches[0] as $match) {
            ...
            $_world_regions = preg_replace(
                '/\)$/i', '', preg_replace('/^wr\(/i', '', $match)
            );
        ...
        $sql = sprintf(
            "SELECT `Country Code` FROM kbase.`Country Dimension` WHERE `World Region Code` IN (%s)", "'".join("','", $wregion_codes)."'"
        );
        $res = mysql_query($sql);
    ...
    $where = preg_replace(
        '/^\s*or\s*/i', '', $where_wr.$where_c.$where_pc.$where_t
    );
    if ($where != '') {
        $where = ' and '.$where;
    }
