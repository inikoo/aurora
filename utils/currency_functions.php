...
function currency_conversion($db, $currency_from, $currency_to, $update_interval="-1 hour") {
	...
	if (  in_array($currency_from, $valid_currencies ) and in_array($currency_to, $valid_currencies ) ) {
		...
		$contents=json_decode(file_get_contents(sprintf('http://api.fixer.io/latest?base=%s&symbols=%s', $currency_from, $currency_to)), true);
	...
	if (is_numeric($exchange) and $exchange>0) {
		...
		$sql=sprintf("insert into kbase.`Currency Exchange Dimension`  (`Currency Pair`,`Exchange`,`Currency Exchange Last Updated`,`Currency Exchange Source`) values (%s,%f,NOW(),%s)  ON DUPLICATE KEY update `Exchange`=%f,`Currency Exchange Last Updated`=NOW(),`Currency Exchange Source`=%s",
			prepare_mysql($currency_from.$currency_to),
			$exchange,
