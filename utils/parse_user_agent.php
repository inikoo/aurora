...
/*
 ...
 Created: 2 October 2018 at 15:35:34 GMT+8, Kuala Lumpur, Malaysia
...
function parse_user_agent($user_agent, $db) {
    ...
    $user_agent_hash = md5(strtolower($user_agent));

    $sql = sprintf('select * from kbase.`User Agent` where `User Agent Hash`=%s', prepare_mysql($user_agent_hash));
    if ($result = $db->query($sql)) {
        if ($row = $result->fetch()  ) {
            ...
            return array(
                'User Agent'       => $result_json['parse']['user_agent'],
                'Software'         => $result_json['parse']['simple_software_string'],
                'Software Details' => $result_json['parse']['simple_sub_description_string'],
                'Device'           => $result_json['parse']['simple_operating_platform_string'],
                'OS Code'          => $result_json['parse']['operating_system_name_code'],
...
function get_user_agent_icon($os_code, $platform, $user_agent) {


    if ($os_code == 'linux') {
        $icon = 'fa-server';
    } elseif ($os_code == 'windows') {
        $icon = 'fa-desktop';
    } elseif ($os_code == 'macos' or $os_code == 'mac-os-x') {
        $icon = 'fa-desktop-alt';
    } elseif ($os_code == 'ios') {
        if (preg_match('/ipad/i', $platform)) {
            $icon = 'fa-tablet';
        ...
        if (check_if_is_tablet($user_agent)) {
            {
