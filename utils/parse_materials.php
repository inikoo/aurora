...
function parse_materials($value, $editor='') {
		...
		if (preg_match('/\s*\(.+\s*\%\s*\)$/', $material, $match)) {
			...
			if ($material!='') {
				...
				if (array_key_exists(strtolower($material), $materials)) {
					...
					$materials[strtolower($material)]=array('name'=>$material, 'ratio'=>$ratio, 'may_contain'=>'No','id'=>'');
				}
			}
			...
			$material=_trim($material);
			if ($material!='') {
				$materials[strtolower($material)]=array('name'=>$material, 'ratio'=>'', 'may_contain'=>'Yes','id'=>'');
			}
		}else {

			$materials[strtolower($material)]=array('name'=>$material, 'ratio'=>'', 'may_contain'=>'No','id'=>'');
	...
	foreach ($materials as $key=>$_value) {
		$material_data=array('Material Name'=>$_value['name'], 'editor'=>$editor);
