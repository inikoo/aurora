...
function get_user_fields(User $object, PDO $db,$options): array
{
    ...
    $object_fields = array(
        array(
            ...
            'fields'     => array(
                ...
                array(
                    'render'          => is_array($options) and in_array($options['parent'], array(
                                                                                               'supplier',
                                                                                               'agent',
                                                                                               'contractor'
                                                                                           )),
                    ...
                    'type'            => ((is_array($options) and in_array($options['parent'], array(
                                                                                                 'supplier',
                                                                                                 'agent',
                                                                                                 'contractor'
                                                                                             ))) ? 'value' : '')
                ...
                array(
                    'render'          => ($new ? ($options['parent'] == 'Staff' ? true : false) : in_array($object->get('User Type'), array(
                                                                                                                                        'Staff',
                                                                                                                                        'Contractor'
                                                                                                                                    ))),
                    'id'              => 'User_PIN',
                    ...
                    'type'            => ($new ? ($options['parent'] == 'Staff' ? 'value' : '') : (in_array($object->get('User Type'), array(
                                                                                                                                         'Staff',
                                                                                                                                         'Contractor'
                                                                                                                                     )) ? 'value' : ''))
    ...
    if (!$new) {
        if (in_array($object->get('User Type'), array(
                                                  'Staff',
                                                  'Contractor'
                                              ))) {
            include_once 'utils/available_locales.php';
            ...
            $employee = get_object(
                $object->get('User Type'),
                ($object->get('User Parent Key'))
            );
            ...
            foreach ($roles as $_key => $_data) {
                if (in_array(
                    $account->get('Setup Metadata')['size'],
                    $_data['size']
                )) {
                    foreach (
    ...
    $object_fields[] = array(
        ...
        'fields'     => array(
            array(
                ...
                'edit'            => 'option',
                'value'           => ($new ? $account->get('Account Locale') : $object->get(
                    'User Preferred Locale'
                )),
                'formatted_value' => ($new ? $account->get('Locale') : $object->get(
                    'Preferred Locale'
                )),
                'label'           => ucfirst(
