...
class MYPDF extends TCPDF {
    ...
    function Header() {
        if ($this->header_xobjid < 0) {
            ...
            $this->header_xobjid = $this->startTemplate(
                $this->w, $this->tMargin
            );
            ...
            if (($headerdata['logo']) AND ($headerdata['logo'] != K_BLANK_IMAGE)) {
                $imgtype = $this->getImageFileType(
                    K_PATH_IMAGES.$headerdata['logo']
                );
                if (($imgtype == 'eps') OR ($imgtype == 'ai')) {
                    $this->ImageEps(
                        K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']
                    );
                } elseif ($imgtype == 'svg') {
                    $this->ImageSVG(
                        K_PATH_IMAGES.$headerdata['logo'], '', '', $headerdata['logo_width']
            ...
            $cell_height = round(
                ($this->cell_height_ratio * $headerfont[2]) / $this->k, 2
            );
            ...
            $this->SetX($header_x);
            $this->Cell(
                $cw, $cell_height, $headerdata['title'], 0, 1, '', 0, '', 0
            );
            // header string
            ...
            $this->MultiCell(
                $cw, $cell_height, $headerdata['string'], 0, '', 0, 1, '', '', true, 0, false, true, 0, 'T', false
            );
            ...
            $this->Cell(
                ($this->w - $this->original_lMargin - $this->original_rMargin), 0, '', 'T', 0, 'C'
            );
            $this->endTemplate();
        }
    ...
    public function Footer() {
        ...
        // Page number
        $sql = sprintf(
            "SELECT * FROM `Store Dimension` WHERE `Store Key`=%d", $this->get_footer_var()
        );
...
EOD;
        ...
        $this->Cell(
            0, 10, 'Page '.$this->getAliasNumPage().'/'.$this->getAliasNbPages(), 0, false, 'C', 0, '', 0, false, 'T', 'M'
        );
    }
    ...
    public function MultiRow($columns) {
        ...
        foreach ($columns as $column) {
            $this->MultiCell(
                $column['w'], 0, $column['txt'], $column['border'], $column['align'], 0, ($i == $number_columns ? 1 : 2), ($i == 1 ? '' : $this->GetX()), ($i == 1 ? '' : $y_start), true, 0
            );
...
if ($order->get('Order Out of Stock Net Amount') != 0) {
    $net_amount
        = "<tr><td>Out of Stock (N)</td><td width=100 >-{$order->get('Out of Stock Net Amount')}</td></tr>";
}

$charged_amount = '';
if ($order->get('Order Invoiced Charges Amount') != 0) {
    $charged_amount
        = "<tr><td >Charges (N)</td><td width=100 >{$order->get('Invoiced Charges Amount')}</td></tr>";
}

$refund_amount = '';
if ($order->get('Order Invoiced Refund Net Amount') != 0) {
    $refund_amount
        = "<tr><td   ><i>Other Order Refunds</i></td><td width=100 >{$order->get('Invoiced Refund Net Amount')}</td></tr>";
}
...
if ($order->get('Order Invoiced Total Net Adjust Amount') != 0) {
    $net_adjustment
        = "<tr ><td   >Adjusts (N)</td><td width=100 >{$order->get('Invoiced Total Net Adjust Amount')}</td></tr>";
}
...
if ($order->get('Order Net Refund Invoiced Amount') != 0) {
    $net_refund
        = "<tr><td >Net</td><td width=100 >{$order->get('Net Refund Amount')}</td></tr>";
}
...
if ($order->get('Order Invoiced Total Tax Adjust Amount') != 0) {
    $total_tax
        = " <tr style=\"color:red\"><td   >Tax Adjusts</td><td width=100 >{$order->get('Invoiced Total Tax Adjust Amount')}</td></tr>";
}
...
<div >
	 ...
	 <table>
	   ...
	   <tr><td>Delivery Notes:</td><td>{$order->get(
    'Order XHTML Delivery Notes'
)}</td></tr>
	 </table>
...
<div >
	 <table>
	   ...
	   <tr><td   >Total Ordered (N)</td><td width=100 >{$order->get(
    'Total Net Amount'
)}</td></tr>
	    {$net_amount}
	   ...
	   <tr><td   >Items (N)</td><td width=100 >{$order->get(
    'Invoiced Items Amount'
)}</td></tr>
	   
	   <tr><td   >Shipping (N)</td><td width=100 >{$order->get(
    'Invoiced Shipping Amount'
)}</td></tr>
	   {$charged_amount}
	   {$refund_amount}
	   {$net_adjustment}
	   ...
	   <tr>
	     ...
	     <td   >Total (N)</td><td width=100 >{$order->get(
    'Invoiced Total Net Amount'
)}</td>
	   </tr>
	   <tr><td   >Tax</td><td width=100 >{$order->get(
    'Invoiced Total Tax Amount'
)}</td></tr>
	    {$total_tax}
	   <tr><td   >Total</td><td width=100 ><b>{$order->get(
...
$sql = sprintf(
    "SELECT O.`Order Transaction Fact Key`,`Deal Info`,`Operation`,`Quantity`,`Order Currency Code`,`Order Quantity`,`Order Bonus Quantity`,`No Shipped Due Out of Stock`,P.`Product ID` ,P.`Product Code`,`Product XHTML Short Description`,`Shipped Quantity`,(`Invoice Transaction Gross Amount`-`Invoice Transaction Total Discount Amount`) AS amount
...
while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
    ...
    $columns = array(
        array(
            'w'      => 20,
            'txt'    => $row['Order Transaction Fact Key'],
            'border' => 'T',
            'align'  => 'L'
        ),
        array(
            'w'      => 55,
            'txt'    => strip_tags($row['Product XHTML Short Description']),
            'border' => 'T',
            'align'  => 'L'
        ),
        array(
            'w'      => 55,
            'txt'    => $row['Order Quantity'],
            'border' => 'T',
            'align'  => 'R'
        ),
        array(
            'w'      => 20,
            'txt'    => $row['Shipped Quantity'],
            'border' => 'T',
            'align'  => 'R'
        ),
        array(
...
$sql    = sprintf(
    "SELECT * FROM `Store Dimension` WHERE `Store Key`=%d", $store->id
);
$result = mysql_query($sql);
$row    = mysql_fetch_assoc($result);
