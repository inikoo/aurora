...
$mpdf->SetTitle( sprintf(_('Purchase order %s'), $purchase_order->get('Public ID')));
...
if ($result=$db->query($sql)) {
	foreach ($result as $data) {
		...
		if ($data['Purchase Order Quantity']>0) {
			...
			$amount=money($data['Purchase Order Quantity']*$units_per_carton*$data['Supplier Part Historic Unit Cost'], $purchase_order->get('Purchase Order Currency Code'));





			$subtotals.=sprintf(_("%d units"), ($data['Purchase Order Quantity']*$data['Part Units Per Package']*$data['Supplier Part Packages Per Carton']));
		...
		$description=$data['Part Unit Description'].' @'.money($data['Supplier Part Historic Unit Cost'], $purchase_order->get('Purchase Order Currency Code'));
		$description.='<span style="font-style: italic;font-size:90%">';
		if ($data['Part Units Per Package']>1) {
			$description.='<br>'.sprintf(_("packed in %d's"), $data['Part Units Per Package']);
			$description.= ', '.sprintf(ngettext('%s unit per carton', '%s units per carton', $units_per_carton), number($units_per_carton) , number($units_per_carton)  );
			$description.=  ', '.sprintf(ngettext('%s pack per carton', '%s packs per carton', $data['Supplier Part Packages Per Carton']), number($data['Supplier Part Packages Per Carton']) , number($data['Supplier Part Packages Per Carton'])  );



		}else {

			$description.= '<br>'.sprintf(ngettext('%s unit per carton', '%s units per carton', $units_per_carton), number($units_per_carton) , number($units_per_carton)  );
