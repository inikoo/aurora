...
try {
    ...
    $payments=[];
    $payments_amount=$customer->get('Customer Account Balance');
    foreach($order->get_payments('objects') as $payment){
        if($payment->get('Payment Transaction Status')=='Completed'){
            $payments_amount += $payment->get('Payment Transaction Amount');
            $payments[]=[
                'amount'=> money($payment->get('Payment Transaction Amount'),$order->get('Order Currency')),
    ...
    $to_pay=$order->get('Order Total Amount')-$payments_amount;
    ...
    $smarty->assign('to_pay', $to_pay);
    $smarty->assign('to_pay_formatted', money($to_pay,$order->get('Order Currency')));
...
`Product Currency`,`Product History Name`,`Product History Price`,`Product Units Per Case`,`Product History XHTML Short Description`,`Product Name`,`Product RRP`,`Product Tariff Code`,`Product Tariff Code`,P.`Product ID`,`Product History Code` as `Product Code`
 FROM `Order Transaction Fact` O  LEFT JOIN `Product History Dimension` PH ON (O.`Product Key`=PH.`Product Key`) LEFT JOIN
  `Product Dimension` P ON (PH.`Product ID`=P.`Product ID`) WHERE `Order Key`=%d ORDER BY `Product History Code`", $order->id
    );
    //print $sql;exit;
