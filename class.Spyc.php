...
class Spyc {
    ...
    private function loadWithSource($Source) {
        ...
        for ($i = 0; $i < $cnt; $i++) {
            ...
            if ($literalBlockStyle) {
                ...
                while (++$i < $cnt
                    && $this->literalBlockContinues(
                        $Source[$i], $this->indent
                    )) {
                    $literalBlock = $this->addLiteralLine(
                        $literalBlock, $Source[$i], $literalBlockStyle
                    );
                }
                $i--;
            }
            ...
            if ($literalBlockStyle) {
                $lineArray = $this->revertLiteralPlaceHolder(
    ...
    private function nodeContainsGroup($line) {
        ...
        if ($line[0] == '&'
            && preg_match(
                '/^(&['.$symbolsForReference.']+)/', $line, $matches
            )
        ) {
            return $matches[1];
        }
    ...
    private function _inlineEscape($inline) {
        ...
        do {
            ...
            while (preg_match('/\[([^{}\[\]]+)\]/U', $inline, $matchseqs)) {
                $seqs[] = $matchseqs[0];
                $inline = preg_replace(
                    '/\[([^{}\[\]]+)\]/U', ('YAMLSeq'.(count($seqs) - 1).'s'), $inline, 1
                );
            }
            ...
            while (preg_match('/{([^\[\]{}]+)}/U', $inline, $matchmaps)) {
                $maps[] = $matchmaps[0];
                $inline = preg_replace(
                    '/{([^\[\]{}]+)}/U', ('YAMLMap'.(count($maps) - 1).'s'), $inline, 1
                );
            }
    ...
    function revertLiteralPlaceHolder($lineArray, $literalBlock) {
        foreach ($lineArray as $k => $_) {
            if (is_array($_)) {
                $lineArray[$k] = $this->revertLiteralPlaceHolder(
                    $_, $literalBlock
                );
            } else {
    ...
    private function addArray($incoming_data, $incoming_indent) {
        ...
        if ($this->_containsGroupAlias) {
            $value                     = $this->referenceContentsByAlias(
        ...
        if ($this->_containsGroupAnchor) {
            ...
            if (is_array($value)) {
                $k = key($value);
                if (!is_int($k)) {
                    $this->SavedGroups[$this->_containsGroupAnchor][$incoming_indent + 2]
                        = $k;
                }
            }
    ...
    public function dump($array, $indent = false, $wordwrap = false) {
        ...
        if ($array) {
            ...
            foreach ($array as $key => $value) {
                $string .= $this->_yamlize(
                    $key, $value, 0, $previous_key, $first_key
                );
                $previous_key = $key;
            }
        }
    ...
    private function _yamlize($key, $value, $indent, $previous_key = -1, $first_key = 0) {
        if (is_array($value)) {
            if (empty ($value)) {
                return $this->_dumpNode(
                    $key, array(), $indent, $previous_key, $first_key
                );
            }
            // It has children. What to do?
            // Make it the right kind of item
            $string = $this->_dumpNode(
                $key, null, $indent, $previous_key, $first_key
            );
            // Add the indent
            ...
            $string = $this->_dumpNode(
                $key, $value, $indent, $previous_key, $first_key
            );
        }
    ...
    private function _dumpNode($key, $value, $indent, $previous_key = -1, $first_key = 0) {
        ...
        if (is_int($key) && $key - 1 == $previous_key && $first_key === 0) {
            ...
            if ($first_key === 0) {
                throw new Exception(
                    'Keys are all screwy. The first one was zero, now it\'s "'.$key.'"'
                );
            }
            // It's mapped
            if (strpos($key, ":") !== false) {
    ...
    private function _doFolding($value, $indent) {
        ...
        if ($this->_dumpWordWrap !== 0 && is_string($value)
            && strlen($value) > $this->_dumpWordWrap
        ) {
            $indent += $this->_dumpIndent;
