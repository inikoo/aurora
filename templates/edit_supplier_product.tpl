...
<div style="clear:left;margin:0 0px">
    <h1>{t}Editing Product{/t}: <span id="title_name">{$supplier_product->get('Product Name')}</span> (<span id="title_code">{$supplier_product->get('Product Code')}</span>)</h1>
</div>
  ...
  <ul class="tabs" id="chooser_ul">
	...
	<div id="info_name" style="float:left;width:260px;{if !( $edit=='pictures') }display:none{/if}">
	  ...
	  <table    class="show_info_product">
	    <tr>
	      <td colspan="2" class="aright product_name" id="l_product_name" >{$supplier_product->get('Product Name')}</td>
	    </tr>
	    <tr>
	      <td>{t}Code{/t}:</td><td  class="aright">{$supplier_product->get('Product Code')}</td>
	    </tr>
	    <tr>
	      <td>{t}Store{/t}:</td><td  class="aright">{$store->get('Store Name')}</td>
	    </tr>
	    <tr>
	      <td>{t}Family{/t}:</td><td  class="aright">{$supplier_product->get('Product Family Code')}</td>
	    </tr>
	...
	<div  id="info_price"  style="float:left;width:260px;margin-left:20px;display:none">
	  ...
	  <table    class="show_info_product">
	    <tr style="{if $supplier_product->get('Product Units Per Case')==1}display:none{/if}">
	      <td>{t}Unit per Case{/t}:</td><td  class="aright">{$supplier_product->get('Units')}</td>
	    </tr>
	    <tr>
	      <td>{t}Cost{/t}:</td><td  id="l_formated_cost" class="aright">
		{$supplier_product->get('Formated Cost')}
	      </td>
	    </tr>
	    <td>{t}Price{/t}:</td><td  id="l_formated_price" class="price aright">
	      {$supplier_product->get('Formated Price')}
	    </td>
</tr>
<tr id="tr_rrp_per_unit" {if $supplier_product->get('Product RRP')==''}style="display:none"{/if} >
	    <td>{t}RRP{/t}:</td><td id="l_formated_rrp_per_unit" class="aright">{$supplier_product->get('RRP Per Unit')}</td>
	  </tr>
	</table>
</div>
	...
	<table style="margin:5px 0px ;xwidth:500px"  border=0 class="edit">
	  <tr>
		...
		<span {if $supplier_product->get('Product Type')=="Normal"}class="selected"{/if} id="type_prod_normal">{t}Simple{/t}</span>
		<span {if $supplier_product->get('Product Type')=="Shortcut"}class="selected"{/if} id="type_prod_shortcut">{t}Shortcut{/t}</span>
		<span {if $supplier_product->get('Product Type')=="Mix"} class="selected"{/if} id="type_prod_mix">{t}Mixture{/t}</span>
	      </div>
	    </td>
...
<h2>{t}Part List{/t}</h2>
	  ...
	  {else}
	  <div style="text-align:right"  id="product_part_items" product_part_key="{$supplier_product->get_current_product_part_key()}"  >
...
<input id="v_cost" value="{$supplier_product->get('Product Cost')}" type="hidden"/>
...
<td id="price_per_unit" cost="{$supplier_product->get('Product Cost')}"  old_price="{$supplier_product->get('Product Price')}"  units="{$supplier_product->get('Product Units Per Case')}">{$supplier_product->get_formated_price_per_unit()}</td>
...
<tr class="first"><td style="" class="label">{t}Unit Weight{/t}:</td>
   <td  style="text-align:left;width:4em;">
     <div  style="width:4em;position:relative;top:00px" >
       <input style="text-align:left;width:4em" id="Product_Unit_Weight" value="{$supplier_product->get('Net Weight Per Unit')}" ovalue="{$supplier_product->get('Net Weight Per Unit')}" valid="0"> 
...
<tr style="display:none"><td style="" class="label">{t}Outer Weight{/t}:<br/><small>with packing<small/></td>
   <td  style="text-align:left">
     <div  style="width:4em;position:relative;top:00px" >
       <input style="text-align:left;width:4em" id="Product_Outer_Weight" value="{$supplier_product->get('Product Gross Weight')}" ovalue="{$supplier_product->get('Product Gross Weight')}" valid="0">
...
<tr class="first"><td style="" class="label">{t}Units Per Outer{/t}:</td>
   <td  style="text-align:left">
     <div  style="width:15em;position:relative;top:00px" >
       <input style="text-align:left;width:18em" id="Product_Units_Per_Case" value="{$supplier_product->get('Product Units Per Case')}" ovalue="{$supplier_product->get('Product Units Per Case')}" valid="0">
...
<tr class="first"><td style="" class="label">{t}Product Name{/t}:</td>
   <td  style="text-align:left">
     <div  style="width:15em;position:relative;top:00px" >
       <input style="text-align:left;width:18em" id="Product_Name" value="{$supplier_product->get('Product Name')}" ovalue="{$supplier_product->get('Product Name')}" valid="0">
...
<tr><td style="" class="label">{t}Special Characteristic{/t}:</td>
   <td  style="text-align:left">
     <div  style="width:15em;position:relative;top:00px" >
       <input style="text-align:left;width:18em" id="Product_Special_Characteristic" value="{$supplier_product->get('Product Special Characteristic')}" ovalue="{$supplier_product->get('Product Special Characteristic')}" valid="0">
...
<textarea id="Product_Description"  olength="{$supplier_product->get('Product Description Length')}"  value="{$supplier_product->get('Product Description')}"  ovalue="{$supplier_product->get('Product Description')}"  ohash="{$supplier_product->get('Product Description MD5 Hash')}" rows="6" cols="42">{$supplier_product->get('Product Description')}</textarea>
