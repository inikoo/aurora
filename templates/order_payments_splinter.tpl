	...
	<table class="edit" id="pending_payment_confirmations" border="0" style="margin-top:0px;padding-top:0px;width:100%;">
		...
		<tr id="payment_{$payment->get('Payment Key')}" class="payment" payment_key="{$payment->get('Payment Key')}">
			...
			<div class="buttons small left">
				<button style="{if !( $payment->get('Payment Transaction Status')=='Pending')}display:none{/if}" class="negative" onclick="cancel_pending_payment({$payment->get('Payment Key')})">{t}Set as cancelled{/t}</button> <button id="complete_payment_{$payment->id}" style="{if !( $payment->get('Payment Transaction Status')=='Pending')}display:none{/if}" class="positive" onclick="show_complete_payment_dialog({$payment->get('Payment Key')})">{t}Set as completed{/t}</button> <button id="add_refund_{$payment->id}" class="{if  $order->get('Order To Pay Amount')<0}positive{/if}" style="{if  $payment->get('Payment Type')!='Payment' or  $payment->get('Payment Method')=='Account' or $payment->get('Payment Transaction Status')!='Completed'}display:none{/if}" onclick="refund_payment({$payment->get('Payment Key')})"><img style="height:12.5px;width:12.5px" src="art/icons/add.png"> {t}Refund{/t}</button> <button id="add_credit_{$payment->id}" class="{if  $order->get('Order To Pay Amount')<0}positive{/if}" style="{if  $payment->get('Payment Type')!='Payment' or  $payment->get('Payment Method')=='Account' or $payment->get('Payment Transaction Status')!='Completed'}display:none{/if}" onclick="credit_payment({$payment->get('Payment Key')})"><img style="height:12.5px;width:12.5px" src="art/icons/add.png"> {t}Credit{/t}</button> <button class="positive" style="{if  !( $payment->get('Payment Balance')!=0  and $order->get_invoices_to_pay_abs_amount()!=0)  }display:none{/if}" onclick="pay_invoice({$payment->get('Payment Key')})">{t}Pay Invoice{/t}</button> <button id="cancel_payment_{$payment->id}" style="{if !( $payment->get('Payment Transaction Status')=='Completed' and $payment->get('Payment Submit Type')=='Manual')}display:none{/if}" class="negative" onclick="show_cancel_payment_dialog({$payment->get('Payment Key')})">{t}Cancel{/t}</button> 
