...
<div id="bd">
	...
	<div class="top_page_menu" style="border:none">
		<div class="buttons small"  style="position:relative;top:7px">
			<button style="height:18px;" onclick="window.open('dn.pdf.php?id={$delivery_note->id}')"><img style="width:40px;height:12px;position:relative;bottom:1.5px" src="art/pdf.gif" alt=""></button> {if $delivery_note->get('Delivery Note Fraction Picked')==1 and $delivery_note->get('Delivery Note Fraction Packed')==1} {if $delivery_note->get('Delivery Note Approved Done')=='No'} {if $user->get('User Type')!='Warehouse'} 
			<button id="approve_packing" onclick="approve_packing({$delivery_note->id},{$user->get('User Parent Key')},'dn')" ><img id="approve_packing_img_{$delivery_note->id}" src="art/icons/flag_green.png" alt="" /> {t}Approve Picking/Packing{/t}</button> {/if} {else} {if $delivery_note->get('Delivery Note Approved To Dispatch')=='No'} <button onclick="approve_dispatching({$delivery_note->id},{$user->get('User Parent Key')},'dn')"><img id="approve_dispatching_img_{$delivery_note->id}" src="art/icons/package_green.png" alt=""> {t}Approve Dispatching{/t}</button> 
			{else if $delivery_note->get('Delivery Note State')!='Canceled to Restock' } <button  onclick="set_as_restocked({$delivery_note->id},{$user->get('User Parent Key')},'dn')"><img id="set_as_dispatched_img_{$delivery_note->id}" src="art/icons/lorry_go.png" alt=""> {t}Set as Restocked{/t}</button> 

			{else if $delivery_note->get('Delivery Note State')!='Dispatched' } <button  onclick="set_as_dispatched({$delivery_note->id},{$user->get('User Parent Key')},'dn')"><img id="set_as_dispatched_img_{$delivery_note->id}" src="art/icons/lorry_go.png" alt=""> {t}Set as Dispatched{/t}</button> 
			{/if} 
			 {/if} {else if $delivery_note->get('Delivery Note Fraction Picked')==0 and $delivery_note->get('Delivery Note Fraction Packed')==0} {if $delivery_note->get('Delivery Note Assigned Picker Key')} <button " onclick="window.location='order_pick_aid.php?id={$delivery_note->id}'"><img src="art/icons/basket_put.png" alt=""> {t}Picking Aid Sheet{/t}</button> {else} <button id="pick_it_">{t}Process Delivery Note{/t} ({t}Picking{/t})</button> {/if} {else if $delivery_note->get('Delivery Note Fraction Picked')>0 and $delivery_note->get('Delivery Note Fraction Picked')<1 } <button  onclick="window.location='order_pick_aid.php?id={$delivery_note->id}'"><img src="art/icons/basket_put.png" alt=""> {t}Picking Aid Sheet{/t}</button> <button  onclick="window.location='order_pick_aid.php?id={$delivery_note->id}'"><img src="art/icons/basket_put.png" alt=""> {t}Picking Aid Sheet{/t}</button> {if $delivery_note->get('Delivery Note Fraction Packed')>0} <button  onclick="window.location='order_pack_aid.php?id={$delivery_note->id}'"><img src="art/icons/package.png" alt=""> {t}Packing Aid Sheet{/t}</button> {/if} {else if $delivery_note->get('Delivery Note Fraction Picked')==1 } <button onclick="window.location='order_pick_aid.php?id={$delivery_note->id}'"><img src="art/icons/basket_put.png" alt=""> {t}Picking Aid Sheet{/t}</button> {if $delivery_note->get('Delivery Note Assigned Packer Key')} <button onclick="window.location='order_pack_aid.php?id={$delivery_note->id}'"><img src="art/icons/package.png" alt=""> {t}Packing Aid Sheet{/t}</button> {else} <button id="process_dn_packing">{t}Process Delivery Note{/t} ({t}Packing{/t})</button> {/if} {/if} 
		</div>
		<div class="buttons left" style="margin-bottom:3px">
			<span class="main_title" style="margin-bottom:3px">{t}Delivery Note{/t} <span>{$delivery_note->get('Delivery Note ID')}</span> <span class="subtitle">({$delivery_note->get_formated_state()})</span></span> {*} {if isset($referal) and $referal=='store_pending_orders'} <button onclick="window.location='$referal_url'"><img src="art/icons/text_list_bullets.png" alt=""> {t}Pending Orders (Store){/t}</button> {else} <button onclick="window.location='warehouse_orders.php?id={$delivery_note->get('Delivery Note Warehouse Key')}'"><img src="art/icons/basket.png" alt=""> {t}Pending Orders{/t}</button> {/if} {*} 
		</div>
	...
	<div id="control_panel">
		...
		<div id="dn_details_panel" style="display:none;border-top:1px solid #ccc;padding-top:10px;margin-top:10px">
			<div class="buttons small">
				...
				{if $delivery_note->get('Delivery Note State')=='Dispatched'} <button onclick="undo_dispatch()"><img id="undo_dispatch_icon" src="art/icons/arrow_rotate_anticlockwise.png"> {t}Undo dispatch{/t}</button>
				{else if  $delivery_note->get('Delivery Note Type')=='Replacement & Shortages'  or  $delivery_note->get('Delivery Note Type')=='Replacement'  or  $delivery_note->get('Delivery Note Type')=='Shortages' } 
	<button class="negative" onclick="cancel_replacement()"><img id="cancel_replacement_icon" src="art/icons/cross.png"> {t}Cancel Replacement{/t}</button>
				 {/if} 
			</div>
