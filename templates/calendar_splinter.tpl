<div class="branch" id="period_navigation" style="clear:none;width:660px;float:right;text-align:right;font-style:normal">
	<span id="day" class="state_details {if  $quick_period=='day'}selected{/if}" style="margin-left:10px"><img src="art/icons/mini-calendar.png" style="position:relative;bottom:1px" /> {t}Day{/t}</span> <span id="other" class="state_details {if  $quick_period=='other'}selected{/if}" style="margin-left:10px"><img src="art/icons/mini-calendar_interval.png" style="position:relative;bottom:1px" /> {t}Interval{/t}</span> <span id="ytd" class="state_details {if  $quick_period=='ytd'}selected{/if}" style="margin-left:10px">{t}YTD{/t}</span> <span id="mtd" class="state_details {if  $quick_period=='mtd'}selected{/if}" style="margin-left:10px">{t}MTD{/t}</span> <span id="wtd" class="state_details {if  $quick_period=='wtd'}selected{/if}" style="margin-left:10px">{t}WTD{/t}</span> <span id="today" class="state_details {if  $quick_period=='today'}selected{/if}" style="margin-left:10px">{t}today{/t}</span> <span id="yesterday" class="state_details {if  $quick_period=='yesterday'}selected{/if}" style="margin-left:10px">{t}yesterday{/t}</span> <span id="last_w" class="state_details {if  $quick_period=='last_w'}selected{/if}" style="margin-left:10px">{t}last w{/t}</span> <span id="last_m" class="state_details {if  $quick_period=='last_m'}selected{/if}" style="margin-left:10px">{t}last m{/t}</span> <span id="1w" class="state_details {if  $quick_period=='1w'}selected{/if}" style="margin-left:10px">{t}1w{/t}</span> <span id="10d" class="state_details {if  $quick_period=='10d'}selected{/if}" style="margin-left:10px">{t}10d{/t}</span> <span id="1m" class="state_details {if  $quick_period=='1m'}selected{/if}" style="margin-left:10px">{t}1m{/t}</span> <span id="1q" class="state_details {if  $quick_period=='1q'}selected{/if}" style="margin-left:10px">{t}1q{/t}</span> <span id="1y" class="state_details {if  $quick_period=='1y'}selected{/if}" style="margin-left:10px">{t}1y{/t}</span> <span id="3y" class="state_details {if  $quick_period=='3y'}selected{/if}" style="margin-left:10px">{t}3y{/t}</span> <span id="all" class="state_details {if  $quick_period=='all'}selected{/if}" style="margin-left:10px">{t}All{/t}</span> 
</div>
...
<div id="dialog_calendar_date_splinter" style="padding:5px 20px">
	<div class="bd">
		<div class="custom_dates" style="width:100%;margin-top:10px;border-top:1px solid#ccc;font-size:90%">
			<div style="margin-top:10px" id="cal2Container">
			</div>
			<div style="float:left;padding:10px">
				<form action="{$calendar_link}?" method="GET" style="margin-top:10px">
					<table border="0">
						<tr>
							<td> <span id="clear_interval_date" style="font-size:80%;color:#777;cursor:pointer;display:none">{t}clear{/t}</span> 
							<input id="pick_date" type="text" class="text" size="11" maxlength="10" name="from" value="{$from_little_edian}" />
							</td>
							<td> <img style="cursor:pointer;height:15px;margin-top:px" align="absbottom" src="art/icons/application_go.png" style="cursor:pointer" id="submit_interval" alt="{t}Go{/t}" /> </td>
						</tr>
						<tr>
