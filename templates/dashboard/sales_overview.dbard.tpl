...
<div  class="date_chooser" >
	...
	<div onclick="change_sales_overview_period('ytd')"period="ytd" id="ytd" class="fixed_interval {if  $period=='ytd'}selected{/if}"  title="{t}Year to day{/t}">{t}YTD{/t}</div> 
	<div onclick="change_sales_overview_period('mtd')"period="mtd" id="mtd" class="fixed_interval {if  $period=='mtd'}selected{/if}"  title="{t}Month to day{/t}">{t}MTD{/t}</div> 
	<div onclick="change_sales_overview_period('wtd')"period="wtd" id="wtd" class="fixed_interval {if  $period=='wtd'}selected{/if}" title="{t}Week to day{/t}">{t}WTD{/t}</div> 
	<div onclick="change_sales_overview_period('today')" period="today" id="today" class="fixed_interval {if  $period=='today'}selected{/if}" title="{t}Today{/t}">{t}Today{/t}</div> 
	<div onclick="change_sales_overview_period('yesterday')" period="yesterday" id="yesterday" class="fixed_interval {if  $period=='yesterday'}selected{/if}" title="{t}Yesterday{/t}">{t}Y'day{/t}</div> 
	<div onclick="change_sales_overview_period('last_w')" period="last_w" id="last_w" class="fixed_interval {if  $period=='last_w'}selected{/if}" title="{t}Last week{/t}">{t}Last W{/t}</div> 
	<div onclick="change_sales_overview_period('last_m')" period="last_m" id="last_m" class="fixed_interval {if  $period=='last_m'}selected{/if}" title="{t}Last month{/t}">{t}Last M{/t}</div> 
	<div onclick="change_sales_overview_period('1w')" period="1w" id="1w" class="fixed_interval {if  $period=='1w'}selected{/if}" title="{t}1 week{/t}">{t}1W{/t}</div> 
	<div onclick="change_sales_overview_period('10d')" period="10d" id="10d" class="fixed_interval {if  $period=='10d'}selected{/if}" title="{t}10 days{/t}">{t}10d{/t}</div> 
	<div onclick="change_sales_overview_period('1m')" period="1m" id="1m" class="fixed_interval {if  $period=='1m'}selected{/if}" title="{t}1 month{/t}">{t}1m{/t}</div> 
	<div onclick="change_sales_overview_period('1q')" period="1q" id="1q" class="fixed_interval {if  $period=='1q'}selected{/if}" title="{t}1 quarter{/t}">{t}1q{/t}</div> 
	<div onclick="change_sales_overview_period('1y')" period="1y" id="1y" class="fixed_interval {if  $period=='1y'}selected{/if}" title="{t}1 year{/t}">{t}1Y{/t}</div> 
	<div onclick="change_sales_overview_period('3y')" period="3y" id="3y" class="fixed_interval {if  $period=='3y'}selected{/if}" title="{t}3 years{/t}">{t}3Y{/t}</div> 
...
<td class="refunds aright {if !($type=='invoices' or  $type=='invoice_categories')}hide{/if}" title="{t}Change compared with last year{/t}">&Delta; {t}Last Y{/t}</td>

<td class="delivery_notes aright {if $type!='delivery_notes'}hide{/if}" title="{t}Delivery notes{/t}" >{t}Delivery notes{/t}</td>
<td class="delivery_notes aright {if $type!='delivery_notes'}hide{/if}" title="{t}Change compared with last year{/t}">&Delta; {t}Last Y{/t}</td>
...
<td class="last replacements aright {if $type!='delivery_notes'}hide{/if}" title="{t}Percentage of replacements same period last year{/t}">&Delta; {t}Last Y{/t}</td>

<td class="invoices aright {if !($type=='invoices' or  $type=='invoice_categories')}hide{/if}">{t}Invoices{/t}</td>
<td class="invoices aright {if !($type=='invoices' or  $type=='invoice_categories')}hide{/if}">&Delta; {t}Last Y{/t}</td>

<td class="sales aright {if !($type=='invoices' or  $type=='invoice_categories')}hide{/if}" >{t}Sales{/t}</td>
<td class="last sales aright {if !($type=='invoices' or  $type=='invoice_categories')}hide{/if}">&Delta; {t}Last Y{/t}</td>
...
{if isset($record.refunds.view) and $record.label.view!='' }onclick="change_view('{$record.refunds.view}' , { parameters:{ period:'{$period}',elements_type:'type' } ,element:{ type:{ Refund:1,Invoice:''}} } )"
{/if}
...
{if isset($record.invoices.view) and $record.invoices.view!='' }onclick="change_view('{$record.invoices.view}' , { parameters:{ period:'{$period}',elements_type:'type' } ,element:{ type:{ Refund:'',Invoice:1}} } )"
{/if}
