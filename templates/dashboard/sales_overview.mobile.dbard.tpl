<table class="mdl-data-table mdl-js-data-table  mdl-shadow--2dp" style="width:100%">
    ...
    {foreach from=$sales_overview item=record}
        <tr class="{$record.class} small_row">
            <td class=" {if isset($record.label.view) and $record.label.view!='' }link{/if}"
            {if isset($record.label.view) and $record.label.view!='' }onclick="change_view('{$record.label.view}')" {/if} title="{if isset($record.label.title)}{$record.label.title}{else}{$record.label.label}{/if}  ">{$record.label.short_label}</td>
