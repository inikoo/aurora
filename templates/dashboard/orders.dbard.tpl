...
<ul class="flex-container" >
    ...
    <li id="order_flow_submitted" class="flex-item order_flow {if $order_flow=='submitted_not_paid' or  $order_flow=='submitted' }selected{/if}" >
        ...
        <div class="title">
            ...
            <span class="Orders_In_Process_Paid_Number button {if  $order_flow=='submitted'}blue{/if}" title="{t}Paid submitted orders{/t}"
                  onclick="get_orders_table('submitted',{ parent: 'store','parent_key':{$store->id}})">{$store->get('Orders In Process Paid Number')} <i style="font-size: 50%" class="fa fa-usd success" aria-hidden="true"></i> </span>  </div>
        <div >
    ...
    <li id="order_flow_dispatched" class="flex-item order_flow {if $order_flow=='dispatched_today'}selected{/if}">
        ...
        <div class="title"><span class="" >
            ...
            <span class="Orders_Dispatched_Today_Number button" title="{t}Today's dispatched orders{/t}"  onclick="get_orders_table('dispatched_today',{ 'parent': 'account','parent_key':1})">{$store->get('Orders Dispatched Today Number')} <i style="font-size: 50%" class="hide fa fa-paper-plane " aria-hidden="true"></i> </span> </div>
        <div >
...
<div id="widget_details" class="hide" style="clear:both;margin-top:-1px;border-top:1px solid #ccc"></div>
        ...
        <script>
            ...
            get_orders_table('{$order_flow}',{ 'parent': 'store','parent_key':{$store->id}})
        </script>
