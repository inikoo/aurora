...
<div id="bd">
	...
	<div id="top_page_menu" class="top_page_menu">
		...
		<div class="buttons" style="float:right">
			<button id="pack_all" style="height:24px;{if $delivery_note->get('Delivery Note Faction Packed')==1}display:none{/if}" ><img src="art/icons/accept.png" alt="" /> {t}Set all as Packed{/t}</button> 
			<a id="picking_aid" style="height:14px;{if $delivery_note->get('Delivery Note Faction Picked')==1}display:none{/if}" href="order_pick_aid.php?id={$delivery_note->id}"><img src="art/icons/basket.png" alt="" /> {t}Picking Aid{/t}</a> 
			<button id="set_as_done" style="height:24px;{if $delivery_note->get('Delivery Note Faction Picked')!=1}display:none{/if}" ><img src="art/icons/accept.png" alt="" /> {t}Aprove Picking/Packing{/t}</button> 
		...
		<div style="border:1px solid #ccc;text-align:left;padding:10px;margin: 0px 0 10px 0;xheight:15em">
			<div style="xborder:1px solid #ddd;width:350px;float:left">
				<h1 style="padding:0 0 10px 0">
					{t}Packing for Delivery Note{/t} {$delivery_note->get('Delivery Note ID')} 
				</h1>
				<h2 style="padding:0">
				...
				<table style="xdisplay:none;width:100%;xborder-top:1px solid #333;xborder-bottom:1px solid #333;width:100%,padding:0;margin:0;float:right;margin-left:0px">
					<tbody id="resend" style="xdisplay:none">
						...
						<tr>
							...
							<td class="aright">
							<span id="number_packed_transactions">{$number_packed_transactions}</span>/<span id="number_picked_transactions">{$number_picked_transactions}</span>/<span id="number_transactions">{$number_transactions}</span> 
							<span style="margin-left:10px" id="percentage_packed">{$delivery_note->get('Faction Packed')}</span></td>
						</tr>
					</tbody>
				</table>
			</div>
