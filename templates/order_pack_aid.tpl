...
<div id="bd">
	...
	<div class="branch">
		<span>{if $user->get_number_warehouses()>1}<a href="warehouses.php">{t}Warehouses{/t}</a> &rarr; {/if}<a href="warehouse_parts.php?warehouse_id={$warehouse->id}">{t}Inventory{/t}</a> &rarr; <a href="warehouse_orders.php?id={$warehouse->id}">{t}Pending Orders{/t}</a> &rarr; {$delivery_note->get('Delivery Note ID')} ({t}Pack Aid{/t})</span> 
	</div>
	<div id="top_page_menu" class="top_page_menu">
		...
		<div class="buttons" style="float:right">
			<button id="pack_all" style="height:24px;{if $delivery_note->get('Delivery Note Faction Packed')==1}display:none{/if}"><img src="art/icons/accept.png" alt="" /> {t}Set all as Packed{/t}</button> <a id="picking_aid" style="height:14px;{if $delivery_note->get('Delivery Note Faction Picked')==1}display:none{/if}" href="order_pick_aid.php?id={$delivery_note->id}"><img src="art/icons/basket.png" alt="" /> {t}Picking Aid{/t}</a> 
			<button id="set_as_done" style="height:24px;{if $delivery_note->get('Delivery Note Faction Packed')!=1}display:none{/if}"><img src="art/icons/accept.png" alt="" /> {t}Aprove Picking/Packing{/t}</button> 
		</div>
		...
		<div style="border:1px solid #ccc;text-align:left;padding:10px;margin: 0px 0 10px 0;xheight:15em">
			<div style="xborder:1px solid #ddd;width:350px;float:left">
				<h1 style="padding:0 0 10px 0">
					{t}Packing of Delivery Note{/t} <a href="dn.php?id={$delivery_note->id}">{$delivery_note->get('Delivery Note ID')}</a> 
				</h1>
				<h2 style="padding:0">
				...
				<table style="xdisplay:none;width:100%;xborder-top:1px solid #333;xborder-bottom:1px solid #333;width:100%,padding:0;margin:0;float:right;margin-left:0px">
					<tbody id="resend" style="xdisplay:none">
						...
						<tr>
							...
							<td class="aright"> <span id="number_packed_transactions">{$number_packed_transactions}</span>/<span id="number_picked_transactions">{$number_picked_transactions}</span>/<span id="number_transactions">{$number_transactions}</span> <span style="margin-left:10px" id="percentage_packed">{$delivery_note->get('Faction Packed')}</span></td>
						</tr>
					</tbody>
				</table>
			</div>
...
<div id="no_dispatchable_editor_dialog" style="width:200px" xstyle="position:fixed;top:-200px">
	...
	<div class="bd dt-editor">
		<div style="display:none;margin-top:20px" id="todo_error_msg">
			<p>
				{t}Error, the sum of out of stock and not found units are greater than the number of not packed units{/t} 
			</p>
		</div>
		<table border="0" style="margin:0">
			...
			<input type="hidden" id="packed_units" value="0"> 
			<tr>
			...
			<tr>
				<td style="cursor:pointer" onclick="add_no_dispatchable('no_packed_other_units')">+</td>
				<td style="cursor:pointer" onclick="remove_no_dispatchable('no_packed_other_units')">-</td>
				<td> 
				<input id="no_packed_other_units" type="text" style="width:20px;"></td>
