...
editable: false,
sortType: "toggle",
{if $sort_key=='agent_link'}direction: '{if $sort_order==1}descending{else}ascending{/if}',{/if}
...
grid.columns.findWhere({ name: 'agent_link'} ).set("renderable", false)
...
grid.columns.findWhere({ name: 'agent_link'} ).set("renderable", true)
