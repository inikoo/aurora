{*/*
 ...
 Created:  13 November 2019  09:33::55  +0100 Malaga, Spain
...
name: "clients",
label: "{t}Clients{/t}",
renderable:{if !empty($show_customer_clients)}true{else}false{/if},
editable: false,
defaultOrder:1,
sortType: "toggle",
...
defaultOrder:1,
sortType: "toggle",
{if $sort_key=='portfolio'}direction: '{if $sort_order==1}descending{else}ascending{/if}',{/if}
cell: Backgrid.HtmlCell.extend({ className: "aright"} ),
headerCell: integerHeaderCell
},
...
grid.columns.findWhere({ name: 'portfolio'} ).set("renderable", false)
...
grid.columns.findWhere({ name: 'portfolio'} ).set("renderable", true)
