	...
	<div id="orders_table" class="data_table" style="clear:both;margin-top:23px">
		...
		<div id="table_type" class="table_type">
			<div style="font-size:90%" id="transaction_chooser">
				<span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements.ReadytoShip}selected{/if} label_dn_state_ready_to_ship" id="elements_ready_to_ship" table_type="ready_to_ship">{t}Ready to Ship{/t} (<span id="elements_notes_number">{$elements_number.ReadytoShip}</span>)</span> <span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements.PickingAndPacking}selected{/if} label_dn_state_picking_and_packing" id="elements_picking_and_packing" table_type="picking_and_packing">{t}Picking/Packing{/t} (<span id="elements_notes_number">{$elements_number.PickingAndPacking}</span>)</span> <span style="float:right;margin-left:20px;{if !$elements_number.ReadytoRestock}display:none{/if}" class=" table_type transaction_type state_details {if $elements.ReadytoRestock}selected{/if} label_dn_state_ready_to_restock" id="elements_ready_to_restock" table_type="ready_to_restock">{t}Ready to Restock{/t} (<span id="elements_notes_number">{$elements_number.ReadytoRestock}</span>)</span> <span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements.ReadytoPack}selected{/if} label_dn_state_ready_to_pack" id="elements_ready_to_pack" table_type="ready_to_pack">{t}Ready to Pack{/t} (<span id="elements_notes_number">{$elements_number.ReadytoPack}</span>)</span> <span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements.ReadytoPick}selected{/if} label_dn_state_ready_to_pick" id="elements_ready_to_pick" table_type="ready_to_pick">{t}Ready to Pick{/t} (<span id="elements_notes_number">{$elements_number.ReadytoPick}</span>)</span> 
			</div>
		</div>
