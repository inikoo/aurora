...
<div id="bd">
	...
	<input type="hidden" value="Supplier" id="category_subject" />
	<div class="branch">
		<span><a href="index.php"><img style="vertical-align:0px;margin-right:1px" src="art/icons/home.gif" alt="home" /></a>&rarr; <a href="suppliers.php">{t}Suppliers{/t}</a> &rarr; <a href="edit_supplier_categories.php">{t}Suppliers Categories{/t}</a> ({t}Editing{/t})</span> 

	</div>
	<div class="top_page_menu">
		<div class="buttons" style="float:left">
			<span class="main_title">{t}Editing Suppliers Categories{/t}</span> 
		</div>
		<div class="buttons" style="float:right">
			<button onclick="window.location='supplier_categories.php'"><img src="art/icons/door_out.png" alt=""> {t}Exit Edit{/t}</button> <button id="new_category"><img src="art/icons/add.png" alt=""> {t}New Main Category{/t}</button> 
		</div>
	...
	<div class="tabbed_container">
		<div class="edit_block" style="min-height:300px;{if $edit!='subcategory'}display:none{/if}" id="d_subcategory">
			...
			<div class="elements_chooser">
					<span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements.Head}selected{/if} label_supplier_Head" id="elements_Head" table_type="Head">{t}Head{/t} (<span id="elements_Head_number">{$elements_number.Head}</span>)</span> <span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements.Node}selected{/if} label_supplier_Node" id="elements_Node" table_type="Node">{t}Node{/t} (<span id="elements_Node_number">{$elements_number.Node}</span>)</span> <span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements.Root}selected{/if} label_supplier_Root" id="elements_Root" table_type="Root">{t}Root{/t} (<span id="elements_Root_number">{$elements_number.Root}</span>)</span> 
				</div>
	...
	<div class="buttons small" style="margin-top:0;margin-bottom:50px">
		<button id="show_history" style="{if $show_history}display:none{/if};margin-right:0px" onclick="show_history('supplier_categories')">{t}Show changelog{/t}</button> <button id="hide_history" style="{if !$show_history}display:none{/if};margin-right:0px" onclick="hide_history('supplier_categories')">{t}Hide changelog{/t}</button> 
	</div>
...
{include file='new_main_category_splinter.tpl' subject='Supplier'} {include file='footer.tpl'} 