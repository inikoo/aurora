...
<div id="bd">
	...
	<ul class="tabs" id="chooser_ul" style="clear:both">
		...
		<li> <span class="item {if $edit_block_view=='deals'}selected{/if}" id="deals"><span> {t}Offers{/t}</span></span></li>
	...
	<div class="tabbed_container no_padding">
			...
			<table class="edit" style="margin-top:20px;width:900px" border="0">
				...
				<tr style="{if $campaign->get('Deal Campaign Status')=='Finish'}display:none{/if}">
					...
					<div class="buttons small left">
						...
						<img style="{if $campaign->get('Deal Campaign Valid To')==''}display:none{/if}" id="calpop2" src="art/icons/calendar_view_month.png" align="top" alt="" /> <button id="permanent" class="{if $campaign->get('Deal Campaign Valid To')==''}selected{/if}">{t}Permanent{/t}</button> <button style="{if $campaign->get('Deal Campaign Valid To')!=''}display:none{/if}" id="change_valid_to">{t}Set date{/t}</button> <button id="finish">{t}Finish{/t}</button> 
					</div>
					</td>
		...
		<div id="d_deals" class="edit_block" style="width:890px;padding:20px 20px;{if $edit_block_view!='deals'}display:none{/if}">
