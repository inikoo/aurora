...
<div id="hide{$table_id}"  class="hide"><img id="hidder{$table_id}" align="absbottom" src="art/minimize.png"  height=14 state="1"  alt="{t}Hide items{/t}"></div>
<div id="filter{$table_id}" class="filter"   {if !$filter}style="display:none"{/if}  >
  ...
  <div id="filtertitle{$table_id}" class="filtertitle"   > 
    <span  class="filterselector" id="filterselector{$table_id}">{$filter_name}</span>{if !options} {t}filter{/t}:{/if}
  </div> 
 <div id="paginatormenu{$table_id}" class="paginatormenu"> 
    ...
    <img   class="loadingicon" id="loadingicon{$table_id}"   src="art/loading.png" alt=""  /> 
  </div>
  ...
  <div id="paginator{$table_id}" class="paginator"> 
    ...
    <span id="pag{$table_id}">{t}Loading...{/t}</span>
