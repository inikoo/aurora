...
.items td.description{font-size:95%}
.items tr.total td {
  border-top: 0.3mm solid #000;
	...
	<table class="items" width="100%" style="font-size: 9pt; border-collapse: collapse;" cellpadding="8">
		<thead>
			<tr>
				...
				<td style="width:8%;text-align:right">{t}Tax{/t}</td>
		...
		<tbody>
			...
			<tr class="{if $smarty.foreach.products.last}last{/if}">
				...
				<td style="width:10%;text-align:right">{$transaction['Net']}</td>
				<td style="width:8%;text-align:right">{$transaction['Tax']}</td>
		...
		{/if} 
		<tbody>
			{foreach from=$transactions_no_products item=transaction name=no_products} 
			<tr class="{if $smarty.foreach.products.first}top{/if}">
				<td style="width:8%;text-align:left">{$transaction['Product Code']}</td>
				<td class="description"  style="text-align:left" colspan=2>{$transaction['Product XHTML Short Description']}</td>
		...
		<tr class="total">
				<td style="width:8%;text-align:left" colspan=3>{t}Total{/t}</td>
				
				<td style="width:10%;text-align:right">{$invoice->get('Total Net Amount')}</td>
				<td style="width:8%;text-align:right">{$invoice->get('Total Tax Amount')}</td>
				<td style="width:10%;text-align:right">{$invoice->get('Total Amount')}</td>
		...
		<td style="border:none"  colspan="6" ></td>
			...
			<tr class="total_net">
				<td style="border:none" colspan="3" rowspan="10"></td>
