...
<li><b>{t}Part barcode{/t}</b> <i>({t}optional{/t})</i> [12 or 13 digits] <i>{t}or{/t}</i> {t}or click{/t} <span  class="fa-stack fa-lg fa-fw" ><i class="fa fa-barcode fa-stack-1x"></i>
 <i class="fa fa-bolt fa-inverse fa-stack-1x"></i>
</span> {t}for automatically assign next available barcode{/t}</li>
			<li><b>{t}Minimum order{/t}</b> <i>({t}required{/t})</i> [{t}number{/t}] {t}minimum number cartons supplier willing to sell{/t}</li>
			...
			<li><b>{t}Unit extra costs{/t}</b> <i>({t}required{/t})</i> [{t}amount (local supplier's currency){/t} <i>{t}or{/t}</i> {t}percentage{/t}] </li>
			<li><b>{t}Unit price{/t}</b> <i>({t}required{/t})</i> [{t}amount{/t} ({$account->get('Currency')})  <i>{t}or{/t}</i> {t}margin{/t}]</li>
			<li><b>{t}Unit RRP{/t}</b> <i>({t}required{/t})</i> [{t}amount{/t} ({$account->get('Currency')})  <i>{t}or{/t}</i> {t}margin{/t}]</li>
