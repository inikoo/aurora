...
<div style="padding-top:20px;">
    ...
    <div id="maintabs" class="tabs" style="border-top:1px solid #ccc">
        <div id="tab_orders_in_basket" class="tab left {if $data['tab']=='orders.website.wget'}selected{/if}"
            ...
            <i class="fa fa-shopping-cart"></i> <span class="label"> {t}Orders{/t} <span class=""></span></span>
        </div>
        <div id="tab_orders_in_basket_purges" class="hide tab left {if $data['tab']=='orders.website.purges.wget'}selected{/if}"
             onclick="change_view('orders/{if $data['parent']=='account'}all{else}{$data['parent_key']}{/if}/dashboard/website/purges')" title="">
            <i class="fa fa-trash-o"></i> <span class="label"> {t}Purges{/t} <span class=""></span></span>
        </div>
        <div id="tab_orders_in_basket_abandoned_emails" class="hide tab left {if $data['tab']=='orders.website.mailshots.wget'}selected{/if}"
             onclick="change_view('orders/{if $data['parent']=='account'}all{else}{$data['parent_key']}{/if}/dashboard/website/mailshots')" title="">
            <i class="fa fa-envelope-o"></i> <span class="label"> {t}Mailshots{/t} <span class=""></span></span>
        </div>
