<div id="inline_form" style="float:right" class="hide"
     data-dropdown_select_metadata="{if isset($data.dropdown_select_metadata)}{$data.dropdown_select_metadata}{/if}"
...
<span id="inline_new_object" class=" hide" object='{$data.object}' key='' parent='{$data.parent}'
      parent_key='{$data.parent_key}' field='{$data.field_id}' data-other_fields='{if isset($data.other_fields)}{$data.other_fields}{else}{literal}{}{/literal}{/if}'>
<span id="{$data.field_id}_container" class="value " _required="true" field_type='{$data.field_edit}'
      field='{$data.field_id}' server_validation=''>
{$data.field_label}

    <input id="{$data.field_id}" class="{$data.field_edit}_input_field inline_input" value=""
...
<script>
    ...
    function get_inline_new_object_items_select() {
        ...
        var request = '/ar_find.php?tipo=find_objects&query=' + fixedEncodeURIComponent(input_element.val()) + '&scope=' + JSON.parse(atob(form_element.data("dropdown_select_metadata"))).scope + '&metadata=' + atob(form_element.data("dropdown_select_metadata")) + '&state=' + JSON.stringify(state)
