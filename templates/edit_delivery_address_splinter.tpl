...
<div style="width:540px;float:right;text-align:left">
	...
	<table>
		<tr id="tr_address_showcase">
			...
			{foreach from=$customer->get_delivery_address_objects('no_contact') item=address key=key name='delivery_addresses'} 
			
			 {if $smarty.foreach.delivery_addresses.index % 2 == 0}
...
<div style="width:260px;position:relative;bottom:-1px">
	...
	<div  id="delivery_address_showcase_bis" style="{if $parent=='order'}display:none{/if}">
		...
		<button id="delivery_use_this{$customer->get('Customer Main Address Key')}" style="margin-top:3px;float:left;{if $parent!='order'}display:none{/if}" class="delivery_use_this small_button small_button_edit" onclick="use_this_delivery_address_in_order({$customer->get('Customer Main Address Key')},true)">{t}Use This{/t}</button> 
