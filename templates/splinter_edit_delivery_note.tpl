<div id="dialog_set_dn_data" style="width:710px;padding:20px 20px 0 20px;">
		...
		<table class="edit" border="0" style="margin-bottom:5px;width:700px">
			...
			<tr>
				...
				<div>
					<input style="width:30px" id="number_parcels" changed="0" type='text' class='text' value="{$delivery_note->get('Delivery Note Number Parcels')}" ovalue="{$delivery_note->get('Delivery Note Number Parcels')}" />
				...
				<div class="buttons small left" id="parcel_type_options">
					<button onclick="change_parcel_type(this)" class="parcel_type {if $delivery_note->get('Delivery Note Parcel Type')=='Pallet'}selected{/if}" id="parcel_Pallet" valor="Pallet">{t}Pallet{/t}</button> 
					<button onclick="change_parcel_type(this)"  class="parcel_type {if $delivery_note->get('Delivery Note Parcel Type')=='Envelope'}selected{/if}" id="parcel_Envelope" valor="Envelope">{t}Envelope{/t}</button> 
					<button onclick="change_parcel_type(this)"  class="parcel_type {if $delivery_note->get('Delivery Note Parcel Type')=='Small Parcel'}selected{/if}" id="parcel_Small Parcel" valor="Small Parcel">{t}Small Parcel{/t}</button> 
					<button onclick="change_parcel_type(this)"  class="parcel_type {if $delivery_note->get('Delivery Note Parcel Type')=='Box'}selected{/if}" id="parcel_Box" valor="Box">{t}Box{/t}</button> 
					<button onclick="change_parcel_type(this)"  class="parcel_type {if $delivery_note->get('Delivery Note Parcel Type')=='None'}selected{/if}"  id="parcel_None" valor="None">{t}None{/t}</button> 
					<button onclick="change_parcel_type(this)"  class="parcel_type {if $delivery_note->get('Delivery Note Parcel Type')=='Other'}selected{/if}"  id="parcel_Other" valor="Other">{t}Other{/t}</button> 
				</div>
			...
			<tr class="buttons">
							...
							<div class="buttons left">
								<button style="margin-right:10px;" id="reset_edit_delivery_note" class="negative disabled">{t}Reset{/t}</button> 

								<button style="margin-right:10px;" id="save_edit_delivery_note" class="positive disabled">{t}Save{/t}</button> 
							</div>
							</td>
			</tr>
		</table>
