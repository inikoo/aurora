	...
	<table class="report_sales1" id="report_sales_invoices" style="width:900px;{if $view!='invoices'}display:none{/if}">
		...
		<table class="report_sales1" id="report_sales_profit" style="width:700px;{if $view!='profits'}display:none{/if}">
			<tr style="border-bottom:1px solid #ccc;margin-bottom:5px">
				<td colspan="7"> 
				<div style="margin-bottom:5px;;color:#999;">
					<span class="state_details selected" style="cursor:default;margin-right:20px">{t}Profit{/t}</span> <span id="profits_invoices_button" view="invoices" class="state_details" style="margin-right:20px">{t}Invoices{/t}</span> <span id="profits_corporate_currency_button" currency="corporate" class="state_details {if $currencies=='corporation'}selected{/if}" style="{if !$mixed_currencies}display:none;{/if}margin-right:5px">{t}HQ Currency{/t}</span> <span id="profits_stores_currency_button" currency="stores" class="state_details {if $currencies!='corporation'}selected{/if}" style="{if !$mixed_currencies}display:none;{/if}">{t}Store Currencies{/t}</span> 
				</div>
				</td>
			</tr>
			<tr>
