...
<div class="no_padding" id="bd" style="{if $product->get('Product Record Type')=='Discontinued'}background-position:300px 30px;background-image:url('art/stamp.discontinued.en.png');background-repeat:no-repeat;{/if}">
			...
			<div id="photo_container" style="margin-top:0px;float:left">
				...
				<div style="border:1px solid #ddd;padding-stop:0;width:220px;xheight:230px;text-align:center;margin:0 10px 0 0px">
					<div id="imagediv" style="border:1px solid #ddd;width:190px;padding:5px 5px;xborder:none;cursor:pointer;xbackground:red;margin: 10px 0 10px 9px;vertical-align:middle">
						<a href="image.php?id={$product->get('Product Main Image Key')}&size=large" class="imgpop"><img id="main_image" class="imgdpop" src="{$product->get('Product Main Image')}" style="vertical-align:middle;display:block;margin:0px auto;width:190px" valign="center" border="1" id="image" alt="{t}Image{/t}" /> </a> 
					</div>
				</div>
				<div style="width:160px;margin:auto;padding-top:5px;{if $product->get_number_of_images()<=1}display:none{/if}">
					<ul class="gallery clearfix">
						...
						<li><a href="{$image.normal_url}" class="imgpop"> <img style="float:left;border:1px solid#ccc;padding:2px;margin:2px;cursor:pointer" src="{$image.thumbnail_url}" alt="{$image.name}" /> </a> {/if} {/foreach} 
					</ul>
				</div>
			</div>
	...
	<ul class="tabs" id="chooser_ul" style="clear:both;margin-top:15px">
		...
		<li> <span class="item {if $block_view=='availability'}selected{/if}" id="availability"> <span> {t}Availability{/t}</span></span></li>
		...
		<li> <span class="item {if $block_view=='timeline'}selected{/if}" id="timeline"> <span> {t}Similar{/t}</span></span></li>
		<li> <span class="item {if $block_view=='pictures'}selected{/if}" id="pictures"> <span> {t}Pictures{/t}</span></span></li>
	</ul>
	<div class="tabs_base">
	</div>
	<div id="block_details" style="padding:20px;min-height:400px;{if $block_view!='details'}display:none;{/if}clear:both;margin:10px 0 40px 0;padding-top:10px">
	    ...
	    <div>
		    {if $product->get('Product Description')==''} <span style="font-style:italic;color:#777">{t}This product don't have public description{/t}</span> {else} {$product->get('Product Description')} {/if} 
	    </div>
    </div>

<div id="block_deals" style="padding:20px;min-height:400px;{if $block_view!='deals'}display:none;{/if}clear:both;margin:0px 0 40px 0">
...
<div id="block_sales" style="padding:10px 20px;min-height:400px;{if $block_view!='sales'}display:none;{/if}clear:both;margin:5px 0 40px 0;">
...
<div id="block_timeline" style="padding:20px;min-height:400px;{if $block_view!='timeline'}display:none;{/if}clear:both;margin:10px 0 40px 0;padding-top:10px">
...
<div id="block_history" style="padding:20px;min-height:400px;{if $block_view!='history'}display:none;{/if}clear:both;margin:10px 0 40px 0;padding-top:10px">
	<span id="table_title" class="clean_table_title">{t}History/Notes{/t}</span> 
	<div class="elements_chooser">
		<span style="float:right;margin-left:20px;" class=" table_type transaction_type state_details {if $elements_product_history.Changes}selected{/if} label_product_history_changes" id="elements_product_history_changes" table_type="elements_changes">{t}Changes History{/t} (<span id="elements_history_Changes_number">{$elements_product_history_number.Changes}</span>)</span> <span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements_product_history.Notes}selected{/if} label_product_history_notes" id="elements_product_history_notes" table_type="elements_notes">{t}Staff Notes{/t} (<span id="elements_history_Notes_number">{$elements_product_history_number.Notes}</span>)</span> <span style="float:right;margin-left:20px" class=" table_type transaction_type state_details {if $elements_product_history.Attachments}selected{/if} label_product_history_attachments" id="elements_product_history_attachments" table_type="elements_attachments">{t}Attachments{/t} (<span id="elements_history_Attachments_number">{$elements_product_history_number.Attachments}</span>)</span> 
	</div>
	<div class="table_top_bar space">
	</div>
...
<div id="block_availability" style="padding:20px;min-height:400px;{if $block_view!='availability'}display:none;{/if}clear:both;margin:10px 0 40px 0;padding-top:10px">
...
<div id="block_web" style="padding:20px;min-height:400px;{if $block_view!='web'}display:none;{/if}clear:both;margin:10px 0 40px 0;padding-top:10px">
...
<div id="block_orders" class="data_table" style="padding:20px;min-height:400px;{if $block_view!='orders'}display:none;{/if}clear:both;margin:10px 0 40px 0;padding-top:10px">
...
<div id="block_customers" class="data_table" style="{if $block_view!='customers'}display:none;{/if}clear:both;margin:10px 0 40px 0;;padding:0px">
	<div class="buttons small left tabs">
		<button class="indented item {if $customers_block_view=='customers_who_order'}selected{/if}" id="customers_who_order" block_id="customers_who_order">{t}Customers who order{/t}</button> 
		<button class=" item {if $customers_block_view=='customers_who_favorited'}selected{/if}" id="customers_who_favorited" block_id="customers_who_favorited">{t}Customers who favorited{/t}</button> 
	</div>
	<div class="tabs_base">
	</div>
	
	<div id="block_customers_who_order" style="{if $customers_block_view!='customers_who_order'}display:none;{/if}clear:both;margin:10px 0px 40px 0px;padding:10px 20px">
...
<div id="block_pictures" class="data_table" style="padding:20px;min-height:400px;{if $block_view!='pictures'}display:none;{/if}clear:both;margin:10px 0 40px 0;;padding-top:10px">
...
<div id="dialog_edit_web_state" style="padding:20px 20px 10px 20px ">
	...
	<div class="buttons" id="edit_web_state_buttons">
		<button class="item" onclick="set_web_configuration(this,'Offline')">{t}Offline{/t}</button> <button class="item" onclick="set_web_configuration(this,'Online Force Out of Stock')">{t}Force Out of Stock{/t}</button> <button class="item" onclick="set_web_configuration(this,'Online Force For Sale')">{t}Force Online{/t}</button> <button class="item" onclick="set_web_configuration(this,'Online Auto')">{t}Link to part{/t}</button> 
	</div>
</div>
