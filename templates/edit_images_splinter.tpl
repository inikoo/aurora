...
<div id="images" class="edit_images" principal="{$parent->get_main_image_key()}">
	...
	<div id="image_container{$image.id}" class="image" image_id="{$image.id}" is_principal="{$image.is_principal}">
		...
		<img style="cursor:pointer" class="delete" src="art/icons/delete.png" alt="{t}Delete{/t}" title="{t}Delete{/t}" onclick="delete_image(this)"> <img class="picture" src="{$image.small_url}" /> 
		...
		<div class="caption" id="caption{$image.id}">{$image.caption}</div> 
