...
<script type="text/javascript" src="external_libs/amstock/amstock/swfobject.js"></script> 
	...
	<div style="padding:0 20px">
		...
		<div class="top_page_menu">
			...
			{t}Deleted Page{/t}: <span class="id"> {$page->get('Page Code')}</span> <span style="text-decoration:line-through;font-size:90%;color:#777">{$page->get('Page URL')}</span>
		</span>
			</div>
	...
	<div id="block_details" style="{if $block_view!='details'}display:none;{/if}clear:both;margin:25px 0 40px 0;padding:0 20px">
		<div style="width:450px;float:left;margin-top:0">
			<table id="page_info" class="show_info_product">
				...
				<tr>
					...
					<td>{$page->get('Page Title')}</td>
				</tr>
				<tr>
					<td style="width:140px">{t}URL{/t}:</td>
					<td style="text-decoration:line-through;">{$page->get('Page URL')}</td>
				</tr>
				<tr>
		...
		<div style="{if !$page->get('Page Preview Snapshot Image Key')}display:none{/if};margin-left:20px;width:450px;float:left;position:relative;top:-12px">
			<span style="font-size:11px;color:#777;">{t}Snapshot{/t}, {$page->get_snapshot_date()}</span> <img id="recapture_page" style="position:relative;top:-1px;cursor:pointer" src="art/icons/camera_bw.png" alt="recapture" /> <img style="width:470px" src="image.php?id={$page->get('Page Preview Snapshot Image Key')}" alt="" /> 
		</div>
