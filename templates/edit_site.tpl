...
<div id="bd">
	...
	<div class="top_page_menu">
		<div class="buttons">
			...
			<button  id="clean_cache" ><img id="clean_cache_img" src="art/icons/wand.png" alt="" /> {t}Clean Cache{/t}</button>  
			...
			<table class="edit" border="0" style="width:890px;clear:both;margin:25px 0px 20px 20px">
				...
				<tr>
					...
					<div class="buttons left" style="margin-left:0px">
						<button id="reset_edit_registration" class="negative disabled">{t}Reset{/t}</button> <button id="save_edit_registration" class="positive disabled">{t}Save{/t}</button> 
					</div>
					<span id="registration_msg"></span> </td>
				</tr>
			</table>
			<div class="buttons left small tabs" style="margin-top:0px;">
				<button id="registration" class="indented {if $users_block_view=='registration'}selected{/if}">{t}Registration{/t}</button> <button id="welcome" class="{if $users_block_view=='welcome'}selected{/if}">{t}Welcome{/t}</button> <button id="forgot_password" class="{if $users_block_view=='forgot_password'}selected{/if}">{t}Forgot Password{/t}</button> <button id="email_provider" class="{if $users_block_view=='email_provider'}selected{/if}">{t}Email Provider{/t}</button> <button id="client_profile" class="{if $users_block_view=='client_profile'}selected{/if}">{t}Client Profile{/t}</button> 
			</div>
			<div class="tabs_base">
			</div>
			...
			<div class="edit_subblock" style="{if $components_block_view!='checkout'}display:none{/if}" id="d_checkout">
				<table class="edit" border="0" style="width:100%">
					...
					<tr class="buttons">
						<td colspan="2"> 
						<div class="buttons left" style="margin-left:250px">
							<button id="reset_edit_site_checkout" class="negative disabled">{t}Reset{/t}</button> <button id="save_edit_site_checkout" class="positive disabled">{t}Save{/t}</button> 
						</div>
						</td>
					</tr>
				</table>
...
<textarea style="width:100%" id="address" changed="0" value="{$site->get('Site Address')}" ovalue="{$site->get('Site Contact Address')}" rows="6" cols="42">{$site->get('Site Contact Address')}</textarea> 
	...
	<div class="edit_block" style="{if $block_view!='style'}display:none{/if}" id="d_style">
		<div class="buttons small left tabs">
			<button id="background" class="indented {if $style_block_view=='background'}selected{/if}">{t}Background{/t}</button> <button id="favicon" class="{if $style_block_view=='favicon'}selected{/if}">{t}Favicon{/t}</button> 
		</div>
		<div class="tabs_base">
		</div>
		<div class="edit_block_content">
			...
			<div class="edit_subblock" style="{if $style_block_view!='favicon'}display:none{/if}" id="d_favicon">
				<table class="edit">
				<tr id="favicon_tr" style="{if $site->get_favicon_url()==''}display:none{/if}">
				<td>{t}Favicon{/t}:</td><td>
				
					<img id="favicon_img" style="width:16px;height:16px" src="{$site->get_favicon_url()}">
					...
					<tr class="space15">
						...
						<form action="upload.php" enctype="multipart/form-data" method="post" id="testForm">
							<input id="upload_favicon_input" style="border:1px solid #ddd;" type="file" name="testFile" />
						</form>
						</td>
						<td> 
						<div class="buttons left">
							<button id="upload_favicon" class="positive">{t}Upload{/t}</button> 
						</div>
						</td>
					</tr>
				</table>
	...
	<div class="edit_block" style="{if $block_view!='pages'}display:none{/if}" id="d_pages">
		...
		<div class="buttons left small tabs" style="margin-top:0px;">
				<button id="pages_list" class="indented {if $pages_block_view=='pages'}selected{/if}">{t}Pages{/t}</button> 
				<button id="site_flags" class="{if $pages_block_view=='flags'}selected{/if}">{t}Flags{/t}</button> 
			</div>
			<div class="tabs_base">
			</div>
		...
		<div class="edit_block_content">
			<div id="d_pages_list" class="data_table" style="clear:both;{if $pages_block_view!='pages'}display:none{/if}">
			...
			<div id="d_site_flags" class="data_table" style="clear:both;{if $pages_block_view!='site_flags'}display:none{/if}">
			
			<table style="margin:0;width:100%" class="edit" border="0">
				
				<tr class="title">
					<td colspan="3"> {t}Page Flags{/t} </td>
				</tr>
				<input type="hidden" id="move_pages_to_default_msg" value="{t}Pages will be moved to default flag{/t}" />
				{foreach from=$flags item=flag} 
				<tr>
					<td class="label"><img style="opacity:{if $flag.display=='Yes'}1{else}0.5{/if}" id="page_flag_icon_{$flag.id}" src="art/icons/{$flag.icon}" /></td>
					<td colspan="2"> 
					<table border="0" style="margin:0;padding:0;width:600px">
						<tr>
							<td style="width:150px"> 
							<div>
								<input style="width:100%" type="text" id="page_flag_label_{$flag.id}" value="{$flag.label}" ovalue="{$flag.label}" valid="1"> 
								<div id="page_flag_label_{$flag.id}_Container">
								</div>
							</div>
							</td>
							<td style="width:16px;padding:0px" id="page_flag_label_{$flag.id}_msg" class="edit_td_alert"> </td>
							<td style="width:80px"> 
							<input id="page_flag_active_{$flag.id}" type="hidden" default="{$flag.default}" flag_id="{$flag.id}" value="{$flag.display}" ovalue="{$flag.display}" />
							<input id="page_flag_number_pages_{$flag.id}" type="hidden" value="{$flag.pages}" />
							<div class="buttons small left" style="height:16px">
								<button id="page_flag_display_{$flag.id}_Yes" class="page_flag_display_{$flag.id}" value="Yes" onclick="change_flag_display(this,{$flag.id})" style="{if $flag.display!='Yes' or $flag.default}display:none{/if}"><img src="art/icons/bullet_green.png">{t}Enabled{/t}</button> <button id="page_flag_display_{$flag.id}_No" class="page_flag_display_{$flag.id}" value="No" onclick="change_flag_display(this,{$flag.id})" style="{if $flag.display=='Yes'  or $flag.default}display:none{/if}"><img src="art/icons/bullet_red.png">{t}Disabled{/t}</button> {if $flag.default}{t}Default{/t}{/if} 
							</div>
							</td>
							<td style="" id="page_flag_active_{$flag.id}_msg" class="edit_td_alert"> </td>
						</tr>
					</table>
					</td>
				...
				<td></td>
					<td colspan="2"> 
					<div class="buttons left" style="margin-left:190px"> 
						<button id="reset_edit_page_flags" class="negative disabled">{t}Cancel{/t}</button> 
												<button id="save_edit_page_flags" class="positive disabled">{t}Save{/t}</button> 
...
<div id="dialog_new_page" style="padding:20px 20px 10px 20px ">
	...
	<div id="new_page_buttons" class="buttons small">
		<button id="show_department_list">{t}Department{/t}</button> <button id="show_family_list">{t}Family{/t}</button> <button id="show_product_list">{t}Product{/t}</button> 
		<button style="display:none" id="show_family_category_list">{t}Family Category{/t}</button> <button  style="display:none"  id="show_product_category_list">{t}Product Category{/t}</button> <button onclick="new_page('site',0)">{t}Custome Information{/t}</button> <button class="negative" id="close_dialog_new_page">{t}Cancel{/t}</button> 
	</div>
</div>
