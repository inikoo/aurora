		...
		<div id="top_page_menu" class="top_page_menu" style="margin-top:10px">
			...
			<div class="buttons" style="float:left">
				<h1 style="padding-bottom:0px">
					{$staff->get('Staff Name')}, <span style="color:SteelBlue"> {$staff->get('Staff Alias')} <i>({$staff->get('Staff ID')})</i></span> 
				</h1>
			</div>
	...
	<div id="block_details" class="data_table" style="{if $block_view!='details'}display:none;{/if}margin:20px 0 40px 0;padding:0 20px">
		<div style="width:350px">
			<table class="show_info_product" border="0" style="padding:0">
				{if $staff->get('Staff Name')}
				<tr>
					<td valign="top" class="aleft">{t}Name{/t}:</td>
					<td valign="top" colspan="2" class="aleft">{$staff->get('Staff Name')}</td>
				</tr>
				{/if} {if $staff->get('Staff Alias')}
				<tr>
					<td valign="top" class="aleft">{t}Code{/t}:</td>
					<td valign="top" colspan="2" class="aleft">{$staff->get('Staff Alias')}</td>
				</tr>
				{/if} {if $staff->get('Staff ID')}
				<tr>
					<td valign="top" class="aleft">{t}Staff ID{/t}:</td>
					<td colspan="2" class="aleft">{$staff->get('Staff ID')}</td>
				</tr>
				{/if} {if $staff->get('Staff Type')}
				<tr>
					<td valign="top" class="aleft">{t}Staff Type{/t}:</td>
					<td colspan="2" class="aleft">{$staff->get('Type')}</td>
				</tr>
				{/if} {if $staff->get('Staff Valid From')}
				<tr>
					<td valign="top" class="aleft">{t}Employed since{/t}:</td>
					<td colspan="2" class="aleft">{$staff->get('Staff Valid From')}</td>
				</tr>
				{/if} {if $staff->get('Staff Currently Working')!='Yes'}
				<tr>
					<td valign="top" class="aleft">{t}Employed Until{/t}:</td>
					<td colspan="2" class="aleft">{$staff->get('Staff Valid To')}</td>
				</tr>
				{/if} 
			</table>
		</div>
