...
<div id="salary" class="salary hide">
    <table border="0">
        ...
        <tr id="monthly_pay_day" class=" pay_day monthly {if !($frequency=='monthly' or $frequency=='')}hide{else}{/if}">
            ...
            <td> {t}Pay day{/t} </td>
            <td colspan="3">
                <input id="pay_day_day_of_the_month" class="salary_input_field potentially_valid day_of_the_month  "
                       max="31" maxlength="2" value="{$payday}"> {t}th of the month{/t} </td>
        </tr>
        <tr id="weekly_pay_day"
            class="pay_day weekly {if isset($frequency) and $frequency!='weekly'}hide{else}hide{/if}">
            <td class="label"> {t}Pay day{/t} </td>
            <td id="salary_pay_day_of_week" colspan="3" class="inline_options salary_pay_day_of_week small">
                <span value="1"
                      class="{if isset($salary.data.payday) and $frequency=='weekly' and $salary.data.payday==1 }selected{else}selected{/if}">{t}Mon{/t}</span>
                <span value="2"
                      class="{if isset($salary.data.payday) and $frequency=='weekly' and $salary.data.payday==2 }selected{/if}">{t}Tue{/t}</span>
                <span value="3"
                      class="{if isset($salary.data.payday) and $frequency=='weekly' and $salary.data.payday==3 }selected{/if}">{t}Wed{/t}</span>
                <span value="4"
                      class="{if isset($salary.data.payday) and $frequency=='weekly' and $salary.data.payday==4 }selected{/if}">{t}Thu{/t}</span>
                <span value="5"
                      class="{if isset($salary.data.payday) and $frequency=='weekly' and $salary.data.payday==5 }selected{/if}">{t}Fri{/t}</span>
                <span value="6"
                      class="{if isset($salary.data.payday) and $frequency=='weekly' and $salary.data.payday==6 }selected{/if}">{t}Sat{/t}</span>
                <span value="7"
                      class="{if isset($salary.data.payday) and $frequency=='weekly' and $salary.data.payday==7 }selected{/if}">{t}Sun{/t}</span>
            </td>
        <tr>
            <td> {t}Type{/t} </td>
            ...
            <td colspan="3">
                <table border=0>
                    <tr id="compressed_salary_amount_components" class="{if $amount==''}hide{/if}">
                        <td colspan="3">
                            <i id="expand_rate_components"
                               class="fa fa-expand fa-fw  {if isset($salary.type) and $salary.type!=prorata_hour}hide{else}hide{/if} "
                            "></i>
                            <input id="salary_amount" class="salary_input_field amount potentially_valid"
                                   value="{$amount}"></td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td>
                            <input id="salary_amount_weekdays" class="salary_input_field amount potentially_valid"
                                   value="{$amount_weekdays}"></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td class="small_label">{t}Saturday{/t}</td>
                        <td>
                            <input id="salary_amount_saturday" class="salary_input_field amount potentially_valid"
                                   value="{$amount_saturday}"></td>
                    </tr>
                    <tr>
                        <td></td>
                        <td class="small_label">{t}Sunday{/t}</td>
                        <td>
                            <input id="salary_amount_sunday" class="salary_input_field amount potentially_valid"
                                   value="{$amount_sunday}"></td>
                    <tr>
                </table>
            </td>
        </tr>
