                ...
                {foreach from=$field_group.fields item=field name=fields}
                    ...
                    {if $edit=='address' or $edit=='filled_address' }
                        <tbody id="{$field.id}_address_fields" class="address_fields"  field="{$field.id}" has_been_changed="0"  _required="{$required}" >
                            ...
                            <input  id="{$field.id}_country" class="input_field" value="" type="hidden" field_name="Address Country 2 Alpha Code" >
            ...
            <input type="hidden"  id="{$field.id}_filled_address_value"  value='{$field.value}'   >
                ...
                {/if}
                    ...
                    {elseif $edit=='timezone'  }
                        {include file="timezone.edit.tpl" field=$field  timezones=$field.timezones mode='new'}
                ...
                $('#' + field+'_Next_Available').val('No')
                   ...
                   &nbsp;
                    ...
                    <div id="{$field.id}_options" class="dropcontainer option_multiple_choices  hide" style="position: relative;top:0px">
                        <ul>
                            {foreach from=$field.options item=option key=value}
                            <li id="{$field.id}_option_{$value}" label="{$option.label}" value="{$value}" is_selected="{$option.selected}" onclick="select_option_multiple_choices('{$field.id}','{$value}','{$option.label}' )"><i class="far fa-fw checkbox {if $option.selected}fa-check-square{else}fa-square{/if}"></i> {$option.label} <i class="fa fa-circle fw current_mark {if $option.selected}current{/if}"></i></li>
                            {/foreach}
                        </ul>
                    </div>
