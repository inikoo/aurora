...
<input type="hidden" id="scope_subject" value="{$scope_subject}"  />
<input type="hidden" id="post_create_action" value="{$post_create_action}"  />
...
<div id="bd">
	...
	<div class="branch">
		...
		{else if $scope_subject=='Customer List'}
		<span><a href="index.php"><img style="vertical-align:0px;margin-right:1px" src="art/icons/home.gif" alt="home" /></a>&rarr; {if $user->get_number_stores()>1}<a href="customers_server.php">{t}Customers{/t}</a> &rarr; {/if}<a href="customers.php?store={$store->id}">{$store->get('Store Code')} {t}Customers{/t}</a> &rarr; <a href="customers_lists.php?store={$store->id}">{t}Lists{/t}</a> &rarr; {$list->get('List Name')} ({t}New Offer{/t})</span> 
	...
	<div class="top_page_menu">
		<div class="buttons" style="float:left">
			...
			{else if $scope_subject=='Campaign'} <span class="main_title">{t}New Offer{/t} @ {t}Campaign{/t} <span class="id">{$campaign->get('Deal Campaign Name')}</span></span> 
			...
			{else if $scope_subject=='Customer List'} <span class="main_title">{t}New Offer{/t} @ {t}Customer List{/t} <span class="id">{$list->get('List Name')}</span></span> 
	...
	<table id="new_deal" class="edit" border="0">
			...
			<tr  class="space10" style="{if $scope_subject!='Campaign'}display:none{/if}">
				...
				<div class="styled-select">
					<select id="trigger_select" onchange="trigger_changed(this.value)">
						...
						<option value="Customer List">{t}Customer Static List{/t}</option>
					</select>
				</div>
				</td>
		...
		<tr class="space10">
			...
			<td id="deal_terms_msg" class="messages edit_td_alert"> </td>
		</tr>
		...
		<tr class="space10">
			...
			<td class="input"> 
			<div>
				<input id="deal_name" value="{$name}" ovalue="{$name}"> 
		...
		<tr id="deal_description_tr" style="display:none">
			<td class="label">{t}Description{/t}:</td>
			<td class="input"> 
			<div>
...
<div id="dialog_deals_list" style="position:absolute;left:-1000;top:0;width:600px">
	<div class="splinter_cell" style="padding:10px 15px 10px 0;border:none;width:570px">
		<div class="data_table">
