...
<span id="show_countdown" class="{if $content._launch_date!=''}button{/if}  " onClick="change_webpage_element_visibility(this)">
        <i class="far fa-clock discreet" style="margin-left:20px" aria-hidden="true"></i> <span class="{if $content._launch_date==''}strikethrough{/if}">{t}Countdown{/t}</span> <i
...
<span id="show_email_form" class="button " onClick="change_webpage_element_visibility(this)">
        <i class="far envelope discreet" style="margin-left:20px" aria-hidden="true"></i> {t}Email{/t} <i class="fa fa-check {if $content.show_email_form==1}success{else}very_discreet{/if}" aria-hidden="true"></i>
    </span>
