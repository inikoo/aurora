...
<div id="block_{$key}" data-block_key="{$key}" block="{$data.type}" class="    _block {if !$data.show}hide{/if}" top_margin="{$top_margin}" bottom_margin="{$bottom_margin}"
    ...
    <h1 id="overwrite_title" class="products_title {if !$block.show_title}hide{/if}" style="margin-left:20px;" contenteditable="true">{if !empty($data.overwrite_title)}{$data.overwrite_title}{elseif !empty($lables._see_also)}{$lables._see_also}{else}{t}See also{/t}{/if}</h1>
