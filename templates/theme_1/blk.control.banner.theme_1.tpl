...
<div id="edit_mode_{$key}" class=" edit_mode " type="{$block.type}" key="{$key}" style="height: 22px;line-height: 22px">
  ...
  {*      <div id="banner_height_edit_block_{$key}" name="banner_height_edit_block" class="hide edit_block" style="position:absolute;padding:10px;background-color: #FFF;border:1px solid #ccc">
        ...
        <span class="banner_slide">
            <ul id="columns1" class="sortable_banner_blocks columns " style="width:1100px;" >
                <li id="banner_slide_{$key}" class="column  unselectable  {if !$block.show}very_discreet{/if}" key="{$key}" block="{$block.type}">slide_1.jpg
                    <i class="far fa-trash-alt padding_left_5" aria-hidden="true"></i><i class="fa handle2 fa-arrows ui-sortable-handle" aria-hidden="true"></i>
                </li>
                <li id="banner_slide_{$key}" class="column  unselectable  {if !$block.show}very_discreet{/if}" key="{$key}" block="{$block.type}">slide_2.jpg
                    <i class="far fa-trash-alt padding_left_5" aria-hidden="true"></i><i class="fa handle2 fa-arrows ui-sortable-handle" aria-hidden="true"></i>
                </li>
                <li id="banner_slide_{$key}" class="column  unselectable  {if !$block.show}very_discreet{/if}" key="{$key}" block="{$block.type}">slide_3.jpg
                    <i class="far fa-trash-alt padding_left_5" aria-hidden="true"></i><i class="fa handle2 fa-arrows ui-sortable-handle" aria-hidden="true"></i>
                </li>
                <li class="column unselectable button"  style="min-width:auto;padding:4px 16px 4px 16px;" >
...
{*<span id="block_label_4" class="column  unselectable  " key="4" block="images"><i class="fa button  fa-eye block_show" aria-hidden="true"></i></span>*}
...
<script>
    $('.sortable_banner_blocks').sortable({
        handle:'.handle2',
