...
<!--
 ...
 Created: 16 June 2017 at 16:35:07 GMT+8, Cyberjaya, Malaysia
...
<div id="webpage_section_{$block_name}"  section="{$block_name}"   class="webpage_section {if $show_block!=1}hide{/if}"   >
    <div class="container">
        ...
        <div id="grid-filters-container" class="cbp-l-filters-alignRight">
            ...
            {foreach from=$content.products.filters  item=filter key=filters_key }
        ...
        <div id="grid-container" class="cbp">
            ...
            {foreach from=$content.products.items  item=item key=key }
                <div id="cbp_item_{$item.product_id}" style="border:1px solid #eee;padding-left:10px;padding-right:10px;padding-bottom: 50px" class="cbp-item {$item.tags}" data-product_id="{$item.product_id}" data-webpage_key="{$item.webpage_key}" data-key="{$key}" data-guest="{$item.guest}">
                    <a href="" class="cbp-caption ">


                        <div class="cbp-caption-defaultWrap"    >
                            <img src="{if $item.image_375x250==''}/art/image_375x250.png{else}{$item.image_375x250}{/if}" data-src="{$item.image_375x250}" width="375" height="275"  />
                        </div>
                    ...
                    <div style="color:#777">
                            ...
                            <span class="fa-stack fa-lg" style="float:right;font-size: 15px;position: relative;top:-180px;left:-10px">
                    ...
                    <i class="fa fa-heart" aria-hidden="true" style="color:red;float:right;position: relative;top:15px;left:10px"></i>
        ...
        <tr>
            <td>
                ...
                <input id="add_item_dropdown_select_label" field="add_item" style="width:200px" scope="category_webpages" parent="store"
                       ...
                       parent_key="{$store->id}" class=" dropdown_select" value="" has_been_valid="0" placeholder="{t}Product code{/t}" action="add_category_to_webpage"
...
<div id="edit_item_container" class="hide" style="clear:both;width:100%;min-height: 200px;background: #fff;padding:0px 20px 20px 20px; position:fixed;top:0px">
    ...
    <div class="container" style="border-top: 1px solid #ccc;padding-top:20px">
        ...
        <ul id="sortable">
            {foreach from=$content.products.items  item=item key=key }
                <li class="cbp_thumbnail_item like_button" id="cbp_thumbnail_item_{$item.product_id}" product_id="{$item.product_id}">
...
<script>
    ...
    $('#edit_item_container').on('click', '.cbp_thumbnail_item', function (e) {
        var item_container = $('#cbp_item_' + $(this).attr('product_id'))
    ...
    $('#edit_item_container').on('click', '.image_container img', function () {
        ...
        if ($(this).closest('.image_data_container').hasClass('image_data_container_ok')) {
            ...
            $('#cbp_item_' + $('#catalogue_code').data('product_id')).find('.cbp-caption-defaultWrap img').attr('src', src).data('src', src)
            $('#cbp_thumbnail_item_' + $('#catalogue_code').data('product_id')).find('img').attr('src', src)
    ...
    function render_image_dialog(item_container) {
        ...
        var request = '/ar_products.php?tipo=product_data&key=' + item_container.data('product_id')

        $.getJSON(request, function (r) {


            $('#catalogue_code').html(r.data.code).data('product_id', r.data.product_id)
            $('#catalogue_name').html(r.data.label)
            $('#catalogue_price').html(r.data.price)
    ...
    $('#edit_item_container').on('change', '#upload_item_image', function () {
        ...
        ajaxData.append("parent_key", $('#catalogue_code').data('product_id'))
    ...
    $('#next_item').click(function (event) {
        ...
        var next_item = $('#cbp_item_' + $('#catalogue_code').data('product_id')).next('.cbp-item')
        if (next_item.length) {
    ...
    $('#prev_item').click(function (event) {
        ...
        var prev_item = $('#cbp_item_' + $('#catalogue_code').data('product_id')).prev('.cbp-item')
        if (prev_item.length) {
    ...
    $('#disassociate_category').click(function (event) {
        ...
        $('#cbp_thumbnail_item_' + $('#catalogue_code').data('product_id')).remove()

        $("#grid-container").cubeportfolio('remove', $('#cbp_item_' + $('#catalogue_code').data('product_id')));
        $('#cbp_item_' + $('#catalogue_code').data('product_id')).remove()
    ...
    function select_dropdown_item(element) {
        ...
        $.getJSON(request, function (r) {
            ...
            if($('#cbp_thumbnail_item_'+r.data.product_id).length){
                swal({
            ...
            var anchor_item_key = $('#catalogue_code').data('product_id');
            ...
            var new_item_html = '\
                <div id="cbp_item_' + r.data.product_id + '" class="cbp-item" data-product_id="' + r.data.product_id + '" data-webpage_key="' + r.data.webpage_key + '" data-guest="true"  > \
                <a href="" class="cbp-caption ">  \
                <div class="cbp-caption-defaultWrap"> \
...
console.log(anchor_item_key)
            ...
            var new_item_thumbnail_html = '<li class="cbp_thumbnail_item like_button" id="cbp_thumbnail_item_' + r.data.product_id + '" product_id="'+r.data.product_id+'">\
    ...
    $('#grid-container').on('input', '.item_code', function () {
        $('#cbp_thumbnail_item_'+$(this).closest('.cbp-item').data('product_id')).find('div').html($(this).html())
    ...
    $('#edit_item_container').on('input', '#edit_title', function () {
        ...
        $('#cbp_item_'+$('#catalogue_code').data('product_id')).find('.cbp-l-caption-title').html($(this).html())
        $('#cbp_item_'+$('#catalogue_code').data('product_id')).find('.item_code').html($(this).html())

        $('#cbp_thumbnail_item_'+$('#catalogue_code').data('product_id')).find('div').html($(this).html())
    ...
    $('#edit_item_container').on('input', '#edit_desc', function () {
        ...
        $('#cbp_item_'+$('#catalogue_code').data('product_id')).find('.cbp-l-caption-desc').html($(this).html())
        $('#cbp_item_'+$('#catalogue_code').data('product_id')).find('.item_desc').html($(this).html())
