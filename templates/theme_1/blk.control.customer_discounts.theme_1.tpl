...
<!--
 ...
 Created: 11 Jun 2021 at 17:43 GMT+8, Kaula Lumpur, Malaysia
...
<div id="edit_mode_{$key}" class=" edit_mode " type="{$block.type}" key="{$key}" style="height: 22px;line-height: 22px">
    <div id="edit_mode_main_{$key}" class="main" style="float:left;margin-right:20px;min-width: 200px;">
        ...
        <span class="with_items button unselectable  " onClick="change_webpage_customer_discounts('with_items')"><i class="far fa-box-full  fa-fw"></i> {t}With discounts{/t}</span>
        <span class="no_items button unselectable hide " onClick="change_webpage_customer_discounts('no_items')"><i class="far fa-box-open  fa-fw"></i> {t}No discounts{/t}</span>
    </div>
...
<script>
    ...
    function change_webpage_customer_discounts(view) {
        ...
        $('#preview')[0].contentWindow.change_webpage_customer_discounts({$key}, view)
