    ...
    <div class="site_wrapper">
        ...
        <div id="bottom_header" class="">
            ...
            {foreach from=$header_data.menu.columns item=column key=key}
                <a id="menu_{$key}" class="menu _column {if $column.type=='nothing'}only_link{else}dropdown{/if}"  data-column_type="{$column.type}"  href="" data-key="{$key}"><i class="{$column.icon} menu_icon" icon="{$column.icon}" ></i> <span class="menu_label" contenteditable="true">{$column.label|strip_tags}</span> <i class="down_cadet fal fa-fw fa-angle-down"></i></a>
            {/foreach}
            </div>
