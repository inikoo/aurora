...
<tr class="title">
<td colspan=2>{t}Customer{/t} <i  onClick="change_view('customers/{$customer->get('Store Key')}/{$customer->id}')" class="fa fa-link link"></i></td>
</tr>

<tr>
<td class="label">{$customer->get_field_label('Customer Name')|capitalize}</td><td>{$customer->get('Name')}</td>
</tr>
<tr>
<td class="label">{$customer->get_field_label('Customer Main Plain Email')|capitalize}</td><td>{$customer->get('Main Plain Email')}</td>
</tr>
