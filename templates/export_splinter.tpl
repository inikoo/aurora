	...
	<table class="edit" style="width:500px" border="0">
		<tbody id="dialog_export_form_{$id}">
			<tr>
				...
				<div class="buttons left">
					<button id="export_xls_{$id}" style="width:70px"><img src="art/icons/page_excel.png" alt=""> Excel</button> <button id="export_csv_{$id}" style="width:70px"><img src="art/icons/page_white_text.png" alt=""> CSV</button> <span id="export_no_field_msg_{$id}" style="display:none;" class="error">There isn't any field to export</span> 
				</div>
				</td>
			</tr>
			...
			<tr class="top">
				...
				<div class="buttons small">
					<button style="display:none" onclick="show_new_export_map('{$id}')" id="dialog_export_new_map_{$id}">{t}New map{/t}</button> <button onclick="show_export_map_library('{$id}')" id="dialog_export_map_library_{$id}">{t}Maps{/t}</button> <button onclick="show_export_map_fields('{$id}')" id="dialog_export_map_fields_{$id}">{t}Fields{/t}</button> 
				</div>
				</td>
			</tr>
		</tbody>
			...
			<tr style="border-bottom:1px solid #ccc">
				...
				<table style="width:100%" border="0">
					<tr id="field_map_buttons_{$id}" style="display:none">
						<td> 
						<div class="buttons small">
							<button onclick="reset_export_fields('{$id}')" id="export_fields_reset_{$id}" class="disabled">{t}Reset{/t}</button> <button>{t}Save As{/t}</button> <button style="{if $is_map_default}display:none{/if}">{t}Save{/t}</button> 
						</div>
						</td>
					</tr>
		...
		<tbody id="dialog_export_result_{$id}" style="display:none">
			<tr id="export_result_wait_{$id}">
				<td> <img src="art/loading.gif"> <span>{t}Processing Request{/t}</span> <span style="margin-left:20px" id="dialog_export_progress_{$id}"></span> </td>
			</tr>
			<tr id="export_result_download_{$id}" style="display:none">
				<td> 
				<div class="buttons left">
					<a id="export_result_download_link_{$id}" href="">{t}Download{/t}</a> 
				</div>
								<div style="float:left;margin-left:30px;position:relative;top:6px">(<span id="export_result_download_info_{$id}"></span>)</div>
