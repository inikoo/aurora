        ...
        <div class="data_container">
            ...
            <div class="data_field {if $agent->get('Agent Main Contact Name')=='' or $agent->get('Main Contact Name')==$agent->get('Name')}hide{/if}">
                <i title="{t}Contact name{/t}" class="fa fa-user"></i> <span
                        class="Agent_Main_Contact_Name">{$agent->get('Agent Main Contact Name')}</span>
            </div>
        ...
        <div class="data_container">
            <div id="Agent_Main_Plain_Email_display"
                ...
                <i class="fa fa-fw fa-at"></i> <span
                        id="Agent_Other_Email_mailto">{if $agent->get('Agent Main Plain Email')}{mailto address=$agent->get('Main Plain Email')}{/if}</span>
            </div>
            {foreach $agent->get_other_emails_data() key=other_email_key item=other_email}
                <div id="Agent_Other_Email_{$other_email_key}_display" class="data_field ">
                    <i class="fa fa-fw fa-at discreet"></i> <span
                            id="Agent_Other_Email_{$other_email_key}_mailto">{mailto address=$other_email.email}</span>
                </div>
            {/foreach}
            ...
            <span id="display_telephones"></span> {if $agent->get('Agent Preferred Contact Number')=='Mobile'}
                <div id="Agent_Main_Plain_Mobile_display"
                     class="data_field {if !$agent->get('Agent Main Plain Mobile')}hide{/if}">
                    <i class="fa fa-fw fa-mobile"></i> <span
                            class="Agent_Main_Plain_Mobile">{$agent->get('Main Plain Mobile')}</span>
                </div>
                <div id="Agent_Main_Plain_Telephone_display"
                     class="data_field {if !$agent->get('Agent Main Plain Telephone')}hide{/if}">
                    <i class="fa fa-fw fa-phone"></i> <span
                            class="Agent_Main_Plain_Telephone">{$agent->get('Main Plain Telephone')}</span>
                </div>
            {else}
                <div id="Agent_Main_Plain_Telephone_display"
                     class="data_field {if !$agent->get('Agent Main Plain Telephone')}hide{/if}">
            ...
            {/if}
            <div id="Agent_Main_Plain_FAX_display"
                 class="data_field {if !$agent->get('Agent Main Plain FAX')}hide{/if}">
        ...
        <div id="overviews">
            <table border="0" class="overview">
                ...
                <tr>
                    ...
                    <td class="Delivery_Time aright">{$agent->get('Delivery Time')}</td>
                </tr>
            </table>
            {if $agent->get('Agent Orders')>0}
                <table class="overview">
                    {if $agent->get('Agent Type by Activity')=='Lost'}
                        <tr>
                            <td><span style="color:white;background:black;padding:1px 10px">{t}Lost Agent{/t}</span>
                            </td>
                        </tr>
                    {/if} {if $agent->get('Agent Type by Activity')=='Losing'}
                        <tr>
                            <td>
                                <span style="color:white;background:black;padding:1px 10px">{t}Warning!, loosing agent{/t}</span>
                            </td>
                        </tr>
                    {/if}
                    <tr>
                        <td class="text"> {if $agent->get('Agent Orders')==1}
                            ...
                            {elseif $agent->get('Agent Orders')>1 } {$agent->get('Name')} {if $agent->get('Agent Type by Activity')=='Lost'}{t}placed{/t}{else}{t}has placed{/t}{/if}
                            <b>{$agent->get('Agent Orders')}</b> {if $agent->get('Agent Type by Activity')=='Lost'}{t}orders{/t}{else}{t}orders so far{/t}{/if}
                            , {t}which amounts to a total of{/t} <b>{$agent->get('Net Balance')}</b> {t}plus tax{/t}
                            ({t}an average of{/t} {$agent->get('Total Net Per Order')} {t}per order{/t}
                            ). {if $agent->get('Agent Orders Invoiced')}
                            </p>
                            <p>
                                {if $agent->get('Agent Type by Activity')=='Lost'}{t}This agent used to place an order every{/t}{else}{t}This agent usually places an order every{/t}{/if} {$agent->get('Order Interval')}
                                .{/if} {else} Agent has not place any order yet. {/if}
                            </p>
                        </td>
                    </tr>
                </table>
