...
<div class="order" style="display: flex;" data-object="{$object_data}">
	...
	<div class="block " style="align-items: stretch;flex: 1;">
		<div class="state" style="height:30px;margin-bottom:10px;position:relative;top:-5px">
			...
			<span style="float:left;padding-left:10px;padding-top:5px" class="Purchase_Order_State"> {$order->get('Agent State')} </span> 
			...
			<div id="forward_operations">
				...
				<div id="create_spo_operations" class="order_operation {if $order->get('Purchase Order State')!='Submitted'}hide{/if}">
					<div id="create_spo_operation" class="square_button right {if $order->get('Purchase Order Number Items')==0}hide{/if} " title="{t}Create supplier's orders{/t}">
						<i class="fa fa-clipboard   " aria-hidden="true" onclick="toggle_order_operation_dialog('create_spo')"></i> 
						<table id="create_spo_dialog" border="0" class="order_operation_dialog hide">
							...
							<tr class="changed buttons">
								<td><i class="fa fa-sign-out fa-flip-horizontal button" aria-hidden="true" onclick="close_dialog('create_spo')"></i></td>
								<td class="aright"><span data-data='{  "field": "Purchase Order Agent State","value": "Submitted","dialog_name":"create_spo"}' id="create_spo_save_buttons" class="valid save button" onclick="save_order_operation(this)"><span class="label">{t}Save{/t}</span> <i class="fa fa-cloud fa-fw  " aria-hidden="true"></i></span> </td>
							</tr>
						</table>
					</div>
				</div>
		...
		<div id="crete_delivery" class="delivery_node {if ({$order->get('State Index')|intval} < 35 or ($order->get('Purchase Order Ordered Number Items')-$order->get('Purchase Order Number Supplier Delivery Items'))==0)  }hide{/if}" style="height:30px;clear:both;border-top:1px solid #ccc;border-bottom:1px solid #ccc">
			<div id="back_operations">
			</div>
	...
	<div class="block " style="align-items: stretch;flex: 1 ">
		<table border="0" class="info_block acenter">
			<tr>
				<td> 
				<span style=""><i class="fa fa-ship fa-fw discreet" aria-hidden="true"></i> <span class="Purchase_Order_Number_Suppliers">{$order->get('Number Suppliers')}</span></span> 
				<span style=""><i class="fa fa-stop fa-fw discreet padding_left_20" aria-hidden="true"></i> <span class="Purchase_Order_Number_Items">{$order->get('Number Items')}</span></span> 
				<span class="{if $order->get('State Index')<60}super_discreet{/if} padding_left_20" ><i class="fa fa-arrow-circle-down  fa-fw discreet" aria-hidden="true"></i> <span class="Purchase_Order_Number_Supplier_Delivery_Items">{$order->get('Number Supplier Delivery Items')}</span></span> 
			</tr>
			<tr>
