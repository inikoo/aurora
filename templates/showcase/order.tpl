...
<div class="order" style="display: flex;" data-object="{$object_data}">
	<div class="block" style=" align-items: stretch;flex: 1">
		<div class="data_container"  style="padding:5px 10px">
			<div class="data_field">
				<i class="fa fa-user fa-fw" aria-hidden="true" title="{t}Customer{/t}"></i> <span onclick="change_view('customers/{$order->get('Order Store Key')}/{$order->get('Order Customer Key')}')" class="button Order_Customer_Name">{$order->get('Order Customer Name')}</span> <span onclick="change_view('customers/{$order->get('Order Store Key')}/{$order->get('Order Customer Key')}')" class="link Order_Customer_Key">{$order->get('Order Customer Key')}</span> 
			</div>
			<div class="data_field  " style="padding:10px 0px 20px 0px;" >
				 ...
				 <div style="margin-bottom:10px"> <i class="fa fa-truck button" aria-hidden="true" "></i>{t}Address{/t}</div>
				 ...
				 <div style="margin-bottom:10px"> <i class="fa fa-dollar button" aria-hidden="true" "></i>{t}Address{/t}</div>
	...
	<div class="block " style="align-items: stretch;flex: 1;">
		<div class="state" style="height:30px;margin-bottom:10px;position:relative;top:-5px">
			<div id="back_operations">
				<div id="delete_operations" class="order_operation {if $order->get('Order State')!='InProcess'}hide{/if}">
					<div class="square_button left" xstyle="padding:0;margin:0;position:relative;top:-5px" title="{t}delete{/t}">
						...
						<table id="delete_dialog" border="0" class="order_operation_dialog hide">
							...
							<tr class="changed buttons">
								...
								<td class="aright"><span data-data='{ "object": "PurchaseOrder", "key":"{$order->id}"}' id="delete_save_buttons" class="error save button" onclick="delete_object(this)"><span class="label">{t}Delete{/t}</span> <i class="fa fa-trash fa-fw  " aria-hidden="true"></i></span> </td>

							</tr>
						</table>
					</div>
				</div>
				<div id="cancel_operations" class="order_operation {if $order->get('Order State')=='InProcess'}hide{/if}">
				...
				<div id="undo_submit_operations" class="order_operation {if $order->get('Order State')!='Submitted'}hide{/if}">
					<div class="square_button left"  title="{t}Undo submit{/t}">
						...
						<table id="undo_submit_dialog" border="0" class="order_operation_dialog hide">
							...
							<tr class="changed buttons">
								...
								<td class="aright"><span  data-data='{  "field": "Order State","value": "InProcess","dialog_name":"undo_submit"}' id="undo_submit_save_buttons" class="valid save button" onclick="save_order_operation(this)"><span class="label">{t}Save{/t}</span> <i class="fa fa-cloud fa-fw  " aria-hidden="true"></i></span> </td>
							</tr>
						</table>
					</div>
				</div>
			...
			<div id="forward_operations">
				<div id="submit_operations" class="order_operation {if $order->get('Order State')!='InProcess'}hide{/if}">
					<div id="submit_operation"  class="square_button right {if $order->get('Order Number Items')==0}hide{/if} "  title="{t}Submit{/t}">
						...
						<table id="submit_dialog" border="0" class="order_operation_dialog hide">
							...
							<tr class="changed buttons">
								...
								<td class="aright"><span data-data='{  "field": "Order State","value": "Submitted","dialog_name":"submit"}' id="submit_save_buttons" class="valid save button" onclick="save_order_operation(this)"><span class="label">{t}Save{/t}</span> <i class="fa fa-cloud fa-fw  " aria-hidden="true"></i></span> </td>
							</tr>
						</table>
					</div>
				</div>
	...
	<div class="block " style="align-items: stretch;flex: 1 ">
	    	...
	    	<div id="create_delivery" class="delivery_node {if {$order->get('State Index')|intval}<30 or ($order->get('Order Ordered Number Items')-$order->get('Order Number Supplier Delivery Items'))==0  }hide{/if}" style="height:30px;clear:both;border-top:1px solid #ccc;border-bottom:1px solid #ccc">
		...
		<div class="invoices" style="margin-bottom:10px">
	    	<div id="create_invoice" class="delivery_node {if {$order->get('State Index')|intval}<30 or ($order->get('Order Ordered Number Items')-$order->get('Order Number Supplier Delivery Items'))==0  }hide{/if}" style="height:30px;clear:both;border-top:1px solid #ccc;border-bottom:1px solid #ccc">
			...
			<span style="float:left;padding-left:10px;padding-top:5px" > <span class="button" onClick="change_view('{$order->get('Order Parent')|lower}/{$order->get('Order Parent Key')}/delivery/{$invoice->id}')">{$payment->get('Payment Account Code')}</span> </span> 
	...
	<div class="block " style="align-items: stretch;flex: 1 ">
		<table border="0" class="totals" style="position:relative;top:-5px">
			...
			<tr class="{if $account->get('Account Currency')==$order->get('Order Currency Code')}hide{/if}">
				<td colspan="2" class="Order_Total_Amount_Account_Currency aright ">{$order->get('Total Amount Account Currency')}</td>
			</tr>
