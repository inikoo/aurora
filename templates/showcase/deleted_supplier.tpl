        ...
        <div class="data_container">
            ...
            <div class="data_field {if $supplier->get('Supplier Main Contact Name')==''}hide{/if}">
                <i title="{t}Contact name{/t}" class="fa fa-user"></i> <span
                        class="Supplier_Main_Contact_Name">{$supplier->get('Supplier Main Contact Name')}</span>
            </div>
            ...
            {if $supplier->get('Supplier Orders')>0}
                <table class="overview">
                    {if $supplier->get('Supplier Type by Activity')=='Lost'}
                        <tr>
                            <td><span style="color:white;background:black;padding:1px 10px">{t}Lost Supplier{/t}</span>
                            </td>
                        </tr>
                    {/if} {if $supplier->get('Supplier Type by Activity')=='Losing'}
                        <tr>
                            <td>
                                <span style="color:white;background:black;padding:1px 10px">{t}Warning!, loosing supplier{/t}</span>
                            </td>
                        </tr>
                    {/if}
                    <tr>
                        <td class="text"> {if $supplier->get('Supplier Orders')==1}
                            ...
                            {elseif $supplier->get('Supplier Orders')>1 } {$supplier->get('Name')} {if $supplier->get('Supplier Type by Activity')=='Lost'}{t}placed{/t}{else}{t}has placed{/t}{/if}
                            <b>{$supplier->get('Supplier Orders')}</b> {if $supplier->get('Supplier Type by Activity')=='Lost'}{t}orders{/t}{else}{t}orders so far{/t}{/if}
                            , {t}which amounts to a total of{/t} <b>{$supplier->get('Net Balance')}</b> {t}plus tax{/t}
                            ({t}an average of{/t} {$supplier->get('Total Net Per Order')} {t}per order{/t}
                            ). {if $supplier->get('Supplier Orders Invoiced')}
                            </p>
                            <p>
                                {if $supplier->get('Supplier Type by Activity')=='Lost'}{t}This supplier used to place an order every{/t}{else}{t}This supplier usually places an order every{/t}{/if} {$supplier->get('Order Interval')}
                                .{/if} {else} Supplier has not place any order yet. {/if}
                            </p>
                        </td>
                    </tr>
                </table>
