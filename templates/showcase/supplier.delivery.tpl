...
<div class="timeline_horizontal">
	...
	<ul class="timeline" id="timeline">
		...
		<div class="timestamp">
			<span class="Purchase_Order_Submitted_Date">&nbsp;{$delivery->get('PO Submitted Date')}</span> <span class="start_date Purchase_Order_Creation_Date">{$delivery->get('PO Creation Date')} </span> 
		</div>
		<div class="dot">
		</div>
		</li>
...
<div class="order" style="display: flex;" data-object="{$object_data}">
	<div class="block" style=" align-items: stretch;flex: 1">
		<div class="data_container" style="padding:5px 10px">
			...
			<div class="data_field">
				<i class="fa fa-share fa-fw" aria-hidden="true" title="Incoterm"></i> <span class="Supplier_Delivery_Incoterm">{$delivery->get('Supplier Delivery Incoterm')}</span> 
			</div>
			<div class="data_field">
				<i class="fa fa-arrow-circle-right fa-fw" aria-hidden="true" title="{t}Port of export{/t}"></i> <span class="Supplier_Delivery_Port_of_Export">{$delivery->get('Port of Export')}</span> 
			</div>
			<div class="data_field">
				<i class="fa fa-arrow-circle-left fa-fw" aria-hidden="true" title="{t}Port of import{/t}"></i> <span class="Supplier_Delivery_Port_of_Import">{$delivery->get('Port of Import')}</span> 
			</div>
		</div>
	...
	<div class="block " style="align-items: stretch;flex: 1;">
		<div class="state" style="height:30px;margin-bottom:10px;position:relative;top:-5px">
			<div id="back_operations">
				...
				<div id="undo_dispatched_operations" class="order_operation {if $delivery->get('Supplier Delivery State')!='Dispatched'}hide{/if}">
					...
					<div class="square_button right" title="{t}received{/t}">
						...
						<table id="received_dialog" border="0" class="order_operation_dialog hide">
							...
							<tr class="top">
								...
								<td> 
								<input id="received_date" type="hidden" value="{$smarty.now|date_format:'%Y-%m-%d'}" ovalue="{$smarty.now|date_format:'%Y-%m-%d'}" has_been_valid="0" />
								<input id="received_date_time" type="hidden" value="{$smarty.now|date_format:'%H:%M:%S'}" />
								<input id="received_date_formatted" class="option_input_field" data-settings='{ "type": "datetime","id":"received_date", "field": "Supplier Delivery Received Date"}'    style="width:8em" value="{$smarty.now|date_format:'%d/%m/%Y'}" />
				...
				<div id="dispatched_operations" class="order_operation {if $delivery->get('Supplier Delivery State')!='InProcess'}hide{/if}">
					<div class="square_button right"  title="{t}Mark as dispatched{/t}">
						...
						<table id="dispatched_dialog" border="0" class="order_operation_dialog hide">
							<tr class="top">
								<td class="label" colspan="2">{if $delivery->get('Supplier Delivery Parent')=='Supplier'}{t}Delivery dispatched by supplier{/t}{else}Delivery dispatched by agent{/if}</td>
							</tr>
							<tr class="top">
								<td class="label">{t}Date{/t}</td>
								<td> 
								<input id="dispatched_date" type="hidden" value="{$smarty.now|date_format:'%Y-%m-%d'}" ovalue="{$smarty.now|date_format:'%Y-%m-%d'}" has_been_valid="0" />
								<input id="dispatched_date_time" type="hidden" value="{$smarty.now|date_format:'%H:%M:%S'}" />
								<input id="dispatched_date_formatted" class="option_input_field" data-settings='{ "type": "datetime","id":"dispatched_date", "field": "Supplier Delivery Dispatched Date"}'    style="width:8em" value="{$smarty.now|date_format:'%d/%m/%Y'}" />
							...
							<tr class="buttons">
								<td><i class="fa fa-sign-out fa-flip-horizontal button" aria-hidden="true" onclick="close_dialog('dispatched')"></i></td>
								<td class="aright">
								<span data-data='{ "value": "Dispatched","dialog_name":"dispatched", "field": "Supplier Delivery State"}' id="dispatched_save_buttons" class="valid save button changed" onclick="save_order_operation(this)">
								<span class="label">{t}Save{/t}</span> <i class="fa fa-cloud fa-fw  " aria-hidden="true"></i></span> </td>
							</tr>
						</table>
					</div>
				</div>
