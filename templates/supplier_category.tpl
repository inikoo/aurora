...
<div id="bd" class="no_padding">
	...
	<div id="block_subjects" style="{if $block_view!='subjects'}display:none;{/if}clear:both;margin:20px 0 40px 0;padding:0 20px">
		<div id="children_table" class="data_table">
			...
			<div class="clusters">
				<div class="buttons small left cluster">
					<button class="{if $suppliers_view=='general'}selected{/if}" id="suppliers_general">{t}General{/t}</button> <button class="{if $suppliers_view=='contact'}selected{/if}" id="suppliers_contact">{t}Contact{/t}</button> <button class="{if $suppliers_view=='products'}selected{/if}" id="suppliers_products">{t}Products{/t}</button> <button style="{if !$view_stock}display:none{/if}" class="{if $suppliers_view=='stock'}selected{/if}" id="suppliers_stock">{t}Stock{/t}</button> 
					<button style="{if !$view_sales}display:none{/if}" class="{if $suppliers_view=='sales'}selected{/if}" id="suppliers_sales">{t}Sales{/t}</button> 
					<button style="{if !$view_sales}display:none{/if}" class="{if $suppliers_view=='sales_year'}selected{/if}" id="suppliers_sales_year">{t}Sales/Year{/t}</button> 
...
<script type="text/javascript">
	...
	<h2>{t}Number of suppliers assigned{/t}</h2>
	<div style="float:left" id="plot_referral_1" style="border:1px solid #ccc">
			<strong> You need to upgrade your Flash Player </strong> 
		</div>
<script type="text/javascript">
		// <![CDATA[		
		var so = new SWFObject("external_libs/ampie/ampie/ampie.swf", "ampie", "350", "350", "1", "#FFFFFF");
		so.addVariable("path", "external_libs/ampie/ampie/");
		so.addVariable("settings_file", encodeURIComponent("conf/pie_settings.xml.php"));                // you can set two or more different settings files here (separated by commas)
		so.addVariable("data_file", encodeURIComponent("plot_data.csv.php?tipo=category&subject=Supplier&category_key={$category->id}")); 
		so.addVariable("loading_settings", "LOADING SETTINGS"); 
...
<script type="text/javascript">
		// <![CDATA[		
		var so = new SWFObject("external_libs/ampie/ampie/ampie.swf", "ampie", "500", "500", "8", "#FFFFFF");
		so.addVariable("path", "external_libs/ampie/ampie/");
		so.addVariable("settings_file", encodeURIComponent("conf/pie_settings.xml.php"));                // you can set two or more different settings files here (separated by commas)
