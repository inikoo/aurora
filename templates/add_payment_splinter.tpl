...
<div id="dialog_add_payment" style="padding:20px 20px 10px 20px">
	...
	<div id="add_payment_payment_account_container" style="clear:both" class="buttons left small">
		...
		<button valid_payment_methods="{$payment_account.valid_payment_methods}" class="item" onclick="add_payment_change_account('{$payment_account.key}')" id="add_payment_payment_account_{$payment_account.key}">{if file_exists('art/icons/psp_{$payment_account.service_provider_code}.png')}<img style="height:14px;width:auto;display:none" src="art/icons/psp_{$payment_account.service_provider_code}.png" /> {/if}{$payment_account.service_provider_name}</button> 
		{/foreach} 
	</div>
