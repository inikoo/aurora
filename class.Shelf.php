    ...
    */
    ...
    function create($data, $options = '') {
        ...
        if (mysql_query($sql)) {
            ...
            $details = _('Shelf')." ".$this->data['Shelf Code']." "._(
                    'created'
                );
    ...
    function get_data($key, $tag) {
        ...
        if ($key == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Shelf Dimension` WHERE `Shelf Key`=%d", $tag
            );
        } else {
            if ($key == 'name') {
                $sql = sprintf(
                    "SELECT  *  FROM `Shelf Dimension` WHERE `Shelf Name`=%s ", prepare_mysql($tag)
                );
            } else {
                return;
            }
    ...
    function find($raw_data, $options) {
        ...
        $sql = sprintf(
            "SELECT `Shelf Key` FROM `Shelf Dimension` WHERE  `Shelf Code`=%s", prepare_mysql($data['Shelf Code'])
        );
    ...
    function update_children() {
        $sql              = sprintf(
            'SELECT count(*) AS number FROM `Location Dimension` WHERE `Location Shelf Key`=%d', $this->id
        );
        $res              = mysql_query($sql);
        $number_locations = 0;
        if ($row = mysql_fetch_array($res)) {
        ...
        $sql = sprintf(
            'UPDATE `Shelf Dimension` SET `Shelf Number Locations`=%d WHERE `Shelf Key`=%d', $number_locations
