...
class EmailSend extends DB_Table {
    ...
    function EmailSend($arg1 = false, $arg2 = false) {
        ...
        $this->ignore_fields = array(
            'Email Send Key',
            'Email Send First Read Date',
            'Email Send Last Read Date',
            'Email Send Number Reads',
            'Email Send Date'
        );
        if (is_numeric($arg1)) {
    ...
    function get_data($key, $tag) {
        ...
        if ($key == 'id') {
            $sql = sprintf(
                "SELECT *  FROM `Email Send Dimension` WHERE `Email Send Key`=%d", $tag
            );
        } else {
            return;
        }
    ...
    function create($data) {
        ...
        $sql    = sprintf(
            "INSERT INTO `Email Send Dimension` %s %s", $keys, $values
        );
        // print $sql;
        if (mysql_query($sql)) {
    ...
    function update_read() {
        ...
        $sql = sprintf(
            "SELECT count(*) AS number_reads, min(`Email Send Read Date`) AS first, max(`Email Send Read Date`) last FROM `Email Send Read Fact` WHERE `Email Send Key`=%s  ", $this->id
        );
        $res = mysql_query($sql);
        ...
        $sql = sprintf(
            "UPDATE `Email Send Dimension` SET  `Email Send Number Reads`=%d,`Email Send First Read Date`=%s,`Email Send Last Read Date`=%s WHERE `Email Send Key`=%d",
            $this->data['Email Send Number Reads'], prepare_mysql($this->data['Email Send First Read Date']), prepare_mysql($this->data['Email Send Last Read Date']), $this->id
        );
        mysql_query($sql);
