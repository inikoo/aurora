    ...
    */
    ...
    function create($data, $options = '') {
        ...
        if ($this->data['Shelf Type Location Height'] != '') {
            list($this->data['Shelf Type Location Height'], $dump)
                = parse_distance($this->data['Shelf Type Location Height']);
        }
        if ($this->data['Shelf Type Location Length'] != '') {
            list($this->data['Shelf Type Location Length'], $dump)
                = parse_distance($this->data['Shelf Type Location Length']);
        }
        if ($this->data['Shelf Type Location Deep'] != '') {
            list($this->data['Shelf Type Location Deep'], $dump)
                = parse_distance($this->data['Shelf Type Location Deep']);
        }
        if ($this->data['Shelf Type Location Max Weight'] != '') {
            list($this->data['Shelf Type Location Max Weight'], $dump)
                = parse_weight($this->data['Shelf Type Location Max Weight']);
        }
        if ($this->data['Shelf Type Location Max Weight'] != '') {
            list($this->data['Shelf Type Location Max Weight'], $dump)
                = parse_weight($this->data['Shelf Type Location Max Weight']);
        ...
        $sql = sprintf(
            "INSERT INTO `Shelf Type Dimension` %s %s", $keys, $values
        );
        //print "$sql\n";
        // exit;
        if (mysql_query($sql)) {
            ...
            $details = _('Shelf Type')." ".$this->data['Shelf Type Name']." "._(
                    'created'
                );
    ...
    function get_data($key, $tag) {
        ...
        if ($key == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Shelf Type Dimension` WHERE `Shelf Type Key`=%d", $tag
            );
        } else {
            if ($key == 'name') {
                $sql = sprintf(
                    "SELECT  *  FROM `Shelf Type Dimension` WHERE `Shelf Type Name`=%s ", prepare_mysql($tag)
                );
            } else {
                return;
            }
    ...
    function find($raw_data, $options) {
        ...
        $sql = sprintf(
            "SELECT `Shelf Type Key` FROM `Shelf Type Dimension` WHERE  `Shelf Type Name`=%s", prepare_mysql($data['Shelf Type Name'])
        );
