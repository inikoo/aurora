...
var change_view=function(e){
	...
	if(table.view!=tipo){
	    ...
	    table.hideColumn('desupplierments');
		...
		table.showColumn('desupplierments');
	...
	table.view=tipo
	YAHOO.util.Connect.asyncRequest('POST','ar_sessions.php?tipo=update&keys=supplier_categories-view&value=' + escape(tipo) );
	}
  }
...
request="ar_categories.php?tipo=main_categories&parent=supplier_categories&tableid=1";
//alert(request)
...
function change_history_elements(e, table_id) {
 ...
 function init() {
     ...
     //YAHOO.util.Event.addListener('show_details', "click",show_details,'supplier_categories');
     YAHOO.util.Event.addListener('show_percentages', "click", show_percentages, 'supplier_categories');
