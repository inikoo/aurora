...
function new_sku(){
  ...
  function create($data){
    ...
    if(isset($data['units factor']) and is_numeric($data['units factor']) and $data['units factor']>0)
      ...
      $units_factor=1;
    $sql=sprintf("insert into  `Product Dimension` (`Product SKU number`,`Product Alphanumeric Code`,`Product Code`,`Product Status`
,`Product Units Factor`) values (%s,%s,%s,'%s',%f)",
