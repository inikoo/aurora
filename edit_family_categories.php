...
/*
  ...
  Created: 2 December 2013 15:36:19 CET Malaga ESP	
...
if (isset($_REQUEST['store_id']) and is_numeric($_REQUEST['store_id']) ) {
	...
	if (count($user->stores)==0) {
		...
		header('Location: edit_family_categories.php?store_id='.$user->stores[0]);
		exit;
	}
}
...
$tipo_filter=$_SESSION['state']['family_categories']['edit_categories']['f_field'];
$smarty->assign('filter0',$tipo_filter);
$smarty->assign('filter_value0',$_SESSION['state']['family_categories']['edit_categories']['f_value']);
...
$sql=sprintf("select count(*) as num ,`Category Branch Type` from  `Category Dimension` where  `Category Store Key`=%d and `Category Subject`='Family' group by  `Category Branch Type`",
$store->id
);
//print_r($sql);
$res=mysql_query($sql);
...
$smarty->assign('elements',$_SESSION['state']['family_categories']['edit_categories']['elements']);
...
$smarty->display('edit_family_categories.tpl');
?>
