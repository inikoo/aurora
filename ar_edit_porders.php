...
function po_transactions_to_process() {
    ...
    if ($show_all) {
        ...
        $where=sprintf('where `Supplier Key`=%d   ',$supplier_key);
        $sql_qty=sprintf(',IFNULL((select sum(`Purchase Order Quantity`) from `Purchase Order Transaction Fact` where `Supplier Product Historic Key`=`Supplier Product Current Key` and `Purchase Order Key`=%d),0) as `Purchase Order Quantity`, IFNULL((select sum(`Purchase Order Net Amount`) from `Purchase Order Transaction Fact` where `Supplier Product Historic Key`=`Supplier Product Current Key` and `Purchase Order Key`=%d),0) as `Purchase Order Net Amount` ',$purchase_order_key,$purchase_order_key);
        ...
        $table='  `Purchase Order Transaction Fact` OTF
               left join `Supplier Product History Dimension` PHD on (`SPH Key`=OTF.`Supplier Product Historic Key`)
               left join `Supplier Product Dimension` PD on (`SPH Key`=PD.`Supplier Product Current Key`) ';
