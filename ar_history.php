...
function list_history($asset_type) {
    ...
    $sql=sprintf("select  * from `History Dimension` H left join `User Dimension` U on (U.`User Key`=H.`Subject Key`)   $where $wheref order by $order  limit $start_from,$number_results ");
    // print $sql;
    $result=mysql_query($sql);
    $adata=array();
