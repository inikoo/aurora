...
function delivery_checking_items($_data, $db, $user) {
	...
	if ($result=$db->query($sql)) {
		foreach ($result as $data) {
			...
			$edit_sko_checked_quantity=sprintf('<span data-settings=\'{"field": "Supplier Delivery Checked Quantity", "transaction_key":%d,"item_key":%d, "item_historic_key":%d ,"on":1 }\' class="checked_quantity"  ><input class="checked_qty width_50" value="%s" ovalue="%s"> <i onClick="save_item_qty_change(this)" class="fa  fa-plus fa-fw button %s" aria-hidden="true"></span>',
				$data['Purchase Order Transaction Fact Key'],
				$data['Supplier Part Key'],
				$data['Supplier Part Historic Key'],
