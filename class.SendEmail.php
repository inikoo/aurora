...
class SendEmail extends DB_Table {
    ...
    function set($data) {
        ...
        $this->email_send_data = array(
            ...
            'Email Send Creation Date'  => date(
                'Y-m-d H:i:s', strtotime('now +0:00')
            ),
        ...
        if (array_key_exists('Email Send Type', $data)) {
            $this->email_send_data['Email Send Type']
                = $data['Email Send Type'];
        }

        if (array_key_exists('madmimi_auto_subscribe', $data)) {
            $this->email_send_data['Madmimi Auto Subscribe']
                = $data['madmimi_auto_subscribe'];
        }
    ...
    function retry($type) {
        ...
        switch ($type) {
            case 'plain':
                $sql = sprintf(
                    "SELECT * FROM `Email Queue Dimension` WHERE `Status`='No' AND `Type`='Plain'"
                );
                ...
                while ($row = mysql_fetch_array($result)) {
                    $sql = sprintf(
                        "SELECT * FROM `Email Queue Attachement Dimension` WHERE `Email Queue Key` = %d", $row['Email Queue Key']
                    );
                    $res = mysql_query($sql);
                    ...
                    if ($res['msg'] == 'ok') {
                        $sql = sprintf(
                            "UPDATE `Email Queue Dimension` SET `Status`='Yes' WHERE `Email Queue Key`=%d", $row['Email Queue Key']
                        );
                        if (mysql_query($sql)) {
                            $success++;
                        }
                ...
                $response = sprintf(
                    "%d emails sent. %d has failed", $success, $fail
                );
                break;

            case 'html':
                $sql = sprintf(
                    "SELECT * FROM `Email Queue Dimension` WHERE `Status`='No' AND `Type`='HTML'"
                );
    ...
    function send_actual_email() {
        ...
        $email_credentials = new EmailCredentials(
            $this->email_send_data['Email Credential Key']
        );
        ...
        switch ($this->method) {
            case 'SMTP':
                ...
                $this->message_object->localhost
                                      = "localhost";   /* This computer address */
                $this->message_object->smtp_host
                                      = "localhost";   /* SMTP server address */
                $this->message_object->smtp_direct_delivery
                                      = 0;  /* Deliver directly to the recipients destination SMTP server */
                $this->message_object->smtp_exclude_address
                                      = ""; /* In directly deliver mode, the DNS may return the IP of a sub-domain of the default domain for domains that do not exist. If that is your case, set this variable with that sub-domain address. */
                /*
                ...
                */
                ...
                $this->message_object->smtp_password
                    = "";        /* authentication password */
                $this->message_object->smtp_pop3_auth_host
                    = "";  /* if you need POP3 authetntication before SMTP delivery, specify the host name here. The smtp_user and smtp_password above should set to the POP3 user and password */
                $this->message_object->smtp_debug
                    = 0;            /* Output dialog with SMTP server */
                $this->message_object->smtp_html_debug
                    = 1;       /* If smtp_debug is 1, set this to 1 to output debug information in HTML */
                ...
                list($to, $subject, $html_message, $plain_message)
                    = $this->get_message_data();

                if ($email_credentials->data['Email Provider'] == 'Gmail') {
                    $this->from_name
                        = $email_credentials->data['Email Address Gmail'];
                    $this->from_address
                        = $email_credentials->data['Email Address Gmail'];
                } else {
                    $this->from_name
                        = $email_credentials->data['Email Address Gmail'];
                    $this->from_address
                        = $email_credentials->data['Email Address Other'];
                }
                ...
                if ($email_credentials->data['Email Provider'] == 'Gmail') {
                    ...
                    $this->message_object->smtp_host
                        = $email_credentials->data['Outgoing Mail Sever'];
                }
                ...
                $this->subject                = mb_convert_encoding(
                    $subject, 'ISO-8859-1'
                );
                ...
                if (strlen($this->to_address) == 0) {
                    die("Please set the messages recipient address in line ".$this->recipient_line." of the script ".basename(
                            __FILE__
                        )."\n");
                }
                ...
                if ($email_credentials->data['Email Provider'] == 'Gmail') {
                    ...
                    $this->message_object->smtp_user
                        = $email_credentials->data['Login Other'];
                }

                $this->message_object->smtp_password
                    = $email_credentials->get_password($this->secret_key);
                ...
                $this->message_object->SetEncodedEmailHeader(
                ...
                $this->message_object->SetEncodedEmailHeader(
                    "Reply-To", $this->reply_address, $this->reply_name
                );
                $this->message_object->SetHeader(
                    "Return-Path", $this->error_delivery_address
                );
                $this->message_object->SetEncodedEmailHeader(
                    "Errors-To", $this->error_delivery_address, $this->error_delivery_name
                );
                ...
                $this->message_object->SetEncodedHeader(
                ...
                foreach ($data['attachement'] as $value) {
                    if ($value['attachement_type'] == 'Text') {
                        ...
                        if ($value['attachement_type'] == 'Image') {
                            $image_attachment[] = array(
                ...
                switch ($this->email_send_data['Email Type']) {
                    case 'Plain':
                        ...
                        $this->message = mb_convert_encoding(
                            $plain_message, 'ISO-8859-1'
                        );

                        $this->text_message = $this->message;
                        $this->message_object->CreateQuotedPrintableTextPart(
                            $this->message_object->WrapText(
                                $this->text_message
                            ), "", $this->text_part
                        );
                    ...
                    case 'HTML':
                        ...
                        if (isset($html_message) and $html_message) {
                            $html_msg = mb_convert_encoding(
                                $html_message, 'ISO-8859-1'
                            );
                        } else {
                            $html_msg = '';
                        }
                        ...
                        $this->message_object->CreateQuotedPrintableHTMLPart(
                            $this->html_message, "", $this->html_part
                        );
                ...
                $this->message_object->AddAlternativeMultipart(
                    $this->alternative_parts
                );
                ...
                for (
                    $recipient = 0, reset($this->message_object->invalid_recipients); $recipient < count(
                    $this->message_object->invalid_recipients
                ); next($this->message_object->invalid_recipients), $recipient++
                ) {
                    $response = "Invalid recipient: ".key(
                            $this->message_object->invalid_recipients
                        )." Error: ".$this->message_object->invalid_recipients[key(
                            $this->message_object->invalid_recipients
                        )]."\n";
                }
                if (strcmp($error, "")) {
            ...
            case 'Amazon':
                ...
                switch ($this->email_send_data['Email Type']) {
                    ...
                    case 'HTML':
                        $this->type = 'HTML';
                        $this->m->setMessageFromString(
                            $plain_message, $html_message.$this->get_track_code()
                        );
                        break;
            ...
            case 'MadMimi':
                ...
                $username
                            = $email_credentials->data['API Email Address MadMimi'];
                $promotion_name
                            = $this->email_send_data['Email Promotion Name'];
    ...
    */
                ...
                if ($this->email_send_data['Madmimi Auto Subscribe']) {
                    $response_add_membership = $mailer->AddMembership(
                        $to, $this->email_send_data['Madmimi Auto Subscribe']
                    );
            ...
            case 'PHPMail':
                ...
                $from
                    = $email_credentials->data['Email Address Direct Mail'];//"registration@ancientwisdom.biz";
                switch ($this->email_send_data['Email Type']) {
                    case 'Plain':
    ...
    function get_message_data() {
        return array(
            $this->email_send_data['Email Send To'],
            $this->email_send_data['Email Send Subject'],
            $this->email_send_data['Email Send HTML'],
            $this->email_send_data['Email Send Plain']
        );
    }
    ...
    function get_track_code() {
        ...
        $code = sprintf(
            '<img src="%s/track.php?s=%s" />', $public_path, $this->send_key
        );
        $code = '';
    ...
    function send() {
        ...
        switch ($this->email_send_data['Email Matter']) {
            case 'Marketing':
                $sql = sprintf(
                    "UPDATE `Email Campaign Mailing List`  SET `Email Send Key`=%d WHERE `Email Campaign Mailing List Key`=%d ", $this->send_key, $this->email_send_data['Email Matter Key']
                );
                // mysql_query($sql);
                break;
            default:
        ...
        if ($send_result['state'] == 200) {
            $sql = sprintf(
                "UPDATE `Email Send Dimension` SET `Email Send Date`=%s WHERE `Email Send Key`=%d", prepare_mysql(date('Y-m-d H:i:s', strtotime('now +0:00'))), $this->send_key
            );
            mysql_query($sql);
            ...
            switch ($this->email_send_data['Email Matter']) {
                case 'Marketing':
                    $email_campaign = new EmailCampaign(
                        $this->email_send_data['Email Matter Parent Key']
                    );
    ...
    function store_in_queue($result, $files = false, $data) {
        if (preg_match(
            '/^could not resolve the host domain/', $result['msg']
        )) {
            ...
            $sql = sprintf(
                "INSERT INTO `Email Queue Dimension` (`To`, `Type`, `Subject`, `Plain`, `HTML`, `Email Credentials Key`, `BCC`) VALUES (%s, %s, %s, %s, %s, %d, %s)	", prepare_mysql($data['to']),
                prepare_mysql($data['type']), prepare_mysql($data['subject']), prepare_mysql($data['plain']), prepare_mysql($html_msg), $data['email_credentials_key'], prepare_mysql($bcc)
            );
            ...
            if (isset($files)) {
                foreach ($files as $value) {
                    ...
                    $sql = sprintf(
                        "INSERT INTO `Email Queue Attachement Dimension` (`Email Queue Key`, `Data`, `FileName`, `Name`, `Content-Type`, `Disposition`, `Type`) VALUES (%d, %s, %s, %s, %s, %s, %s)",
                        $email_queue_key, prepare_mysql($data_temp), prepare_mysql($file_name), prepare_mysql($name), prepare_mysql($value['Content-Type']), prepare_mysql($value['Disposition']),
            ...
            if ($stat) {
                $result = array(
                    'state' => 400,
                    'msg'   => _('Message will send shortly')
                );
            } else {
                $result = array(
                    'state' => 400,
                    'msg'   => 'Error: Message could not be sent'
                );
            }
...
class SimpleEmailService {
    ...
    public function listVerifiedEmailAddresses() {
        ...
        $response['RequestId']
                               = (string)$rest->body->ResponseMetadata->RequestId;
    ...
    public function __triggerError($functionname, $error) {
        if ($error == false) {
            trigger_error(
                sprintf(
                    "SimpleEmailService::%s(): Encountered an error, but no description given", $functionname
                ), E_USER_WARNING
            );
        } else {
            if (isset($error['curl']) && $error['curl']) {
                trigger_error(
                    sprintf(
                        "SimpleEmailService::%s(): %s %s", $functionname, $error['code'], $error['message']
                    ), E_USER_WARNING
                );
            } else {
                if (isset($error['Error'])) {
                    $e       = $error['Error'];
                    $message = sprintf(
                        "SimpleEmailService::%s(): %s - %s: %s\nRequest Id: %s\n", $functionname, $e['Type'], $e['Code'], $e['Message'], $error['RequestId']
                    );
                    trigger_error($message, E_USER_WARNING);
                } else {
                    trigger_error(
                        sprintf(
                            "SimpleEmailService::%s(): Encountered an error: %s", $functionname, $error
                        ), E_USER_WARNING
                    );
                }
            }
    ...
    public function getSendQuota() {
        ...
        $response['Max24HourSend']
            = (string)$rest->body->GetSendQuotaResult->Max24HourSend;
        $response['MaxSendRate']
            = (string)$rest->body->GetSendQuotaResult->MaxSendRate;
        $response['SentLast24Hours']
            = (string)$rest->body->GetSendQuotaResult->SentLast24Hours;
        $response['RequestId']
            = (string)$rest->body->ResponseMetadata->RequestId;
    ...
    public function sendEmail($sesMessage) {
        ...
        if (is_array($sesMessage->bcc)) {
            ...
            foreach ($sesMessage->bcc as $bcc) {
                $rest->setParameter(
                    'Destination.BccAddresses.member.'.$i, $bcc
                );
                $i++;
            }
        }
        ...
        if ($sesMessage->subject != null && strlen($sesMessage->subject) > 0) {
            ...
            if ($sesMessage->subjectCharset != null
                && strlen(
                    $sesMessage->subjectCharset
                ) > 0
            ) {
                $rest->setParameter(
                    'Message.Subject.Charset', $sesMessage->subjectCharset
                );
            }
        }


        if ($sesMessage->messagetext != null
            && strlen($sesMessage->messagetext) > 0
        ) {
            $rest->setParameter(
                'Message.Body.Text.Data', $sesMessage->messagetext
            );
            if ($sesMessage->messageTextCharset != null
                && strlen(
                    $sesMessage->messageTextCharset
                ) > 0
            ) {
                $rest->setParameter(
                    'Message.Body.Text.Charset', $sesMessage->messageTextCharset
                );
            }
        }

        if ($sesMessage->messagehtml != null
            && strlen($sesMessage->messagehtml) > 0
        ) {
            $rest->setParameter(
                'Message.Body.Html.Data', $sesMessage->messagehtml
            );
            if ($sesMessage->messageHtmlCharset != null
                && strlen(
                    $sesMessage->messageHtmlCharset
                ) > 0
            ) {
                $rest->setParameter(
                    'Message.Body.Html.Charset', $sesMessage->messageHtmlCharset
                );
            }
        }
...
final class SimpleEmailServiceRequest {
    ...
    public function getResponse() {
        ...
        $auth .= ',Algorithm=HmacSHA256,Signature='.$this->__getSignature(
                $date
            );
        ...
        curl_setopt(
            $curl, CURLOPT_SSL_VERIFYHOST, ($this->ses->verifyHost() ? 0 : 0)
        );
        curl_setopt(
            $curl, CURLOPT_SSL_VERIFYPEER, ($this->ses->verifyPeer() ? 0 : 0)
        );
        ...
        curl_setopt(
            $curl, CURLOPT_WRITEFUNCTION, array(
                &$this,
        ...
        if ($this->response->error === false && isset($this->response->body)) {
            $this->response->body = simplexml_load_string(
                $this->response->body
            );
            ...
            if (!in_array(
                ...
                $output['RequestId']
                                            = (string)$this->response->body->RequestId;
    ...
    private function __getSignature($string) {
        return base64_encode(
            hash_hmac('sha256', $string, $this->ses->getSecretKey(), true)
        );
    }
