...
function _convert_units($value, $from, $to) {
...
function money_locale($amount,$locale='',$currency_code='') {
	...
	if ($currency_code) {
		...
		$money=$currency_code.money_format($format,round($amount,2));
		}
	...
	if (preg_match('/[A-Z]{3}/',$money,$match)) {
		...
		$money=preg_replace('/\s*[A-Z]{3}\s*/',currency_symbol($match[0]),$money);
...
function _currency_symbol($currency) {
	switch ($currency) {
	case('GBP'):
		return '£';
		break;
...
function _weight($w,$unit='Kg',$number_decimals=3,$simplify=false) {
	if ($w=='') return '';
	if ($simplify) {
		if ($w==0) {
			return '0Kg';
