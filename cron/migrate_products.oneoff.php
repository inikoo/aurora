...
if ($result=$db->query($sql)) {
	...
	foreach ($result as $row) {
		...
		$old_product=new Product('pid', $product->id);
		$parts_data=$old_product->get_part_list();
		...
		foreach ($parts_data as $part_data) {
			...
			$sql=sprintf('insert into `Store Product Part Bridge` (`Store Product Key`,`Part SKU`,`Parts Per Product`,`Note`) values (%d,%d,%f,%s)',
				$product->id,
				$part->id,
			...
			if ($row['Product Use Part Properties']=='Yes') {
				...
				$sql=sprintf("select `Linked Fields` from `Store Product Part Bridge` where `Store Product Key`=%d and `Part SKU`=%d ",
					$product->id,
					$part->id
				);
				if ($result2=$db->query($sql)) {
					if ($row2 = $result2->fetch()) {
						...
						$linked_fields['Part Package Weight']='Store Product Outer Weight';
					...
					$sql=sprintf("update `Store Product Part Bridge` set `Linked Fields`=%s where `Store Product Key`=%d and `Part SKU`=%d ",
			...
			if ($row['Product Use Part Tariff Data']=='Yes') {
				...
				if ($result2=$db->query($sql)) {
					if ($row2 = $result2->fetch()) {
						...
						$linked_fields['Part Tarrif Code']='Store Product Outer Tariff Code';
						$linked_fields['Part Duty Rate']='Store Product Outer Duty Rate';
			...
			if ($row['Product Use Part H and S']=='Yes') {
				...
				if ($result2=$db->query($sql)) {
					if ($row2 = $result2->fetch()) {
						...
						$linked_fields['Part UN Number']='Store Product Outer UN Number';
						$linked_fields['Part UN Class']='Store Product Outer UN Class';
						$linked_fields['Part Packing Group']='Store Product Outer Packing Group';
						$linked_fields['Part Proper Shipping Name']='Store Product Outer Proper Shipping Name';
						$linked_fields['Part Hazard Indentification Number']='Store Product Outer Hazard Indentification Number';
...
//$sql=sprintf('select `Store Key` from `Store Dimension` where `Store Key`=1 ');
			...
			foreach ($result2 as $row2) {
				...
				if ($result3=$db->query($sql)) {
					foreach ($result3 as $row3) {
						...
						if ($product->id) {
							$product->update(array('Store Product Family Category Key'=>$family->id));
						}
