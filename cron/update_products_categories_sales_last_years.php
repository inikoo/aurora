...
/*
 ...
 Created: 28 September 2016 at 22:49:17 GMT+8, Kuala Lumpur, Malaysia
...
function update_parts_sales($db, $print_est) {
	...
	if ($result=$db->query($sql)) {
		foreach ($result as $row) {
			...
			if ($print_est) {
				print 'PC '.percentage($contador, $total, 3)."  lap time ".sprintf("%.2f", ($lap_time1-$lap_time0)/$contador)." EST  ".sprintf("%.1f", (($lap_time1-$lap_time0)/$contador)*($total-$contador)/3600)  ."h  ($contador/$total) \r";
			}
