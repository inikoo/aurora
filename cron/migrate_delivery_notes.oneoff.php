...
$sql = sprintf("select count(*) as num FROM `Delivery Note Dimension` O left join `Store Dimension` on (`Store Key`=`Delivery Note Store Key`)  where `Store Version`=1 ");
...
$sql = sprintf('SELECT `Delivery Note Key` FROM `Delivery Note Dimension`  left join `Store Dimension` on (`Store Key`=`Delivery Note Store Key`) where `Store Version`=1  order by `Delivery Note Key` desc ');
if ($result = $db->query($sql)) {
