...
$sql = sprintf('select * from tenant_sk.`Supplier Dimension`  where `Supplier Key` in (%s) and `Supplier Type`="Free"   ',$suppliers_to_copy);
...
while ($row2 = $stmt2->fetch()) {
    ...
    $sql = sprintf(
        'select * from sk.`Supplier Part Dimension` left join sk.`Part Dimension` on (`Part SKU`=`Supplier Part Part SKU`) left join sk.`Category Dimension` on (`Category Key`=`Part Family Category Key`) where  `Part Status` in ("In Use","In Process") and  `Supplier Part Supplier Key`=?   '
    );
    ...
    while ($row4 = $stmt4->fetch()) {
        ...
        if (is_object($supplier_part) and $supplier->new_object ) {
            ...
            while ($row3 = $stmt3->fetch()) {
                ...
                $file = preg_replace('/^img/', '/data/img/AWEU', $row3['Image Path']);
