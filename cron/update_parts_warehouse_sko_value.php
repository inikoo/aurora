...
if ($result = $db->query($sql)) {
    foreach ($result as $row) {
        ...
        $sql = sprintf(
            'select  *, POTF.`Supplier Key`,`Date`,(`Inventory Transaction Amount`/`Inventory Transaction Quantity`) as value_per_sko ,`ITF POTF POTF Key` from    `ITF POTF Bridge` B  left join     `Inventory Transaction Fact` ITF   on  (B.`ITF POTF ITF Key`=`Inventory Transaction Key`)  
    left join `Purchase Order Transaction Fact` POTF on  (`Purchase Order Transaction Fact Key`=`ITF POTF POTF Key`) where  `Inventory Transaction Amount`>0 and `Inventory Transaction Quantity`>0 and   B.`ITF POTF ITF State`="InvoiceChecked"  and  `Inventory Transaction Section`="In"    and ITF.`Part SKU`=%d    and POTF.`Supplier Key`!=%d order by `Date` desc  limit 1 ',
