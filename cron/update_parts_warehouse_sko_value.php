...
if ($result = $db->query($sql)) {
    foreach ($result as $row) {
        ...
        $sql = sprintf(
            'select  POTF.`Supplier Key`,`Date`,(`Inventory Transaction Amount`/`Inventory Transaction Quantity`) as value_per_sko ,`ITF POTF Costing Done POTF Key` 
...
where  `Inventory Transaction Amount`>0 and `Inventory Transaction Quantity`>0   and  `Inventory Transaction Section`="In"    and ITF.`Part SKU`=%d    and POTF.`Supplier Key`!=%d order by `Date` desc  limit 1 ',
