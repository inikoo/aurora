...
$sql = sprintf("select count(*) as num FROM `Invoice Dimension` O left join `Store Dimension` on (`Store Key`=`Invoice Store Key`)  where `Store key` in (%s) and `Invoice Type`='Invoice' ",$store_keys);
...
$sql = sprintf('SELECT `Invoice Key` FROM `Invoice Dimension` left join `Store Dimension` on (`Store Key`=`Invoice Store Key`)  where `Store key`  in (%s)  and `Invoice Type`="Invoice" order by `Invoice Key` desc ',$store_keys);
