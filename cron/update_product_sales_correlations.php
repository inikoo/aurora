...
$sql = "SELECT P.`Product ID` FROM `Product Dimension` P left join `Product DC Data` D on (P.`Product ID`=D.`Product ID`)  where `Product Ignore Correlation`='No' and `Product Status` in ('Active','Discontinuing')  order by  RAND()   ";
$stmt = $db->prepare($sql);
$stmt->execute();
