...
/*
 ...
 Created:  11 March 2020  22:59::18  +0800, Kuala Lumpur, Malaysia
...
$editor = array(
    ...
    'Subject'      => 'System',
    'Subject Key'  => 0,
    'Author Name'  => 'System (Stack store data feeds)',
    'Author Alias' => 'System (Stack store data feeds)',


);

$sql  = "SELECT count(*) AS num FROM `Stack Dimension`  where `Stack Operation`='store_data_feed'";
$stmt = $db->prepare($sql);
$stmt->execute();
if ($row = $stmt->fetch()) {
    $total = $row['num'];
...
$sql  = "SELECT `Stack Key`,`Stack Object Key` FROM `Stack Dimension`  where `Stack Operation`='store_data_feed' ORDER BY RAND() ";
    ...
    if ($store->id) {
        ...
        if ($row2 = $stmt2->fetch()) {
            ...
            if($website->id and $website->get('Website Status')=='Active'){
                ...
                file_put_contents('EcomB2B/data_feeds/data_feed_website_'.$website->id.'_json.txt',json_encode($data));
