...
foreach ($store_keys as $store_key) {
    ...
    if ($stmt->execute(
        ...
        while ($row = $stmt->fetch()) {
            $sql = sprintf(
                'insert into `Payment Account Store Bridge` (`Payment Account Store Store Key`,`Payment Account Store Website Key`,`Payment Account Store Payment Account Key`,
