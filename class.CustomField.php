...
class CustomField extends DB_Table {
    ...
    function get_data($key, $tag) {
        ...
        if ($key == 'id') {
            $sql = sprintf(
                "SELECT * FROM `Custom Field Dimension` WHERE `Custom Field Key`=%d", $tag
            );
        }
    ...
    function find($raw_data, $options) {
        ...
        $sql = sprintf(
            "SELECT `Custom Field Key` FROM `Custom Field Dimension` WHERE `Custom Field Name` =%s AND `Custom Field Store Key`=%s", prepare_mysql($data['Custom Field Name']),
    ...
    function create($data) {
        ...
        $sql    = sprintf(
            "INSERT INTO `Custom Field Dimension` %s %s", $keys, $values
        );

        //print $sql;
        if (mysql_query($sql)) {
            $sql       = sprintf(
                "SELECT `Custom Field Key` FROM `Custom Field Dimension` ORDER BY `Custom Field Key` DESC"
            );
            $res       = mysql_query($sql);
            ...
            if ($base_data['Custom Field Type'] == 'Enum') {
                ...
                $sql = sprintf(
                    "ALTER TABLE `%s` ADD `%s` %s", $table, $custom_id, $base_data['Custom Field Type']
                );
                ...
                if ($base_data['Custom Field Type'] == 'Mediumint') {
                    ...
                    $sql = sprintf(
                        "ALTER TABLE `%s` ADD `%s` %s(8) DEFAULT '%s'", $table, $custom_id, $base_data['Custom Field Type'], $base_data['Default Value']
                    );

                } elseif ($base_data['Custom Field Type'] == 'varchar') {
                    $sql = sprintf(
                        "ALTER TABLE `%s` ADD `%s` %s(255) DEFAULT '%s'", $table, $custom_id, $base_data['Custom Field Type'], $base_data['Default Value']
                    );
                }
            }
