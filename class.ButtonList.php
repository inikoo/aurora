...
class ButtonList {
    ...
    function get_products_from_family($code, $store) {
        ...
        if ($family->id) {
            $sql = sprintf(
                "SELECT `Product ID`,`Product Code` FROM `Product Dimension` WHERE `Product Sales Type`='Public Sale' AND  `Product Family Key`=%d  ", $family->id
            );
            // print $sql;
            $res = mysql_query($sql);
            while ($row = mysql_fetch_assoc($res)) {
                $this->products[$row['Product Code']] = new Product(
                    'pid', $row['Product ID']
                );
            }
        }
    ...
    function display() {
        ...
        switch ($this->method) {
            case 'awr':
                ...
                $form = sprintf(
                    '%s<table class="Order" border=0><FORM METHOD="POST" ACTION="%s"><INPUT TYPE="HIDDEN" NAME="userid" VALUE="%s"><input type="hidden" name="return" value="%s">', $style,
                    addslashes($site_checkout_address), addslashes($site_checkout_id), $site_url.$_SERVER['PHP_SELF']
                );
                ...
                foreach ($this->products as $product) {
                    echo $product->get_order_list_form(
                        array(
                            'counter'  => $i,
                            'options'  => '',
                            'currency' => $currency
                ...
                $form = sprintf(
                    '<tr id="submit_tr"><td id="submit_td" colspan="3" ><input name="Submit" type="submit" class="text" value="%s"> <input name="Reset" type="reset" class="text"  id="Reset" value="%s"></td></tr></form></table>',
                    $order_txt, $reset_txt
                );
