...
$sql="select C.`Customer Key`,C.`Customer Store Key`,C.`Customer Main XHTML Email`,C.`Customer Main Location`,C.`Customer Name`,C.`Customer Type`,C.`Customer Main XHTML Telephone` from `Customer Dimension` C right join `Customer List Customer Bridge` CLCB on (C.`Customer Key`=CLCB.`Customer Key`) left join `Customer List Dimension` CLD on (CLD.`Customer List Key`=CLCB.`Customer List Key`) where CLD.`Customer List Key`=$static_list_id order by $order  $order_direction limit $start_from,$number_results";
//print $sql;
    $adata=array();
...
$sql="select count(distinct `Customer List Key`) as total from `Customer List Customer`  $where  ";  
    ...
    if ($wheref!='') {
        $sql="select count(*) as total_without_filters from `Customer List Customer` $where $wheref ";
        $res=mysql_query($sql);
        if ($row=mysql_fetch_array($res, MYSQL_ASSOC)) {
...
$cusomer_list_name=" <a href='customer_list.php?id=".$data['Customer List key']."'>".$data['Customer List Name'].'</a>';
