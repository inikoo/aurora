...
$sql="select * from `Invoice Dimension` where `Invoice Tax Code`='UNK' and `Invoice Store Key`=3";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result, MYSQL_ASSOC)   ){
$id=preg_replace('/[^\d]/i','',$row['Invoice Metadata']);
...
$sql="select * from `Invoice Dimension` where `Invoice Tax Code`='UNK' and `Invoice Store Key`=5";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result, MYSQL_ASSOC)   ){
$id=preg_replace('/[^\d]/i','',$row['Invoice Metadata']);
...
$sql="select * from `Invoice Dimension` where `Invoice Tax Code`='UNK' and `Invoice Store Key`=7";
$result=mysql_query($sql);
while($row=mysql_fetch_array($result, MYSQL_ASSOC)   ){
$id=preg_replace('/[^\d]/i','',$row['Invoice Metadata']);
