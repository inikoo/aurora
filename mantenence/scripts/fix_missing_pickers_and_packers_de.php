...
$store_key=$store->id;
$sql="select `Delivery Note Metadata` as id ,`Delivery Note Key` from `Delivery Note Dimension` where  `Delivery Note Store Key`=$store_key and ( `Delivery Note Assigned Picker Key` IS NULL or `Delivery Note Assigned Packer Key` IS NULL) ";
$result3=mysql_query($sql);
while ($row3=mysql_fetch_array($result3, MYSQL_ASSOC)   ) {

	$id=preg_replace('/D/i','',$row3['id']);
	//print $id."\n";
	$sql=sprintf("select * from  de_orders_data.orders  where de_orders_data.orders.id =%d",$id);
