...
while($row=mysql_fetch_array($res)){
 ...
 $res2=mysql_query($sql);
  while($row2=mysql_fetch_array($res2)){
    ...
    $sql=sprintf("select `Date`,`Inventory Transaction Type` from `Inventory Transaction Fact` where  `Part SKU`=%d and `Location Key`=%d  order by `Date`,`Event Order`   ",$part->sku,$location_key);
    //print "$sql\n";
    ...
    $part_location->set_audits();
     ...
     foreach($intervals as $interval){
       $part_location->update_stock_history_interval($interval['From'],($interval['To']?$interval['To']:date('Y-m-d',strtotime('now -1 day'))));
     }
