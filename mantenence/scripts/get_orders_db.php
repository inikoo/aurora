...
while ($row2=mysql_fetch_array($res, MYSQL_ASSOC)) {
	...
	if ($row=mysql_fetch_array($result, MYSQL_ASSOC)) {
		...
		foreach ($transactions as $transaction) {
			...
			if (  preg_match('/^bonus\-/i',$code)  or   preg_match('/\-st\d$/i',$code)  or   preg_match('/\-pack$/i',$__code)  or    preg_match('/\-pst$/i',$__code)  or    preg_match('/\-kit2$/i',$__code)  or  preg_match('/\-kit1$/i',$__code)  or preg_match('/\-st$/i',$__code)  or     preg_match('/Bag-02Mx|Bag-04mx|Bag-05mx|Bag-06mix|Bag-07MX|Bag-12MX|Bag-13MX|FishP-Mix|IncIn-ST|IncB-St|LLP-ST|L\&P-ST|EO-XST|AWRP-ST/i',$__code) or       $__code=='eo-st' or $__code=='mol-st' or  $__code=='jbb-st' or $__code=='lwheat-st' or  $__code=='jbb-st'
