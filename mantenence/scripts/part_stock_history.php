...
$sql=sprintf('select `Part SKU`,`Part XHTML Currently Used In`  from `Part Dimension` where `Part Status`="In Use"   %s  order by `Part Total Sold Amount` desc ,`Part Valid From` desc  ',$where);
//print $sql;
$res=mysql_query($sql);
$count=0;
while($row=mysql_fetch_array($res)){
