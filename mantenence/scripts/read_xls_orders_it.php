...
$orders_array_full_path = glob("/mnt/z/Orders-Italy/*.xls");
...
foreach($orders_array_full_path as $key=>$order){
  ...
  $tmp=preg_replace('/.*rders-Italy\//i','',$tmp);
  $orders_array[]=$tmp;
}
...
foreach($orders_array as $order_index=>$order){
  if(preg_match('/^IT\-\d{5}r$|^IT\-\d{5}ref$|^IT\-\d{5}\s?refund$|^IT\-\d{5}rr$|^IT\-\d{5}ra$|^IT\-\d{5}r2$|^IT\-\d{5}\-2ref$|^IT\-\d{5}rpl$|^IT\-\d{5}sht?$|^IT\-\d{5}rfn$/i',$order)){
     $good_files[]=$orders_array_full_path[$order_index];
    $good_files_number[]=$order;
  }
...
foreach($good_files_number as $order_index=>$order){
  ...
  $sql=sprintf("select * from it_orders_data.orders where  `filename`=%s",prepare_mysql($filename));
  $result=mysql_query($sql);
  // print "$sql\n";
  if($row=mysql_fetch_array($result, MYSQL_ASSOC)){
    $sql=sprintf("update it_orders_data.orders set last_checked=NOW(),date=%s,timestamp=%d where id=%d",
	...
	$handle_csv = fopen($csv_file, "r");
	unlink($csv_file);
	$sql=sprintf("update it_orders_data.orders set last_read=NOW() where id=%d",$row['id']);
	mysql_query($sql);
	$updated=true;
	$id =$row['id'];
      }
    ...
    $sql=sprintf("insert into it_orders_data.orders (directory,filename,checksum,date,timestamp,last_checked,last_read) values (%s,%s,%s,%s,%s,NOW(),NOW())"
    ...
    $id =mysql_insert_id();
     ...
     $sql=sprintf("insert into it_orders_data.data (id) values (%d)"
		  ,$id
		 );
    mysql_query($sql);
    ...
    //$sql=sprintf("update it_orders_data.orders set filename_cvs=%s where id=%d",prepare_mysql($cvs_repo.$cvs_filename),$id);
   // mysql_query($sql);
    $updated=true;
  ...
  if($updated){
    ...
    $prod_map=$y_map;
      ...
      // print "Updating  $filename\n";
      $sql=sprintf("update it_orders_data.order_data set checksum_header=%s,checksum_prod=%s where id=%d"
		   ,prepare_mysql($checksum_header)
		   ,prepare_mysql($checksum_products)
		   ,$id);
      mysql_query($sql);
      $sql=sprintf("update it_orders_data.data set header=%s ,products=%s  where id=%d"
