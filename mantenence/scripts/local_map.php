...
function get_tipo_order($ltipo,$header){
 ...
 if(preg_match('/proforma/i',$ltipo)){
    ...
    $tipo=3;
    $header['notes2']=preg_replace('/^cancel order?$/i','',$header['notes2']);


  }elseif(preg_match('/^SAMPLE|muestras/i',$ltipo)){
    //    print_r($header);
    $header['notes']=preg_replace('/^samples?$/i','',$header['notes']);
    $header['notes2']=preg_replace('/^samples?$/i','',$header['notes2']);
    $tipo=4;
    }elseif(preg_match('/donation/i',$ltipo)){
    $tipo=5; 
  }elseif(preg_match('/^\s*REPLACEMENT|Replcement|^replace|Replacenment|^reemplazo por roturas|^replacement|Damaged item|REPLACMENT|DELIVERY COLLECTION|repplacements|Remplacement|repalcements|Repalcement|Replaceement/i',$ltipo)){
    $tipo=6;
