...
$days=0;
while($today<strtotime('now -2 day') and  $days<5000  ){
  ...
  while($row=mysql_fetch_array($result, MYSQL_ASSOC)   ){
    ...
    print "Part sku ".$row['Part SKU']." ".prepare_mysql(date("Y-m-d",$today))."\r";

     $amount_sold=0;
     $qty_inicio='NULL';
     $value_inicio='NULL';
     $sql=sprintf("select IFNULL(`Quantity on Hand`,'NULL'),IFNULL(`Value at Cost`,'NULL'),IFNULL(`Value at Cost`,'NULL')   from `Inventory Spanshot Fact` where `Snapshot Period`='Day' and Date=%s",date("Y-m-d",$yesterday));
     $result2=mysql_query($sql);
     if($row2=mysql_fetch_array($result2, MYSQL_ASSOC)   ){
      $qty_inicio=$row2['Quantity on Hand'];
       $value_inicio=$row2['Value At'];
     }
     //   print "$qty_inicio\n";
    $sql=sprintf("delete from  `Inventory Transition Fact` where  `Inventory Transaction Type`='Adjust' and `Part Sku`=%s  and `Date`=%s ",prepare_mysql($row['Part SKU']),prepare_mysql(date("Y-m-d",$today)));
    mysql_query($sql);


    $sql=sprintf("select * from `Inventory Transition Fact` where  `Part Sku`=%s  and `Date`=%s order by `Date`",prepare_mysql($row['Part SKU']),prepare_mysql(date("Y-m-d",$today)));
	...
	if(is_numeric($qty_inicio)){
	  ...
	  $sql=sprintf("insert into `Inventory Transition Fact` (`Date`,`Part SKU`,`Inventory Transaction Type`,`Inventory Transaction Quantity`,`Inventory Transaction Amount`) values (%s,%s,'In',%s,%s)",prepare_mysql($row2['Date']),prepare_mysql($part_sku),prepare_mysql($adjust_qty),prepare_mysql($adjust_amount),prepare_mysql($notes));
	  // print "$sql\n";
	  if(!mysql_query($sql))
	...
	if(is_numeric($value_inicio))
	  $qty_inicio+=$row2['Inventory Transaction Amount'];
