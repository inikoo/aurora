...
foreach($orders_array as $order_index=>$order){
  if(preg_match('/^\d{8}r$|^\d{8}ref$|^\d{8}\s?refund$|^\d{8}rr$|^\d{8}ra$|^\d{8}r2$|^\d{8}\-2ref$|^\d{8}rpl$|^\d{8}sht?$|^\d{8}rfn$/i',$order)){
     $good_files[]=$orders_array_full_path[$order_index];
    $good_files_number[]=$order;
  }
...
foreach($good_files_number as $order_index=>$order){
  ...
  $sql=sprintf("select * from ci_orders_data.orders where  `filename`=%s",prepare_mysql($filename));
  $result=mysql_query($sql);
  // print "$sql\n";
  if($row=mysql_fetch_array($result, MYSQL_ASSOC)){
    $sql=sprintf("update ci_orders_data.orders set last_checked=NOW(),date=%s,timestamp=%d where id=%d",
	...
	$handle_csv = fopen($csv_file, "r");
	unlink($csv_file);
	$sql=sprintf("update ci_orders_data.orders set last_read=NOW() where id=%d",$row['id']);
	mysql_query($sql);
	$updated=true;
	$id =$row['id'];
      }
    ...
    $sql=sprintf("insert into ci_orders_data.orders (directory,filename,checksum,date,timestamp,last_checked,last_read) values (%s,%s,%s,%s,%s,NOW(),NOW())"
    ...
    $id =mysql_insert_id();
     ...
     $sql=sprintf("insert into ci_orders_data.data (id) values (%d)"
		  ,$id
		 );
    mysql_query($sql);
    ...
    //$sql=sprintf("update ci_orders_data.orders set filename_cvs=%s where id=%d",prepare_mysql($cvs_repo.$cvs_filename),$id);
   // mysql_query($sql);
    $updated=true;
  ...
  if($updated){
    ...
    $prod_map=$y_map;
      ...
      // print "Updating  $filename\n";
      $sql=sprintf("update ci_orders_data.order_data set checksum_header=%s,checksum_prod=%s where id=%d"
		   ,prepare_mysql($checksum_header)
		   ,prepare_mysql($checksum_products)
		   ,$id);
      mysql_query($sql);
      $sql=sprintf("update ci_orders_data.data set header=%s ,products=%s  where id=%d"
