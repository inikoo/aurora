...
$view='subcategory';
$_SESSION['state']['product_categories']['edit']=$view;
...
if (isset($_REQUEST['store_id']) and is_numeric($_REQUEST['store_id']) ) {
	...
	if (count($user->stores)==0) {
		...
		header('Location: edit_product_categories.php?store_id='.$user->stores[0]);
		exit;
	}
}
...
$smarty->assign('show_history',$_SESSION['state']['product_categories']['show_history']);
...
$smarty->assign('title', _('Edit Product Categories'));
...
$tipo_filter=$_SESSION['state']['product_categories']['edit_categories']['f_field'];
$smarty->assign('filter0',$tipo_filter);
$smarty->assign('filter_value0',$_SESSION['state']['product_categories']['edit_categories']['f_value']);
...
$tipo_filter=$_SESSION['state']['product_categories']['history']['f_field'];
$smarty->assign('filter1',$tipo_filter);
$smarty->assign('filter_value1',$_SESSION['state']['product_categories']['history']['f_value']);
$filter_menu=array(
...
$sql=sprintf("select count(*) as num ,`Category Branch Type` from  `Category Dimension` where  `Category Store Key`=%d and `Category Subject`='Product' group by  `Category Branch Type`",
$store->id
);
//print_r($sql);
$res=mysql_query($sql);
...
$smarty->assign('elements',$_SESSION['state']['product_categories']['edit_categories']['elements']);
