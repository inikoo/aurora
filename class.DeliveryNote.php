...
class DeliveryNote extends DB_Table {
    ...
    */
    ...
    function update_xhtml_state() {
            ...
            if ($this->data['Delivery Note State'] == 'Ready to be Picked') {
                ...
                if ($this->data['Delivery Note State'] == 'Dispatched') {
                    ...
                    if ($this->data['Delivery Note State'] == 'Cancelled to Restock') {
                        ...
                        if ($this->data['Delivery Note State'] == 'Approved') {
                            $state = _('Ready to Ship');
                        } else {
                            if ($this->data['Delivery Note State'] == 'Cancelled') {
                                $state = _('Cancelled');
                            } else {
                                if ($this->data['Delivery Note State'] == 'Picking' and $this->data['Delivery Note Assigned Picker Alias'] == '') {
                                    $state = _('Picking');
                                } else {
                                    ...
                                    if ($this->data['Delivery Note Assigned Picker Alias']) {
                                        ...
                                        if ($this->data['Delivery Note Fraction Picked'] == 0) {
                                            ...
                                            if ($this->data['Delivery Note Date Start Picking'] == '') {
                                                ...
                                                $_tmp = _('Picking').' ('.percentage(
                                                        $this->data['Delivery Note Fraction Picked'], 1, 0
                                                    ).')';
                                            }
                                    ...
                                    if ($this->data['Delivery Note Assigned Packer Alias']) {
                                        ...
                                        if ($this->data['Delivery Note Fraction Packed'] == 0) {
                                            ...
                                            if ($this->data['Delivery Note Date Start Packing'] == '') {
                                                ...
                                                $_tmp = _('Packing').' ('.percentage(
                                                        $this->data['Delivery Note Fraction Packed'], 1, 0
                                                    ).')';
                                            }
        ...
        $sql                                     = sprintf(
            'UPDATE `Delivery Note Dimension` SET `Delivery Note XHTML State`=%s WHERE `Delivery Note Key`=%d  ', prepare_mysql($state), $this->id
        );
        mysql_query($sql);
    ...
    function update_order_transaction_after_create_dn($order) {
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            $sql = sprintf(
                ...
                , $this->data ['Delivery Note Key'], prepare_mysql(
                    $this->data ['Delivery Note Country 2 Alpha Code']
        ...
        foreach ($this->get_orders_objects() as $order_key => $order) {
            $sql = sprintf(
                "INSERT INTO `Order Delivery Note Bridge` VALUES (%d,%d)", $order_key, $this->id
            );
            // print "caca $sql\n";
            mysql_query($sql);
    ...
    function update_item_totals() {
        ...
        $distinct_items   = 0;
        $sql              = sprintf(
            'SELECT sum((`Order Bonus Quantity`+`Order Quantity`)*`Product Package Weight`) AS estimated_weight,count(*) AS num FROM `Order Transaction Fact` OTF LEFT JOIN `Product History Dimension` PH  ON (OTF.`Product Key`=PH.`Product Key`)  LEFT JOIN `Product Dimension` P  ON (PH.`Product ID`=P.`Product ID`)     WHERE `Delivery Note Key`=%d  ',
            $this->id
        );
        //    print "$sql\n";
        //  exit;
        ...
        $sql = sprintf(
            "UPDATE   `Delivery Note Dimension` SET `Delivery Note Distinct Items`=%d,`Delivery Note Estimated Weight`=%f WHERE `Delivery Note Key`=%d", $distinct_items, $estimated_weight, $this->id
        );
        mysql_query($sql);
    }
    ...
    function get_orders_ids() {
        $sql = sprintf(
            "SELECT `Order Key` FROM `Order Transaction Fact` WHERE `Delivery Note Key`=%d GROUP BY `Order Key`", $this->id
        );
    ...
    function update_xhtml_orders() {
        ...
        foreach ($this->get_orders_objects() as $order) {
            $this->data ['Delivery Note XHTML Orders'] .= sprintf(
                '%s <a href="order.php?id=%d">%s</a>,', $prefix, $order->data ['Order Key'], $order->data ['Order Public ID']
            );
        }
        $this->data ['Delivery Note XHTML Orders'] = _trim(
            preg_replace(
                '/\,$/', '', $this->data ['Delivery Note XHTML Orders']
            )
        );
        ...
        $sql = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note XHTML Orders`=%s WHERE `Delivery Note Key`=%d ", prepare_mysql($this->data['Delivery Note XHTML Orders']), $this->id
        );
        mysql_query($sql);
    }
    ...
    function get_formatted_parcels() {
        ...
        switch ($this->data['Delivery Note Parcel Type']) {
            case('Box'):
                $parcel_type = ngettext(
                    'box', 'boxes', $this->data['Delivery Note Number Parcels']
                );
                break;
            case('Pallet'):
                $parcel_type = ngettext(
                    'pallet', 'pallets', $this->data['Delivery Note Number Parcels']
                );
                break;
            case('Envelope'):
                $parcel_type = ngettext(
                    'envelope', 'envelopes', $this->data['Delivery Note Number Parcels']
                );
                break;
            case('Small Parcel'):
                $parcel_type = ngettext(
                    'small parcel', 'small parcels', $this->data['Delivery Note Number Parcels']
                );
                break;
            case('Other'):
                $parcel_type = ngettext(
                    'container (other)', 'containers (other)', $this->data['Delivery Note Number Parcels']
                );
                break;
    ...
    function get_invoices_ids() {
        $invoices = array();
        $sql      = sprintf(
            "SELECT `Invoice Key` FROM `Order Transaction Fact` WHERE `Delivery Note Key`=%d GROUP BY `Invoice Key`", $this->id
        );
        ...
        $sql = sprintf(
            "SELECT `Refund Key` FROM `Order Transaction Fact` WHERE `Delivery Note Key`=%d GROUP BY `Refund Key`", $this->id
        );
        $res = mysql_query($sql);
        while ($row = mysql_fetch_array($res, MYSQL_ASSOC)) {
            if ($row['Refund Key']) {
    ...
    function set_weight($weight) {
        ...
        if (is_numeric($weight) and $weight >= 0) {
            ...
            $this->update_field(
                'Delivery Note Weight', $this->data['Delivery Note Estimated Weight']
            );
    ...
    function update_xhtml_invoices() {
        ...
        foreach ($this->get_invoices_objects() as $invoice) {
            ...
            if ($invoice->get('Invoice Paid') == 'Yes') {
                ...
                $state
                    = '<img src="art/icons/money_bw.png" style="width:14px">';
            }

            $this->data ['Delivery Note XHTML Invoices'] .= sprintf(
                ...
                $state, $invoice->data ['Invoice Key'],

                $prefix, $invoice->data ['Invoice Public ID'], $invoice->data ['Invoice Key']
            );
        }
        $this->data ['Delivery Note XHTML Invoices'] = _trim(
            preg_replace(
                '/\<br\/\>$/', '', $this->data ['Delivery Note XHTML Invoices']
            )
        );
        $sql                                         = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note XHTML Invoices`=%s WHERE `Delivery Note Key`=%d ", prepare_mysql($this->data['Delivery Note XHTML Invoices']), $this->id
        );
        mysql_query($sql);
    ...
    function create_orphan_inventory_transaction_fact($date) {
        $skus_data = array();
        $sql       = sprintf(
            'SELECT  OTF.`Product Code`,OTF.`Product Key`,`Product Package Weight`,`Delivery Note Quantity`,`Order Transaction Fact Key` FROM `Order Transaction Fact` OTF LEFT JOIN `Product History Dimension` PH  ON (OTF.`Product Key`=PH.`Product Key`)  LEFT JOIN `Product Dimension` P  ON (PH.`Product ID`=P.`Product ID`)     WHERE `Current Dispatching State` IN ("Submitted by Customer","In Process") AND `Delivery Note Key`=%d ',
            $this->id
        );
        $res       = mysql_query($sql);
        while ($row = mysql_fetch_assoc($res)) {
            ...
            $part_index                  = 0;
            $multipart_data              = sprintf(
                '<a href="product.php?id=%d">%s</a>', $row['Product Key'], $row['Product Code']
            );
            ...
            foreach ($part_list as $part_data) {
                ...
                if (count($supplier_products) > 0) {
                    $supplier_products_rnd_key = array_rand(
                        $supplier_products, 1
                    );
                    $supplier_products_keys    = preg_split(
                        '/,/', $supplier_products[$supplier_products_rnd_key]['Supplier Product Keys']
                    );
                    $supplier_product_key
                                               = $supplier_products_keys[array_rand(
                        $supplier_products_keys
                    )];
                    $supplier_key
                                               = $supplier_products[$supplier_products_rnd_key]['Supplier Key'];
                    $supplier_product_id
                                               = $supplier_products[$supplier_products_rnd_key]['Supplier Product ID'];
                }

                $product = new product ($row ['Product Key']);

                $quantity_to_be_taken = $part_data['Parts Per Product'] * $row ['Delivery Note Quantity'];
                $locations            = $part->get_picking_location_key(
                    $date, $quantity_to_be_taken
                );

                $a = sprintf(
                    '<a href="product.php?id=%d">%s</a> <a href="dn.php?id=%d">%s</a>', $product->id, $product->code, $this->id, $this->data['Delivery Note ID']
                );
                ...
                foreach ($locations as $location_data) {
                    ...
                    if ($location_key) {
                        if ($location_key == 1) {
                            $a .= ' '._('Taken from an')." ".sprintf(
                                    "<a href='location.php?id=1'>%s</a>", _('Unknown Location')
                                );
                        } else {
                            $location = new Location($location_key);
                            $a .= ' '._('Taken from').": ".sprintf(
                                    "<a href='location.php?id=%d'>%s</a>", $location->id, $location->data['Location Code']
                                );
                        }
                    ...
                    $sql = sprintf(
                        "INSERT INTO `Inventory Transaction Fact`  (`Map To Order Transaction Fact Parts Multiplicity`,`Map To Order Transaction Fact XHTML Info`,`Inventory Transaction Record Type`,`Inventory Transaction Section`,`Dispatch Country Code`,`Picking Note`,`Inventory Transaction Weight`,`Date Created`,`Date`,`Delivery Note Key`,`Part SKU`,`Location Key`,`Inventory Transaction Quantity`,`Inventory Transaction Type`,`Inventory Transaction Amount`,`Required`,`Given`,`Amount In`,`Metadata`,`Note`,`Supplier Product ID`,`Supplier Product Historic Key`,`Supplier Key`,`Map To Order Transaction Fact Key`,`Map To Order Transaction Fact Metadata`) VALUES
					(%d,%s,%s,%s,%s,%s,%f,%s,%s,%d,%s,%d,%s,%s,%.2f,%f,%f,%f,%s,%s,%d,%d,%d,%d,%s) ", $multipart_data_multiplicity, prepare_mysql($multipart_data), "'Movement'", "'OIP'",
                        prepare_mysql(
                            $this->data['Delivery Note Country Code']
                        ), prepare_mysql($part_data['Product Part List Note']), 0, prepare_mysql($date), prepare_mysql($date), $this->id, prepare_mysql($part_data['Part SKU']), $location_key, 0,
                        "'Order In Process'", 0, $quantity_taken_from_location, 0, 0, prepare_mysql($this->data ['Delivery Note Metadata']), prepare_mysql($note), $supplier_product_id,
                        $supplier_product_key, $supplier_key, $map_to_otf_key, prepare_mysql(
                            prepare_mysql(
                                $part_index.';'.$part_data['Parts Per Product'].';'.$location_index
                            )
                        )
                    );
                    mysql_query($sql);
                    ...
                    $part_location = new PartLocation(
                        $part_data['Part SKU'].'_'.$location_key
                    );
    ...
    function create_post_order_inventory_transaction_fact($order_key, $date) {
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            $state               = 'In Warehouse';
            $sql                 = sprintf(
                "UPDATE `Order Post Transaction Dimension` SET `State`=%s ,`Delivery Note Key`=%d WHERE `Order Post Transaction Key`=%d  ", prepare_mysql($state), $this->id,
            ...
            foreach ($part_list as $part_data) {
                ...
                foreach ($locations as $location_data) {
					...
					(%d,%s,%s,%s,%s,%f,%s,%s,%d,%s,%d,%s,%s,%.2f,%f,%f,%f,%s,%s,%d,%d,%d,%d,%s) ", $multipart_data_multiplicity, prepare_mysql($multipart_data), "'Movement'", "'OIP'", prepare_mysql(
                            $this->data['Delivery Note Country Code']
                        ), // prepare_mysql ($part_data['Product Part List Note']),
                        0, prepare_mysql($date), prepare_mysql($date), $this->id, prepare_mysql($part_data['Part SKU']), $location_key, 0, "'Order In Process'", 0, $quantity_taken_from_location, 0, 0,
                        prepare_mysql($this->data ['Delivery Note Metadata']), prepare_mysql($note), $supplier_product_id, $supplier_product_key, $supplier_key, $map_to_otf_key, prepare_mysql(
                            $part_index.';'.$part_data['Parts Per Product'].';'.$location_index
                        )
                    );
                    //print $sql;
    ...
    function actualize_inventory_transaction_facts($actualization_date = false) {
        ...
        $last_used_index        = 0;
        $sql                    = sprintf(
            "SELECT * FROM `Inventory Transaction Fact` WHERE `Delivery Note Key`=%d  ", $this->id
        );
        $res                    = mysql_query($sql);
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            $metadata = preg_split(
                '/;/', $row['Map To Order Transaction Fact Metadata']
            );
            ...
            if ($to_pick == 0) {
                ...
                if ($row['Picked'] > 0) {
                    $sql = sprintf(
                        "UPDATE `Inventory Transaction Fact`  SET `Required`=%d  WHERE `Inventory Transaction Key`=%d ", $row['Required'] - $to_pick, $row['Inventory Transaction Key']
                    );
                    mysql_query($sql);
                } else {
                    $sql = sprintf(
                        "DELETE FROM `Inventory Transaction Fact`  WHERE `Inventory Transaction Key`=%d ", $row['Inventory Transaction Key']
                    );
                    mysql_query($sql);
            ...
            if ($to_pick > 0) {
                ...
                $sql         = sprintf(
                    "SELECT `Product Key` FROM `Order Transaction Fact` WHERE `Order Transaction Fact Key`=%d ", $row['Map To Order Transaction Fact Key']
                );
                ...
                $inventory_to_actualize[$row['Map To Order Transaction Fact Key']][$row['Part SKU']]
                    = $transaction_data;
        ...
        foreach ($inventory_to_actualize as $otf => $transactions_parts) {
            ...
            foreach ($transactions_parts as $part_sku => $transaction_locations) {
                ...
                $locations = $part->get_picking_location_key(
                    $actualization_date, $transaction_locations['qty']
                );
                // continue;

                $product        = new Product(
                    $transaction_locations['product_key']
                );
                $picking_note   = $transaction_locations['picking_note'];
                $map_to_otf_key = $transaction_locations['otf'];
                $parts_per_product
                                = $transaction_locations['parts_per_product'];
                ...
                foreach ($locations as $location_data) {
                    ...
                    if ($location_key) {
                        if ($location_key == 1) {
                            $a .= ' '._('To be taken from an')." ".sprintf(
                                    "<a href='location.php?id=1'>%s</a>", _('Unknown Location')
                                );
                        } else {
                            $location = new Location($location_key);
                            $a .= ' '._('To be taken from').": ".sprintf(
                                    "<a href='location.php?id=%d'>%s</a>", $location->id, $location->data['Location Code']
                                );
                        }
                    ...
                    $sql = sprintf(
                        ...
                        $transaction_locations['parts_multiplicity'], prepare_mysql($transaction_locations['otf_info']), "'Movement'", "'OIP'", prepare_mysql(
                            $this->data['Delivery Note Country Code']
                        ), prepare_mysql($picking_note), 0, prepare_mysql($date), prepare_mysql($date), $this->id, prepare_mysql($part->sku), $location_key, 0, "'Order In Process'", 0,
                        $quantity_taken_from_location, 0, 0, prepare_mysql($this->data ['Delivery Note Metadata']), prepare_mysql($note), $supplier_product_id, $supplier_product_key, $supplier_key,
                        $map_to_otf_key, prepare_mysql(
                            $part_index.';'.$parts_per_product.';'.$location_index
                        )
                    );
                    mysql_query($sql);
                    //print "a $sql\n\n\n";
    ...
    function update_inventory_transaction_fact($otf_key, $quantity, $date = false) {
        ...
        $sql = sprintf(
            "SELECT `Inventory Transaction Key` FROM `Inventory Transaction Fact` WHERE `Delivery Note Key`=%d AND `Map To Order Transaction Fact Key`=%d ", $this->id, $otf_key
        );
        ...
        if ($row = mysql_fetch_assoc($res)) {
            ...
            $sql = sprintf(
                "UPDATE `Inventory Transaction Fact` SET `Required`=%f WHERE `Inventory Transaction Key`=%d ", $quantity, $row['Inventory Transaction Key']
            ...
            $sql = sprintf(
                'SELECT `No Shipped Due No Authorized`,OTF.`Product Key`,`Product Package Weight`,`Order Bonus Quantity`,`Order Quantity`,`Supplier Metadata`,`Order Bonus Quantity`,`Order Transaction Fact Key` FROM `Order Transaction Fact` OTF LEFT JOIN `Product History Dimension` PH  ON (OTF.`Product Key`=PH.`Product Key`)  LEFT JOIN `Product Dimension` P  ON (PH.`Product ID`=P.`Product ID`)     WHERE `Order Transaction Fact Key`=%d ',
                $otf_key
            );
            $res = mysql_query($sql);
            ...
            while ($row = mysql_fetch_assoc($res)) {
                ...
                $this->create_inventory_transaction_fact_item(
                    $row['Product Key'], $row['Order Transaction Fact Key'], $row['Order Quantity'] + $row['Order Bonus Quantity'] - $row['No Shipped Due No Authorized'], $date,
    ...
    function create_inventory_transaction_fact_item($product_key, $map_to_otf_key, $to_sell_quantity, $date, $supplier_metadata_array, $bonus_qty) {
        ...
        $multipart_data              = sprintf(
            '<a href="product.php?id=%d">%s</a>', $product->id, $product->data['Product Code']
        );
        $multipart_data_multiplicity = count($part_list);
        ...
        foreach ($part_list as $part_data) {
            ...
            if ($part->sku) {
                ...
                if (array_key_exists($part->sku, $supplier_metadata) and $supplier_metadata[$part->sku]) {
                    ...
                    $supplier_product_id
                        = $supplier_metadata[$part->sku]['supplier_product_pid'];
                    $supplier_product_key
                        = $supplier_metadata[$part->sku]['supplier_product_key'];
                    $supplier_key
                        = $supplier_metadata[$part->sku]['supplier_key'];
                ...
                foreach ($locations as $location_data) {
					...
					%s,%s,%d,%d,%d,%d,%s) ", $multipart_data_multiplicity, prepare_mysql($multipart_data), "'Movement'", "'OIP'", prepare_mysql(
                            $this->data['Delivery Note Country Code']
                        ), prepare_mysql($picking_note),

                        0, prepare_mysql($date), prepare_mysql($date), $this->id, prepare_mysql($part_data['Part SKU']), $location_key,
    ...
    function create_inventory_transaction_fact($order_key, $date = false, $extra_data = false) {
        ...
        $sql = sprintf(
            'SELECT OTF.`Product Code`,OTF.`Order Quantity`,`No Shipped Due No Authorized`,OTF.`Product Key`,`Product Package Weight`,`Order Quantity`,`Supplier Metadata`,`Order Bonus Quantity`,`Order Transaction Fact Key` FROM `Order Transaction Fact` OTF LEFT JOIN `Product History Dimension` PH  ON (OTF.`Product Key`=PH.`Product Key`)  LEFT JOIN `Product Dimension` P  ON (PH.`Product ID`=P.`Product ID`)
		WHERE `Order Key`=%d  AND `Current Dispatching State` IN ("Submitted by Customer","In Process","In Process by Customer")  ', $order_key
        );
        $res = mysql_query($sql);
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            $this->create_inventory_transaction_fact_item(
                $row['Product Key'], $row['Order Transaction Fact Key'], $items_to_dispatch, $date, $row['Supplier Metadata'], $row['Order Bonus Quantity']
            );
    ...
    function undo_dispatch() {
        ...
        $sql = sprintf(
            "DELETE FROM  `Inventory Transaction Fact` WHERE `Inventory Transaction Record Type`=%s AND `Inventory Transaction Section`=%s AND `Inventory Transaction Type`=%s AND `Delivery Note Key`=%d",
            "'Movement'", "'Audit'", "'Adjust'", $this->id

        );
        mysql_query($sql);

        $sql = sprintf(
            "UPDATE `Inventory Transaction Fact` SET `Inventory Transaction Type` = 'Order In Process',`Inventory Transaction Section`='OIP' WHERE `Delivery Note Key`=%d AND `Inventory Transaction Type` = 'No Dispatched' AND `Inventory Transaction Section`='NoDispatched'  ",
        ...
        $sql = sprintf(
            "UPDATE `Inventory Transaction Fact` SET `Inventory Transaction Type` = 'Order In Process',`Inventory Transaction Section`='OIP' WHERE `Delivery Note Key`=%d AND `Inventory Transaction Type` = 'Sale' AND `Inventory Transaction Section`='Out'  ",
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            if ($this->update_stock) {
                $part_location = new PartLocation(
                    $row['Part SKU'].'_'.$row['Location Key']
                );
                $part_location->update_stock();
            }
        ...
        $sql = sprintf(
            "SELECT `Delivery Note Quantity`,`Order Transaction Fact Key` FROM `Order Transaction Fact` WHERE `Delivery Note Key`=%s  AND `Current Dispatching State`='Dispatched'  ", $this->id
        );
        ...
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            ...
            $sql = sprintf(
                "UPDATE  `Order Transaction Fact` SET `Actual Shipping Date`=NULL,`Shipped Quantity`=0, `Current Dispatching State`=%s WHERE   `Order Transaction Fact Key`=%d",
                prepare_mysql('Ready to Ship'), $row['Order Transaction Fact Key']
            );
            mysql_query($sql);
            //print "$sql\n";
        }
        ...
        if (in_array(
            ...
            $sql = sprintf(
                "UPDATE `Order Post Transaction Dimension` SET `State`=%s  WHERE `Delivery Note Key`=%d   ", prepare_mysql('In Warehouse'), $this->id
            );
            mysql_query($sql);
        }
        ...
        $this->data['Delivery Note Date']
                                           = $this->data['Delivery Note Date Dispatched Approved'];

        $sql = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note State`=%s,`Delivery Note Date`=%s WHERE `Delivery Note Key`=%d", prepare_mysql($this->data['Delivery Note State']),
            prepare_mysql($this->data['Delivery Note Date']), $this->id
        );
        mysql_query($sql);
    ...
    function dispatch($data) {
        ...
        $customer              = new Customer(
            $this->data['Delivery Note Customer Key']
        );
        $numbers_of_times_used = 0;
        $sql                   = sprintf(
            'SELECT count(*) AS num FROM `Delivery Note Dimension` WHERE `Delivery Note Customer Key`=%d AND `Delivery Note Ship To Key`=%d', $customer->id, $this->data['Delivery Note Ship To Key']
        );
        $res2                  = mysql_query($sql);
        if ($row2 = mysql_fetch_assoc($res2)) {
        ...
        $sql = sprintf(
            'SELECT `Ship To From Date`,`Ship To Last Used` FROM `Customer Ship To Bridge` WHERE `Customer Key`=%d AND `Ship To Key`=%d', $customer->id, $this->data['Delivery Note Ship To Key']
        );
        ...
        if (strtotime($this->data['Delivery Note Date']) < strtotime(
                $from_date
            )
        ) {
        ...
        $sql = sprintf(
            'UPDATE `Customer Ship To Bridge` SET `Times Used`=%d ,`Ship To From Date`=%s,`Ship To Last Used`=%s  WHERE `Customer Key`=%d AND `Ship To Key`=%d', $numbers_of_times_used,
            prepare_mysql($from_date), prepare_mysql($to_date),

            $customer->id, $this->data['Delivery Note Ship To Key']
        );
        mysql_query($sql);
    ...
    private function handle_to_customer($data) {
        ...
        while ($row = mysql_fetch_assoc($res)) {
            ...
            if ($row['Out of Stock'] > 0) {
                ...
                $note = _('Out of Stock');
                $sql  = sprintf(
                    "INSERT INTO `Inventory Transaction Fact`  (`Inventory Transaction Record Type`,`Inventory Transaction Section`,`Dispatch Country Code`,`Inventory Transaction Weight`,`Date Created`,`Date`,`Delivery Note Key`,`Part SKU`,`Location Key`,`Inventory Transaction Quantity`,`Inventory Transaction Type`,`Inventory Transaction Amount`,`Required`,`Given`,`Amount In`,`Metadata`,`Note`,`Supplier Product ID`) VALUES (%s,%s,%s,%f,%s,%s,%d,%s,%d,%s,%s,%.2f,%f,%f,%f,%s,%s,%s) ",
                    "'Movement'", "'Audit'", prepare_mysql($this->data['Delivery Note Country Code']), 0, prepare_mysql(
                        $this->data['Delivery Note Date Finish Picking']
                    ), prepare_mysql(
                        $this->data['Delivery Note Date Finish Picking']
                    ), $this->id, $row['Part SKU'], $row['Location Key'], 0, "'Adjust'", 0, 0, 0, 0, prepare_mysql($row['Metadata']), prepare_mysql($note), $row['Supplier Product ID']
            ...
            if ($row['Inventory Transaction Quantity'] == 0) {
                $sql = sprintf(
                    "UPDATE `Inventory Transaction Fact` SET `Inventory Transaction Type` = 'No Dispatched',`Inventory Transaction Section`='NoDispatched' WHERE `Delivery Note Key`=%d  AND `Inventory Transaction Key`=%d  ",
                    $this->id, $row['Inventory Transaction Key']
                ...
                $sql = sprintf(
                    "UPDATE `Inventory Transaction Fact` SET `Date Shipped`=%s,`Inventory Transaction Type` = 'Sale',`Inventory Transaction Section`='Out' WHERE `Delivery Note Key`=%d  AND `Inventory Transaction Key`=%d  ",
        ...
        $sql = sprintf(
            "SELECT `Delivery Note Quantity`,`Order Transaction Fact Key` FROM `Order Transaction Fact` WHERE `Delivery Note Key`=%s  AND `Current Dispatching State`='Ready to Ship'  ", $this->id
        );
        ...
        while ($row = mysql_fetch_array($result, MYSQL_ASSOC)) {
            ...
            $sql = sprintf(
                "UPDATE  `Order Transaction Fact` SET `Actual Shipping Date`=%s,`Shipped Quantity`=%f, `Current Dispatching State`=%s WHERE   `Order Transaction Fact Key`=%d",
                prepare_mysql($data['Delivery Note Date']), $row['Delivery Note Quantity'], prepare_mysql('Dispatched'), $row['Order Transaction Fact Key']
            );
            mysql_query($sql);
            //print "$sql\n";
        }
    ...
    function set_parcels($parcels, $parcel_type = 'Box') {
        ...
        if (is_numeric($parcels)) {
            $sql                                        = sprintf(
                "UPDATE `Delivery Note Dimension` SET `Delivery Note Number Parcels`=%d,`Delivery Note Parcel Type`=%s WHERE `Delivery Note Key`=%d", $parcels, prepare_mysql($parcel_type), $this->id
            );
            $this->data['Delivery Note Number Parcels'] = $parcels;
        } else {
            $sql                                        = sprintf(
                "UPDATE `Delivery Note Dimension` SET `Delivery Note Number Parcels`=NULL,`Delivery Note Parcel Type`=%s WHERE `Delivery Note Key`=%d"

                , prepare_mysql($parcel_type), $this->id
            );
    ...
    function cancel($note = '', $date = false, $force = false) {
        ...
        if (preg_match('/Dispatched/', $this->data ['Delivery Note State'])) {
            $this->msg = _(
                'Delivery Note can not be cancelled,because has already been dispatched'
            );
        ...
        if ($this->data ['Delivery Note State'] == 'Cancelled') {
            ...
            if (preg_match(
                    '/Ready to be Picked/', $this->data ['Delivery Note State']
                ) or $force
            ) {
            ...
            $sql = sprintf(
                "UPDATE `Delivery Note Dimension` SET `Delivery Note State`=%s ,`Delivery Note Dispatch Method`=%s WHERE `Delivery Note Key`=%d", prepare_mysql($this->data ['Delivery Note State']),
    ...
    function delete() {
        ...
        if (in_array(
            ...
            $sql = sprintf(
                "DELETE FROM `Order Transaction Fact` WHERE `Delivery Note Key`=%d AND `Order Transaction Type`='Resend'", $this->id
            );
            mysql_query($sql);
    ...
    function suspend($note = '', $date = false) {
        ...
        if (preg_match('/Dispatched/', $this->data ['Delivery Note State'])) {
            $this->msg = _(
                'Delivery Note can not be suspended, because has already been dispatched'
            );

            return;
        } elseif (preg_match(
            '/Suspended/', $this->data ['Delivery Note State']
        )) {
            $this->_('Order is already suspended');

            return;
        } elseif (preg_match(
            '/Cancelled/', $this->data ['Delivery Note State']
        )) {
            ...
            $sql = sprintf(
                "UPDATE `Delivery Note Dimension` SET `Delivery Note State`=%s ,`Delivery Note Dispatch Method`=%s WHERE `Delivery Note Key`=%d", prepare_mysql($this->data ['Delivery Note State']),
                prepare_mysql($this->data ['Delivery Note Dispatch Method']), $this->id
            );
            mysql_query($sql);
            $this->suspended = true;
    ...
    function assign_picker($staff_key) {
        ...
        if (!preg_match(
            '/^(Ready to be Picked|Picker Assigned|Picking)$/', $this->data ['Delivery Note State']
        )
        ) {
            $this->error = true;
            $this->msg   = $this->data ['Delivery Note State'].' '._(
                    'Delivery Note can not be assigned to a picker,because has already been picked'
                );
        ...
        $this->data ['Delivery Note Assigned Picker Alias']
                                                          = $staff->data['Staff Alias'];
        $this->data ['Delivery Note XHTML Pickers']       = sprintf(
            '<a href="staff.php?id=%d">%s</a>', $staff->id, ucfirst($staff->data['Staff Name'])
        );
        $this->data ['Delivery Note Number Pickers']      = 1;
        $sql                                              = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note Number Pickers`=%d,`Delivery Note XHTML Pickers`=%s,`Delivery Note State`=%s ,`Delivery Note Assigned Picker Key`=%d ,`Delivery Note Assigned Picker Alias`=%s WHERE `Delivery Note Key`=%d",
            $this->data ['Delivery Note Number Pickers'], prepare_mysql($this->data ['Delivery Note XHTML Pickers']), prepare_mysql($this->data ['Delivery Note State']),
            $this->data ['Delivery Note Assigned Picker Key'], prepare_mysql($this->data ['Delivery Note Assigned Picker Alias']), $this->id
        );
        mysql_query($sql);
        //print $sql;
    ...
    function update_state($state) {
        ...
        $sql                               = sprintf(
            'UPDATE `Delivery Note Dimension` SET `Delivery Note State`=%s WHERE `Delivery Note Key`=%d  ', prepare_mysql($state), $this->id
        );
    ...
    function get_state() {
        ...
        } elseif ($this->data['Delivery Note Assigned Picker Alias'] and $this->data['Delivery Note Assigned Packer Alias'] and $this->data['Delivery Note Fraction Picked'] == 0
            and $this->data['Delivery Note Fraction Packed'] == 0
        ) {
    ...
    function assign_packer($staff_key, $force = false) {
        ...
        if (preg_match(
                '/^(Picked|Packer Assigned|Picking \& Packer Assigned|Picking)$/', $this->data ['Delivery Note State']
            ) or $force
        ) {

        } else {
            if (preg_match(
                '/^(Ready to be Picked|Picker Assigned)$/', $this->data ['Delivery Note State']
            )) {
                $this->error = true;
                $this->msg   = $this->data ['Delivery Note State'].' '._(
                        'Delivery Note can not be assigned to a packer,because has not been picked'
                    );

                return;
            } elseif (preg_match(
                '/^(Packed|Packing|Picking \& Packing)$/', $this->data ['Delivery Note State']
            )) {
                $this->error = true;
                $this->msg   = $this->data ['Delivery Note State'].' '._(
                        'Packer has been already assigned'
                    );
                ...
                $this->error = true;
                $this->msg   = $this->data ['Delivery Note State'].'; '._(
                        'Delivery Note has been already packed'
                    );
        ...
        $this->data ['Delivery Note Assigned Packer Alias']
                                                          = $staff->data['Staff Alias'];
        $this->data ['Delivery Note XHTML Packers']       = sprintf(
            '<a href="staff.php?id=%d">%s</a>', $staff->id, ucfirst($staff->data['Staff Name'])
        );
        ...
        $sql = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note Number Packers`=%d,`Delivery Note XHTML Packers`=%s ,`Delivery Note Assigned Packer Key`=%d ,`Delivery Note Assigned Packer Alias`=%s WHERE `Delivery Note Key`=%d",
            $this->data ['Delivery Note Number Packers'], prepare_mysql($this->data ['Delivery Note XHTML Packers']), $this->data ['Delivery Note Assigned Packer Key'],
    ...
    function start_picking($staff_key, $date = false) {
        ...
        if (!$staff_key) {
            ...
            $staff_key     = $staff->id;
            $xhtml_pickers = sprintf(
                '<a href="staff.php?id=%d">%s</a>', $staff_key, ucfirst($staff_alias)
            );
        ...
        $sql    = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note Date Start Picking`=%s,`Delivery Note State`=%s ,`Delivery Note XHTML Pickers`=%s ,`Delivery Note Number Pickers`=%d ,`Delivery Note Assigned Picker Key`=%s,`Delivery Note Assigned Picker Alias`=%s WHERE `Delivery Note Key`=%d",
            prepare_mysql($date), prepare_mysql($this->data ['Delivery Note State']), prepare_mysql($this->data ['Delivery Note XHTML Pickers']), $this->data ['Delivery Note Number Pickers'],
            prepare_mysql($staff_key), prepare_mysql($staff_alias, false), $this->id
        );
        $result = mysql_query($sql);
        // print $sql;
        $this->get_data('id', $this->id);
        ...
        $sql = sprintf(
            "UPDATE `Order Transaction Fact` SET `Start Picking Date`=%s  WHERE `Delivery Note Key`=%d", prepare_mysql($date), $this->id
        );
        mysql_query($sql);
    ...
    function update_picking_percentage($date = false) {
        ...
        $sql = sprintf(
            'UPDATE `Delivery Note Dimension` SET `Delivery Note Date Finish Picking`=%s ,`Delivery Note Fraction Picked`=%f WHERE `Delivery Note Key`=%d  ', prepare_mysql($finish_picking_date),
            $percentage_picked, $this->id
        );
        mysql_query($sql);
    ...
    function get_picking_percentage() {
        $sql             = sprintf(
            "SELECT `Required`,`Not Found`,`No Picked Other`,`Out of Stock`,ifnull(`Part Package Weight`,0) AS `Part Package Weight`,`Picked` ,`Given`,`Inventory Transaction Quantity` FROM   `Inventory Transaction Fact` ITF           LEFT JOIN `Part Dimension` P ON (P.`Part SKU`=ITF.`Part SKU`) WHERE `Delivery Note Key`=%d  ",
            $this->id
    ...
    function start_packing($staff_key, $date = false) {
        ...
        if (!preg_match(
            '/^(Picked|Picking|Packer Assigned|Picking & Packer Assigned)$/', $this->data ['Delivery Note State']
        )
        ) {
            $this->error = true;
            $this->msg   = $this->data ['Delivery Note State'].'<'._(
                    'Delivery Note can not be assigned to a packer,because is been packed'
                );
        ...
        $sql = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note Date Start Packing`=%s,`Delivery Note State`=%s ,`Delivery Note XHTML Packers`=%s ,`Delivery Note Number Packers`=%d ,`Delivery Note Assigned Packer Key`=%s,`Delivery Note Assigned Packer Alias`=%s WHERE `Delivery Note Key`=%d",
            prepare_mysql($date), prepare_mysql($this->data ['Delivery Note State']), prepare_mysql($this->data ['Delivery Note XHTML Packers']), $this->data ['Delivery Note Number Packers'],
            prepare_mysql($staff_key), prepare_mysql($staff_alias, false), $this->id
        );
        // print $sql;
        mysql_query($sql);
        ...
        $sql = sprintf(
            "UPDATE `Order Transaction Fact` SET `Start Packing Date`=%s  WHERE `Delivery Note Key`=%d", prepare_mysql($date), $this->id
        );
        //  print $sql;
        mysql_query($sql);
    ...
    function update_packing_percentage($date = false) {
        ...
        $sql = sprintf(
            'UPDATE `Delivery Note Dimension` SET `Delivery Note Date Finish Packing`=%s ,`Delivery Note Fraction Packed`=%f WHERE `Delivery Note Key`=%d  ', prepare_mysql($finish_packing_date),
            $percentage_packed, $this->id
        );
        mysql_query($sql);
    ...
    function get_packing_percentage() {
        $sql             = sprintf(
            "SELECT `Required`,`Out of Stock`,ifnull(`Part Package Weight`,0) AS `Part Package Weight`,`Not Found`,`No Picked Other`,`Packed` ,`Given`,`Inventory Transaction Quantity` FROM   `Inventory Transaction Fact` ITF   LEFT JOIN `Part Dimension` P ON (P.`Part SKU`=ITF.`Part SKU`) WHERE `Delivery Note Key`=%d  ",
            $this->id
    ...
    function get_number_transactions() {
        ...
        $sql = sprintf(
            "SELECT count(*) AS number FROM   `Inventory Transaction Fact` ITF        WHERE `Delivery Note Key`=%d ", $this->id
    ...
    function get_number_picked_transactions() {
        ...
        $sql = sprintf(
            "SELECT count(*) AS number FROM   `Inventory Transaction Fact` ITF        WHERE `Delivery Note Key`=%d AND (`Given`+`Required`=`Out of Stock`+`Picked`+`Not Found`+`No Picked Other`) ",
            $this->id
    ...
    function get_number_packed_transactions() {
        ...
        $sql = sprintf(
            "SELECT count(*) AS number FROM   `Inventory Transaction Fact` ITF        WHERE `Delivery Note Key`=%d AND (`Picked`=`Packed` AND `Picked`>0) ", $this->id
    ...
    function approve_packed($date = false) {
        ...
        if (!$date) {
            $this->data['Delivery Note Date Done Approved'] = gmdate(
                'Y-m-d H:i:s'
            );
        } else {
        ...
        $sql = sprintf(
            'UPDATE `Delivery Note Dimension` SET `Delivery Note State`=%s,`Delivery Note Approved Done`="Yes" ,`Delivery Note Date Done Approved`=%s WHERE `Delivery Note Key`=%d',
            prepare_mysql($this->data['Delivery Note State']), prepare_mysql($this->data['Delivery Note Date Done Approved']), $this->id
        );
        mysql_query($sql);

        $sql = sprintf(
            'UPDATE `Order Transaction Fact` SET `Current Dispatching State`="Packed Done"  WHERE `Delivery Note Key`=%d AND `Current Dispatching State`="Packed"'
        ...
        foreach ($this->get_orders_objects() as $order) {
            ...
            $order->update_xhtml_delivery_notes();
            $order->update(
                array('Order Packed Done Date' => $this->data['Delivery Note Date Done Approved'])
            );
        }
        foreach ($this->get_invoices_objects() as $invoice) {
            $invoice->update_xhtml_delivery_notes();
    ...
    function approved_for_shipping($date = false) {
        ...
        if (!$date) {
            $this->data['Delivery Note Date Dispatched Approved'] = gmdate(
                'Y-m-d H:i:s'
            );
        } else {
        ...
        $sql = sprintf(
            'UPDATE `Delivery Note Dimension` SET `Delivery Note State`=%s,`Delivery Note Approved To Dispatch`="Yes" ,`Delivery Note Date Dispatched Approved`=%s WHERE `Delivery Note Key`=%d',
            prepare_mysql($this->data['Delivery Note State']), prepare_mysql($this->data['Delivery Note Date Dispatched Approved']), $this->id
        );
        mysql_query($sql);

        $sql = sprintf(
            'UPDATE `Order Transaction Fact` SET `Current Dispatching State`="Ready to Ship"  WHERE `Delivery Note Key`=%d AND `Current Dispatching State`="Packed Done"'
    ...
    function set_as_picked($itf_key, $qty, $date = false, $picker_key = false) {
        ...
        $sql = sprintf(
            "SELECT `Given`,`Map To Order Transaction Fact Key`,`Location Key`,`Out of Stock`,`No Picked Other`,`Not Found`,`Part SKU`,`Required`,`Picked`,`Packed`,`Map To Order Transaction Fact Key`  FROM   `Inventory Transaction Fact` WHERE `Inventory Transaction Key`=%d  ",
            $itf_key
        );
        ...
        if ($row = mysql_fetch_assoc($res)) {
            ...
            $sql = sprintf(
                "UPDATE `Inventory Transaction Fact` SET `Note`=%s,`Picked`=%f,`Inventory Transaction Quantity`=%f,`Inventory Transaction Amount`=%f,`Date Picked`=%s,`Date`=%s ,`Picker Key`=%s WHERE `Inventory Transaction Key`=%d  ",
                prepare_mysql($note), $qty, -1 * $qty, $transaction_value, prepare_mysql($date), prepare_mysql($date), prepare_mysql($picker_key), $itf_key
            );
            mysql_query($sql);
            ...
            $sql = sprintf(
                "UPDATE `Order Transaction Fact` SET `Picked Quantity`=%f,`Current Dispatching State`=%s,`Picking Finished Date`=%s,`Picker Key`=%s,`Picking Factor`=%f ,`Cost Supplier`=%f,`Cost Storing`=%f WHERE `Order Transaction Fact Key`=%d  ",
                $qty, prepare_mysql($state), prepare_mysql($date), prepare_mysql($picker_key), $picking_factor, $transaction_value, $cost_storing, $otf_key
            );
            mysql_query($sql);
    ...
    function set_as_out_of_stock($itf_key, $qty, $date = false, $picker_key = false) {
        ...
        $sql = sprintf(
            "SELECT `Inventory Transaction Quantity`,`Inventory Transaction Amount`,`Part SKU`,`Required`,`Picked`,`Map To Order Transaction Fact Key`,`Map To Order Transaction Fact Metadata` FROM   `Inventory Transaction Fact` WHERE `Inventory Transaction Key`=%d  ",
            $itf_key
        );
        $res = mysql_query($sql);
        ...
        if ($row = mysql_fetch_assoc($res)) {
            ...
            $transaction_value = $this->get_transaction_value(
                $sku, $qty, $date
            );

            $sql = sprintf(
                "UPDATE `Inventory Transaction Fact` SET `Out of Stock`=%f ,`Out of Stock Lost Amount`=%f ,`Out of Stock Tag`='Yes'  WHERE   `Inventory Transaction Key`=%d ", $qty,
                -1 * $transaction_value, $itf_key
            );
            mysql_query($sql);
            //print_r($row);
            //print "$sql\n";
            ...
            $sql = sprintf(
                "UPDATE `Order Transaction Fact` SET  `Current Dispatching State`=%s,`No Shipped Due Out of Stock`=%f,`Packing Finished Date`=%s,`Picker Key`=%s ,`Picking Factor`=%f WHERE `Order Transaction Fact Key`=%d  ",

                prepare_mysql($state), $qty / $parts_per_product, prepare_mysql($date), prepare_mysql($picker_key), $picking_factor, $otf_key
            );
            mysql_query($sql);

            $sql = sprintf(
                "UPDATE `Order Transaction Fact` SET `Order Out of Stock Lost Amount`=IFNULL(`Order Transaction Amount`*`No Shipped Due Out of Stock`/`Order Quantity`,0)  WHERE `Order Transaction Fact Key`=%d  ",
                $otf_key
            );
            mysql_query($sql);
    ...
    function get_transaction_value($sku, $qty, $date = false) {
        ...
        $sql           = sprintf(
            "SELECT sum(ifnull(`Inventory Transaction Quantity`,0)) AS stock ,ifnull(sum(`Inventory Transaction Amount`),0) AS value FROM `Inventory Transaction Fact` WHERE  `Date`<%s AND `Part SKU`=%d ",
            prepare_mysql($date), $sku
        ...
        $transaction_value = $this->get_value_change(
            $sku, -1 * $qty, $old_qty, $old_value, $date
        );
    ...
    function update_unpicked_transaction_data($itf_key, $data) {
        ...
        $sql = sprintf(
            "SELECT * FROM `Inventory Transaction Fact` WHERE `Inventory Transaction Key`=%d AND `Delivery Note Key`=%d", $itf_key, $this->id
        );
        $res = mysql_query($sql);
        if ($row = mysql_fetch_assoc($res)) {
            ...
            if ($todo < 0) {
                $this->error = true;
                $this->msg   = _(
                        'Error,the sum of out of stock and not found units are greater than the number of not picked units'
                    )."(".$row['Required']."+".$row['Picked'].")";
            ...
            $sql = sprintf(
                "UPDATE `Inventory Transaction Fact` SET `Date`=%s,`Out of Stock`=%f ,`Not Found`=%f,`No Picked Other`=%f WHERE `Inventory Transaction Key`=%d ", prepare_mysql($date),
                $data['Out of Stock'], $data['Not Found'], $data['No Picked Other'],
            ...
            $transaction_value = $this->get_transaction_value(
                $row['Part SKU'], $data['Out of Stock'], $date
            );

            $sql = sprintf(
                "UPDATE `Inventory Transaction Fact` SET `Out of Stock Lost Amount`=%f ,`Out of Stock Tag`='Yes'  WHERE   `Inventory Transaction Key`=%d ", -1 * $transaction_value, $itf_key
            );
            mysql_query($sql);
            ...
            if ($row['Map To Order Transaction Fact Parts Multiplicity'] != 1) {
                ...
                $sql  = sprintf(
                    'SELECT * FROM `Inventory Transaction Fact` WHERE `Map To Order Transaction Fact Key`=%d  ', $row['Map To Order Transaction Fact Key']
                );
                $res2 = mysql_query($sql);
                ...
                while ($row2 = mysql_fetch_assoc($res2)) {
                    ...
                    $metadata_data = preg_split(
                        '/\;/', $row2['Map To Order Transaction Fact Metadata']
                    );
                ...
                if ($metadata_data[1] != 1) {
                    ...
                    $no_picked_other = floor(
                        $no_picked_other / $metadata_data[1]
                    );
                    $out_of_stock    = floor($out_of_stock / $metadata_data[1]);

                    if (fmod($not_found, $metadata_data[1]) or fmod(
                            $no_picked_other, $metadata_data[1]
                        ) or fmod($out_of_stock, $metadata_data[1])
                    ) {
                        $multipart_partially_no_picked = 'Yes';
                    }
                }
            ...
            $sql = sprintf(
                "UPDATE `Order Transaction Fact` SET `Multipart Partically No Picked`=%s,`No Shipped Due Not Found`=%f,`No Shipped Due Other`=%f,`No Shipped Due Out of Stock`=%f,
			`Current Dispatching State`=%s,`Picking Finished Date`=%s,`Picker Key`=%s,`Picking Factor`=%f WHERE `Order Transaction Fact Key`=%d  ", prepare_mysql($multipart_partially_no_picked),
                $not_found, $no_picked_other, $out_of_stock, prepare_mysql($state), prepare_mysql($date), prepare_mysql($picker_key), $picking_factor,
            ...
            $sql = sprintf(
                "UPDATE `Delivery Note Dimension` SET `Delivery Note Date Finish Picking`=%s WHERE `Delivery Note Key`=%d", prepare_mysql($date), $this->id
            );
            mysql_query($sql);
    ...
    function set_as_packed($itf_key, $qty, $date = false, $packer_key = false) {
        ...
        $sql = sprintf(
            "SELECT `Given`,`Not Found`,`No Picked Other`,`Note`,`Inventory Transaction Amount`,`Inventory Transaction Storing Charge Amount`,`Part SKU`,`Required`,`Picked`,`Packed`,`Out of Stock`,`Map To Order Transaction Fact Key`,`Map To Order Transaction Fact Metadata`  FROM   `Inventory Transaction Fact` WHERE `Inventory Transaction Key`=%d  ",
            $itf_key
        );
        $res = mysql_query($sql);
        if ($row = mysql_fetch_assoc($res)) {
            ...
            $sql = sprintf(
                "UPDATE `Inventory Transaction Fact` SET `Inventory Transaction Weight`=%f,`Packed`=%f,`Date Packed`=%s,`Date`=%s ,`Packer Key`=%s WHERE `Inventory Transaction Key`=%d  ", $weight,
                $qty, prepare_mysql($date), prepare_mysql($date), prepare_mysql($packer_key)
            ...
            $sql = sprintf(
                "UPDATE `Order Transaction Fact` SET `Estimated Dispatched Weight`=%f,`Current Dispatching State`=%s,`Delivery Note Quantity`=%f,`Packing Finished Date`=%s,`Packer Key`=%s ,`Packing Factor`=%f  WHERE `Order Transaction Fact Key`=%d  ",
                $weight / $parts_per_product, prepare_mysql($state), $qty / $parts_per_product, prepare_mysql($date), prepare_mysql($packer_key), $packing_factor,
            ...
            $sql    = sprintf(
                "UPDATE `Delivery Note Dimension` SET `Delivery Note Estimated Weight`=%f  WHERE `Delivery Note Key`=%d", $weight,
    ...
    function get_packed_estimated_weight() {
        $weight = 0;
        $sql    = sprintf(
            "SELECT sum(`Estimated Dispatched Weight`) AS weight FROM `Order Transaction Fact` WHERE `Order Quantity`!=0 AND `Delivery Note Key`=%d", $this->id
        );
        $res    = mysql_query($sql);
        while ($row = mysql_fetch_assoc($res)) {
            $weight = $row['weight'];
    ...
    function add_orphan_transactions($data) {
		...
		`Current Dispatching State`,`Current Payment State`,`Customer Key`,`Delivery Note Quantity`,`Ship To Key`,`Order Transaction Gross Amount`,`Order Transaction Total Discount Amount`,`Metadata`,`Store Key`,`Units Per Case`,`Customer Message`)
                         VALUES (%s,%s,%s,%d,%s,%f,%s,%f,%s,%s,%s, %s,%d,%d,%s,%s,%s,%d,%s,%s,%.2f,%.2f,%s,%s,%f,'') ", prepare_mysql($order_date), prepare_mysql($order_key),
            prepare_mysql($order_public_id),

            $this->id, prepare_mysql($this->data['Delivery Note ID']),

            $bonus_quantity, prepare_mysql('Resend'), $data['Order Tax Rate'], prepare_mysql($data['Order Tax Code']), prepare_mysql($data['Order Currency']), $data['Estimated Weight'],

            prepare_mysql($data ['Date']), $product->id, $product->pid, prepare_mysql($product->code), prepare_mysql($data ['Current Dispatching State']),
            prepare_mysql($data ['Current Payment State']), prepare_mysql($data['Order Customer Key']),

            $data['Quantity'], prepare_mysql($data['Ship To Key']), $data['Gross'], 0, prepare_mysql($data ['Metadata'], false), prepare_mysql($data['Order Store Key']), $data ['units_per_case']
        ...
        $sql = sprintf(
            "INSERT INTO `Order Post Transaction Dimension` (`Delivery Note Key`,`Order Transaction Fact Key`,`Order Post Transaction Fact Key`,`Order Key`,`Quantity`,`Operation`,`Reason`,`To Be Returned`,`State`,`Order Post Transaction Metadata`) VALUES (%d,%s,%d,%s,%f,%s,%s,%s,%s,%s)  ",
            $this->id, prepare_mysql($data ['Order Transaction Fact Key']), $otf_key, prepare_mysql($order_key), $data['Quantity'], "'Resend'", prepare_mysql($data['Reason']), "'No'", "'Dispatched'",
    ...
    function update_is_for_collection($_value) {
        ...
        if ($value == 'Collection') {
            ...
            $sql = sprintf(
                "UPDATE `Delivery Note Dimension` SET
				...
				WHERE `Delivery Note Key`=%d", prepare_mysql($value), prepare_mysql($store_country_code), prepare_mysql($store_2_alpha_country_code), prepare_mysql($store_world_region_code),
                prepare_mysql($store_town_code), prepare_mysql($store_postal_code), prepare_mysql($store_address), $this->id
            );
...
`Delivery Note Dispatch Method`=%s ,
				...
				WHERE `Delivery Note Key`=%d", prepare_mysql($value)

                , prepare_mysql($ship_to->data['Ship To Country Code']), prepare_mysql($ship_to->data['Ship To Country 2 Alpha Code']), prepare_mysql($ship_to->get('World Region Code')),
                prepare_mysql($ship_to->data['Ship To Town']), prepare_mysql($ship_to->data['Ship To Postal Code'])

                , prepare_mysql($ship_to->display('xhtml')), $ship_to->id, $this->id
            );
    ...
    function update_ship_to($ship_to_key) {
        ...
        $sql = sprintf(
            "UPDATE `Delivery Note Dimension` SET `Delivery Note Dispatch Method`='Dispatch' ,`Delivery Note Ship To Key`=%d,
		`Delivery Note Country 2 Alpha Code`=%s,`Delivery Note XHTML Ship To`=%s,`Delivery Note Country Code`=%s  ,
		`Delivery Note World Region Code`=%s,`Delivery Note Town`=%s,`Delivery Note Postal Code`=%s   WHERE `Delivery Note Key`=%d", $ship_to->id,
            prepare_mysql($ship_to->data['Ship To Country 2 Alpha Code']), prepare_mysql($ship_to->data['Ship To XHTML Address']), prepare_mysql($ship_to->data['Ship To Country Code']),
