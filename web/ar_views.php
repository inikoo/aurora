...
function view($db, $smarty, $user, $data) {

    require_once 'utils/parse_request.php';


    $state = parse_request($data, $db, $user);
    ...
    $sql = sprintf(
        ...
        prepare_mysql(gmdate('Y-m-d H:i:s')), prepare_mysql($state['module']), prepare_mysql($state['section']), prepare_mysql(($state['subtab'] != '' ? $state['subtab'] : $state['tab'])),
    ...
    switch ($showcase) {
        ...
        case 'category':
            ...
            if ($data['_object']->get('Category Scope') == 'Product') {
                ...
                if ($data['_object']->id == $data['store']->get('Store Family Category Key') or $data['_object']->id == $data['store']->get('Store Department Category Key')) {
                    $html = '';
...
function get_navigation($user, $smarty, $data, $db, $account) {
    ...
    switch ($data['module']) {
        ...
        case ('suppliers'):
            ...
            switch ($data['section']) {
                ...
                case ('order'):
                    ...
                    break;
                case ('deleted_order'):
                    return get_deleted_purchase_order_navigation($data, $smarty, $user, $db, $account);
                    break;
        ...
        case ('account'):
            ...
            switch ($data['section']) {
                ...
                case ('user'):
                    ...
                    break;
                case ('deleted.user'):
                    return get_deleted_user_navigation($data, $smarty, $user, $db, $account);
                    break;
                case ('suppliers.user'):
        ...
        case 'agent_client_orders':
            ...
            switch ($data['section']) {
                case ('orders'):
                    return get_agent_client_orders_navigation($data, $smarty, $user, $db, $account);
                    break;
                case ('client_order'):
        ...
        case 'agent_client_deliveries':
            ...
            switch ($data['section']) {
                case ('agent_deliveries'):
                    return get_agent_client_deliveries_navigation($data, $smarty, $user, $db, $account);
                    break;
                case ('agent_delivery'):
    ...
    switch ($state['module']) {
            ...
            if ($state['section'] == 'store') {
                ...
                if ($state['parent'] == 'store') {
                    ...
                    $branch[] = array(
                        ...
                        'icon'      => 'sitemap',
                        'reference' => 'products/'.$category->get('Store Key').'/categories'
                    );
                    ...
                    foreach ($parent_category_keys as $category_key) {
                        ...
                        if ($parent_category->id) {
                            ...
                            $branch[] = array(
                                'label'     => $parent_category->get('Code'),
                                'icon'      => '',
                    ...
                    $branch[] = array(
                        'label'     => _('Order').' '.$order->get('Order Public ID'),
                        'icon'      => 'shopping-cart',
                ...
                if ($state['parent'] == 'store') {
                    ...
                    $branch[] = array(
                        ...
                        'icon'      => 'sitemap',
                        'reference' => 'services/'.$category->get('Store Key').'/categories'
                    );
        ...
        case 'customers':
            ...
            switch ($state['section']) {
                ...
                case 'statistics':
                    $branch[] = array(
                        'label'     => _("Customer's stats").' '.$store->data['Store Code'],
                        'icon'      => 'line-chart',
        ...
        case 'suppliers':
            if ($state['section'] == 'suppliers') {
                ...
                $branch[] = array(
                    ...
                    'icon'      => 'ship',
                    'reference' => 'supplier/'.$state['_parent']->get('Purchase Order Parent Key')
                );
                $branch[] = array(
                    'label'     => '<span class="Purchase_Order_Public_ID">'.$state['_parent']->get('Public ID').'</span>',
                    'icon'      => 'clipboard',
                    'reference' => 'supplier/'.$state['_parent']->get('Purchase Order Parent Key').'/order/'.$state['parent_key']
                );
                ...
                $branch[] = array(
                    ...
                    'icon'      => 'user-secret',
                    'reference' => 'agent/'.$state['_parent']->get('Purchase Order Parent Key')
                );
                $branch[] = array(
                    'label'     => '<span class="Purchase_Order_Public_ID">'.$state['_parent']->get('Public ID').'</span>',
                    'icon'      => 'clipboard',
        ...
        case 'invoices_server':
            ...
            if ($state['section'] == 'categories') {
                $branch[] = array(
                    'label'     => _("Invoice's categories").' ('._('All stores').')',
                    'icon'      => '',
                    'reference' => ''
                );
        ...
        case 'delivery_notes':
            ...
            switch ($state['section']) {
                case 'delivery_notes':
                    ...
                    $branch[] = array(
                        'label'     => _('Delivery Notes').' '.$store->data['Store Code'],
                        'icon'      => '',
                ...
                case 'order':
                    ...
                    $branch[] = array(
                        'label'     => $parent->get('Delivery Note ID'),
                        'icon'      => 'truck fa-flip-horizontal',
        ...
        case 'invoices':
            ...
            switch ($state['section']) {
                ...
                case 'delivery_note':
                    ...
                    $branch[] = array(
                        'label'     => $parent->get('Invoice Public ID'),
                        'icon'      => 'file-text-o',
            ...
            switch ($state['section']) {
                ...
                case 'timesheets':
                    ...
                    if ($state['parent'] == 'year') {
                        ...
                        $branch[] = array(
                            'label'     => sprintf(_('%s week (starting %s %s)'), get_ordinal_suffix($week), strftime('%a', $date), get_ordinal_suffix(strftime('%d', $date))),
                            'icon'      => '',
        ...
        case 'inventory':
            ...
            switch ($state['section']) {
                ...
                case 'barcode':
                    ...
                    $branch[] = array(
                        'label'     => $state['_object']->get('Number'),
                        'icon'      => 'barcode',
                        'reference' => ''
                    );
                ...
                case 'deleted_barcode':
                    ...
                    $branch[] = array(
                        'label'     => $state['_object']->get('Deleted Number').' <i class="fa fa-trash" aria-hidden="true"></i>',
                        'icon'      => 'barcode',
                        'reference' => ''
                    );
                ...
                case 'upload':
                    ...
                    $branch[] = array(
                        'label'     => _('Upload').' '.sprintf('%04d', $state['_object']->get('Key')),
                        'icon'      => 'upload',
                        'reference' => ''
                    );
                ...
                case 'stock_history.day':
                    ...
                    $branch[] = array(
                        'label'     => strftime("%a %e %b %Y", strtotime($state['key'].' +0:00')),
                        'icon'      => '',
                        'reference' => ''
                    );
                    break;
        ...
        case 'warehouses':
            ...
            switch ($state['section']) {
                ...
                case 'part':
                    ...
                    $branch[] = array(
                        'label'     => '<span class="id Part_Reference">'.$state['_object']->get('Reference').'</span> (<span class="id">'.$state['_object']->get('SKU').'</span>)',
                        'icon'      => 'square',
                        'reference' => ''
                    );
            ...
            switch ($state['section']) {
                ...
                case 'website.node':
                    ...
                    $branch[] = array(
                        ...
                        'icon'      => ($state['_object']->get('Website Node Icon') == '' ? 'file-o' : $state['_object']->get('Website Node Icon')),
                        'reference' => ''
                    );
...
*/
                ...
                $branch[] = array(
                    'label'     => _('Payment service provider').'  <span id="id">'.$psp->get('Code').'</span>',
                    'icon'      => '',
                    'reference' => ''
                );
                ...
                if ($state['parent'] == 'account') {
                    ...
                    $branch[]        = array(
                        'label'     => _('Payment account').'  <span id="id">'.$payment_account->get('Payment Account Code').'</span>',
                        'icon'      => '',
        ...
        case 'account':
            ...
            if ($state['section'] == 'users') {
                ...
                $branch[] = array(
                    'label'     => _('Payment service provider').'  <span id="id">'.$state['_object']->get('Payment Service Provider Code').'</span>',
                    'icon'      => '',
                    'reference' => 'account/payment_service_provider/'.$state['_object']->id
                );
...
*/
        ...
        case 'reports':
            ...
            if ($state['section'] == 'billingregion_taxcategory') {
                $branch[] = array(
                    'label'     => _('Billing region & Tax code report'),
                    'icon'      => '',
...
function create_node_breadcrumbs($db, $node_key, $branch) {
    ...
    if ($result = $db->query($sql)) {
        if ($row = $result->fetch()) {
            ...
            array_unshift(
                $branch, array(
                'label'     => trimStringToFullWord(16, $row['Webpage Name']),
