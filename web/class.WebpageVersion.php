...
class WebpageVersion extends DB_Table{
	...
	function get_data($key, $tag,$tag2=false) {
		...
		if ($key=='id')
			$sql=sprintf("select * from `Webpage Version Dimension` where `Webpage Version Key`=%d", $tag);
		else if ($key=='webpage_code')
			$sql=sprintf("select  * from `Webpage Version Dimension` where `Webpage Version Webpage Key`=%d and `Webpage Version Code`=%s ", $tag,prepare_mysql($tag2));
				else
			return;
		...
		if ($this->data = $this->db->query($sql)->fetch()) {
			...
			$this->metadata=json_decode($this->data['Webpage Version Metadata'],true);
	...
	function find($raw_data, $options) {
		...
		$sql=sprintf("select `Webpage Version Key` from `Webpage Version Dimension` where `Webpage Version Webpage Key`=%d and  `Webpage Version Code`=%s",
			$data['Webpage Version Webpage Key'],
	...
	function create($data) {
		...
		$sql=sprintf("insert into `Webpage Version Dimension` %s %s", $keys, $values);
	...
	function append_block($data, $position=0) {
		...
		$sql=sprintf('insert into `Webpage Version Block Bridge` (`Webpage Version Block Webpage Version Key`,`Webpage Version Block Position`,`Webpage Version Block Template`,`Webpage Version Block Settings`) values (%d,%d,%s,%s) ',
			$this->id,
			(10*$position)+5,
			prepare_mysql($data['Webpage Version Block Template']),
			prepare_mysql(json_encode($data['Webpage Version Block Settings']))
		);
		...
		$sql.=sprintf("update  `Webpage Version Block Bridge` set `Webpage Version Block Position` = (@new_ordering := @new_ordering + @ordering_inc) where `Webpage Version Block Webpage Version Key`=%d order by `Webpage Version Block Position` desc",
			$this->id
		);
		$this->db->exec($sql);
	}
