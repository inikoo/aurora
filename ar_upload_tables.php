	...
	if ($result=$db->query($sql)) {
		foreach ($result as $data) {
			...
			case 'InProcess':
				...
				break;
			case 'OK':
				$state='<i class="fa success fa-check padding_left_10"></i>';
				$msg=sprintf('<span class="link" onClick="change_view(\'%s%d\')">%s</span>', $link, $data['Upload Record Object Key'], $data['object_name']);

				break;
			case 'Warning':
				$state='<i class="fa warning fa-check padding_left_10"></i>';
				$msg=sprintf('<span class="link">%s</span>', $data['object_name']);

				break;
			case 'Cancelled':
				$state='<i class="fa discret fa-ban padding_left_10"></i>';
				$msg=_('Cancelled');

				break;
			case 'Error':
				$state='<i class="fa error fa-exclamation-circle padding_left_10"></i>';
				$msg='';
			...
			$adata[]=array(
				...
				'row'=>sprintf('<span title="%s">%s</span>', $data['Upload File Name'], number($data['Upload Record Row Index'])),
				'date'=>(($data['Upload Record Date']=='' ) ?'': strftime("%c", strtotime($data['Upload Record Date'].' +0:00'))),
				'state'=>$state,
				'msg'=>$state.' '.$msg
			);
	...
	if ($result=$db->query($sql)) {
		foreach ($result as $data) {
			...
			$adata[]=array(
				...
				'date'=>(($data['Upload Created']=='' ) ?'': strftime("%c", strtotime($data['Upload Created'].' +0:00'))),
				'state'=>$state,
