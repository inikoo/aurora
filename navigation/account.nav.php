...
function get_new_api_key_navigation($data, $smarty, $user, $db, $account) {
    ...
    $up_button      = array(
        'icon'      => 'arrow-up',
        'title'     => _("Staff user"),
        'reference' => 'account/user/'.$data['parent_key']
    );
    $left_buttons   = array();
...
function get_user_navigation($data, $smarty, $user, $db, $account) {
        ...
        if ($data['parent'] == 'group') {
            ...
            if ($prev_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-left',
                    'title'     => $prev_title,
                    'reference' => 'account/user/'.$prev_key
                );
            ...
            if ($next_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-right',
                    'title'     => $next_title,
                    'reference' => 'account/user/'.$next_key
                );
    ...
    switch ($object->get('User Type')) {
        ...
        case 'Supplier':
            ...
            $parent_icon
                              = '<i style="font-size:80%;padding-left:10px" class="fa fa-ship" aria-hidden="true"></i>';
            break;
        case 'Agent':
            $parent_reference = 'agent/'.$object->get('User Parent Key');
            $parent_icon
                              = '<i style="font-size:80%;padding-left:10px" class="fa fa-user-secret" aria-hidden="true"></i>';
    ...
    if ($data['parent']) {
        ...
        if ($data['parent'] == 'user') {
            ...
            $up_button = array(
                ...
                'reference' => 'account/user/'.$system_user->id
            );
            ...
            if ($prev_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-left',
                    'title'     => $prev_title,
                    'reference' => 'account/user/'.$system_user->id.'/api_key/'.$prev_key
                );
            ...
            if ($next_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-right',
                    'title'     => $next_title,
                    'reference' => 'account/user/'.$system_user->id.'/api_key/'.$next_key
                );
