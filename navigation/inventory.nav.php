...
function get_part_navigation($data, $smarty, $user, $db, $account) {
    ...
    if ($data['parent']) {
        ...
        $sql = sprintf(
            "select `Part Reference` object_name,P.`Part SKU` as object_key from %s  
	                and ($_order_field < %s OR ($_order_field = %s AND P.`Part SKU` < %d))  order by $_order_field desc , P.`Part SKU` desc limit 1", "$table $where $wheref", prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
        );
        ...
        $sql = sprintf(
            "select `Part Reference` object_name,P.`Part SKU` as object_key from %s
	                and ($_order_field  > %s OR ($_order_field  = %s AND P.`Part SKU` > %d))  order by $_order_field   , P.`Part SKU`  limit 1", "$table $where $wheref",prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
        );
            ...
            case 'category':
                ...
                if ($prev_key) {
                    $left_buttons[] = array(
                        'icon'      => 'arrow-left',
                        'title'     => $prev_title,
                        'reference' => 'category/'.$data['parent_key'].'/part/'.$prev_key
                    );
                ...
                if ($next_key) {
                    $left_buttons[] = array(
                        'icon'      => 'arrow-right',
                        'title'     => $next_title,
                        'reference' => 'category/'.$data['parent_key'].'/part/'.$next_key
                    );
    ...
    if ($supplier_part->id) {
        if ($object->get('Part Production') == 'Yes') {
            ...
            $title .= ' <small class="padding_left_10"> <i class="fa fa-long-arrow-left padding_left_10"></i> <i class="fa fa-industry button" title="'._("Supplier's product").'" onCLick="change_view(\'/production/'.$supplier_part->get('Supplier Part Supplier Key')
                .'/part/'.$supplier_part->id.'\')" ></i> <span class="Supplier_Part_Reference button"  onCLick="change_view(\'production/'.$supplier_part->get('Supplier Part Supplier Key').'/part/'.$supplier_part->id.'\')">'.$supplier_part->get('Reference').'</small>';
        } else {
...
function get_part_image_navigation($data, $smarty, $user, $db, $account) {
    ...
    if ($data['parent']) {
        ...
        if ($result2 = $db->query($sql)) {
            if ($row2 = $result2->fetch() and $row2['num'] > 1) {
                ...
                $sql = sprintf(
                    "select `Part Reference` object_name,P.`Part SKU` as object_key from $table   $where $wheref
                ...
                $sql = sprintf(
                    "select `Part Reference` object_name,P.`Part SKU` as object_key from $table   $where $wheref
	                and ($_order_field  > %s OR ($_order_field  = %s AND P.`Part SKU` > %d))  order by $_order_field   , P.`Part SKU`  limit 1", prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
                );
    ...
    $_content = array(
        ...
        'left_buttons'  => $left_buttons,
        'right_buttons' => $right_buttons,
        'title'         => _('Part').' <span class="id Part_Reference">'.$data['_parent']->get('Part Reference').'</span> (<span class="id Part_SKU">'.$data['_parent']->get('SKU').'</span>) '._(
                'Image'
            ).' ('.$data['_object']->get('Subject Order').'/'.$data['_parent']->get_number_images().')',
        'search'        => array(
            'show'        => true,
            'placeholder' => _('Search Inventory')
        )
...
function get_stock_history_day_navigation($data, $smarty, $user, $db, $account) {
    ...
    $left_buttons[] = array(
        'icon'      => 'arrow-left',
        'title'     => strftime(
            "%a %e %b %Y", strtotime($data['key'].' -1 day  +0:00')
        ),
        'reference' => 'inventory/stock_history/day/'.strftime(
                "%Y-%m-%d", strtotime($data['key'].' - 1 day +0:00')
            )
    );
    $left_buttons[] = array(
        'icon'      => 'arrow-up',
    ...
    $title = _('Stock history').' <span class="id">'.strftime(
            "%a %e %b %Y", strtotime($data['key'].' +0:00')
        ).'</span>';
...
function get_barcode_navigation($data, $smarty, $user, $db, $account) {
    ...
    if ($data['parent']) {
        ...
        if ($result2 = $db->query($sql)) {
            if ($row2 = $result2->fetch() and $row2['num'] > 1) {
                ...
                $sql = sprintf(
                    "select `Barcode Number` object_name,B.`Barcode Key` as object_key from $table   $where $wheref
                ...
                $sql = sprintf(
                    "select `Barcode Number` object_name,B.`Barcode Key` as object_key from $table   $where $wheref
	                and ($_order_field  > %s OR ($_order_field  = %s AND B.`Barcode Key` > %d))  order by $_order_field   , B.`Barcode Key`  limit 1", prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
                );
                    ...
                    case 'account':
                        ...
                        if ($prev_key) {
                            $left_buttons[] = array(
                                'icon'      => 'arrow-left',
                                'title'     => $prev_title,
                                'reference' => 'inventory/barcode/'.$prev_key
                            );
                        ...
                        if ($next_key) {
                            $left_buttons[] = array(
                                'icon'      => 'arrow-right',
                                'title'     => $next_title,
                                'reference' => 'inventory/barcode/'.$next_key
                            );
    ...
    $title = _('Barcode').' <span class="id Barcode_Number">'.$object->get(
            'Number'
        ).'</span>';
...
function get_parts_new_main_category_navigation($data, $smarty, $user, $db, $account) {
    ...
    $_content = array(
        ...
        'left_buttons'   => $left_buttons,
        'right_buttons'  => $right_buttons,
        'title'          => _("New main Part's category"),
        'search'         => array(
            'show'        => true,
            'placeholder' => _('Search inventry')
        )
...
function get_parts_category_navigation($data, $smarty, $user, $db, $account) {
    ...
    if ($prev_key) {
        $left_buttons[] = array(
            'icon'      => 'arrow-left',
            'title'     => $prev_title,
            'reference' => 'inventory/category/'.$parent_categories.$prev_key
        );
    ...
    if ($next_key) {
        $left_buttons[] = array(
            'icon'      => 'arrow-right',
            'title'     => $next_title,
            'reference' => 'inventory/category/'.$parent_categories.$next_key
        );
...
function get_new_part_attachment_navigation($data, $smarty): array {
    ...
    $title = '<span>'.sprintf(
            _('New attachment for %s'), '<span onClick="change_view(\'part/'.$part->id.'\')" class="button id">'.$part->get('Reference').'</span>'
        ).'</span>';
...
function get_upload_navigation($data, $smarty, $user, $db) {
    ...
    if ($data['parent'] == 'category') {
        $up_button = array(
            'icon'      => 'arrow-up',
            'title'     => sprintf(
                _('Category: %s'), $data['_parent']->get('Code')
            ),
            'reference' => 'category/'.$data['parent_key']
        );

        $title = sprintf(
            _('Upload into category %s'), $data['_parent']->get('Code')
        );
