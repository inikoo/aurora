...
function get_campaigns_navigation($data, $smarty, $user, $db) {
	...
	if ($user->stores>1) {
		...
		if ($result=$db->query($sql)) {
			if ($row = $result->fetch()) {
				$prev_title=_('Deals').' '.$row['Store Code'];
			}else {
				$prev_title='';
			}
		}else {
		...
		if ($result=$db->query($sql)) {
			if ($row = $result->fetch()) {
				$next_title=_('Deals').' '.$row['Store Code'];
			}else {
				$next_title='';
			}
		}else {
		...
		$left_buttons[]=array('icon'=>'arrow-left', 'title'=>$prev_title, 'reference'=>'campaigns/'.$prev_key );
		$left_buttons[]=array('icon'=>'arrow-up', 'title'=>_('Customers (All stores)'), 'reference'=>'marketing/all', 'parent'=>'');

		$left_buttons[]=array('icon'=>'arrow-right', 'title'=>$next_title, 'reference'=>'campaigns/'.$next_key );
	}
...
function get_campaign_navigation($data, $smarty, $user, $db) {
	...
	if ($data['parent']) {
		...
		if ($result2=$db->query($sql)) {
			if ($row2 = $result2->fetch() and $row2['num']>1 ) {
				...
				$sql=sprintf("select `Deal Campaign Name` object_name,C.`Deal Campaign Key` as object_key from $table   $where $wheref
				...
				$sql=sprintf("select `Deal Campaign Name` object_name,C.`Deal Campaign Key` as object_key from $table   $where $wheref
	                and ($_order_field  > %s OR ($_order_field  = %s AND C.`Deal Campaign Key` > %d))  order by $_order_field   , C.`Deal Campaign Key`  limit 1",
					prepare_mysql($_order_field_value),
					prepare_mysql($_order_field_value),
					$object->id
				);
		...
		if ($data['parent']=='store') {
			...
			$up_button=array('icon'=>'arrow-up', 'title'=>_("Campaigns").' '.$data['store']->get('Code'), 'reference'=>'campaigns/'.$data['store']->id);

			if ($prev_key) {
				$left_buttons[]=array('icon'=>'arrow-left', 'title'=>$prev_title, 'reference'=>'campaigns/'.$data['parent_key'].'/'.$prev_key);
			...
			if ($next_key) {
				$left_buttons[]=array('icon'=>'arrow-right', 'title'=>$next_title, 'reference'=>'campaigns/'.$data['parent_key'].'/'.$next_key);
	...
	$title= '<span class="id"><span class="Deal_Campaign_Name">'.$object->get('Name').'</span> </span>';
...
//print $sql;
		...
		if ($result2=$db->query($sql)) {
			if ($row2 = $result2->fetch() and $row2['num']>1 ) {
				...
				$sql=sprintf("select `Deal Name` object_name,D.`Deal Key` as object_key from $table   $where $wheref
				...
				$sql=sprintf("select `Deal Name` object_name,D.`Deal Key` as object_key from $table   $where $wheref
	                and ($_order_field  > %s OR ($_order_field  = %s AND D.`Deal Key` > %d))  order by $_order_field   , D.`Deal Key`  limit 1",
					prepare_mysql($_order_field_value),
					prepare_mysql($_order_field_value),
					$object->id
				);
		...
		if ($data['parent']=='store') {
			...
			$up_button=array('icon'=>'arrow-up', 'title'=>_("Campaigns").' '.$data['store']->get('Code'), 'reference'=>'campaigns/'.$data['store']->id.'&tab=deals');

			if ($prev_key) {
				$left_buttons[]=array('icon'=>'arrow-left', 'title'=>$prev_title, 'reference'=>'deals/'.$data['parent_key'].'/'.$prev_key);
			...
			if ($next_key) {
				$left_buttons[]=array('icon'=>'arrow-right', 'title'=>$next_title, 'reference'=>'deals/'.$data['parent_key'].'/'.$next_key);
		...
		elseif ($data['parent']=='campaign') {
			...
			$up_button=array('icon'=>'arrow-up', 'title'=>_("Campaigns").' '.$data['store']->get('Code'), 'reference'=>'campaigns/'.$data['store']->id.'/'.$data['parent_key']);

			if ($prev_key) {
				$left_buttons[]=array('icon'=>'arrow-left', 'title'=>$prev_title, 'reference'=>'campaigns/'.$data['store']->id.'/'.$data['parent_key'].'/deal/'.$prev_key);
			...
			if ($next_key) {
				$left_buttons[]=array('icon'=>'arrow-right', 'title'=>$next_title, 'reference'=>'campaigns/'.$data['store']->id.'/'.$data['parent_key'].'/deal/'.$next_key);
