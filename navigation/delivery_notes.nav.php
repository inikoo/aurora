...
function get_consignments_navigation($data, $smarty) {
    ...
    $sections['consignments']['selected'] = true;
...
function get_shipper_new_navigation($data, $smarty) {
...
function get_consignment_navigation($data, $smarty, $user, $db) {
    ...
    if ($result2 = $db->query($sql)) {
        if ($row2 = $result2->fetch()) {
            ...
            if ($row2['num'] > 1) {
                ...
                $sql = sprintf(
                    "select `Consignment Public ID` object_name,`Consignment Key` as object_key from %s %s %s
and ($_order_field < %s OR ($_order_field = %s AND `Consignment Key` < %d))  order by $_order_field desc , `Consignment Key` desc limit 1", $table, $where, $wheref, prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
                );
                if ($result = $db->query($sql)) {
                    if ($row = $result->fetch()) {
                        $prev_key   = $row['object_key'];
                        $prev_title = _("Consignment").' '.$row['object_name'];
                    }
                }
                ...
                $sql = sprintf(
                    "select `Consignment Public ID` object_name,`Consignment Key` as object_key from  %s %s %s
and ($_order_field  > %s OR ($_order_field  = %s AND `Consignment Key` > %d))  order by $_order_field   , `Consignment Key`  limit 1", $table, $where, $wheref, prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
                );

                if ($result = $db->query($sql)) {
                    if ($row = $result->fetch()) {
                        $next_key   = $row['object_key'];
                        $next_title = _("Consignment").' '.$row['object_name'];
                    }
                }
                ...
                if ($prev_key) {
                    $left_buttons[] = array(
                        'icon'      => 'arrow-left',
                        'title'     => $prev_title,
                        'reference' => 'consignments/'.$prev_key
                    );
                ...
                if ($next_key) {
                    $left_buttons[] = array(
                        'icon'      => 'arrow-right',
                        'title'     => $next_title,
                        'reference' => 'consignments/'.$next_key
                    );
    ...
    $title = _('Consignment').' <span  class="id Consignment_Public_ID" >'.$data['_object']->get('Public ID').'</span>';
