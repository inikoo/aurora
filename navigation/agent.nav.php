        ...
        elseif ($data['parent'] == 'Agent') {
            ...
            if ($prev_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-left',
                    'title'     => $prev_title,
                    'reference' => 'client_order/'.$prev_key
                );
            ...
            if ($next_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-right',
                    'title'     => $next_title,
                    'reference' => 'client_order/'.$next_key
                );
        ...
        if ($result2 = $db->query($sql)) {
            if ($row2 = $result2->fetch()) {
                if ($row2['num'] > 1) {
	                ...
	                and ($_order_field < %s OR ($_order_field = %s AND D.`Supplier Delivery Key` < %d))  order by $_order_field desc , D.`Supplier Delivery Key` desc limit 1", "$table $where $wheref",
                        prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
                    );
...
function get_agent_supplier_order_navigation($data, $smarty, $user, $db, $account) {
    ...
    if ($data['parent']) {
        ...
        if ($result2 = $db->query($sql)) {
            if ($row2 = $result2->fetch()) {
                if ($row2['num'] > 1) {
                    ...
                    $sql = sprintf(
                        "select `Agent Supplier Purchase Order Public ID` object_name,ASPOD.`Agent Supplier Purchase Order Key` as object_key from %s
	                and ($_order_field < %s OR ($_order_field = %s AND ASPOD.`Agent Supplier Purchase Order Key` < %d))  order by $_order_field desc , ASPOD.`Agent Supplier Purchase Order Key` desc limit 1", "$table $where $wheref",
                        prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
                    );
                    ...
                    $sql = sprintf(
                        "select `Agent Supplier Purchase Order Public ID` object_name,ASPOD.`Agent Supplier Purchase Order Key` as object_key from %s
	                and ($_order_field  > %s OR ($_order_field  = %s AND ASPOD.`Agent Supplier Purchase Order Key` > %d))  order by $_order_field   , ASPOD.`Agent Supplier Purchase Order Key`  limit 1", "$table $where $wheref",
                        prepare_mysql($_order_field_value), prepare_mysql($_order_field_value), $object->id
                    );
        ...
        if ($data['parent'] == 'client_order') {


            $up_button = array(
                'icon'      => 'arrow-up',
                'title'     => _("Client's order").' '.$data['_parent']->get('Code'),
                'reference' => 'client_order/'.$data['parent_key']
            );
            ...
            if ($prev_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-left',
                    'title'     => $prev_title,
                    'reference' => 'client_order/'.$data['parent_key'].'/'.$prev_key
                );
            ...
            if ($next_key) {
                $left_buttons[] = array(
                    'icon'      => 'arrow-right',
                    'title'     => $next_title,
                    'reference' => 'client_order/'.$data['parent_key'].'/'.$next_key
                );
    ...
    $title = _("Supplier order").' <span class="Agent_Supplier_Purchase_Order_Public_ID id">'.$object->get('Public ID').'</span>';
