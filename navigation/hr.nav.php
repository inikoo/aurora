...
function get_timesheet_navigation($data, $smarty, $user, $db) {
	...
	$title= '<span class=" Timesheet_Date">'.$object->get('Date').'</span>, <span onCLick="change_view(\'employee/'.$object->get('Timesheet Staff Key').'\')" class="link Timesheet_Staff_Name">'.$object->get('Timesheet Staff Name').'</span>  <span class="id hide">('.$object->get_formatted_id().')</span>';
...
function get_upload_navigation($data, $smarty, $user, $db) {
	...
	case 'employees':
		...
		if ($result2=$db->query($sql)) {
			if ($row2= $result2->fetch()  and $row2['num']>1) {
				...
				$sql=sprintf("select concat(`Upload Key`,`Upload Created`) object_name,TD.`Upload Key` as object_key from $table   $where $wheref
				...
				$sql=sprintf("select concat(`Upload Key`,`Upload Created`) object_name,TD.`Upload Key` as object_key from $table   $where $wheref
	                and ($_order_field  > %s OR ($_order_field  = %s AND TD.`Upload Key` > %d))  order by $_order_field   , TD.`Upload Key`  limit 1",
					prepare_mysql($_order_field_value),
					prepare_mysql($_order_field_value),
					$object->id
				);
	...
	case 'employees':
		...
		$up_button=array('icon'=>'arrow-up', 'title'=>sprintf(_('Employee: %s'), $employee->get('Name')), 'reference'=>'/hr');
	...
	if ($data['parent']) {
		...
		if ($result2=$db->query($sql)) {
			if ($row2= $result2->fetch()  and $row2['num']>1) {
				...
				$sql=sprintf("select `Staff Deleted Name` object_name,SD.`Staff Deleted Key` as object_key from $table   $where $wheref
				...
				$sql=sprintf("select `Staff Deleted Name` object_name,SD.`Staff Deleted Key` as object_key from $table   $where $wheref
	                and ($_order_field  > %s OR ($_order_field  = %s AND SD.`Staff Deleted Key` > %d))  order by $_order_field   , SD.`Staff Deleted Key`  limit 1",
					prepare_mysql($_order_field_value),
					prepare_mysql($_order_field_value),
					$object->id
				);
...
function get_deleted_contractor_navigation($data, $smarty, $user, $db) {
	...
	if ($data['parent']) {
		...
		if ($data['parent']=='account') {
			...
			$up_button=array('icon'=>'arrow-up', 'title'=>_("Deleted contractor"), 'reference'=>'hr/deleted_contractors');
