...
function get_order_navigation($data, $smarty, $user, $db, $account) {
	...
	if ($data['parent']) {
		...
		if ($result2=$db->query($sql)) {
			if ($row2 = $result2->fetch()) {
				if ($row2['num']>1) {
					...
					$sql=sprintf("select O.`Order Public ID` object_name,O.`Order Key` as object_key from $table   $where $wheref
		...
		if ($data['parent']=='product') {
			...
			$up_button=array('icon'=>'arrow-up', 'title'=>_("Product").' '.$data['_parent']->get('Code'), 'reference'=>'product/'.$data['_parent']->id);

			if ($prev_key) {
				$left_buttons[]=array('icon'=>'arrow-left', 'title'=>$prev_title, 'reference'=>'product/'.$data['_parent']->id.'/order/'.$prev_key);
			...
			if ($next_key) {
				$left_buttons[]=array('icon'=>'arrow-right', 'title'=>$next_title, 'reference'=>'product/'.$data['_parent']->id.'/order/'.$next_key);

			}else {
				$left_buttons[]=array('icon'=>'arrow-right disabled', 'title'=>'', 'url'=>'');

			}
			$sections=get_sections('products', $data['_parent']->get('Product Store Key'));
