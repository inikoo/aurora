...
class nodes {
    ...
    function delete($id) { // delete this category and all categories under it
        ...
        $res1 = mysql_query($sql1) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql1."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));
        $before_ids = array();
        ...
        mysql_query($sql2) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql2."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));

        // ok now lets get the ids after the deletion.
        $sql3 = "SELECT ".$this->table_fields['id']."
                FROM ".$this->table_name." ".$this->sql_condition_where;
        $res3 = mysql_query($sql1) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql3."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));
        $after_ids = array();
        while ($crd_id = mysql_fetch_array($res1)) {
            $after_ids[] = $crd_id;
    ...
    function fetch($id) {
        ...
        $sql = sprintf(
            "SELECT * FROM %s WHERE %s=%d %s", $this->table_name, $this->table_fields['id'], $id, $this->sql_condition
        );
        $res = mysql_query($sql);
        ...
        if ($record = mysql_fetch_array($res)) {
            ...
            $record["prefix"] = $this->get_prefix(
                $record[preg_replace(
                    '/`/', '', $this->table_fields['position']
                )]
            );
            $position_slices  = explode(
                ">", $record[preg_replace(
                '/`/', '', $this->table_fields['position']
            )]
            );
    ...
    function update($id, $parent = 0, $fields = array()) {
        ...
        if ($position != $new_position) {
            ...
            $res = mysql_query($sql1) or die(trigger_error(
                "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql1."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
            ));
            ...
            while ($sub = mysql_fetch_array($res)) {
                ...
                $new_sub_position = str_replace(
                    $position, $new_position, $sub[$this->table_fields['position']]
                );
                $sql2             = "UPDATE ".$this->table_name."
        ...
        mysql_query($sql3) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql3."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));
        ...
        foreach ($fields as $key => $value) {
            if ($key == $this->table_fields['id']) {
                continue;
            }  // no change will be done on the id
            if ($key == $this->table_fields['position']) {
                continue;
            } // position change have been done in the section above
            $sql .= "".$key." = '".$value."',";
        }
        ...
        mysql_query($sql) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));
    }
    ...
    function get_deep($id) {
        ...
        $sql = "SELECT ".$this->table_fields['position']." as position
               FROM ".$this->table_name."
               WHERE ".$this->table_fields['id']." = '".$id."' ".$this->sql_condition;
        $res = mysql_query($sql) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));
        $record = mysql_fetch_array($res);
    ...
    function html_output($id = 0, $clickable = false) {
        ...
        if (!$clickable) {
            ...
            $tree = $this->build_list(
                $id, $clickable
            ); // display clickable list (one sub-level list)
        }
            ...
            for ($i = 0; $i < $end; $i++) {
                ...
                if ($next_loop_level > $c['prefix']) {
                    ...
                    if ($c['prefix'] > 1) {
                        ...
                        $body .= "\n<!-- Using LevelOpenTag -->\n";
                        if ($c[$this->table_fields['id']] == $id) {
                            $body .= $this->HtmlTree['LevelOpenTagSelected'];
                        } else {
                            $body .= $this->HtmlTree['LevelOpenTag'];
                        }
                    } else {
                        $body .= "\n<!-- Using FirstLevelOpenTag -->\n";
                        // we are on the roots.
                        if ($c[$this->table_fields['id']] == $id) {
                            $body .= $this->HtmlTree['FirstLevelOpenTagSelected'];
                        } else {
                            $body .= $this->HtmlTree['FirstLevelOpenTag'];
                        }
                    }
                    ...
                    if ($next_loop_level == 1 && $c['prefix'] == 2) {
                        ...
                        $body .= "\n<!-- Using Node -->\n";
                        if ($c['id'] == $id) {
                            $body .= $this->HtmlTree['NodeSelected'];
                        } else {
                            $body .= $this->HtmlTree['Node'];
                        }
                        $body .= "\n<!-- Using FirstLevelCloseTag -->\n";
                        if ($c['id'] == $id) {
                            $body .= $this->HtmlTree['FirstLevelCloseTagSelected'];
                        } else {
                            $body .= $this->HtmlTree['FirstLevelCloseTag'];
                        }
                        ...
                        for ($j = $c['prefix']; $j > $next_loop_level; $j--) {
                            if ($j == 2) {
                                ...
                                $body .= "\n<!-- Using LevelCloseTag -->\n";
                                if ($c[$this->table_fields['id']] == $id) {
                                    $body .= $this->HtmlTree['LevelCloseTagSelected'];
                                } else {
                                    $body .= $this->HtmlTree['LevelCloseTag'];
                                }
                            }
                        }
            ...
            if ($RootPos == $root[$this->table_fields['id']] and $id != 0 and $clickable) {
                ...
                if ($clickable == "" and $id == 0) {
                    $has_children = $this->has_children(
                        $root[$this->table_fields['position']]
                    );
                    if ($has_children == true) {
                        $this->load_children(
                            $root[$this->table_fields['position']], 0
                        );
                    }
                }
            }
    ...
    function list_by_id($id) { //return an array with the categories under the given ID and ordered by name
        ...
        $positions = explode(
            ">", $this_category[$this->table_fields['position']]
        );
    ...
    function has_children($position) { // return TRUE if that position has sub-categories otherwise returns FALSE
        ...
        //print "--> $check_sql\n";
        if ($check[preg_replace('/`/', '', $this->table_fields['id'])] != "") {
            return true;
        } else {
            return false;
        }
    ...
    function load_children($position, $id = 0, $recursive = true) {
        ...
        while ($child = mysql_fetch_array($res)) {
            $child["prefix"] = $this->get_prefix(
                $child[preg_replace('/`/', '', $this->table_fields['position'])]
            );
            ...
            if ($id != 0) {
                ...
                $child['position']     = $child[preg_replace(
                    '/`/', '', $this->table_fields['position']
                )];
                $this->c_list[$child[preg_replace(
    ...
    function html_row_output($id) {
        ...
        foreach ($positions as $nid) {
            ...
            if (!$nid) {
                continue;
            } // thats for the last position its always a null space, so we just ignore it!
    ...
    function order_node($id, $new_order) {
        ...
        $res2 = mysql_query($sql2) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql2."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));
        ...
        $res3 = mysql_query($sql3) or die(trigger_error(
            "<br><storng><u>MySQL Error:</u></strong><br>".mysql_error()."<br><br><storng><u>Query Used:</u></strong><br>".$sql3."<br><br><storng><u>Info:</u></strong><br>", E_USER_ERROR
        ));
    ...
    function load_comb() {
        ...
        foreach ($this->get_children('') as $child) {
            ...
            $this->comb[$child[$id_field]] = array(
                'name'      => $child[preg_replace(
    ...
    function get_children($position) {
        ...
        while ($child = mysql_fetch_assoc($res)) {
            ...
            $child['is_default']   = $child[preg_replace(
                '/`/', '', $this->table_fields['is_default']
            )];
            $child['contador']     = $contador++;
            $children[$child[preg_replace(
                '/`/', '', $this->table_fields['id']
            )]]
                                   = $child;
    ...
    function get_teeth($position, $father) {
        $id_field         = preg_replace('/`/', '', $this->table_fields['id']);
        $position_field   = preg_replace(
            '/`/', '', $this->table_fields['position']
        );
        $is_default_field = preg_replace(
            '/`/', '', $this->table_fields['is_default']
        );
        $teeth            = array();
        $the_default      = 0;
        foreach ($this->get_children($position) as $child) {
            ...
            $this->comb[$father]['teeth'][$position]['elements'][$child[$id_field]]
                = array(
                'key'       => $child[$id_field],
                'name'      => $child[preg_replace(
                    '/`/', '', $this->table_fields['name']
