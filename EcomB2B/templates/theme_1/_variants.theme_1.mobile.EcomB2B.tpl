...
<div id="variant_chooser_dialog_{$master_id}" class="variant_chooser_dialog hide" style="left:-80px;position: absolute;z-index: 4000;background: #FFF" >
    <table class="variant_chooser" >
        ...
        {foreach from=$variants item=$variant  name=variant}
            ...
            >
                <td  class="smaller_font">{$variant->get('Product Variant Short Name')} ({$variant->get('Formatted Units')}x)</td>
                <td   style="text-align: right">{$variant->get('Price')}</td>

                <td  class="smaller_font"  style="text-align: right">{$variant->get('Price Per Unit Bis')}</td>
            </tr>
        {/foreach}
        <tr></tr>
    </table>
