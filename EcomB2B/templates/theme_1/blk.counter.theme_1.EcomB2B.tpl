...
<!--
 ...
 Created: 17 July 2017 at 11:15:00 GMT+8, Kuala Lumpur, Malaysia
        ...
        <div class="container">
            ...
            <div class="counters1 two">
                ...
                {foreach from=$data.columns  key=column_key item=counter_column name=counter_columns}
                    <div  link="{$counter_column.link}" number="{$counter_column.number}"  class="{$column_class}  _counter  {if $smarty.foreach.counter_columns.last}last{/if}"><span id="counter_target_{$key}_{$column_key}">0</span> <h4 >{$counter_column.label}</h4></div>
                {/foreach}
