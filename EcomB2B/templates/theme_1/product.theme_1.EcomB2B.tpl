		...
		<span  class="label sim_button " > <i reminder_key="{$reminder_key}" title="{if $reminder_key>0}{t}Click to remove notification{/t}{else}{t}Click to be notified by email{/t}{/if}"   class="reminder fa {if $reminder_key>0}fa-envelope{else}fa-envelope-o{/if}" aria-hidden="true"></i>  </span>
						...
						<div class="ordering log_in " style="width:200px;position:relative;margin-top:40px" >
							...
							<div class="order_row {if $quantity_ordered!=''}ordered{else}empty{/if}"      >
								<input maxlength=6  style="border-left:1px solid #ccc" class='order_input ' id='but_qty{$product->id}'   type="text"' size='2'  value='{$quantity_ordered}' ovalue='{$quantity_ordered}'>
                                {if $quantity_ordered==''}
               ...
               {/if}
				...
				<div id="product_description" class="product_description_block fr-view {$content.description_block.class}">
	...
	<section class="product_tabs" style="margin-top:20px">
		...
		<div>
			...
			<table class="properties">
				...
				<tr class="{if $Weight==''}hide{/if}"> <td>{$product->get_field_label('Product Unit Weight')|ucfirst}</td> <td>{$Weight}</td></tr>
				<tr class="{if $Dimensions==''}hide{/if}">  <td>{$product->get_field_label('Product Unit Dimensions')|ucfirst}</td> <td>{$Dimensions}</td></tr>
				...
				<tr class="{if $Barcode==''}hide{/if}"> <td>{$product->get_field_label('Product Barcode Number')|ucfirst}</td> <td>{$Barcode}</td></tr>
