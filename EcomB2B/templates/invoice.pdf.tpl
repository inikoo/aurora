	...
	<table class="items" width="100%" style="display:none;font-size: 9pt; border-collapse: collapse;" cellpadding="8">
		...
		<tbody>
			...
			<tr class="{if $smarty.foreach.payments.last}last{/if}">
				<td style="text-align:left">{if $payment->get('Payment Method')=='Account'}{t}Credit{/t}{else}{$payment->get('Method')}{/if}  {if $payment->get('Payment Type')=='Refund'} ({t}Refund{/t}){/if} {if $payment->get('Payment Type')=='Return'} ({t}Return excess payment{/t}){/if}    </td>
