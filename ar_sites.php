...
<<<<<<< HEAD
    //////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////
    //List users with number of hits for a particular page //
    //////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////
    function list_page_stats() {
        ...
        elseif ($parent=='pages'){
            $conf=$_SESSION['state']['site']['hits'];
            $conf_table='site';
            $conf_var='hits';
        }
        elseif ($parent=='top_pages'){
            $conf=$_SESSION['state']['site']['overview'];
            $conf_table='site';
            $conf_var='overview';
        }
        else {
        ...
        switch ($group_by) {
            ...
            case('hits'):
                ...
                break;
            case('site'):
                $sql="select count(*) as total from `User Request Dimension` URD left join `Page Store Dimension` PSD on (URD.`Page Key`=PSD.`Page Key`) $where $wheref";
                break;
            case('top_pages'):
                $sql="select count(*) as total from (select count($group) as tot from `User Request Dimension` URD left join `Page Store Dimension` PSD on (URD.`Page Key`=PSD.`Page Key`) $where $wheref group by $group limit 0,5) as temp ";
                break;
            default:
                $sql="";
                break;
        ...
        switch ($group_by) {
            ...
            case('pages'):
                ...
                break;
            case('site'):
                $sql=sprintf("select CD.`Customer Name`, CD.`Customer Key`, CD.`Customer Main Plain Email`, URD.`Page Key` as current_page_key, URD.`Previous Page Key` previous_page_key, PD.`Page Title` as prev_page_title, URD.`Date`, URD.`IP` from `User Request Dimension` URD left join `User Dimension` UD on (URD.`User Key` = UD.`User Key`) left join `Customer Dimension` CD on (UD.`User Parent Key` = CD.`Customer Key`) left join `Page Dimension` PD on (URD.`Previous Page Key`=PD.`Page Key`) left join `Page Store Dimension` PSD on (PD.`Page Key`=PSD.`Page Key`) $where $wheref order by $order $order_direction limit $start_from,$number_results ");
                break;
            case('top_pages'):
                $sql=sprintf("select URD.`Page Key`, PSD.`Page Store Title` as `Page Title`, count($group) as total_visits from `User Request Dimension` URD left join `Page Store Dimension` PSD on (URD.`Page Key`=PSD.`Page Key`) $where $wheref group by $group order by $order $order_direction limit $start_from,$number_results");
                break;
            default:
                $sql='';
                break;
...
>>>>>>> a178b3f18dd0bbd8ffafb129af24315849fe0149
