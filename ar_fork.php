...
function get_process_bar($data, $db) {
	...
	if ($result=$db->query($sql)) {
		if ($row = $result->fetch()) {
            ...
            $download_info='<i class="fa fa-spinner fa-spin"></i>'._('Saving');
            }else{
            $download_info=percentage($row['Fork Operations Done'], $row['Fork Operations Total Operations']);
            }
			...
			$response= array(
				...
				'percentage'=> percentage($row['Fork Operations Done'], $row['Fork Operations Total Operations']),
