...
class SupplierDelivery extends DB_Table {
        ...
        switch ($key) {
            ...
            case 'Checked Percentage or Date':
                if ($this->get('State Index') < 0) {
                    ...
                    return strftime(
                        "%e %b %Y", strtotime($this->get('Supplier Delivery Checked Date'))
                    );
                    ...
                    return percentage(
                        $this->get('Supplier Delivery Number Checked Items'), $this->get('Supplier Delivery Number Dispatched Items')
                    );
                } else {
            ...
            case 'Placed Percentage or Date':
                ...
                if ($this->get('State Index') < 40) {
                    ...
                    return percentage(
                        $this->get('Supplier Delivery Number Placed Items'), $this->get(
                        'Supplier Delivery Number Received and Checked Items'
                    )
                    );
                } else {
                    if ($this->data['Supplier Delivery Checked Date'] == '') {
            ...
            case 'Total Amount':
                return money(
                    $this->data['Supplier Delivery Total Amount'], $this->data['Supplier Delivery Currency Code']
                );
                break;
    ...
    function update_state($value, $options = '', $metadata = array()) {
        ...
        if ($value == 'InProcess or Dispatched') {
            ...
            if ($this->get('Supplier Delivery Placed Items') == 'Yes') {
                $this->error = true;
                $this->msg
                             = "Can't roll back delivery status with placed items";
        ...
        switch ($value) {
            ...
            case 'Cancelled':
                ...
                $this->update_field(
                    'Supplier Delivery Cancelled Date', $date, 'no_history'
                );
                $this->update_field(
                    'Supplier Delivery State', $value, 'no_history'
                );
        ...
        $this->update_metadata = array(
            'class_html'  => array(
                ...
                'Supplier_Delivery_Dispatched_Date'         => '&nbsp;'.$this->get(
                        'Dispatched Date'
                    ),
                'Supplier_Delivery_Received_Date'           => '&nbsp;'.$this->get(
                        'Received Date'
                    ),
                'Supplier_Delivery_Checked_Date'            => '&nbsp;'.$this->get(
                        'Checked Date'
                    ),
                'Supplier_Delivery_Number_Dispatched_Items' => $this->get(
                    'Number Dispatched Items'
                ),
                'Supplier_Delivery_Number_Received_Items'   => $this->get(
                    'Number Received Items'
                )
    ...
    function update_totals() {
        ...
        $sql = sprintf(
            "SELECT  sum(if(`Supplier Delivery Transaction Placed`='Yes',1,0)) AS placed_items, sum(if(`Supplier Delivery Transaction Placed`='No',1,0)) AS no_placed_items,  sum(`Supplier Delivery Weight`) AS  weight,sum(`Supplier Delivery CBM` )AS cbm ,
        ...
        if ($result = $this->db->query($sql)) {
            if ($row = $result->fetch()) {
                ...
                $total_net = $row['net'] + $this->get(
                        'Supplier Delivery Shipping Net Amount'
                    ) + $this->get('Supplier Delivery Charges Net Amount') + $this->get('Supplier Delivery Other Net Amount');
                $total_tax = $row['tax'] + $this->get(
                        'Supplier Delivery Shipping Tax Amount'
                    ) + $this->get('Supplier Delivery Charges Tax Amount') + $this->get('Supplier Delivery Other Tax Amount');
    ...
    function update_item_delivery_checked_quantity($data) {
        ...
        $qty = $data['qty'] / $supplier_part->get(
                'Supplier Part Packages Per Carton'
            );
        if ($qty < 0) {
            $qty = 0;
        ...
        if ($result = $this->db->query($sql)) {
            if ($row = $result->fetch()) {
                ...
                if ($placed == 'Yes') {
                    $sql = sprintf(
                        'UPDATE `Purchase Order Transaction Fact` SET `Supplier Delivery Transaction State`="Placed"  WHERE `Purchase Order Transaction Fact Key`=%d ',
                    ...
                    $sql = sprintf(
                        'UPDATE `Purchase Order Transaction Fact` SET `Supplier Delivery Transaction State`="Checked"  WHERE `Purchase Order Transaction Fact Key`=%d ',
        ...
        if ($this->get('Supplier Delivery State') == 'Received') {
            ...
            if ($this->get('Supplier Delivery Number Placed Items') == $this->get(
                    'Supplier Delivery Number Received and Checked Items'
                )
            ) {
 ...
 LEFT JOIN  `Part Dimension` P ON (P.`Part SKU`=SP.`Supplier Part Part SKU`)
        ...
        $this->update_metadata = array(
            'class_html'  => array(
                ...
                'Supplier_Delivery_Number_Placed_Items'        => $this->get(
                    'Number Placed Items'
                ),
                'Supplier_Delivery_Checked_Percentage_or_Date' => '&nbsp;'.$this->get('Checked Percentage or Date'),
                'Supplier_Delivery_Placed_Percentage_or_Date'  => '&nbsp;'.$this->get('Placed Percentage or Date')
    ...
    function update_supplier_parts() {
        ...
        $sql = sprintf(
            "SELECT `Supplier Part Key`,`Supplier Delivery Quantity` FROM `Purchase Order Transaction Fact` WHERE `Supplier Delivery Key`=%d", $this->id
        );
