...
*/
				...
				'.$this->site->ip_country.'<span style="display:none;margin-left:10px;margin-right:10px;cursor:pointer">'._('Show prices in').' <span id="alternative_currency"></span>?</span> <img  src="art/basket.jpg" style="height:15px;position:relative;top:3px;margin-left:10px;cursor:pointer"/> <span style="cursor:pointer"  > '._('Total').': <span onClick=\'window.location="basket.php"\'  id="basket_total">'.($this->order->id?$this->order->get('Total Amount'):money_locale(0,$this->site->data['Site Locale'],$this->currency)).'</span> (<span id="number_items">'.($this->order->id?$this->order->get('Number Items'):0).'</span> '._('items').')</span>  <span onClick=\'window.location="basket.php"\' style="color:#ff8000;margin-left:10px" class="link basket"  id="see_basket"  >'._('Basket').'</span>
				 </div>' ;
