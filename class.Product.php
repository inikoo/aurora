...
class product extends DB_Table {
	...
	function get_data($tipo,$tag,$extra=false) {
		...
		elseif ($tipo=='code') {
			$this->mode='code';
			$sql=sprintf("select * from `Product Dimension` where `Product Code`=%s   ",prepare_mysql($tag));
...
*/
	...
	function get_materials() {
		...
		$sql=sprintf("select * from `Product Material Bridge` B left join `Material Dimension` MD on (MD.`Material Key`=B.`Material Key`) where `Product ID`=%d order by `Ratio` desc,`Product Material Key` ",
			$this->pid
		...
		//print $sql;
		while ($row=mysql_fetch_assoc($res)) {
			$materials.=sprintf(', %s',$row['Material Name']);
			$xhtml_materials.=sprintf(', <a href="material.php?id=%d">%s</a>',$row['Material Key'],$row['Material Name']);
	...
	function update_origin_country_from_parts() {
		$countries=array();
		$country_codes='';
	...
	function update_materials($value) {
			...
			if (preg_match('/\s*\(.+\s*\%\s*\)$/',$material,$match)) {
				...
				if ($material!='') {
					...
					if (array_key_exists(strtolower($material),$materials)) {
						...
						$materials[strtolower($material)]=array('name'=>$material,'ratio'=>$ratio);
					}
				}
			}else {

				$materials[strtolower($material)]=array('name'=>$material,'ratio'=>'');
		...
		foreach ($materials as $key=>$_value) {
			...
			if ($material->id) {
				$sql=sprintf("insert into `Product Material Bridge` (`Product ID`,`Material Key`,`Ratio`) values (%d,%d,%s) ",
					$this->pid,
					$material->id,
    ...
    */
	...
	function update_sales_from_order_snapshot($interval) {

		$to_date='';
		list($db_interval,$from_date,$to_date,$from_date_1yb,$to_1yb)=calculate_interval_dates($interval);


		$sql=sprintf("select sum(`Outers Out`) as outers_out,sum(`Sales`) as sales,sum(`Sales DC`) as dc_sales from `Order Spanshot Fact` USE INDEX (`Date`)  where   `Product ID`=%d %s %s ",
			$this->pid,
		...
		sum(`Sales DC`) as dc_sales
		 ...
		 from `Order Spanshot Fact`  USE INDEX (`Date`)   where   `Product ID`=%d and `Date`>=%s %s ",
			...
			$result=mysql_query($sql);
			if ($row=mysql_fetch_array($result, MYSQL_ASSOC)) {
				$this->data["Product $db_interval Acc 1YB Invoiced Amount"]=$row["sales"];
				$this->data["Product $db_interval Acc 1YB Invoiced Delta"]=($row["sales"]==0?-1000000:$this->data["Product $db_interval Acc Invoiced Amount"]/$row["sales"]);
