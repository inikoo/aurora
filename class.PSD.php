...
class PhpPsdReader {
    ...
    function PhpPsdReader($fileName) {
        ...
        if (fread($this->fp, 4) == '8BPS') {
            ...
            $this->infoArray['colorModeDataSectionLength'] = $this->_getInteger(
                4
            );
            fseek(
                $this->fp, $this->infoArray['colorModeDataSectionLength'], SEEK_CUR
            ); // ignore this snizzle

            /*  IMAGE RESOURCES */
            $this->infoArray['imageResourcesSectionLength']
                = $this->_getInteger(4);
            fseek(
                $this->fp, $this->infoArray['imageResourcesSectionLength'], SEEK_CUR
            ); // ignore this snizzle

            /*  LAYER AND MASK */
            $this->infoArray['layerMaskDataSectionLength'] = $this->_getInteger(
                4
            );
            fseek(
                $this->fp, $this->infoArray['layerMaskDataSectionLength'], SEEK_CUR
            ); // ignore this snizzle


            /*  IMAGE DATA */
            $this->infoArray['compressionType'] = $this->_getInteger(
                2
            );
            $this->infoArray['oneColorChannelPixelBytes']
                                                = $this->infoArray['colorDepth'] / 8;
            $this->colorBytesLength
                                                = $this->infoArray['rows'] * $this->infoArray['columns'] * $this->infoArray['oneColorChannelPixelBytes'];

            if ($this->infoArray['colorMode'] == 2) {
                $this->infoArray['error']
                    = 'images with indexed colours are not supported yet';
    ...
    function _getInteger($byteCount = 1) {
        switch ($byteCount) {
            ...
            default:
                return hexdec(
                    $this->_hexReverse(bin2hex(fread($this->fp, $byteCount)))
                );
        }
    }
    ...
    function getImage() {
        ...
        switch ($this->infoArray['compressionType']) {
            ...
            case 1:
                ...
                for (
                    $i = 0; $i < ($this->infoArray['rows'] * $this->infoArray['channels']); $i++
                ) {
                    $this->infoArray['scanLinesByteCounts'][]
                        = $this->_getInteger(2);
                }
                $this->tempFileName = tempnam(
                    realpath('/tmp'), 'decompressedImageData'
                );
                ...
                foreach (
                    ...
                    fwrite(
                        $tfp, $this->_getPackedBitsDecoded(
                        fread($this->fp, $scanLinesByteCount)
                    )
                    );
                }
                fclose($tfp);
        ...
        $image = imagecreatetruecolor(
            $this->infoArray['columns'], $this->infoArray['rows']
        );
                ...
                */
                switch ($this->infoArray['colorMode']) {
                    ...
                    case 0:
                        ...
                        if ($bitPointer == 0) {
                            $currentByteBits = str_pad(
                                base_convert(
                                    bin2hex(fread($this->fp, 1)), 16, 2
                                ), 8, '0', STR_PAD_LEFT
                            );
                        }
                        $r
                            = $g
                            = $b = (($currentByteBits[$bitPointer] == '1') ? 0 : 255);
                        $bitPointer++;
                        if ($bitPointer == 8) {
                            $bitPointer = 0;
                        }
                    ...
                    case 4: // CMYK
                        ...
                        $r = round(
                            ($c * $k) / (pow(2, $this->infoArray['colorDepth']) - 1)
                        );
                        $g = round(
                            ($m * $k) / (pow(2, $this->infoArray['colorDepth']) - 1)
                        );
                        $b = round(
                            ($y * $k) / (pow(2, $this->infoArray['colorDepth']) - 1)
                        );
        ...
        */
        ...
        while (1) {
            if (isset($string[$stringPointer])) {
                $headerByteValue = $this->_unsignedToSigned(
                    hexdec(bin2hex($string[$stringPointer])), 1
                );
            } else {
                return $returnString;
            }
