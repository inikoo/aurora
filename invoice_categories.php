...
$smarty->assign('search_label',_('Invoices'));
$smarty->assign('search_scope','invoices');
...
if (isset($_REQUEST['store_key']) ) {
	...
	if ($_REQUEST['store_key']!=0) {
		$store=new Store($_REQUEST['store_key']);
		if (!$store->id) {
	...
	if (count($user->stores)==0) {
		...
		header('Location: invoice_categories.php?store_key='.$user->stores[0]);
		exit;
...
$block_view=$_SESSION['state']['invoice_categories']['root_block_view'];
...
$tipo_filter=$_SESSION['state']['invoice_categories']['main_categories']['f_field'];
$smarty->assign('filter1',$tipo_filter);
$smarty->assign('filter_value1',$_SESSION['state']['invoice_categories']['main_categories']['f_value']);
...
$sql=sprintf("select count(*) as num ,`Type` from  `Invoice Category History Bridge` where  `Store Key`=%d group by  `Type`",$store_key);
//print_r($sql);
$res=mysql_query($sql);
...
$smarty->display('invoice_categories.tpl');
?>
