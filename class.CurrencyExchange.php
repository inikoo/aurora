...
class CurrencyExchange {
    ...
    function CurrencyExchange($currency_pair = false, $date = false) {
        ...
        if (!$this->exchange) {
            $this->get_data_exchange_from_kbase();
        }
    ...
    function get_data_exchange_from_kbase() {
        ...
        $sql  = sprintf(
            "SELECT `Exchange` FROM kbase.`History Currency Exchange Dimension` WHERE `Currency Pair`=%s AND `Date`=DATE(%s)     ", prepare_mysql($this->currency_pair), prepare_mysql($this->date)
        );
    ...
    function save_exchange() {
        ...
        $sql = sprintf(
            "INSERT INTO kbase.`History Currency Exchange Dimension` VALUES (%s,%s,%f) ON DUPLICATE KEY UPDATE `Exchange`=%f ", prepare_mysql($this->date), prepare_mysql($this->currency_pair),
        ...
        $sql          = sprintf(
            "INSERT INTO kbase.`History Currency Exchange Dimension` VALUES (%s,%s,%f) ON DUPLICATE KEY UPDATE `Exchange`=%f ", prepare_mysql($this->date),
            prepare_mysql($this->currency2.$this->currency1), $inv_exchange, $inv_exchange
        );
        mysql_query($sql);
    }
