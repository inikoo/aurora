...
class Department extends DB_Table {
    ...
    function get_data($tipo, $tag, $tag2 = false) {
        ...
        switch ($tipo) {
            case('id'):
                $sql = sprintf(
                    "SELECT * FROM `Product Department Dimension` WHERE `Product Department Key`=%d ", $tag
                );
                break;
            case('code'):
                $sql = sprintf(
                    "SELECT * FROM `Product Department Dimension` WHERE `Product Department Code`=%s AND `Product Department Most Recent`='Yes'", prepare_mysql($tag)
                );
                break;
            case('code_store'):
                $sql = sprintf(
                    "SELECT * FROM `Product Department Dimension` WHERE `Product Department Code`=%s AND `Product Department Most Recent`='Yes' AND `Product Department Store Key`=%d",
            ...
            default:
                $sql = sprintf(
                    "SELECT * FROM `Product Department Dimension` WHERE `Product Department Type`='Unknown' "
                );
        }
    ...
    */
    ...
    function find($raw_data, $options) {
        ...
        $sql = sprintf(
            "SELECT `Product Department Key`FROM `Product Department Dimension` WHERE `Product Department Store Key`=%d AND `Product Department Code`=%s ", $data['Product Department Store Key'],
        ...
        if (!$this->found & $create) {
            ...
            if ($create) {
                $this->msg = _(
                    'There is already another department with this code'
                );
    ...
    function create($data) {
        ...
        $data['Product Department Currency Code']
        ...
        foreach ($data as $key => $value) {
            $keys .= "`$key`,";
            if (preg_match(
                '/Product Department Description|Marketing|Slogan/i', $key
            )) {
        ...
        $sql    = sprintf(
            "INSERT INTO `Product Department Dimension` %s %s", $keys, $values
        );
        ...
        if (mysql_query($sql)) {
            ...
            $sql = sprintf(
                "INSERT INTO `Product Department Data Dimension`  (`Product Department Key`) VALUES (%d)", $this->id
            );
            mysql_query($sql);


            $sql = sprintf(
                "INSERT INTO `Product Department Default Currency`  (`Product Department Key`) VALUES (%d)", $this->id
            );
            mysql_query($sql);
            ...
            $this->add_history(
                array(
                    ...
                    'History Details'  => _('Department')." ".$this->data['Product Department Name']." (".$this->get(
    ...
    function name_if_duplicated($data) {
        ...
        $sql = sprintf(
            "SELECT * FROM `Product Department Dimension` WHERE `Product Department Name`=%s  AND `Product Department Store Key`=%d ", prepare_mysql($data['Product Department Name']),
        ...
        if ($row = mysql_fetch_assoc($result)) {
            ...
            $number  = 1;
            $sql     = sprintf(
                "SELECT * FROM `Product Department Dimension` WHERE `Product Department Name` LIKE '%s (%%)'  AND `Product Department Store Key`=%d ", addslashes($data['Product Department Name']),
                $data['Product Department Store Key']
            );
            ...
            while ($row2 = mysql_fetch_array($result2, MYSQL_ASSOC)) {
                ...
                if (preg_match(
                    '/\(\d+\)$/', $row2['Product Department Name'], $match
                )) {
    ...
    function get($key) {
        ...
        switch ($key) {
            ...
            case('For Sale Products'):
                return number(
                    $this->data['Product Department For Public Sale Products']
                );
                break;
            case('Families'):
            ...
            case('Total Products'):
                return $this->data['Product Department For Public Sale Products'] + $this->data['Product Department For Private Sale Products'] + $this->data['Product Department In Process Products']
                + $this->data['Product Department Not For Sale Products'] + $this->data['Product Department Discontinued Products'] + $this->data['Product Department Historic Products'];
                break;
    ...
    function load_acc_data() {
        if ($this->id) {
            $sql = sprintf(
                "SELECT * FROM `Product Department Data Dimension` WHERE `Product Department Key`=%d", $this->id
            );
            $res = mysql_query($sql);
    ...
    function update_field_switcher($field, $value, $options = '', $metadata = '') {
        ...
        switch ($field) {
            ...
            case('Sticky Note'):
                $this->update_field(
                    'Product Department '.$field, $value, 'no_null'
                );
    ...
    function update_sales_type($value) {
        if ($value == 'Public Sale' or $value == 'Private Sale' or $value == 'Not For Sale') {
            ...
            $sql = sprintf(
                "UPDATE `Product Department Dimension` SET `Product Department Sales Type`=%s  WHERE  `Product Department Key`=%d ", prepare_mysql($sales_state), $this->id
            );
            //print $sql;
            if (mysql_query($sql)) {
                if ($this->external_DB_link) {
                    mysql_query($sql, $this->external_DB_link);
                }
                ...
                $this->msg     = _(
                    "Error: Department sales type could not be updated "
                );
                $this->updated = false;
    ...
    function update_code($code) {
        ...
        if (!(strtolower($code) == strtolower(
                $this->data['Product Department Code']
            ) and $code != $this->data['Product Department Code'])
        ) {

            $sql = sprintf(
                "SELECT count(*) AS num FROM `Product Department Dimension` WHERE `Product Department Store Key`=%d AND `Product Department Code`=%s  COLLATE utf8_general_ci",
                $this->data['Product Department Store Key'], prepare_mysql($code)
            );
            $res = mysql_query($sql);
        ...
        $sql       = sprintf(
            "UPDATE `Product Department Dimension` SET `Product Department Code`=%s WHERE `Product Department Key`=%d ", prepare_mysql($code), $this->id
        );
        if (mysql_query($sql)) {
            ...
            $sql = sprintf(
                "UPDATE `Product Family Dimension` SET `Product Family Main Department Code`=%s WHERE `Product Family Main Department Key`=%d ", prepare_mysql($code), $this->id
            );
            mysql_query($sql);

            $sql = sprintf(
                "UPDATE `Product Dimension` SET `Product Main Department Code`=%s WHERE `Product Main Department Key`=%d ", prepare_mysql($code), $this->id
            );
            mysql_query($sql);
            ...
            $this->add_history(
                array(
                    ...
                    'History Details'  => _('Store')." ".$this->data['Product Department Name']." "._(
                            'code changed from'
    ...
    function update_name($name) {
        ...
        $sql = sprintf(
            "SELECT count(*) AS num FROM `Product Department Dimension` WHERE `Product Department Store Key`=%d AND `Product Department Name`=%s  COLLATE utf8_general_ci",
            $this->data['Product Department Store Key'], prepare_mysql($name)
        );
        $res = mysql_query($sql);
        $row = mysql_fetch_array($res);
        if ($row['num'] > 0) {
        ...
        $sql       = sprintf(
            "UPDATE `Product Department Dimension` SET `Product Department Name`=%s WHERE `Product Department Key`=%d ", prepare_mysql($name), $this->id
        );
        ...
        if (mysql_query($sql)) {
            ...
            $sql = sprintf(
                "UPDATE `Product Family Dimension` SET `Product Family Main Department Name`=%s WHERE `Product Family Main Department Key`=%d ", prepare_mysql($name), $this->id
            );
            mysql_query($sql);

            $sql = sprintf(
                "UPDATE `Product Dimension` SET `Product Main Department Name`=%s WHERE `Product Main Department Key`=%d ", prepare_mysql($name), $this->id
            );
            mysql_query($sql);
            ...
            $this->add_history(
                array(
                    ...
                    'History Details'  => _('Product Department')." ("._('Code').":".$this->data['Product Department Code'].") "._(
                            'name changed from'
                        ).' '.$old_value." "._('to').' '.$this->get(
            ...
            foreach ($this->get_pages_keys() as $page_key) {
                ...
                if ($page->data['Page Type'] == 'Store' and $page->data['Page Store Content Display Type'] == 'Template') {
                    $page->update(
                        array('Page Store Title' => $this->data['Product Department Name'])
                    );
    ...
    function get_pages_keys() {
        $page_keys = array();
        $sql       = sprintf(
            "SELECT `Page Key` FROM `Page Store Dimension` WHERE `Page Store Section Type`='Department' AND  `Page Parent Key`=%d", $this->id
        );
        $res       = mysql_query($sql);
    ...
    function delete() {
        ...
        if ($this->get('Total Products') == 0) {
            $store = new Store($this->data['Product Department Store Key']);
            $sql   = sprintf(
                "DELETE FROM `Product Department Dimension` WHERE `Product Department Key`=%d", $this->id
            );
            if (mysql_query($sql)) {
            ...
            $sql    = sprintf(
                "SELECT * FROM `Product Family Dimension` WHERE `Product Family Main Department Key` = %d", $this->id
            );
            $result = mysql_query($sql);
            while ($row = mysql_fetch_assoc($result)) {
                $family = new Family($row['Product Family Key']);
                $family->update_department(
                    $store->data['Store No Products Department Key']
                );
                if (!$family->updated) {
    ...
    function update_product_data() {
        ...
        $sql = sprintf(
            "UPDATE `Product Department Dimension` SET `Product Department In Process Products`=%d,`Product Department For Public Sale Products`=%d ,`Product Department For Private Sale Products`=%d,`Product Department Discontinued Products`=%d ,`Product Department Not For Sale Products`=%d ,`Product Department Historic Products`=%d, `Product Department Optimal Availability Products`=%d , `Product Department Low Availability Products`=%d ,`Product Department Critical Availability Products`=%d ,`Product Department Out Of Stock Products`=%d,`Product Department Unknown Stock Products`=%d ,`Product Department Surplus Availability Products`=%d  WHERE `Product Department Key`=%d  ",
            $in_process, $public_sale, $private_sale,
    ...
    */
    ...
    function get_family_keys() {
        $family_keys = array();
        $sql         = sprintf(
            'SELECT `Product Family Key` FROM `Product Family Dimension` WHERE `Product Family Main Department Key`=%d', $this->id
        );
        $res         = mysql_query($sql);
        if ($row = mysql_fetch_assoc($res)) {
    ...
    function add_family($family_id, $args = false) {
        ...
        if ($family->id) {
            $sql = sprintf(
                "INSERT INTO `Product Family Department Bridge` (`Product Family Key`,`Product Department Key`) VALUES (%d,%d)", $family->id, $this->id
            );
            mysql_query($sql);

            $sql    = sprintf(
                "SELECT count(*) AS num FROM `Product Family Department Bridge`  WHERE `Product Department Key`=%d", $this->id
            );
            $result = mysql_query($sql);
            if ($row = mysql_fetch_assoc($result)) {
                $sql = sprintf(
                    "UPDATE `Product Department Dimension` SET `Product Department Families`=%d   WHERE `Product Department Key`=%d  ", $row['num'], $this->id
                );
                //  print "$sql\n";exit;
                mysql_query($sql);
            }
            ...
            if (preg_match('/principal/', $args)) {
                $sql = sprintf(
                    "UPDATE  `Product Family Dimension` SET `Product Family Main Department Key`=%d ,`Product Family Main Department Code`=%s,`Product Family Main Department Name`=%s WHERE `Product Family Key`=%s    ",
                    $this->id, prepare_mysql($this->get('Product Department Code')), prepare_mysql($this->get('Product Department Name')), $family->id
                );
                mysql_query($sql);
            }
        }
    ...
    function add_product($product_id, $args = false) {
        ...
        if ($product->id) {
            $sql = sprintf(
                "INSERT INTO `Product Department Bridge` (`Product Key`,`Product Department Key`) VALUES (%d,%d)", $product->id, $this->id
            );
            mysql_query($sql);
            $this->update_product_data();
            ...
            if (preg_match('/principal/', $args)) {
                $sql = sprintf(
                    "UPDATE  `Product Dimension` SET `Product Main Department Key`=%d ,`Product Main Department Code`=%s,`Product Main Department Name`=%s WHERE `Product ID`=%d    ", $this->id,
                    prepare_mysql($this->get('Product Department Code')), prepare_mysql($this->get('Product Department Name')), $product->pid
                );
    ...
    function update_sales_from_invoices($interval) {
        ...
        $sql = sprintf(
            "update `Product Department Data Dimension` set
                     ...
                     where `Product Department Key`=%d ", $this->data["Product Department $db_interval Acc Invoiced Discount Amount"],
            $this->data["Product Department $db_interval Acc Invoiced Amount"], $this->data["Product Department $db_interval Acc Invoices"], $this->data["Product Department $db_interval Acc Profit"],
            $this->data["Product Department $db_interval Acc Customers"], $this->data["Product Department $db_interval Acc Quantity Ordered"],
            $this->data["Product Department $db_interval Acc Quantity Invoiced"], $this->data["Product Department $db_interval Acc Quantity Delivered"]
        ...
        $sql = sprintf(
            "update `Product Department Default Currency` set
                             ...
                             where `Product Department Key`=%d ", $this->data["Product Department DC $db_interval Acc Invoiced Discount Amount"],
            $this->data["Product Department DC $db_interval Acc Invoiced Gross Amount"]

            , $this->data["Product Department DC $db_interval Acc Invoiced Amount"], $this->data["Product Department DC $db_interval Acc Profit"], $this->id
        );
        ...
        if ($from_date_1yb) {
            ...
            if ($row = mysql_fetch_assoc($result)) {
                ...
                $this->data["Product Department $db_interval Acc 1YB Invoiced Amount"]
                    = $row["net"];
                $this->data["Product Department $db_interval Acc 1YB Invoiced Delta"]
                    = ($row["net"] == 0 ? -1000000 : $this->data["Product Department $db_interval Acc Invoiced Amount"] / $row["net"]);
            ...
            $sql = sprintf(
                "update `Product Department Data Dimension` set
                        ...
                        `Product Department $db_interval Acc 1YB Invoiced Delta`=%f,
                         ...
                         where `Product Department Key`=%d ", $this->data["Product Department $db_interval Acc 1YB Invoiced Discount Amount"],
                $this->data["Product Department $db_interval Acc 1YB Invoiced Amount"], $this->data["Product Department $db_interval Acc 1YB Invoiced Delta"],
                $this->data["Product Department $db_interval Acc 1YB Invoices"], $this->data["Product Department $db_interval Acc 1YB Profit"], $this->id
            );
    ...
    function update_averages_per_item() {
        ...
        $sql                              = sprintf(
            "SELECT *  FROM `Product Dimension` WHERE `Product Main Department Key`=%d  ", $this->id
        );
        $res                              = mysql_query($sql);
        ...
        $this->data['Product Department Total Acc Avg Week Profit Per Product']
            = $avg_weekly_profit_per_product;
        $this->data['Product Department 1 Year Acc Avg Week Sales Per Product']
            = $avg_weekly_sales_per_product_1y;
        $this->data['Product Department 1 Year Acc Avg Week Profit Per Product']
            = $avg_weekly_profit_per_product_1y;
        $this->data['Product Department 1 Quarter Acc Avg Week Sales Per Product']
            = $avg_weekly_sales_per_product_1q;
        $this->data['Product Department 1 Quarter Acc Avg Week Profit Per Product']
            = $avg_weekly_profit_per_product_1q;
        $this->data['Product Department 1 Month Acc Avg Week Sales Per Product']
            = $avg_weekly_sales_per_product_1m;
        $this->data['Product Department 1 Month Acc Avg Week Profit Per Product']
            = $avg_weekly_profit_per_product_1m;
        $this->data['Product Department 1 Week Acc Avg Week Sales Per Product']
            = $avg_weekly_sales_per_product_1w;
        $this->data['Product Department 1 Week Acc Avg Week Profit Per Product']
            = $avg_weekly_profit_per_product_1w;


        $sql = sprintf(
            "UPDATE `Product Department Dimension` SET `Product Department Total Acc Avg Week Sales Per Product`=%.2f , `Product Department Total Acc Avg Week Profit Per Product`=%.2f ,`Product Department 1 Year Acc Avg Week Sales Per Product`=%.2f , `Product Department 1 Year Acc Avg Week Profit Per Product`=%.2f,`Product Department 1 Quarter Acc Avg Week Sales Per Product`=%.2f , `Product Department 1 Quarter Acc Avg Week Profit Per Product`=%.2f,`Product Department 1 Month Acc Avg Week Sales Per Product`=%.2f , `Product Department 1 Month Acc Avg Week Profit Per Product`=%.2f ,`Product Department 1 Week Acc Avg Week Sales Per Product`=%.2f , `Product Department 1 Week Acc Avg Week Profit Per Product`=%.2f WHERE `Product Department Key`=%d   ",
            $this->data['Product Department Total Acc Avg Week Sales Per Product'], $this->data['Product Department Total Acc Avg Week Profit Per Product'],
            $this->data['Product Department 1 Year Acc Avg Week Sales Per Product'], $this->data['Product Department 1 Year Acc Avg Week Profit Per Product'],
            $this->data['Product Department 1 Quarter Acc Avg Week Sales Per Product'], $this->data['Product Department 1 Quarter Acc Avg Week Profit Per Product'],
            $this->data['Product Department 1 Month Acc Avg Week Sales Per Product'], $this->data['Product Department 1 Month Acc Avg Week Profit Per Product'],
            $this->data['Product Department 1 Week Acc Avg Week Sales Per Product'], $this->data['Product Department 1 Week Acc Avg Week Profit Per Product']
    ...
    function update_customers() {
        ...
        $sql = sprintf(
            "UPDATE `Product Department Dimension` SET 
		...
		WHERE `Product Department Key`=%d  ", $this->data['Product Department Active Customers'], $this->data['Product Department Active Customers More 0.75 Share'],
            $this->data['Product Department Active Customers More 0.5 Share'], $this->data['Product Department Active Customers More 0.25 Share'], $this->data['Product Department Losing Customers'],
            $this->data['Product Department Losing Customers More 0.75 Share'], $this->data['Product Department Losing Customers More 0.5 Share'],
            $this->data['Product Department Losing Customers More 0.25 Share'], $this->data['Product Department Lost Customers'], $this->data['Product Department Lost Customers More 0.75 Share'],
            $this->data['Product Department Lost Customers More 0.5 Share'], $this->data['Product Department Lost Customers More 0.25 Share'], $this->id
        );
        //	 print "$sql\n";
        mysql_query($sql);
    ...
    function update_families() {
        $sql    = sprintf(
            "SELECT count(*) AS num FROM `Product Family Dimension`  WHERE`Product Family Main Department Key`=%d", $this->id
        );
        $result = mysql_query($sql);
        if ($row = mysql_fetch_assoc($result)) {
            $this->data['Product Department Families'] = $row['num'];
            $sql                                       = sprintf(
                "UPDATE `Product Department Dimension` SET `Product Department Families`=%d  WHERE `Product Department Key`=%d  ", $this->data['Product Department Families'], $this->id
            );
            mysql_query($sql); //print "$sql\n";
        }

        $sql    = sprintf(
            "SELECT count(*) AS num FROM `Product Family Dimension`  WHERE `Product Family Main Department Key`=%d AND `Product Family Sales Type`='Public Sale' AND `Product Family Record Type` IN ('New','Normal','Discontinuing')  ",
            $this->id
        );
        $result = mysql_query($sql);
        if ($row = mysql_fetch_assoc($result)) {
            $this->data['Product Department For Public For Sale Families']
                 = $row['num'];
            $sql = sprintf(
                "UPDATE `Product Department Dimension` SET `Product Department For Public For Sale Families`=%d  WHERE `Product Department Key`=%d  ",
                $this->data['Product Department For Public For Sale Families'], $this->id
            );
            mysql_query($sql); //print "$sql\n";
        }

        $sql    = sprintf(
            "SELECT count(*) AS num FROM `Product Family Dimension`  WHERE `Product Family Main Department Key`=%d  AND `Product Family Sales Type`='Public Sale' AND `Product Family Record Type`='Discontinued'    ",
            $this->id
        );
        $result = mysql_query($sql);
        if ($row = mysql_fetch_assoc($result)) {
            $this->data['Product Department For Public Discontinued Families']
                 = $row['num'];
            $sql = sprintf(
                "UPDATE `Product Department Dimension` SET `Product Department For Public Discontinued Families`=%d  WHERE `Product Department Key`=%d  ",
                $this->data['Product Department For Public Discontinued Families'], $this->id
            );
            //   print "$sql\n";
            mysql_query($sql);
        }
    ...
    function remove_image($image_key) {
        ...
        if ($row = mysql_fetch_assoc($res)) {
            ...
            $sql = sprintf(
                "DELETE FROM `Image Bridge` WHERE `Subject Type`='Department' AND `Subject Key`=%d  AND `Image Key`=%d", $this->id, $image_key
            );
            mysql_query($sql);
            $this->updated = true;
            ...
            if ($number_images == 0) {
                $main_image_src = '';
                $main_image_key = 0;
                $this->data['Product Department Main Image']
                                = 'art/nopic.png';
                $this->data['Product Department Main Image Key']
                                = $main_image_key;
                $sql            = sprintf(
                    "UPDATE `Product Department Dimension` SET `Product Department Main Image`=%s ,`Product Department Main Image Key`=%d WHERE `Product Department Key`=%d",
                    prepare_mysql($main_image_src), $main_image_key, $this->id
                );
                ...
                $sql  = sprintf(
                    "SELECT `Image Key` FROM `Image Bridge` WHERE `Subject Type`='Department' AND `Subject Key`=%d  ", $this->id
                );
    ...
    function get_number_of_images() {
        $number_of_images = 0;
        $sql              = sprintf(
            "SELECT count(*) AS num FROM `Image Bridge` WHERE `Subject Type`='Department' AND `Subject Key`=%d ", $this->id
        );
        $res              = mysql_query($sql);
        if ($row = mysql_fetch_assoc($res)) {
    ...
    function update_main_image($image_key) {
        ...
        $sql = sprintf(
            "SELECT `Image Key` FROM `Image Bridge` WHERE `Subject Type`='Department' AND `Subject Key`=%d  AND `Image Key`=%d", $this->id, $image_key
        );
        $res = mysql_query($sql);
        ...
        $sql = sprintf(
            "UPDATE `Image Bridge` SET `Is Principal`='No' WHERE `Subject Type`='Department' AND `Subject Key`=%d  ", $this->id
        );
        mysql_query($sql);
        $sql = sprintf(
            "UPDATE `Image Bridge` SET `Is Principal`='Yes' WHERE `Subject Type`='Department' AND `Subject Key`=%d  AND `Image Key`=%d", $this->id, $image_key
        );
        mysql_query($sql);
        ...
        $sql                                             = sprintf(
            "UPDATE `Product Department Dimension` SET `Product Department Main Image`=%s ,`Product Department Main Image Key`=%d WHERE `Product Department Key`=%d", prepare_mysql($main_image_src),
    ...
    function update_image_caption($image_key, $value) {
        ...
        $sql = sprintf(
            "UPDATE `Image Bridge` SET `Image Caption`=%s WHERE  `Subject Type`='Department' AND `Subject Key`=%d  AND `Image Key`=%d", prepare_mysql($value), $this->id, $image_key
        );
        mysql_query($sql);
        //print $sql;
    ...
    function add_image($image_key) {
        ...
        $sql = sprintf(
            "INSERT INTO `Image Bridge` VALUES ('Department',%d,%d,%s,'')", $this->id, $image_key, prepare_mysql($principal)
        ...
        $sql = sprintf(
            "SELECT `Is Principal`,ID.`Image Key`,`Image Caption`,`Image Filename`,`Image File Size`,`Image File Checksum`,`Image Width`,`Image Height`,`Image File Format` FROM `Image Bridge` PIB LEFT JOIN `Image Dimension` ID ON (PIB.`Image Key`=ID.`Image Key`) WHERE `Subject Type`='Department' AND   `Subject Key`=%d AND  PIB.`Image Key`=%d",
            $this->id, $image_key
        );
    ...
    function get_sales_delta($interval) {
        $delta = delta(
            $this->data["Product Department $interval Acc Invoiced Amount"], $this->data["Product Department $interval Acc 1YB Invoiced Amount"]
        );;
    ...
    function post_add_history($history_key, $type = false) {
        ...
        $sql = sprintf(
            "INSERT INTO  `Product Department History Bridge` (`Department Key`,`History Key`,`Type`) VALUES (%d,%d,%s)", $this->id, $history_key, prepare_mysql($type)
        );
        mysql_query($sql);
    ...
    function update_sales_averages() {
        ...
        $sql = sprintf(
            "SELECT sum(`Sales`) AS sales,sum(`Availability`) AS availability  FROM `Order Spanshot Fact` WHERE `Product Department Key`=%d   GROUP BY `Date`;", $this->id
        );
        $res = mysql_query($sql);
        ...
        $sql        = sprintf(
            "SELECT sum(`Sales`) AS sales  FROM `Order Spanshot Fact` WHERE `Product Department Key`=%d AND sales>0  GROUP BY `Date`;", $this->id
        );
        $res        = mysql_query($sql);
        $data_sales = array();
        $max_value  = 0;
        ...
        $sql = sprintf(
            "UPDATE `Product Department Dimension` SET `Product Department Number Days on Sale`=%d,`Product Department Avg Day Sales`=%d,`Product Department Number Days Available`=%f,`Product Department Number Days with Sales`=%d,`Product Department Avg with Sale Day Sales`=%f,`Product Department STD with Sale Day Sales`=%f,`Product Department Max Day Sales`=%f WHERE `Product Department Key`=%d",
            $this->data['Product Department Number Days on Sale'], $this->data['Product Department Avg Day Sales'], $this->data['Product Department Number Days Available'],
            $this->data['Product Department Number Days with Sales'], $this->data['Product Department Avg with Sale Day Sales'], $this->data['Product Department STD with Sale Day Sales'],
            $this->data['Product Department Max Day Sales'], $this->id
        );
        mysql_query($sql);
    ...
    function get_formated_discounts() {
        $formated_discounts = '';
        $sql                = sprintf(
            "SELECT `Deal Description`,`Deal Name`,D.`Deal Key`,`Deal Component Allowance Description` FROM `Deal Target Bridge`  B LEFT JOIN `Deal Component Dimension` DC ON (DC.`Deal Component Key`=B.`Deal Component Key`) LEFT JOIN `Deal Dimension` D ON (D.`Deal Key`=B.`Deal Key`) WHERE `Subject`='Department' AND `Subject Key`=%d ",
            $this->id, $this->id
        );
        $res                = mysql_query($sql);
        while ($row = mysql_fetch_assoc($res)) {
            $formated_discounts .= ', <span title="'.$row['Deal Description'].'"><a href="deal.php?id='.$row['Deal Key'].'">'.$row['Deal Name'].'</a> <b>'.$row['Deal Component Allowance Description']
                .'</b></span>';
        }
