...
class DeliveryNote extends DB_Table {
    ...
    */
	...
	private function handle_to_customer($data) {
		...
		while ($row=mysql_fetch_assoc($res)) {
			...
			if ($row['Inventory Transaction Quantity']==0) {
				$sql=sprintf("update `Inventory Transaction Fact` set `Inventory Transaction Type` like 'No Dispatched' where `Delivery Note Key`=%d  and `Inventory Transaction Key`=%d  ",
					$this->id,
					$row['Inventory Transaction Key']
				...
				$sql=sprintf("update `Inventory Transaction Fact` set `Date Shipped`=%s,`Inventory Transaction Type` like 'Sale' where `Delivery Note Key`=%d  and `Inventory Transaction Key`=%d  ",
	...
	function start_picking($staff_key,$date=false) {
		...
		//print $sql;
		$this->assigned=true;
		$operations='<a href="order_pick_aid.php?id='.$this->id.'" >'._('Picking')." ($staff_alias)</a>";
		// $operations.=' <img src="art/icons/edit.gif" alt="'._('edit').'" style="cursor:pointer"  onClick="assign_picker(this,'.$this->id.')">';
		$this->operations=$operations;
		$this->dn_state=_('Picking');
	...
	function get_state() {
		...
		}elseif ($this->data['Delivery Note Fraction Picked']==1 and !$this->data['Delivery Note Assigned Packer Alias']) {
			$state='Picked';
