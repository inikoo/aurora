...
if ( $row['Transaction Type']=='Adjust') {
	...
	function set_order_post_actions_as_dispatched($date) {
		...
		$sql=sprintf("update `Order Dimension` set `Order Post Transactions Dispatched Date`=%s , `Order Current XHTML Dispatch State`=%s ,`Order Current Dispatch State`=%s,`Order Current XHTML Payment State`=%s  where `Order Key`=%d"
			,prepare_mysql($date)
			,prepare_mysql($this->data['Order Current XHTML Dispatch State'])
			,prepare_mysql($this->data['Order Current Dispatch State'])
			,prepare_mysql($this->data['Order Current XHTML Payment State'])
	...
	function update_discounts_family_tigger() {
		...
		while ($row_lines=mysql_fetch_array($res_lines)) {
			...
			$sql=sprintf("select * from `Deal Component Dimension` DM left join `Deal Dimension` D on  (D.`Deal Key`=DM.`Deal Key`)   where `Deal Component Trigger`='Family' and `Deal Component Trigger Key` =%d  and `Deal Component Status`='Active' ",
				$family_key
			);
			$res=mysql_query($sql);
			while ($row=mysql_fetch_assoc($res)) {
