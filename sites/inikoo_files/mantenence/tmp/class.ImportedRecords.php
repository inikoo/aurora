...
class ImportedRecords extends DB_Table {
    ...
    function find($raw_data,$options) {
        ...
        if ($data['Imported Records Parent']=='' ) {
            $this->error=true;
            $this->msg='Imported Records Parent empty';
            return;
        }
        ...
        $sql=sprintf("select `Imported Records Key` from `Imported Records Dimension` where `Imported Records Parent`=%s and `Imported Records Parent Key`=%s and `Imported Records File Checksum`=%s  and `Imported Records Start Date` is NULL",
                     prepare_mysql($data['Imported Records Parent']),
                     prepare_mysql($data['Imported Records Parent Key']),
