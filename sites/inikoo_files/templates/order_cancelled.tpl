...
<input type="hidden" id="order_key" value="{$order->id}"/>
	 ...
	 <h2 style="padding:0"><a style="position:relative;z-index:5" href="customer.php?id={$order->get('order customer key')}">{$order->get('order customer name')} (ID:{$page->customer->get_formated_id()})</a></h2>
       <br/>
