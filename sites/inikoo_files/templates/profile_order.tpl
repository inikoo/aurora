...
<div id="order_container">
	...
	<div id="control_panel"  class="{$order->get('Order Current Dispatch State')|replace:' ':''}"  >
		<div id="addresses">
			<h1 style="padding:0 0 5px 0;font-size:140%">
				Order {$order->get('Order Public ID')}, <span style="font-style:italic;color:#555">{$order->get_formated_dispatch_state()}</span>
			</h1>
			<h1 style="padding:0 0 5px 0;font-size:125%">
				{$order_date}
			</h1>
		...
		<div id="totals">
			...
			<div style="text-align:right;padding-top:4px;;clear:both;{if $distinct_set_currency}display:none{/if}">
				<img src="art/info.png" style="height:14px;position:relative;bottom:-1px" /> {t}Please note, the prices are just a reference value, the amount payable is in {/t} <b>{$store->get('Store Currency Code')}</b> with a total of <b><span id="store_currency_total_balance">{$total_in_store_currency}</span></b> 
			</div>
		</div>
		<div style="clear:both">
		</div>
